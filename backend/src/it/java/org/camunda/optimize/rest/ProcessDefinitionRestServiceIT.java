...
public class ProcessDefinitionRestServiceIT extends AbstractDefinitionRestServiceIT {
  ...
  public void getProcessDefinitions(final String processDefinitionType) {
    ...
    final ProcessDefinitionOptimizeDto processDefinitionOptimizeDto =
        addDefinitionToElasticsearch(KEY, processDefinitionType);
  ...
  public void getProcessDefinitionsExcludesDeletedDefinitions() {
    ...
    final ProcessDefinitionOptimizeDto definition =
        addProcessDefinitionToElasticsearch(KEY, "1", null, false);
  ...
  public void getProcessDefinitionsReturnOnlyThoseAuthorizedToSeeAndAllEventProcessDefinitions() {
    ...
    final String authorizedProcessId =
        addProcessDefinitionToElasticsearch(authorizedDefinitionKey1).getId();
    final String authorizedEventProcessId1 =
        databaseIntegrationTestExtension
            .addEventProcessDefinitionDtoToDatabase(
                authorizedDefinitionKey2, new UserDto(KERMIT_USER))
            .getId();
    final String authorizedEventProcessId2 =
        databaseIntegrationTestExtension
            .addEventProcessDefinitionDtoToDatabase(
                authorizedDefinitionKey3, new UserDto(KERMIT_USER))
            .getId();
    ...
    // then we only get 3 definitions, the one kermit is authorized to see and all event based
    // definitions
    assertThat(definitions)
        .extracting(DefinitionOptimizeResponseDto::getId)
        .containsExactlyInAnyOrder(
            authorizedProcessId, authorizedEventProcessId1, authorizedEventProcessId2);
  }
  ...
  public void getProcessDefinitionsWithXml(final String processDefinitionType) {
    ...
    // then
    assertThat(definitions.get(0).getId()).isEqualTo(processDefinitionOptimizeDto.getId());
    assertThat(definitions.get(0).getBpmn20Xml())
  ...
  public void getProcessDefinitionXml(final String processDefinitionType) {
    // given
    ProcessDefinitionOptimizeDto expectedDto =
        addDefinitionToElasticsearch(KEY, processDefinitionType);

    // when
    String actualXml =
        definitionClient.getProcessDefinitionXml(
            expectedDto.getKey(), expectedDto.getVersion(), null);
  ...
  public void getAllProcessDefinitionXml(final String processDefinitionType) {
    // given
    ProcessDefinitionOptimizeDto expectedDto1 =
        addDefinitionToElasticsearch(KEY, "1", processDefinitionType);
    ProcessDefinitionOptimizeDto expectedDto2 =
        addDefinitionToElasticsearch(KEY, "2", processDefinitionType);
  ...
  public void getProcessDefinitionXmlByTenant() {
    ...
    ProcessDefinitionOptimizeDto firstTenantDefinition =
        addProcessDefinitionToElasticsearch(KEY, firstTenantId);
    ProcessDefinitionOptimizeDto secondTenantDefinition =
        addProcessDefinitionToElasticsearch(KEY, secondTenantId);

    // when
    final String actualXmlFirstTenant =
        definitionClient.getProcessDefinitionXml(KEY, "1", firstTenantId);
    final String actualXmlSecondTenant =
        definitionClient.getProcessDefinitionXml(KEY, "1", secondTenantId);
    // then
  ...
  public void getSharedProcessDefinitionXmlByNullTenant() {
    ...
    ProcessDefinitionOptimizeDto firstTenantDefinition =
        addProcessDefinitionToElasticsearch(KEY, firstTenantId);
    ProcessDefinitionOptimizeDto secondTenantDefinition =
        addProcessDefinitionToElasticsearch(KEY, null);
  ...
  public void getSharedProcessDefinitionXmlByTenantWithNoSpecificDefinition(
      final String processDefinitionType) {
    // given
    final String firstTenantId = "tenant1";
    ProcessDefinitionOptimizeDto sharedTenantDefinition =
        addDefinitionToElasticsearch(KEY, processDefinitionType);
  ...
  public void getProcessDefinitionXmlWithoutAuthorization() {
    ...
    engineIntegrationExtension.grantUserOptimizeAccess(KERMIT_USER);
    final ProcessDefinitionOptimizeDto processDefinitionOptimizeDto =
        addProcessDefinitionToElasticsearch(definitionKey);
  ...
  public void getEventProcessDefinitionXmlWithoutAuthorization() {
    ...
    engineIntegrationExtension.addUser(KERMIT_USER, KERMIT_USER);
    engineIntegrationExtension.grantUserOptimizeAccess(KERMIT_USER);
    final ProcessDefinitionOptimizeDto expectedDefinition =
        databaseIntegrationTestExtension.addEventProcessDefinitionDtoToDatabase(definitionKey);
  ...
  @ParameterizedTest(
      name = "Get {0} process definition with nonexistent version returns 404 message.")
  @MethodSource("processDefinitionTypes")
  public void getProcessDefinitionXmlWithNonsenseVersionReturns404Code(
    ...
    final Response response =
        embeddedOptimizeExtension
            .getRequestExecutor()
            .buildGetProcessDefinitionXmlRequest(
                processDefinitionOptimizeDto.getKey(), "nonsenseVersion")
            .execute();
  ...
  public void getProcessDefinitionXmlWithNonsenseKeyReturns404Code(
    ...
    final Response response =
        embeddedOptimizeExtension
            .getRequestExecutor()
            .buildGetProcessDefinitionXmlRequest(
                "nonsense", processDefinitionOptimizeDto.getVersion())
            .execute();
  ...
  private ProcessDefinitionOptimizeDto addDefinitionToElasticsearch(
      final String key, final String type) {
    return addDefinitionToElasticsearch(key, "1", type);
  }

  private ProcessDefinitionOptimizeDto addDefinitionToElasticsearch(
      final String key, final String version, final String type) {
    switch (type) {
      case EVENT_BASED:
  ...
  private ProcessDefinitionOptimizeDto addProcessDefinitionToElasticsearch(
      final String key, final String version, final String tenantId, final boolean deleted) {
