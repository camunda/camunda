...
public class EventBasedProcessAutogenerationMultipleSourceConnectionsIT
  ...
  public void createFromExternalAndCamundaSources_noExternalEndEvents_singleCamundaStartEvent() {
    ...
    final CamundaEventSourceEntryDto camundaSource =
        deployDefinitionWithInstanceAndCreateEventSource(modelInstance, EventScopeType.START_END);
    final EventTypeDto camundaStart =
        createCamundaEventTypeDto(PROCESS_ID_1, START_EVENT_ID_1, START_EVENT_ID_1);
    final EventTypeDto camundaEnd =
        createCamundaEventTypeDto(PROCESS_ID_1, END_EVENT_ID_1, END_EVENT_ID_1);

    final List<EventSourceEntryDto<?>> sources =
        Arrays.asList(createExternalEventAllGroupsSourceEntry(), camundaSource);
    ...
    assertCorrectMappingsAndContainsEvents(
        mappings, generatedInstance, Arrays.asList(EVENT_A, EVENT_B, camundaStart, camundaEnd));
    ...
    // We cannot assert on the actual connection between the models as we don't know which start
    // event will be connected
    // to the camunda start events
    assertNodeConnection(
        idOf(camundaStart), INTERMEDIATE_EVENT, idOf(camundaEnd), END_EVENT, generatedInstance);
  ...
  public void createFromExternalAndCamundaSources_noExternalEndEvents_multipleCamundaStartEvent() {
    ...
    final CamundaEventSourceEntryDto camundaSource =
        deployDefinitionWithInstanceAndCreateEventSource(modelInstance, EventScopeType.START_END);
    final EventTypeDto camundaStart1 =
        createCamundaEventTypeDto(PROCESS_ID_1, START_EVENT_ID_1, START_EVENT_ID_1);
    final EventTypeDto camundaStart2 =
    ...
    // The additional Flow nodes are the gateway from the Camunda source and the diverging
    // connecting gateway
    assertThat(generatedInstance.getModelElementsByType(FlowNode.class))
        .hasSize(mappings.size() + 2);
    ...
    final String connectingId =
        generateConnectionGatewayIdForDefinitionKey(
            Diverging, camundaSource.getConfiguration().getProcessDefinitionKey());
    // We cannot assert on the actual connections between the models as we don't know which start
    // event will be
    ...
    assertNodeConnection(
        ...
        generatedInstance);
    assertNodeConnection(
        convergingId, EXCLUSIVE_GATEWAY, idOf(camundaEnd), END_EVENT, generatedInstance);
    assertNodeConnection(idOf(camundaEnd), END_EVENT, null, null, generatedInstance);
    ...
    // We cannot assert on the connecting gateway as we do not know what start event will be its
    // source
  ...
  public void createFromExternalAndCamundaSources_singleExternalStartEndEvents() {
    ...
    assertNodeConnection(
        idOf(EVENT_B), INTERMEDIATE_EVENT, idOf(EVENT_C), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_C), INTERMEDIATE_EVENT, idOf(EVENT_D), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_D),
        INTERMEDIATE_EVENT,
        idOf(camundaStart),
  ...
  public void createFromExternalAndCamundaSources_multipleExternalStartEndEvents() {
    ...
    assertCorrectMappingsAndContainsEvents(
        ...
        generatedInstance,
        Arrays.asList(EVENT_A, EVENT_B, EVENT_C, EVENT_D, camundaStart, camundaEnd));
    // The extra flow nodes are the connecting gateway and the one to split the two end events from
    // the external
    // source model
    assertThat(generatedInstance.getModelElementsByType(FlowNode.class))
        .hasSize(mappings.size() + 2);
    ...
    // then the model elements are of the correct type and connected to expected nodes correctly
    final String connectingId =
        generateConnectionGatewayIdForDefinitionKey(Converging, EXTERNAL_EVENTS_INDEX_SUFFIX);
    final String externalId = generateGatewayIdForNode(EVENT_B, Diverging);
    assertNodeConnection(
        idOf(EVENT_A), START_EVENT, idOf(EVENT_B), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_B), INTERMEDIATE_EVENT, externalId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        externalId, EXCLUSIVE_GATEWAY, idOf(EVENT_C), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        externalId, EXCLUSIVE_GATEWAY, idOf(EVENT_D), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_C), INTERMEDIATE_EVENT, connectingId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_D), INTERMEDIATE_EVENT, connectingId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        connectingId, EXCLUSIVE_GATEWAY, idOf(camundaStart), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(camundaStart), INTERMEDIATE_EVENT, idOf(camundaEnd), END_EVENT, generatedInstance);
    assertNodeConnection(idOf(camundaEnd), END_EVENT, null, null, generatedInstance);
  ...
  public void createFromThreeSourcesWithOrder_external_processStartEnd_startEndEvents() {
    ...
    final String processNodeId =
        generateTaskIdForDefinitionKey(
            camundaProcessSource.getConfiguration().getProcessDefinitionKey());

    final CamundaEventSourceEntryDto startEndSource =
        deployDefinitionWithInstanceAndCreateEventSource(
    ...
    final List<EventSourceEntryDto<?>> sources =
        Arrays.asList(
            createExternalEventAllGroupsSourceEntry(), camundaProcessSource, startEndSource);
    final EventProcessMappingCreateRequestDto createRequestDto =
        buildAutogenerateCreateRequestDto(sources);
    ...
    assertNodeConnection(
        idOf(EVENT_C), INTERMEDIATE_EVENT, idOf(EVENT_D), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_D), INTERMEDIATE_EVENT, processNodeId, CALL_ACTIVITY, generatedInstance);
    assertNodeConnection(
        processNodeId, CALL_ACTIVITY, idOf(camundaStart), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(camundaStart), INTERMEDIATE_EVENT, idOf(camundaEnd), END_EVENT, generatedInstance);
    assertNodeConnection(idOf(camundaEnd), END_EVENT, null, null, generatedInstance);
  ...
  public void createFromThreeSourcesWithOrder_processStartEnd_startEndEvents_external() {
    ...
    final List<EventSourceEntryDto<?>> sources =
        Arrays.asList(
            camundaProcessSource, startEndSource, createExternalEventAllGroupsSourceEntry());
    final EventProcessMappingCreateRequestDto createRequestDto =
        buildAutogenerateCreateRequestDto(sources);
    ...
    // then the model elements are of the correct type and connected to expected nodes correctly
    assertNodeConnection(
        idOf(processStart), START_EVENT, idOf(processEnd), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(processEnd),
        INTERMEDIATE_EVENT,
        idOf(camundaStart),
    ...
    assertNodeConnection(
        ...
        generatedInstance);
    assertNodeConnection(
        idOf(camundaEnd), INTERMEDIATE_EVENT, idOf(EVENT_A), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_A), INTERMEDIATE_EVENT, idOf(EVENT_B), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_B), INTERMEDIATE_EVENT, idOf(EVENT_C), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_C), INTERMEDIATE_EVENT, idOf(EVENT_D), END_EVENT, generatedInstance);
  ...
  public void createFromThreeSourcesWithOrder_startEndEvents_external_processStartEnd()
    ...
    final CamundaEventSourceEntryDto startEndSource =
        createCamundaSourceEntry(
            startEndInstance.getProcessDefinitionKey(), EventScopeType.START_END);
    ...
    final List<EventSourceEntryDto<?>> sources =
        Arrays.asList(
            startEndSource, createExternalEventAllGroupsSourceEntry(), camundaProcessSource);
    final EventProcessMappingCreateRequestDto createRequestDto =
        buildAutogenerateCreateRequestDto(sources);
    ...
    // then the model elements are of the correct type and connected to expected nodes correctly
    assertNodeConnection(
        idOf(camundaStart), START_EVENT, idOf(camundaEnd), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(camundaEnd), INTERMEDIATE_EVENT, idOf(EVENT_A), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_A), INTERMEDIATE_EVENT, idOf(EVENT_B), INTERMEDIATE_EVENT, generatedInstance);
    ...
    assertNodeConnection(
        ...
        generatedInstance);
    assertNodeConnection(
        idOf(processStart), INTERMEDIATE_EVENT, idOf(processEnd), END_EVENT, generatedInstance);
  ...
  public void createFromThreeSourcesWithOrder_complexModelConfiguration() {
    ...
    final String connectingId =
        generateConnectionGatewayIdForDefinitionKey(
            Diverging, startEndSource.getConfiguration().getProcessDefinitionKey());

    assertNodeConnection(
        idOf(EVENT_A), START_EVENT, divergingId1, PARALLEL_GATEWAY, generatedInstance);
    assertNodeConnection(
        divergingId1, PARALLEL_GATEWAY, idOf(EVENT_B), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        divergingId1, PARALLEL_GATEWAY, idOf(EVENT_C), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_B), INTERMEDIATE_EVENT, convergingId1, PARALLEL_GATEWAY, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_C), INTERMEDIATE_EVENT, convergingId1, PARALLEL_GATEWAY, generatedInstance);
    assertNodeConnection(
        convergingId1, PARALLEL_GATEWAY, idOf(EVENT_D), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(EVENT_D), INTERMEDIATE_EVENT, processNodeId, CALL_ACTIVITY, generatedInstance);
    assertNodeConnection(
        processNodeId, CALL_ACTIVITY, connectingId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        connectingId,
        EXCLUSIVE_GATEWAY,
        idOf(camundaStart1),
    ...
    assertNodeConnection(
        divergingId2, EXCLUSIVE_GATEWAY, idOf(camundaEnd1), END_EVENT, generatedInstance);
    assertNodeConnection(
        divergingId2, EXCLUSIVE_GATEWAY, idOf(camundaEnd2), END_EVENT, generatedInstance);
  ...
  private EventMappingDto getMappingForStartEnd(
      final EventTypeDto processStart, final EventTypeDto processEnd) {
