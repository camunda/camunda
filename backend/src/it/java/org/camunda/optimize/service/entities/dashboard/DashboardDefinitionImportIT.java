...
public class DashboardDefinitionImportIT extends AbstractExportImportEntityDefinitionIT {
  ...
  public void importDashboard() {
    ...
    final List<EntityIdResponseDto> importedIds =
        importClient.importEntitiesAndReturnIds(
            Sets.newHashSet(
                dashboardExport, processReportExport, combinedReportExport, decisionReportExport));
    ...
    assertThat(importedDashboard.get().getOwner())
        .isEqualTo(DEFAULT_FIRSTNAME + " " + DEFAULT_LASTNAME);
    assertThat(importedDashboard.get().getLastModifier())
        .isEqualTo(DEFAULT_FIRSTNAME + " " + DEFAULT_LASTNAME);
    assertThat(importedDashboard.get().getCreated()).isEqualTo(LocalDateUtil.getCurrentDateTime());
    assertThat(importedDashboard.get().getLastModified())
        .isEqualTo(LocalDateUtil.getCurrentDateTime());
    assertThat(importedDashboard.get().getCollectionId()).isNull();
    assertThat(importedDashboard.get().getAvailableFilters())
        .isEqualTo(dashboardExport.getAvailableFilters());
  ...
  public void importDashboard_containsSameReportTwice() {
    ...
    final DashboardDefinitionExportDto dashboardExport =
        createDashboardExportDtoWithResources(
            Arrays.asList(processReportExport.getId(), processReportExport.getId()));

    // when
    final List<EntityIdResponseDto> importedIds =
        importClient.importEntitiesAndReturnIds(
            Sets.newHashSet(dashboardExport, processReportExport));
    ...
    // the report within the dashboard is imported once and referenced correctly in the new
    // dashboard
  ...
  public void importDashboard_validDescription() {
    ...
    // when
    final EntityIdResponseDto entityIdResponseDto =
        importClient.importEntityAndReturnId(dashboardExport);

    // then
    assertThat(retrieveImportedDashboard(List.of(entityIdResponseDto)))
        .isPresent()
        .get()
        .satisfies(
            dashboard -> assertThat(dashboard.getDescription()).isEqualTo(dashboardDescription));
  }
  ...
  public void importDashboardIntoCollection() {
    ...
    final List<EntityIdResponseDto> importedIds =
        importClient.importEntitiesIntoCollectionAndReturnIds(
            collectionId, Sets.newHashSet(dashboardExport, reportExport));
  ...
  public static Set<String> getExternalResourceUrls(
    ...
    return dashboardDefinitionRestDto.getTiles().stream()
        ...
        .map(
            tile ->
                gson.fromJson(String.valueOf(tile.getConfiguration()), JsonElement.class)
                    .getAsJsonObject()
                    .get("external")
                    .getAsString())
        .collect(toSet());
