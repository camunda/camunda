...
public abstract class AbstractProcessInstanceFrequencyByInstanceDateByVariableReportEvaluationIT
  ...
  public void numberVariable_invalidBaseline_returnsEmptyResult() {
    ...
    reportData
        .getConfiguration()
        .setDistributeByCustomBucket(
            CustomBucketDto.builder().active(true).baseline(200.0).bucketSize(100.0).build());
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
  ...
  public void worksForAutomaticIntervalSelection() {
    ...
    final ProcessInstanceEngineDto procInstance2 =
        engineIntegrationExtension.startProcessInstance(
            procInstance.getDefinitionId(), Collections.singletonMap("boolVar", true));
    ...
    ProcessReportDataDto reportData =
        createReportData(
            procInstance, VariableType.BOOLEAN, "boolVar", AggregateByDateUnit.AUTOMATIC);
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
