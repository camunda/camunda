...
public class ExecutedFlowNodeInstanceLevelFilterIT extends AbstractFilterIT {
  ...
  public void filterByOneFlowNodeWithUnequalOperator() {
    ...
    final ProcessInstanceEngineDto runningInstance =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    importAllEngineEntitiesFromScratch();
    ...
    assertThat(result.getData())
        .singleElement()
        .satisfies(
            data -> assertThat(data.getProcessInstanceId()).isEqualTo(runningInstance.getId()));
  }
  ...
  public void filterMultipleProcessInstancesByOneFlowNodeWithUnequalOperator() {
    ...
    instanceEngineDto = engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    engineIntegrationExtension.finishAllRunningUserTasks(instanceEngineDto.getId());
    final ProcessInstanceEngineDto firstRunningInstance =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    final ProcessInstanceEngineDto secondRunningInstance =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    importAllEngineEntitiesFromScratch();
    ...
    assertThat(result.getData())
        .hasSize(2)
        .extracting(RawDataProcessInstanceDto::getProcessInstanceId)
        .containsExactlyInAnyOrder(firstRunningInstance.getId(), secondRunningInstance.getId());
  }
  ...
  public void filterByMultipleAndCombinedFlowNodes() {
    ...
    assertThat(result.getData())
        .hasSize(2)
        .extracting(RawDataProcessInstanceDto::getProcessInstanceId)
        .containsExactlyInAnyOrder(firstCompletedInstance.getId(), secondCompletedInstance.getId());
  }
  ...
  public void filterByMultipleAndCombinedFlowNodesWithUnequalOperator() {
    ...
    assertThat(result.getData())
        ...
        .satisfies(
            data ->
                assertThat(data.getProcessInstanceId()).isEqualTo(firstRunningInstance.getId()));
    ...
    assertThat(result.getData())
        .hasSize(2)
        .extracting(RawDataProcessInstanceDto::getProcessInstanceId)
        .containsExactlyInAnyOrder(secondRunningInstance.getId(), thirdRunningInstance.getId());
  }
  ...
  public void filterByMultipleOrCombinedFlowNodesWithUnequalOperator() {
    ...
    ProcessDefinitionEngineDto processDefinition =
        deployProcessWithGatewayAndOneUserTaskEachBranch();
    ...
    assertThat(result.getData())
        .singleElement()
        .satisfies(
            data -> assertThat(data.getProcessInstanceId()).isEqualTo(filteredInstance.getId()));
  }
  ...
  public void sameFlowNodeInDifferentProcessDefinitionDoesNotDistortResult() {
    ...
    // when
    List<ProcessFilterDto<?>> executedFlowNodes =
        ProcessFilterBuilder.filter().executedFlowNodes().id(END_EVENT).add().buildList();
    ReportResultResponseDto<List<RawDataProcessInstanceDto>> result =
        evaluateReportWithFilter(processDefinition, executedFlowNodes);
    ...
    assertThat(result.getData())
        .hasSize(2)
        .extracting(RawDataProcessInstanceDto::getProcessInstanceId)
        .containsExactlyInAnyOrder(firstInstance.getId(), secondInstance.getId());
  }
  ...
  public void validationExceptionOnNullValueField() {
    // given
    List<ProcessFilterDto<?>> filterDtos =
        ProcessFilterBuilder.filter().executedFlowNodes().id(null).add().buildList();
