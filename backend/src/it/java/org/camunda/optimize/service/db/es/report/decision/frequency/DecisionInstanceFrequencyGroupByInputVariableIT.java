...
public class DecisionInstanceFrequencyGroupByInputVariableIT extends AbstractDecisionDefinitionIT {
  ...
  public void
    ...
    DecisionDefinitionEngineDto decisionDefinitionDto1 =
        engineIntegrationExtension.deployDecisionDefinition();
    startDecisionInstanceWithInputVars(decisionDefinitionDto1.getId(), createInputs(10.0, "Misc"));
    startDecisionInstanceWithInputVars(decisionDefinitionDto1.getId(), createInputs(20.0, "Misc"));
  ...
  public void reportEvaluationMultiBucket_negativeVariableValues_defaultBaselineWorks() {
    ...
    DecisionDefinitionEngineDto decisionDefinitionDto1 =
        engineIntegrationExtension.deployDecisionDefinition();
    startDecisionInstanceWithInputVars(decisionDefinitionDto1.getId(), createInputs(-1.0, "Misc"));
    startDecisionInstanceWithInputVars(decisionDefinitionDto1.getId(), createInputs(-5.0, "Misc"));
  ...
  public void reportEvaluationMultiBucketsSpecificVersionGroupByNumberInputVariableFilterByValue() {
    ...
    final DecisionReportDataDto reportData =
        DecisionReportDataBuilder.create()
            ...
            .setVariableType(VariableType.DOUBLE)
            .setFilter(
                Lists.newArrayList(createNumericInputVariableFilter(INPUT_AMOUNT_ID, IN, "200.0")))
            .build();
  ...
  public void dateVariableGroupByWithOneInstance() {
    ...
    final DecisionDefinitionEngineDto definition =
        deploySimpleInputDecisionDefinition(inputClauseId, camInputVariable, DecisionTypeRef.DATE);
    ...
    final String expectedKey =
        embeddedOptimizeExtension.formatToHistogramBucketKey(
            now.atZoneSimilarLocal(ZoneId.systemDefault()).toOffsetDateTime(), ChronoUnit.MONTHS);
    assertThat(result.getFirstMeasureData()).isNotNull();
    assertThat(result.getFirstMeasureData()).hasSize(1);
  ...
  public void dateVariableGroupByWorksWithAllStaticUnits(final AggregateByDateUnit unit) {
    ...
    dateVariableValue =
        dateVariableValue.minus(numberOfInstances - 1, chronoUnit); // reset to first variable value
    for (int i = 0; i < numberOfInstances; i++) {
