 ...
 */
package org.camunda.optimize.service.db.es.report.process.single.flownode.frequency.groupby.date.distributedby.flownode;
...
import org.camunda.optimize.service.db.es.report.process.single.ModelElementFrequencyByModelElementDateByModelElementReportEvaluationIT;
...
public abstract class FlowNodeFrequencyByFlowNodeDateByFlowNodeReportEvaluationIT
  ...
  public void simpleReportEvaluationById() {
    ...
    // when
    final AuthorizedProcessReportEvaluationResponseDto<List<HyperMapResultEntryDto>>
        evaluationResponse = reportClient.evaluateHyperMapReportById(reportId);
  ...
  public void resultContainsAlsoNotExecutedUserTasks() {
    ...
    engineIntegrationExtension.startProcessInstance(
        processDefinition.getId(), ImmutableMap.of("goToTask1", true));
    engineIntegrationExtension.finishAllRunningUserTasks();
  ...
  private static Stream<Arguments> viewLevelAssigneeFilterScenarios() {
    return Stream.of(
        Arguments.of(IN, new String[] {SECOND_USER}, Map.of(USER_TASK_2, 1.)),
        Arguments.of(
            IN,
  ...
  public void viewLevelAssigneeFilterOnlyIncludesFlowNodesMatchingFilter(
    ...
    final OffsetDateTime referenceDate =
        dateFreezer().freezeDateAndReturn().truncatedTo(ChronoUnit.DAYS);
    ...
    HyperMapAsserter.asserter()
        ...
        .distributedByContains(
            USER_TASK_1, expectedResults.getOrDefault(USER_TASK_1, null), USER_TASK_1_NAME)
        .distributedByContains(
            USER_TASK_2, expectedResults.getOrDefault(USER_TASK_2, null), USER_TASK_2_NAME)
        .distributedByContains(
            USER_TASK_3, expectedResults.getOrDefault(USER_TASK_3, null), USER_TASK_3_NAME)
        .doAssert(result);
    // @formatter:on
  }
  ...
  private static Stream<Arguments> viewLevelCandidateGroupFilterScenarios() {
    return Stream.of(
        Arguments.of(IN, new String[] {SECOND_CANDIDATE_GROUP_ID}, Map.of(USER_TASK_2, 1.)),
        Arguments.of(
            IN,
  ...
  public void multipleVersionsRespectLatestNodesOnlyWhereLatestHasMoreNodes() {
    ...
    // when
    final ProcessReportDataDto reportData = createGroupedByDayReport(latestDefinition);
    reportData.setProcessDefinitionVersions(
        Arrays.asList(firstDefinition.getVersionAsString(), latestDefinition.getVersionAsString()));
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
