 ...
 */
package org.camunda.optimize.service.db.es.report.process.single.processinstance.duration.groupby.date.distributedby.variable;
...
public abstract class AbstractProcessInstanceDurationByInstanceDateByVariableReportEvaluationIT
  ...
  public void simpleReportEvaluation() {
    ...
    final ZonedDateTime startOfReferenceDate =
        truncateToStartOfUnit(OffsetDateTime.now(), ChronoUnit.DAYS);
    // @formatter:off
  ...
  public void customOrderOnResultKeyIsApplied() {
    ...
    final ProcessInstanceEngineDto procInstance3 =
        engineIntegrationExtension.startProcessInstance(
            procInstance1.getDefinitionId(), Collections.singletonMap("stringVar", "a string"));
  ...
  public void worksWithAllVariableTypes() {
    ...
    for (Map.Entry<String, Object> entry : variables.entrySet()) {
      ...
      ProcessReportDataDto reportData =
          createReportData(processInstanceDto, variableType, entry.getKey());
      final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
          reportClient.evaluateHyperMapReport(reportData).getResult();
  ...
  public void worksWithVariablesWithMultipleValues() {
    ...
    // when
    ProcessReportDataDto reportData =
        createReportData(procInstance, VariableType.STRING, "listVar");
    AuthorizedProcessReportEvaluationResponseDto<List<HyperMapResultEntryDto>> evaluationResponse =
        reportClient.evaluateHyperMapReport(reportData);
  ...
  public void variableTypeIsImportant() {
    ...
    final ProcessInstanceEngineDto procInstance2 =
        engineIntegrationExtension.startProcessInstance(
            procInstance1.getDefinitionId(), Collections.singletonMap("stringVar", 1.0));
    adjustProcessInstanceDatesAndDuration(procInstance2.getId(), referenceDate, 0, 2L);
  ...
  public void otherVariablesDoNotAffectResult() {
    ...
    // when
    ProcessReportDataDto reportData =
        createReportData(procInstance, VariableType.STRING, "stringVar1");
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
  ...
  public void distributeByDateVariableWorksForAllStaticUnits(final AggregateByDateUnit unit) {
    ...
    // then
    final ZonedDateTime truncatedReferenceDate =
        truncateToStartOfUnit(referenceDate, ChronoUnit.DAYS);
    final ZonedDateTime truncatedDateVariableValue =
        truncateToStartOfUnit(dateVariableValue, chronoUnit);
    // @formatter:off
    HyperMapAsserter.asserter()
        .processInstanceCount(1L)
        .processInstanceCountWithoutFilters(1L)
        ...
        .allSatisfy(
            resultEntries -> {
              ...
              assertThat(resultEntries.get(0))
                  ...
                  .isEqualTo(1000.);
              assertThat(
                      resultEntries.get(NUMBER_OF_DATA_POINTS_FOR_AUTOMATIC_INTERVAL_SELECTION - 1))
                  .extracting(MapResultEntryDto::getValue)
                  .isEqualTo(2000.);
            });
  }
  ...
  public void numberVariable_customBuckets() {
    ...
    final ProcessInstanceEngineDto procInstance2 =
        engineIntegrationExtension.startProcessInstance(
            procInstance1.getDefinitionId(), Collections.singletonMap("doubleVar", 200.0));
    adjustProcessInstanceDatesAndDuration(procInstance2.getId(), referenceDate, 0, 1L);

    final ProcessInstanceEngineDto procInstance3 =
        engineIntegrationExtension.startProcessInstance(
            procInstance1.getDefinitionId(), Collections.singletonMap("doubleVar", 300.0));
    ...
    reportData
        .getConfiguration()
        .setDistributeByCustomBucket(
            CustomBucketDto.builder().active(true).baseline(50.0).bucketSize(100.0).build());
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
  ...
  public void numberVariable_invalidBaseline_returnsEmptyResult() {
    ...
    reportData
        .getConfiguration()
        .setDistributeByCustomBucket(
            CustomBucketDto.builder().active(true).baseline(500.0).build());
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
  ...
  public void doubleVariable_bucketKeysHaveTwoDecimalPlaces() {
    ...
    // when
    ProcessReportDataDto reportData =
        createReportData(procInstance, VariableType.DOUBLE, "doubleVar");
    final ReportResultResponseDto<List<HyperMapResultEntryDto>> result =
        reportClient.evaluateHyperMapReport(reportData).getResult();
  ...
  public void missingVariablesAggregationWorksForUndefinedAndNullVariables() {
    ...
    final ProcessInstanceEngineDto procInstance3 =
        engineIntegrationExtension.startProcessInstance(
            procInstance1.getDefinitionId(), Collections.singletonMap("stringVar", null));
    ...
    final ProcessInstanceEngineDto procInstance4 =
        engineIntegrationExtension.startProcessInstance(
            ...
            Collections.singletonMap(
                "stringVar", new EngineVariableValue(null, VariableType.STRING.getId())));
  ...
  @SneakyThrows
  private String getExpectedKeyForVariableType(
      final VariableType variableType, final Object variableValue) {
    switch (variableType) {
      case STRING:
      case INTEGER:
      case BOOLEAN:
