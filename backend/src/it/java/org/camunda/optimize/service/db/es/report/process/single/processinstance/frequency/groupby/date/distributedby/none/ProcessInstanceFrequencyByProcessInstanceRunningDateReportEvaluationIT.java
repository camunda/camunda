...
public class ProcessInstanceFrequencyByProcessInstanceRunningDateReportEvaluationIT
  ...
  public void countRunningInstances_instancesFallIntoMultipleBuckets(
      final AggregateByDateUnit unit) {
    // given
    // first instance starts within a bucket (as opposed to on the "edge" of a bucket)
    final OffsetDateTime startOfFirstInstance =
        OffsetDateTime.parse("2020-06-15T12:00:00+02:00").withSecond(10);
    ...
    ProcessReportDataDto reportData =
        getGroupByRunningDateReportData(
            instance.getProcessDefinitionKey(), instance.getProcessDefinitionVersion(), unit);
    ...
    assertThat(resultData).isNotNull().hasSize(expectedNumberOfBuckets);

    // bucket keys exist for each unit between start date of first and end date of last instance
    final ZonedDateTime lastBucketStartDate =
        ...
        .forEach(
            i -> {
              final String expectedBucketKey =
                  convertToExpectedBucketKey(
                      startOfFirstInstance.plus(i, mapToChronoUnit(unit)), unit);
  ...
  public void countRunningInstances_runningInstancesOnly(final AggregateByDateUnit unit) {
    ...
    final OffsetDateTime startOfSecondInstance =
        OffsetDateTime.parse("2020-01-05T12:00:00+02:00").plus(bucketWidth);
    ...
    engineDatabaseExtension.changeProcessInstanceStartDate(
        instance2.getId(), startOfSecondInstance);
    importAllEngineEntitiesFromScratch();

    // when
    ProcessReportDataDto reportData =
        getGroupByRunningDateReportData(
            processDefinition.getKey(), processDefinition.getVersionAsString(), unit);
    ReportResultResponseDto<List<MapResultEntryDto>> result =
        reportClient.evaluateMapReport(reportData).getResult();
    ...
    assertThat(resultData.get(0).getKey())
        .isEqualTo(convertToExpectedBucketKey(startOfFirstInstance, unit));
    assertThat(resultData.get(1).getKey())
        .isEqualTo(convertToExpectedBucketKey(startOfSecondInstance, unit));
  }

  @SneakyThrows
  @ParameterizedTest
  @MethodSource("staticAggregateByDateUnits")
  public void countRunningInstances_latestStartDateAfterLatestEndDate(
      final AggregateByDateUnit unit) {
    ...
    final OffsetDateTime latestStartDate =
        OffsetDateTime.parse("2020-01-05T12:00:10+02:00").plus(bucketWidth.multipliedBy(3));
    ProcessDefinitionEngineDto processDefinition = deployUserTaskProcess();
    ProcessInstanceEngineDto instance1 =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    ProcessInstanceEngineDto instance2 =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    ProcessInstanceEngineDto instance3 =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    ...
    engineDatabaseExtension.changeProcessInstanceStartAndEndDate(
        instance2.getId(), earliestStartDate, earliestStartDate.plus(bucketWidth));
    ...
    assertThat(resultData.get(0).getKey())
        .isEqualTo(convertToExpectedBucketKey(earliestStartDate, unit));
    assertThat(resultData.get(3).getKey())
        .isEqualTo(convertToExpectedBucketKey(latestStartDate, unit));
  }

  @Override
  protected void assertStartDateResultMap(
        ...
        .forEach(
            i -> {
              final String expectedDateString =
                  localDateTimeToString(finalStartOfUnit.minus((i), unit));
  ...
  private ProcessReportDataDto getGroupByRunningDateReportData(
      final String key, final String version, final AggregateByDateUnit unit) {
  ...
  private String convertToExpectedBucketKey(
      final OffsetDateTime date, final AggregateByDateUnit unit) {
    return localDateTimeToString(truncateToStartOfUnit(date, mapToChronoUnit(unit)));
  }
