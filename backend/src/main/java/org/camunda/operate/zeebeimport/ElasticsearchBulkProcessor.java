  ...
  public void persistZeebeRecords(List<Record> zeebeRecords, ImportValueType importValueType) throws PersistenceException {
    ...
    logger.debug("Writing [{}] Zeebe records to Elasticsearch", zeebeRecords.size());
      ...
      case WORKFLOW_INSTANCE:
        Map<Long, List<RecordImpl<WorkflowInstanceRecordValueImpl>>> groupedWIRecords =
            zeebeRecords.stream()
              .map(obj -> (RecordImpl<WorkflowInstanceRecordValueImpl>) obj)
              .collect(Collectors.groupingBy(obj -> obj.getValue().getWorkflowInstanceKey()));
        listViewZeebeRecordProcessor.processWorkflowInstanceRecord(groupedWIRecords, bulkRequest);
        Map<Long, List<RecordImpl<WorkflowInstanceRecordValueImpl>>> groupedWIRecordsPerActivity =
            zeebeRecords.stream()
                .map(obj -> (RecordImpl<WorkflowInstanceRecordValueImpl>) obj)
