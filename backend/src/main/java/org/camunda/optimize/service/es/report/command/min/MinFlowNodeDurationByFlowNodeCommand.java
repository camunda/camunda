package org.camunda.optimize.service.es.report.command.min;
...
public class MinFlowNodeDurationByFlowNodeCommand extends FlowNodeGroupingCommand {
  ...
  private static final String MIN_DURATION_AGGREGATION = "aggregatedDuration";

  @Override
  protected MapReportResultDto evaluate() {

    logger.debug("Evaluating minimum flow node duration grouped by flow node report " +
  ...
  private Map<String, Long> processAggregations(Aggregations aggregations) {
    ...
    for (Terms.Bucket b : terms.getBuckets()) {
      InternalMin minimumDuration = b.getAggregations().get(MIN_DURATION_AGGREGATION);
      long roundedDuration = Math.round(minimumDuration.getValue());
