...
public class RunningProcessInstanceWriter {
  ...
  public List<ImportRequestDto> generateProcessInstanceImports(final List<ProcessInstanceDto> processInstanceDtos) {
    ...
    return processInstanceDtos.stream()
      .map(instance ->
             importRequestDtoFactory.createImportRequestForProcessInstance(instance, UPDATABLE_FIELDS, IMPORT_ITEM_NAME)
      )
      .toList();
  }
  ...
  public void importProcessInstancesForProcessDefinitionIds(final Map<String, Map<String, String>> definitionKeyToIdToStateMap) {
    for (Map.Entry<String, Map<String, String>> definitionKeyEntry : definitionKeyToIdToStateMap.entrySet()) {
      ...
      for (Map.Entry<String, String> definitionStateEntry : definitionKeyEntry.getValue().entrySet()) {
        indexRepository.createMissingIndices(PROCESS_INSTANCE_INDEX, READ_ONLY_ALIASES, Set.of(definitionKey));
        processInstanceRepository.updateProcessInstanceStateForProcessDefinitionId(
          IMPORT_ITEM_NAME, definitionKey, definitionStateEntry.getKey(), definitionStateEntry.getValue()
        );
      }
    }
  }
  ...
  public void importProcessInstancesForProcessDefinitionKeys(final Map<String, String> definitionKeyToNewStateMap) {
    indexRepository.createMissingIndices(PROCESS_INSTANCE_INDEX, READ_ONLY_ALIASES, definitionKeyToNewStateMap.keySet());
