...
public class ESVersionChecker {
  ...
  static {
    ...
    supportedVersions.add("8.7.0");
    supportedVersions.add("8.8.0");
  }
  ...
  public static void checkESVersionSupport(final RestHighLevelClient esClient,
    ...
    if (!isCurrentVersionSupported(currentVersion)) {
      if (doesVersionNeedWarning(currentVersion)) {
  ...
  public static boolean doesVersionNeedWarning(String currentVersion) {
    return (Integer.parseInt(getMajorVersionFrom(currentVersion))
      == Integer.parseInt(getMajorVersionFrom(getLatestSupportedESVersion())))
      && (Integer.parseInt(getMinorVersionFrom(currentVersion)) > Integer.parseInt(getMinorVersionFrom(getLatestSupportedESVersion())));
  }
