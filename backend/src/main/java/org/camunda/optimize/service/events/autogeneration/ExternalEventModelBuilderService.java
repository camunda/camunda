...
public class ExternalEventModelBuilderService {
  ...
  public AbstractFlowNodeBuilder<?, ?> createOrExtendModelWithExternalEventSource(final ProcessBuilder processBuilder,
    ...
    if (!canGenerateModelFromGraph(autogeneratedEventGraphDto)) {
    ...
    for (EventTypeDto rootNode : autogeneratedEventGraphDto.getStartEvents()) {
      final String nodeId = generateNodeId(rootNode);
      log.debug("Adding event with id {} to autogenerated model", nodeId);
      final AbstractFlowNodeBuilder<?, ?> nextBuilder;
      if (currentBuilder == null) {
        nextBuilder = addStartEvent(rootNode, nodeId, processBuilder);
      } else {
      ...
      builderToReturn = depthTraverseGraph(
        rootNode,
        autogeneratedEventGraphDto.getAdjacentEventTypesDtoMap()
          .get(rootNode)
          .getSucceedingEvents(),
    ...
    for (EventTypeDto nodeToAdd : nodesToAdd) {
      ...
      if (graphDto.getEndEvents().contains(nodeToAdd)) {
        ...
        if (!nodesExistInParallel(nodeToAdd, previouslyAddedNode, graphDto)) {
          ...
          depthTraverseOrConnect(graphDto, currentNodeBuilder, mappings, nodeToAdd, isFinalSourceInSeries);
        }
      }
    }
  ...
  private void depthTraverseOrConnect(final AutogeneratedEventGraphDto graphDto,
    ...
    if (!nonParallel.isEmpty()) {
      depthTraverseGraph(nodeToAdd, nonParallel, graphDto, currentNodeBuilder, mappings, isFinalSourceInSeries);
    } else {
  ...
  private AbstractFlowNodeBuilder<?, ?> addOrConnectToIntermediateEventNode(final AutogeneratedEventGraphDto graphDto,
    ...
    if (!nodeAlreadyAddedToModel(nodeId, currentNodeBuilder)) {
      ...
      mappings.put(nodeId, EventMappingDto.builder().start(nodeToAdd).build());
      depthTraverseOrConnect(graphDto, nextBuilder, mappings, nodeToAdd, isFinalSourceInSeries);
    } else {
  ...
  private boolean canGenerateModelFromGraph(final AutogeneratedEventGraphDto autogeneratedEventGraphDto) {
    ...
    if (autogeneratedEventGraphDto.getEndEvents().isEmpty()) {
      log.warn("Cannot generate an external events model as no eligible end events found");
      return false;
    }
    return true;
  }
