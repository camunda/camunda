...
public class CCSMIdentityService extends AbstractIdentityService {
    ...
    if (identity.users().isAvailable()) {
      ...
      if (token.isPresent()) {
        return new IdentitySearchResultResponseDto(
            searchForIdentityUsingSearchTerm(token.get(), searchString, maxResults));
      } else {
      ...
      log.debug(
          "Cannot search for identities because user search not available in Camunda identity.");
      return new IdentitySearchResultResponseDto(Collections.emptyList());
    }
  }
    ...
    if (identity.users().isAvailable()) {
      ...
      if (token.isPresent()) {
        return lowerCasedEmails.stream()
            .flatMap(
                email ->
                    searchForIdentityUsingSearchTerm(
                        token.get(), email, 1 // we are only expecting 1 matching user per email
                        )
                        .stream())
  ...
  private List<IdentityWithMetadataResponseDto> searchForIdentityUsingSearchTerm(
      final String token, final String searchString, final int maxResults) {
    if (identity.users().isAvailable()) {
      try {
        return identity.users().withAccessToken(token).search(searchString).stream()
            .limit(maxResults)
            .map(this::mapToUserDto)
