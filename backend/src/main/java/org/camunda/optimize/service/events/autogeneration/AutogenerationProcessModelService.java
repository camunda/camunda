...
public class AutogenerationProcessModelService {
  ...
  public AutogenerationProcessModelService(EventTraceStateServiceFactory eventTraceStateServiceFactory,
  ...
  public AutogenerationProcessModelDto generateModelFromEventSources(List<EventSourceEntryDto<?>> eventSources) {
    ...
    if (eventSources.stream().anyMatch(source -> EventSourceType.EXTERNAL.equals(source.getSourceType()))) {
      final List<EventSequenceCountDto> externalSequenceCounts = eventTraceStateService.getAllSequenceCounts();
      autogeneratedEventGraphDto = EventModelBuilderUtil.generateExternalEventGraph(externalSequenceCounts);
      sampleTraceListsByTracingId = getSampleTraceListsByTracingId(
    ...
    List<EventSourceEntryDto<?>> orderedEventSources = sortEventSourcesForModelOrder(
      eventSources,
      externalSampleTraceIds
    );
  ...
  private List<EventSourceEntryDto<?>> sortEventSourcesForModelOrder(final List<EventSourceEntryDto<?>> eventSources,
                                                                     final List<String> externalEventTracingIds) {
    if (eventSources.size() <= 1) {
      return eventSources;
    }
    final Map<String, EventSourceEntryDto<?>> eventSourceByIdentifier = eventSources.stream()
      .collect(toMap(EventSourceEntryDto::getSourceIdentifier, Function.identity()));
    final List<String> sampleCorrelationValuesForSources = getSampleCorrelationValuesForCamundaSources(eventSources);
    ...
    orderedEventSources.addAll(
      correlatedTraces
        ...
        .map(trace -> trace.getInstances()
          .stream()
          .map(instance -> eventSourceByIdentifier.get(instance.getSourceIdentifier()))
          .collect(Collectors.toList())).orElse(Collections.emptyList()));
    log.debug(
      "Trace found containing all following source identifiers in order: {}",
      orderedEventSources.stream().map(EventSourceEntryDto::getSourceIdentifier).collect(Collectors.toList())
    );
    ...
    List<String> identifiersForPlacedSources = orderedEventSources.stream()
  ...
  private void tryToAddUnplacedSourceInOrderedSources(final Map<String, EventSourceEntryDto<?>> sourceByIdentifier,
    ...
    if (correlatedInstancesForCorrelatedTrace.size() > 1) {
      final Optional<CorrelatedInstanceDto> instanceForUnplacedSource =
        correlatedInstancesForCorrelatedTrace
          .stream()
          .filter(instance -> instance.getSourceIdentifier().equals(unplacedSource.getSourceIdentifier()))
          .findFirst();
      ...
      if (unplacedSourceIndex == 0) {
        ...
        final EventSourceEntryDto<?> placedSource = sourceByIdentifier.get(
          correlatedInstancesForCorrelatedTrace.get(unplacedSourceIndex + 1).getSourceIdentifier());
      ...
      // We have now placed this source so can add it to our placed identifiers list to help future placements
  ...
  private List<CorrelatedInstanceDto> findCorrelatedInstancesContainingUnplacedSourceAndAtLeastOnePlaced(
    ...
    return correlatedTrace
      ...
      .filter(trace -> trace.getInstances()
        ...
        .anyMatch(identifier -> identifier.equals(unplacedSource.getSourceIdentifier())))
      .map(traceContainingUnplaced ->
             traceContainingUnplaced.getInstances().stream()
               .filter(instance -> identifiersForPlacedSources.contains(instance.getSourceIdentifier()) ||
  ...
  private List<String> getSampleCorrelationValuesForCamundaSources(final List<EventSourceEntryDto<?>> eventSources) {
    final List<CamundaEventSourceEntryDto> camundaSources = eventSources.stream()
      ...
      .collect(Collectors.toList());
    return correlatedInstanceService.getCorrelationValueSampleForCamundaEventSources(camundaSources);
  }

  private boolean isFinalSourceInSeries(final EventSourceEntryDto<?> source,
                                        List<EventSourceEntryDto<?>> allSources) {
    return allSources.indexOf(source) == allSources.size() - 1;
  }

  private Map<String, List<EventTypeDto>> getSampleTraceListsByTracingId(final List<EventTypeDto> startEvents,
    ...
    final List<EventTraceStateDto> sampleTraces =
      eventTraceStateService.getTracesContainingAtLeastOneEventFromEach(
        startEvents,
        endEvents,
        MAX_EXTERNAL_SAMPLE_RESULTS
      );
    return sampleTraces
      .stream()
      .collect(Collectors.toMap(
        EventTraceStateDto::getTraceId,
        trace -> trace.getEventTrace().stream()
