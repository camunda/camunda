...
public class AutogenerationProcessModelService {
  ...
  public AutogenerationProcessModelDto generateModelFromEventSources(List<EventSourceEntryDto> eventSources) {
    AutogenerationEventGraphDto autogenerationEventGraphDto = null;
    Map<String, List<EventTypeDto>> sampleTraceListsByTracingId = new HashMap<>();
    if (eventSources.stream().anyMatch(source -> EventSourceType.EXTERNAL.equals(source.getType()))) {
      final List<EventSequenceCountDto> externalSequenceCounts = eventTraceStateService.getAllSequenceCounts();
      autogenerationEventGraphDto = EventModelBuilderUtil.generateExternalEventGraph(externalSequenceCounts);
    ...
    List<EventSourceEntryDto> orderedEventSources = sortEventSourcesForModelOrder(eventSources, externalSampleTraceIds);
