...
public class ZeebeUserTaskImportService extends ZeebeProcessInstanceSubEntityImportService<ZeebeUserTaskRecordDto> {
  ...
  public static final Set<UserTaskIntent> INTENTS_TO_IMPORT = Set.of(
    CREATING,
    ASSIGNED,
    COMPLETED,
    CANCELED
  ...
  private void updateUserTaskDurations(final FlowNodeInstanceDto userTaskToAdd) {
    if (userTaskToAdd.getStartDate() != null && userTaskToAdd.getEndDate() != null) {
      userTaskToAdd.setTotalDurationInMs(userTaskToAdd.getStartDate().until(userTaskToAdd.getEndDate(), ChronoUnit.MILLIS));
    }
    ...
    if (userTaskToAdd.getStartDate() != null
      && (userTaskToAdd.getEndDate() != null || !userTaskToAdd.getAssigneeOperations().isEmpty())) {
      ...
      if (!userTaskToAdd.getAssigneeOperations().isEmpty()) {
        ...
        final List<OffsetDateTime> allUnclaimTimestamps = userTaskToAdd.getAssigneeOperations().stream()
          .filter(operation -> UNCLAIM_OPERATION_TYPE.toString().equals(operation.getOperationType()))
        ...
        final List<OffsetDateTime> allClaimTimestamps = userTaskToAdd.getAssigneeOperations().stream()
          .filter(operation -> CLAIM_OPERATION_TYPE.toString().equals(operation.getOperationType()))
          .map(AssigneeOperationDto::getTimestamp)
          .sorted(Comparator.naturalOrder())
          .toList();

        // Calculate idle time, which is the sum of differences between claim and unclaim timestamp pairs, ie (claim_n -
        // unclaim_n)
        ...
        // Calculate work time, which is the sum of differences between unclaim and previous claim timestamp pairs, ie
        // (unclaim_n+1 - claim_n)
        ...
        if (allUnclaimTimestamps.size() - allClaimTimestamps.size() == 2) {
          ...
          OffsetDateTime secondToLastUnclaim = allUnclaimTimestamps.get(allUnclaimTimestamps.size() - 2);
  ...
  private void updateUserTaskAssigneeOperations(final ZeebeUserTaskRecordDto zeebeUserTaskRecord,
                                                final FlowNodeInstanceDto flowNodeToAdd) {
    flowNodeToAdd.getAssigneeOperations().add(createAssigneeOperationDto(zeebeUserTaskRecord));
  }

  private AssigneeOperationDto createAssigneeOperationDto(final ZeebeUserTaskRecordDto zeebeUserTaskRecord) {
    return new AssigneeOperationDto()
      ...
      .setOperationType(StringUtil.isNullOrEmpty(zeebeUserTaskRecord.getValue().getAssignee())
