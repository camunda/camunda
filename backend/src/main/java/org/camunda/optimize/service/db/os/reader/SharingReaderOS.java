...
public class SharingReaderOS implements SharingReader {
  ...
  public Optional<ReportShareRestDto> getReportShare(String shareId) {
    ...
    GetRequest.Builder getReqBuilder = new GetRequest.Builder()
      .index(REPORT_SHARE_INDEX_NAME)
      .id(shareId);

    final String errorMessage = String.format("Could not fetch report share with id [%s]", shareId);
    GetResponse<ReportShareRestDto> getResponse = osClient.get(getReqBuilder, ReportShareRestDto.class, errorMessage);
  ...
  public Optional<DashboardShareRestDto> findDashboardShare(String shareId) {
    ...
    final String errorMessage = String.format("Could not fetch dashboard share with id [%s]", shareId);
    GetResponse<DashboardShareRestDto> getResponse = osClient.get(getReqBuilder, DashboardShareRestDto.class, errorMessage);
    if (getResponse.found()) {
      return Optional.ofNullable(getResponse.source());
    } else {
      return Optional.empty();
  ...
  public Optional<ReportShareRestDto> findShareForReport(String reportId) {
    ...
    BoolQuery.Builder boolQueryBuilder = new BoolQuery.Builder()
      .must(QueryDSL.term(ReportShareIndex.REPORT_ID, reportId));
  ...
  public Optional<DashboardShareRestDto> findShareForDashboard(String dashboardId) {
    ...
    SearchResponse<DashboardShareRestDto> searchResponse = performSearchShareForDashboardIdRequest(dashboardId);
  ...
  public long getShareCount(final String indexName) {
    final String errorMessage = String.format("Was not able to retrieve count for type: %s", indexName);
  ...
  private SearchResponse<DashboardShareRestDto> performSearchShareForDashboardIdRequest(String dashboardId) {
    ...
    final String errorMessage = String.format("Was not able to fetch share for dashboard with id [%s]", dashboardId);
    return osClient.search(searchReqBuilder, DashboardShareRestDto.class, errorMessage);
  }

  private <T> List<T> findSharesByQuery(BoolQuery query,
                                        final String index,
