...
@DisplayName("Invoice Approval Process Test")
public class InvoiceApprovalNestedTest {
  ...
  @Nested
  @DisplayName("Tests for exceptional paths")
  class ExceptionalPathTests {
    ...
    public void testArchiveSystemError() throws Exception {
      ...
      //  this should lead to the exception being thrown and the process to end up in the user task
      // to
      // handle the problem
      assertThatUserTask(byElementId("UserTask_ManuallyArchiveInvoice"))
          .isCreated()
          .hasCandidateGroup(
              "archive-team"); // probably not worth to test as it limits flexibility in model
      // changes
