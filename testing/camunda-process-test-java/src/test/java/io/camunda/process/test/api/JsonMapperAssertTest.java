...
public class JsonMapperAssertTest {
  ...
  @Test
  void shouldHaveVariableWithDeprecatedJsonMapper() {
    // given a mapper configured through the deprecated overload, which 8.9 still supports
    CamundaAssert.setJsonMapper(new io.camunda.zeebe.client.impl.ZeebeObjectMapper());
  ...
  @Test
  @CamundaAssertExpectFailure
  void shouldReportMismatchWithDeprecatedJsonMapper() {
    // given a mapper configured through the deprecated overload, which 8.9 still supports
    CamundaAssert.setJsonMapper(new io.camunda.zeebe.client.impl.ZeebeObjectMapper());
    ...
    // when the failure message is rendered, which serializes through the mapper as well
    Assertions.assertThatThrownBy(
            () ->
                CamundaAssert.assertThatProcessInstance(
                        ProcessInstanceSelectors.byKey(PROCESS_INSTANCE_KEY))
