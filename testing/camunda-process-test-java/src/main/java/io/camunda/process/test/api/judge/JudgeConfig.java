 ...
 *
 * <p>Configuration for the LLM judge used in judge assertions. Instances are immutable; {@code
...
public interface JudgeConfig {
   ...
   *
   * @return a new JudgeConfig instance with default settings Creates a config with default settings
   *     and no chat model. A chat model must be set via {@link
   ...
   *
   * @param customPrompt the custom evaluation criteria prompt, or {@code null} to use the default
   * @return a new JudgeConfig instance with the custom prompt Replaces only the default evaluation
   *     criteria (the "You are an impartial judge..." preamble). The system still controls
   *     expectation/value injection, the scoring rubric, and the JSON output format. Pass {@code
   *     null} to fall back to the default criteria.
   */
   ...
   * results or received as user input. Toggles whether Camunda document references found in the
   * asserted variable are downloaded and attached to the judge call as structured content blocks.
   * Disabled by default to avoid token cost.
   *
   ...
   *     MultimodalChatModelAdapter}; otherwise attachment is skipped with a warning and the judge
   *     sees only the raw variable JSON.
   */
