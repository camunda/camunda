...
public class CoverageCreator {
  ...
  public static Coverage createCoverage(
    ...
    enhanceSequenceFlowsByEventBaseGateway(takenSequenceFlows, takenElementInstances, model);
  ...
  public static List<Coverage> aggregateCoverages(
    ...
    final Map<String, List<Coverage>> coveragesByProcessDefinition = new HashMap<>();
    coverages.forEach(
        coverage -> {
          coveragesByProcessDefinition
              .computeIfAbsent(coverage.getProcessDefinitionId(), k -> new ArrayList<>())
              .add(coverage);
        });
        ...
        (processDefinitionId, coveragesForProcessInstance) -> {
          final List<String> takenElements =
              coveragesForProcessInstance.stream()
                  .flatMap(c -> c.getTakenElements().stream())
                  .distinct()
                  .collect(Collectors.toList());
          final List<String> takenSequenceFlows =
              coveragesForProcessInstance.stream()
   ...
   * @param model The process model containing definition information
   */
  private static void enhanceSequenceFlowsByEventBaseGateway(
      final List<String> takenSequenceFlows,
