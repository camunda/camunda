...
public class BucketBufferArrayTest {
  ...
  public void shouldCreateBucketArray() {
    ...
    assertThat(bucketBufferArray.getCapacity())
        .isEqualTo(
            BUCKET_BUFFER_HEADER_LENGTH
                + ALLOCATION_FACTOR * bucketBufferArray.getMaxBucketLength());
  }
  ...
  public void shouldAllocateNewBucketBufferOnCreatingBuckets() {
    ...
    assertThat(bucketBufferArray.getCapacity())
        .isEqualTo(bucketBufferArray.getMaxBucketBufferLength() * 2);
  ...
  public void shouldAllocateBucketOnBufferWhichHasFreeBuckets() {
    ...
    for (int i = ALLOCATION_FACTOR - 1; i >= ALLOCATION_FACTOR - 11; i--) {
      final int bucketOffset =
          i * bucketBufferArray.getMaxBucketLength()
              + BucketBufferArrayDescriptor.BUCKET_BUFFER_HEADER_LENGTH;
      ...
      bucketBufferArray.removeBlock(
          bucketAddress,
          bucketBufferArray.getFirstBlockOffset() + bucketBufferArray.getBlockLength());
    ...
    final int bucketOffset =
        (ALLOCATION_FACTOR - 11) * bucketBufferArray.getMaxBucketLength()
            + BucketBufferArrayDescriptor.BUCKET_BUFFER_HEADER_LENGTH;
  ...
  public void shouldIncreaseAddressArrayOnCreatingBuckets() {
    ...
    assertThat(newBucketAddress)
        .isEqualTo(getBucketAddress(32, bucketBufferArray.getFirstBucketOffset()));
  ...
  public void shouldCreateBlock() {
    ...
    assertThat(bucketBufferArray.getBucketLength(newBucketAddress))
        .isEqualTo(BUCKET_DATA_OFFSET + getBlockLength(MAX_KEY_LEN, MAX_VALUE_LEN));
    ...
    assertThat(bucketBufferArray.getBlockLength())
        .isEqualTo(getBlockLength(MAX_KEY_LEN, MAX_VALUE_LEN));

    final boolean keyEquals =
        bucketBufferArray.keyEquals(keyHandler, newBucketAddress, newBlockOffset);
  ...
  public void shouldAddMoreBlocksThanMinimalFitSize() {
    ...
    assertThat(bucketBufferArray.getBucketCount()).isEqualTo(1);
    assertThat(bucketBufferArray.getBucketLength(newBucketAddress))
        .isEqualTo(BUCKET_DATA_OFFSET + 2 * getBlockLength(MAX_KEY_LEN, MAX_VALUE_LEN));
  ...
  public void shouldOverflowBucketOnRelocate() {
    ...
    keyHandler.theKey = 10;
    final boolean keyEquals =
        bucketBufferArray.keyEquals(keyHandler, bucketOverflowPointer, newBlockOffset);
    assertThat(keyEquals).isTrue();
  ...
  public void shouldRelocateBlocksFromBucket() {
    ...
    assertThat(
            bucketBufferArray.keyEquals(keyHandler, newBucketAddress, newBlockOffset + blockLength))
        .isTrue();
    keyHandler.theKey = 10;
  ...
  public void shouldNotRelocateBlocksFromBucketIfNotFit() {
    ...
    expectedException.expectMessage(
        "Blocks can't be relocate from bucket "
            + firstBucketAddress
            + " to bucket "
            + newBucketAddress
            + ". Not enough space on destination bucket.");
    expectedException.expect(IllegalArgumentException.class);
  ...
  public void shouldOverflowBucket() {
    ...
    // then
    assertThat(bucketBufferArray.getBucketOverflowPointer(newBucketAddress))
        .isEqualTo(overflowBucketAddress);
  ...
  public void shouldUseOverflowBucketIfAddBlockOnFullBucket() {
    ...
    assertThat(bucketBufferArray.getBucketLength(overflowBucketAddress))
        .isEqualTo(BUCKET_DATA_OFFSET + getBlockLength(MAX_KEY_LEN, MAX_VALUE_LEN));
    assertThat(bucketBufferArray.getBucketFillCount(overflowBucketAddress)).isEqualTo(1);
    assertThat(bucketBufferArray.getBlockCount()).isEqualTo(3);

    final boolean keyEquals =
        bucketBufferArray.keyEquals(keyHandler, overflowBucketAddress, newBlockOffset);
    assertThat(keyEquals).isTrue();
  ...
  public void shouldOverflowOverflowBucket() {
    ...
    assertThat(bucketBufferArray.getBucketDepth(overflowBucketAddress)).isEqualTo(OVERFLOW_BUCKET);
    assertThat(bucketBufferArray.getBucketId(overflowBucketAddress)).isEqualTo(1);
    assertThat(bucketBufferArray.getBucketOverflowPointer(overflowBucketAddress))
        .isEqualTo(newOverflowBucketAddress);

    assertThat(bucketBufferArray.getBucketDepth(newOverflowBucketAddress))
        .isEqualTo(OVERFLOW_BUCKET);
  ...
  public void shouldRemoveOverflowBucketInBetween() {
    ...
    assertThat(bucketBufferArray.getBucketOverflowPointer(bucketAddress))
        .isEqualTo(firstOverflowBucketAddress);
    assertThat(bucketBufferArray.getBucketOverflowPointer(firstOverflowBucketAddress))
        .isEqualTo(thirdOverflowBucketAddress);
    assertThat(bucketBufferArray.getBucketOverflowPointer(secondOverflowBucketAddress))
        .isEqualTo(0);
  }
  ...
  @Test
  public void
      shouldReturnFalseIfIsRemovableBucketAfterOtherBucketsAreRemovedButHasOverflowBucketInOtherBuffer() {
    // given
    final long firstBucketAddress = bucketBufferArray.allocateNewBucket(1, 1);
  ...
  public void shouldReleaseBucketBufferIfEmptyAfterRemoveBucket() {
    ...
    // then
    final int lastBucketOffsetInFirstBuffer =
        BUCKET_BUFFER_HEADER_LENGTH + 31 * bucketBufferArray.getMaxBucketLength();
    assertThat(nextRemovableBucket).isEqualTo(getBucketAddress(0, lastBucketOffsetInFirstBuffer));
  ...
  public void shouldNotReleaseBucketBufferIfNotEmptyAfterRemoveBucket() {
    ...
    // then
    assertThat(nextRemovableBucket).isEqualTo(getBucketAddress(1, BUCKET_BUFFER_HEADER_LENGTH));
  ...
  public void shouldNotReleaseBucketBufferInBetween() {
    ...
    for (int i = 31; i >= 0; i--) {
      ...
      final int bucketOffset =
          BUCKET_BUFFER_HEADER_LENGTH + i * bucketBufferArray.getMaxBucketLength();
  ...
  public void shouldResolveNextRemovableAddressIfBucketBufferIsNotReleased() {
    ...
    final long nextRemovableBucket =
        bucketBufferArray.removeBucket(getBucketAddress(1, BUCKET_BUFFER_HEADER_LENGTH));
  ...
  public void shouldResolveNextRemovableAddressIfNextEmptyBucketBufferWasReleased() {
    ...
    final long nextRemovableBucket =
        bucketBufferArray.removeBucket(getBucketAddress(2, BUCKET_BUFFER_HEADER_LENGTH));
  ...
  @Test
  public void
      shouldReleaseBucketBufferInBetweenAndResizeAddressBufferAndResolveNextRemovableAddress() {
    // given
    ...
    final long nextRemovableBucket =
        bucketBufferArray.removeBucket(getBucketAddress(48, BUCKET_BUFFER_HEADER_LENGTH));
  ...
  public void shouldShrinkAddressBuffer() {
    ...
    // then realAddresses is shrinked -> since 31 (buffer count) is less then half of address buffer
    // length (=64)
  ...
  public void shouldNextNotFilledBucketBufferIdStartOnZero() {
    ...
    // then bucket is allocated in first bucket and last not full bucket buffer id is not updated
    // since bucket buffer is not full
    assertThat(bucketBufferArray.getBucketCount(0)).isEqualTo(1);
    assertThat(bucketBufferArray.nextNotFullBucketBuffer).isEqualTo(0);
  }
  ...
  private float getExpectedLoadFactor(float blockCount, int bucketCount) {
    return bucketCount == 0 ? 0 : blockCount / ((float) bucketCount * 2);
  }
