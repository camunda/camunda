...
public class FsTemporarySnapshotWriterTest
{
    ...
    {
        // given
        final byte[] checksumBytes = MessageDigest.getInstance(config.getChecksumAlgorithm()).digest(SNAPSHOT_DATA);
        ...
        assertThat(new String(Files.readAllBytes(checksumFile.toPath()))).isEqualTo(config.checksumContent(checksum, snapshotFile.getName()));
    }
}
