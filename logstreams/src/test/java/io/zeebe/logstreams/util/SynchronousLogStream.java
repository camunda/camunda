 ...
 *
 * We do not want to pollute the real log stream interface with synchronous method's.
 * Synchronous method's would be error prone and it is likely that
 * we use these synchronous methods in production then by mistake, which would fail due to our actor model.
 *
 * *Note:* Actor's are not allowed to join on a future.
 */
