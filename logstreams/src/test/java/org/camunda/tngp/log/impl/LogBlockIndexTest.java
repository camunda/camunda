...
public class LogBlockIndexTest
{
    ...
    public void shouldAddBlock()
    {
        ...
        for (int i = 0; i < capacity; i++)
        {
            ...
            assertThat(blockIndex.addBlock(virtPos, physPos)).isEqualTo(expectedSize);
    ...
    public void shouldNotAddBlockWithEqualPos()
    {
        ...
        exception.expectMessage("Illegal value for virtual position");
    ...
    public void shouldLookupBlocks()
    {
        ...
        for (int i = 0; i < capacity; i++)
        {
            final int virtPos = (i + 1) * 10;
            final int physPos = (i + 1) * 100;

            blockIndex.addBlock(virtPos, physPos);
        }
        ...
        for (int i = 0; i < capacity; i++)
        {
            final int expectedPhysPos = (i + 1) * 100;

            for (int j = 0; j < 10; j++)
            {
                final int vpos = ((i + 1) * 10) + j;

                assertThat(blockIndex.lookupBlockAddress(vpos)).isEqualTo(expectedPhysPos);
            }
        }
    }
}
