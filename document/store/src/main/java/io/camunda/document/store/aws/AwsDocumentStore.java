...
public class AwsDocumentStore implements DocumentStore {
  ...
  private static final Logger log = LoggerFactory.getLogger(AwsDocumentStore.class);
  ...
  private Either<DocumentError, Void> verifyContentHashInternal(
    ...
    try {
      ...
      if (!documentInfo.metadata().containsKey(CONTENT_HASH_METADATA_KEY.toLowerCase())) {
  ...
  private Either<DocumentError, DocumentReference> uploadDocument(
    ...
    final var updatedMetadata = buildMetadataWithContentHash(request, documentId, contentHash);

    final PutObjectRequest putObjectRequest =
        PutObjectRequest.builder()
            .key(resolveKey(documentId))
            .bucket(bucketName)
            .metadata(toS3MetaData(updatedMetadata))
