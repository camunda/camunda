...
public final class CommandProcessorImpl<T extends UnifiedRecordValue>
  ...
  @Override
  public void processRecord(final TypedRecord<T> command, final SideEffects sideEffects) {

    entityKey = command.getKey();

    final boolean shouldRespond = wrappedProcessor.onCommand(command, this, sideEffects);
    ...
    if (isAccepted) {
      ...
      wrappedProcessor.afterAccept(
          commandWriter, stateWriter, entityKey, newState, updatedValue, sideEffects);
      if (respond) {
