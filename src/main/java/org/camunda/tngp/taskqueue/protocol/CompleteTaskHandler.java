...
public class CompleteTaskHandler implements SbeRequestHandler, ResponseCompletionHandler
{
    ...
    protected final HashIndex taskInstanceIndex;
    ...
    public CompleteTaskHandler(TaskQueueContext context)
    {
        log = context.getLog();
        writeBuffer = log.getWriteBuffer();
        taskInstanceIndex = context.getTaskInstanceIndex().getIndex();
    }
            ...
            final int sbeSchemaVersion)
    {
        ...
        if (response.allocate(ackResponseLength()))
        {
            ...
            if (lastTaskPosition != -1)
            {
                ...
                {
                    final long claimedPosition = claimLogFragment(payloadLength, taskInstanceReader);
                    ...
                    if (claimedPosition >= 0)
                    {
                        ...
                        writeAck(response, taskId);
                        response.defer(claimedPosition, this, null);
                    }
                    else
                    {
    ...
    protected void writeAck(DeferredResponse response, long taskInstanceId)
    {
    ...
    protected long claimLogFragment(final int payloadLength, final TaskInstanceReader taskInstanceReader)
    {
        ...
        do
        {
            claimedPosition = writeBuffer.claim(claimedFragment, taskInstanceLength);
        }
