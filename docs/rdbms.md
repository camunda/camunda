...
The new RDBMS module in Camunda 8 introduces support for relational databases as an alternative to
Elasticsearch (ES) and OpenSearch (OS) for the second level storage.

Key Features & Benefits:

* Seamless RDBMS Integration: Supports SQL-based databases, enabling customers to leverage existing
  DBA expertise.
* Simplified Onboarding & Testing: Removes ES/OS dependency, allowing in-memory databases for local
  development.
...
Like for all other modules in camunda as well, spring should not be used in any spring module.
The only place where spring is allowed is in dist folder to bring together the components for the
application.
...
flowchart TD
  ...
  subgraph CAMUNDA ["Camunda"]
    ...
    subgraph broker["`**Broker (Partition)** - _Command Side_`"]
      ...
      RDBMS_EXP["RDBMS Exporter"]
      RECORD_EXPORTER --> EXPORT_CONTAINER --> RDBMS_EXP
    end
