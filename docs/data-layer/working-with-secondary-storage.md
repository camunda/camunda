...
Before starting any data-layer-affecting work, answer the following questions in the PDP or design
document. Treat these as a checklist — missing answers are a signal that the design is not yet
ready.
...
Microbenchmarks are located in `microbenchmarks/` and use
the [Java Microbenchmark Harness (JMH)](https://github.com/openjdk/jmh).
For full build instructions, available benchmarks, JMH options (iterations, forks, profilers, GC
analysis, result export), and tips for accurate results, see [
...
Compare the `score` and `scoreError` fields between `before.json` and `after.json`. A change is
significant only if it exceeds the error margin. Store both JSON files as PR artifacts or paste the
table output in the PR description.

### 2.3 Load Tests (End-to-End)

Load tests run against a Kubernetes cluster and test realistic workloads. They support all secondary
storage backends:
...
See [Manual set up a load test](../../load-tests/setup/README.md) for full setup instructions,
including how to build and push a custom Docker image with your changes.
...
3. Compare the two Grafana snapshots side-by-side. Include screenshots or a metrics link in the PR
   description.
...
Look for `Seq Scan` (full table scan) where you expect an `Index Scan`. A sequential scan on a large
table is a performance problem that must be fixed before the change is merged.
...
- **`top_hits` inside terms aggregation**: Especially dangerous on high-cardinality fields. See §4.2
  and §5.2.
...
**Rule:** Any new aggregation on a high-cardinality field requires a benchmark before merging (see
§2).
