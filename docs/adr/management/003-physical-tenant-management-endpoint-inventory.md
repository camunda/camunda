...
| `clock` (`ActorClockEndpoint`)                                                                         | Unchanged: mutates the node-global `ControlledActorClock` (all tenants + actor timers; requires controlled-clock mode) | Per-PT overlay on that tenant's stream clocks                            | Every partition already consumes time through its own `ControllableStreamClock` overlay (pin/offset modification) over the `ActorClock` source (`LogStreamPartitionTransitionStep.java:54-56`) — the same seam the `/v2/clock` engine feature mutates. Per-PT pin/add/reset applies the modification to the tenant's partitions' stream clocks and leaves the shared `ActorClock` untouched. Limitation (as with `/v2/clock`): governs stream/processing time; actor-scheduler timers stay node-global. |
