...
- Secondary storage schema initialization needs rework to not block startup when one physical tenant's database is uninitialized.
  - How that rework behaves — what startup waits for and what "ready" asserts — is decided in [004](004-per-physical-tenant-schema-initialization.md), which keeps D2's single-tenant clause intact.
