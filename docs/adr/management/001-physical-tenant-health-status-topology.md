...
- Secondary storage schema initialization needs rework to not block startup when one physical tenant's database is uninitialized.
  ...
  - How a degraded tenant is reported outside the probes, on the catch-all `/actuator/health`, is decided in [005](005-per-physical-tenant-schema-initialization.md) D7.
