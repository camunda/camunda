...
- Secondary storage schema initialization needs rework to not block startup when one physical tenant's database is uninitialized.
  - How that rework behaves — what startup waits for, what "ready" asserts, and the refinement of D2's single-tenant clause — is decided in [004](004-per-physical-tenant-schema-initialization.md).
