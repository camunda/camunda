...
* **Scalability:** The module is designed to handle large volumes of data efficiently, making it
  suitable for high-throughput applications. It should scales horizontally with the number of
...
| `modifiactionPlan`  | The optional description of a process instance modification. Only used for type `MODIFY_PROCESS_INSTANCE` |
...
collection of itemKeys eventually needs to be split in severa chunks when the number of itemKeys
