...
**D6. The merge lands now; assignment is gated on [#56652](https://github.com/camunda/camunda/issues/56652).** The merge (D3/D4) is immediately effective on the #57202 wiring and strictly improves live behavior. Assignment's narrowing (removing unassigned catalog entries) must wait: while the enable state is generated from the root `BrokerCfg` only, a narrowed-away id stays *enabled* on the tenant's partitions with no descriptor, becoming a `BlockingExporter` that stalls exporting and log compaction — turning an isolation hazard into an availability incident. Until #56652 makes the enable state per tenant, inherit-all is an accepted, documented interim (it is today's behavior, not a regression).
