...
- A new, small piece of shared infrastructure (the propagating executor) must be maintained, and **every** async offload of an authorization read must go through the managed executor — a bare `supplyAsync` that bypasses it silently reintroduces the bug. This constraint should be enforced by a test and called out for reviewers.
