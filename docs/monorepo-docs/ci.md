...
This page has information and answers questions around how the [C8 monorepo CI](https://github.com/camunda/camunda/actions) and related tools like Renovate work. It should serve as a knowledge base including FAQ for Camundi and external contributors.

For release process information, see [Release Process](./release.md).
...
New reports of issues need to be checked against the [GitHub Issues](https://github.com/camunda/camunda/issues) to avoid duplication: new occurrences of existing issues need to be reported in comments, otherwise raise a new issue labelled `area/build` or reach out via Slack to the Monorepo CI DRI.
...
1. **Impacted functionality:**
   - Highest severity for issues related to workflows that are:
     ...
     - part of the GitHub merge queue to `main`
     - run on `main` branch
...
A: Search the open [GitHub Issues](https://github.com/camunda/camunda/issues) with the failure message to see if the problem is known: If you find an issue for the same problem, leave a comment with the new occurrence. Otherwise raise a new issue labelled `area/build` to start tracking that CI failure or reach out via Slack to the Monorepo CI DRI.

**Q: How to deal with flaky tests that block CI?**

A: Disable the flaky test(s) and comment on existing ticket or create a new one that the flaky test needs to be re-enabled after fixing it. No single test can be more important than the stability of the remaining CI system impacting dozens of developers.
...
A: You can find the up-to-date list [here](https://github.com/camunda/camunda/rules?ref=refs%2Fheads%2Fmain):
- `check-results` check from [ci.yml](https://github.com/camunda/camunda/blob/df3a8296085cb7bbc1065d86e43f2b77bd02b017/.github/workflows/ci.yml#L158) (Unified CI)
...
GitHub Actions pipeline code should be de-duplicated for the same task and moved out from [ci.yml](https://github.com/camunda/camunda/blob/main/.github/workflows/ci.yml) into other reusable workflows named `ci-<subtask>.yml` or composite actions to keep the `ci.yml` short and lean.
...
- high stability (low flakiness)
  ...
  - handle flaky tests gracefully, options:
    a. retry them 3-5 times while staying in the timeout and report them via [detailed test statistics API](https://github.com/camunda/camunda/pull/26715) to CI health
    b. disable them and create a ticket to fix them long-term
...
Suite names must follow a naming convention of `{componentName}{team}TestSuite`. The composite of the component and the team is used by the CI job to select which component and team to run the tests for. For example, `OperateCoreFeaturesTestSuite` is used to run Core Features tests on Operate.
...
For example, Core Features Unit Tests for Tasklist would appear as:
...
[Renovate](https://docs.renovatebot.com/) is a bot and GitHub app that automates dependency updates in software projects by scanning the source code for outdated libraries and applications, then creating Pull Requests to upgrade them to the latest versions, which helps keep the project secure and up-to-date.
...
We also want Renovate to automatically merge dependency updates when CI is green and automated tests are passing. Assuming a nearly complete test coverage the efficiency gains outweigh the risks. This is achieved by Renovate requesting to put every Pull Request into the GitHub Merge Queue - GitHub will then ensure that [required status checks](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-protected-branches/about-protected-branches#require-status-checks-before-merging) pass before merging the PR.
...
To achieve that for CI jobs we can collect metrics like build times, build failures and information about the hardware/runner via the [CI Analytics framework](https://confluence.camunda.com/display/HAN/CI+Analytics). See how to instrument GHA workflows for metrics collection. We use the collected data for visualizations to get an overview of the CI health.
...
2. Use self-hosted runners (with `-default` name suffix) when a workflow needs:
   i. more resources (memory, CPU) than available on GitHub-hosted runners
   ii. ARM CPU architecture
...
- **size**: 10 GiB
  - this is small for a monorepo use case with Java, NodeJS, and many open PRs
...
For more information about release-related backporting, see [Release Process - Backporting Guidelines](./release.md#backporting-guidelines).

## Slack Notifications

All CI workflows in the [camunda/camunda monorepo](https://github.com/camunda/camunda) must use the ["C8 Monorepo Notifications" Slack app](https://api.slack.com/apps/A086JM3CTB7/incoming-webhooks). Messages to Slack should be sent via webhooks. The webhook URLs are secrets and stored in [Vault](https://vault.int.camunda.com/ui/vault/secrets/secret/kv/products%2Fcamunda%2Fci%2Fgithub-actions) for each Slack channel.
...
Tests can be viewed as "flaky" when they are not consistently passing although neither the source code, nor the test code, nor the environment has been meaningfully changed.
...
GitHub Action workflows with Maven testing Java code should use the flaky-test-extractor-maven-plugin and report the resulting [detailed flaky test statistics](https://github.com/camunda/camunda/issues/26930) to our CI health database.
...
- if a test succeeds at least once during the retries, it is classified as "flaky" by this plugin
  - flaky tests get reported via CI Health metrics and can be viewed on [the Flaky tests dashboard](https://dashboard.int.camunda.com/d/ae2j69npxh3b4f/flaky-tests-camunda-camunda-monorepo)
...
The scan is performed by a GitHub Actions workflow for:
...
There can be many factors influencing that and it is sometimes hard to find the root causes. Below list should provide guidance:
