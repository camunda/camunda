...
- Picker rendering: CSL does NOT ship a working login picker — its `OidcWebapp` chain sets a custom `AuthenticationEntryPoint`, which trips Spring Security 7's gate in `DefaultLoginPageConfigurer.configure()` (the auto-picker is only added when `exceptionHandling.authenticationEntryPoint == null`). So neither the unprefixed `/login` nor a prefixed `/physical-tenant/<id>/login` gets the auto-picker out of the box. The PoC therefore renders pickers in two places: the unprefixed `/login` is served by `PhysicalTenantLoginPageController` (lists every registration in the standard `ClientRegistrationRepository`, no `providers.assigned` filtering — accepted trade-off), and the PT-prefixed `prefix + /login` is rendered by an explicit `DefaultLoginPageGeneratingFilter` instance installed in `PerTenantSecurityChainFactory#buildWebappChain` via `addFilterAfter(picker, CsrfFilter.class)`, configured with the PT's per-tenant `ClientRegistrationRepository`. The clean upstream fix is for CSL to either let its `OidcWebapp` chain pass through the auto-picker (drop or restructure the custom entry point so the configurer's gate isn't tripped) or to expose a per-chain picker hook the host can configure.
