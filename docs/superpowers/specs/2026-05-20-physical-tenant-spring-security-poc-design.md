...
- Picker rendering: CSL does NOT ship a working login picker — its `OidcWebapp` chain sets a custom `AuthenticationEntryPoint`, which trips Spring Security 7's gate in `DefaultLoginPageConfigurer.configure()` (the auto-picker is only added when `exceptionHandling.authenticationEntryPoint == null`). So the unprefixed `/login` has no picker out of the box. On the **prefixed PT chains** the PoC installs Spring Security's standard `DefaultLoginPageGeneratingFilter` explicitly in `PerTenantSecurityChainFactory#buildWebappChain` via `addFilterAfter(picker, CsrfFilter.class)`, populated from the PT's per-tenant `ClientRegistrationRepository` — that picker IS Spring's standard class, just installed by hand because the auto-pipeline's gate is incompatible with PT-prefixed routing. On the **unprefixed `/login`** the PoC accepts that no picker exists until the CSL fix lands (see Task 20 follow-up).
- **CSL follow-up (Task 20, tracked upstream as [camunda/camunda-security-library#269](https://github.com/camunda/camunda-security-library/issues/269)):** install `DefaultLoginPageGeneratingFilter` in CSL's `OidcWebappSecurityConfiguration` so the unprefixed `/login` works for multi-IdP deployments without each host having to render its own picker. Same `addFilterAfter(picker, CsrfFilter.class)` pattern this PoC uses for PT chains; install before the `.exceptionHandling(...)` step that registers the custom entry point.
