...
- The `PhysicalTenantLoginPageController` was removed entirely. The unprefixed `/login` is served by CSL's `OidcWebapp` chain whose `DefaultLoginPageGeneratingFilter` lists every registration in the standard `ClientRegistrationRepository` (no `providers.assigned` filtering — accepted trade-off). On the prefixed PT chains, we install our own `DefaultLoginPageGeneratingFilter` instance via `http.addFilterAfter(picker, CsrfFilter.class)` in `PerTenantSecurityChainFactory#buildWebappChain`, configured with `loginPageUrl = prefix + "/login"` and OAuth2 entries derived from the PT's `ClientRegistrationRepository` (PT-scoped to `providers.assigned`). The root cause Spring Security 7's auto-picker did NOT activate on the PT chain was discovered while debugging this: `DefaultLoginPageConfigurer.configure()` only adds the auto-picker when `exceptionHandling.authenticationEntryPoint == null`, and we set a custom entry point to route to the per-PT picker URL. Installing the filter manually is the correct workaround.
