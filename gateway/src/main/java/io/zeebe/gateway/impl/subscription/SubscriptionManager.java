...
import io.zeebe.gateway.impl.record.RecordMetadataImpl;
import io.zeebe.gateway.impl.record.UntypedRecordImpl;
import io.zeebe.gateway.impl.subscription.job.JobSubscriberGroup;
import io.zeebe.gateway.impl.subscription.job.JobSubscriptionSpec;
import io.zeebe.gateway.impl.subscription.topic.TopicSubscriber;
import io.zeebe.gateway.impl.subscription.topic.TopicSubscriberGroup;
import io.zeebe.gateway.impl.subscription.topic.TopicSubscriptionSpec;
...
public class SubscriptionManager extends Actor
    implements SubscribedEventHandler, TransportListener {
  ...
  final IdleStrategy idleStrategy =
      new BackoffIdleStrategy(1000, 100, 1, TimeUnit.MILLISECONDS.toNanos(1));
  ...
  protected void onActorStarting() {
    final SubscribedRecordCollector taskCollector =
        new SubscribedRecordCollector(this, client.getObjectMapper());
  ...
  public boolean onEvent(
    ...
    if (subscribers != null) {
      ...
      if (subscriber == null) {
        if (subscribers.isAnySubscriberOpeningOn(partitionId)) {
          ...
          // and we haven't processed the response yet (=> therefore subscriber is not registered
          // yet),
    ...
    if (subscriber != null && subscriber.isOpen()) {
      ...
      LOGGER.debug(
          "Ignoring event event {} for subscription [type={}, partition={}, key={}]",
          event,
          type,
