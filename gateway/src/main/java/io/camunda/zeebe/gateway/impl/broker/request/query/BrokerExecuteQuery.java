 ...
 */
package io.camunda.zeebe.gateway.impl.broker.request.query;
...
public abstract class BrokerExecuteQuery<T> extends BrokerRequest<T> {
  ...
  public BrokerExecuteQuery(final ValueType valueType, final Intent intent) {
  ...
  protected boolean isRejection() {
    // todo: consider if this requires a new RecordType.QUERY_REJECTION (i'd rather not avoid it))
