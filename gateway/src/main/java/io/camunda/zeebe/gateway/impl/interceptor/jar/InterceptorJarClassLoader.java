 ...
 */
public final class InterceptorJarClassLoader extends URLClassLoader {
  private static final Logger LOGGER = LoggerFactory.getLogger(InterceptorJarClassLoader.class);

  private static final String JAVA_PACKAGE_PREFIX = "java.";
  private static final String JAR_URL_FORMAT = "jar:%s!/";

  private InterceptorJarClassLoader(final URL[] urls) {
    super(urls);
  }

  static InterceptorJarClassLoader ofPath(final Path jarPath) throws InterceptorJarLoadException {
    final URL jarUrl;
    ...
    try {
      ...
      throw new InterceptorJarLoadException(jarPath, "bad JAR url", e);
    }

    return new InterceptorJarClassLoader(new URL[] {jarUrl});
  }
