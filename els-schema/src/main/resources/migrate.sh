...
checkNoSchemaExists(){
  $RESTCLIENT --request GET --url $ES/operate-* | grep "operate" &> /dev/null || success_exit "No operate schema exists."
}

checkIsPreviousVersionInstalled(){
  $RESTCLIENT --request GET --url $ES/operate-*-${schema.previous_version}* | grep "${schema.previous_version}" &> /dev/null || error_exit "Can't migrate current schema to '${schema.version}'. Only migration from ${schema.previous_version} to ${schema.version} is supported."
}

checkIsCurrentVersionAlreadyInstalled(){
  $RESTCLIENT --request GET --url $ES/operate-*-${schema.version}* | grep "${schema.version}" &> /dev/null && success_exit "Schema version '${schema.version}' exists already."
}
