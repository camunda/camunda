...
public class TopicCreationService extends Actor
    implements Service<TopicCreationService>, KnownTopicsListener, TopologyPartitionListener {
  ...
  private void computeStateDifferences(
    ...
    final ActorFuture<List<PendingTopic>> pendingTopicsFuture =
        knownTopics.queryTopics(topics -> computePendingTopics(topics, currentState));
              ...
              if (filterTopicName == null || filterTopicName.equals(topicName)) {
                if (pendingTopic.getMissingPartitions() > 0) {
                  if (!pendingTopicCreationRequests.contains(topicName)) {
                    LOG.debug(
                        "Creating {} partitions for topic {}",
                        pendingTopic.getMissingPartitions(),
                        topicName);
                    ...
                    actor.runDelayed(
                        PENDING_TIMEOUT, () -> pendingTopicCreationRequests.remove(topicName));
                  }
                } else {
                  if (!pendingTopicCompletions.contains(topicName)) {
                    ...
                    pendingTopic
                        .getPartitionIds()
                        .forEach(id -> eventPartitionIds.add().setValue(id));

                    actor.runUntilDone(
                        () ->
                            writeEvent(
                                pendingTopic.getKey(), TopicIntent.CREATE_COMPLETE, topicEvent));

                    pendingTopicCreationRequests.remove(topicName);
                    pendingTopicCompletions.add(topicName);
                    actor.runDelayed(
                        PENDING_TIMEOUT, () -> pendingTopicCompletions.remove(topicName));
                    LOG.debug(
                        "Topic {} with {} partition(s) and replication factor {} created",
                        topicName,
                        partitionCount,
                        replicationFactor);
                  }
  ...
  private List<PendingTopic> computePendingTopics(
      final Iterable<TopicInfo> topics, final ClusterPartitionState currentState) {
    ...
    actor.runOnCompletion(
        ...
        (id, error) -> {
          if (error == null) {
            ...
            LOG.error(
                "Failed to get new partition id for topic {}", pendingTopic.getTopicName(), error);
          }
        });
  }

  private void sendCreatePartitionRequest(
      final PendingTopic pendingTopic, final Integer partitionId) {
    final PartitionInfo newPartition =
        new PartitionInfo(
            pendingTopic.getTopicNameBuffer(), partitionId, pendingTopic.getReplicationFactor());
    final ActorFuture<NodeInfo> nextSocketAddressFuture =
        nodeSelector.getNextSocketAddress(newPartition);
    actor.runOnCompletion(
        ...
        (nodeInfo, error) -> {
          if (error == null) {
            ...
            LOG.error(
                "Problem in resolving next node address to create partition {} for topic {}",
                partitionId,
                pendingTopic.getTopicName(),
                error);
          }
        });
  }
  ...
  private void sendCreatePartitionRequest(
      final PendingTopic pendingTopic, final Integer partitionId, final NodeInfo nodeInfo) {
    ...
    LOG.debug(
        "Send create partition request for topic {} to node {} with partitionId={}",
        pendingTopic.getTopicName(),
    ...
    actor.runOnCompletion(
        ...
        (createPartitionResponse, error) -> {
          if (error == null) {
            LOG.info(
                "Partition {} for topic {} created on node {}",
                partitionId,
                pendingTopic.getTopicName(),
                nodeInfo.getManagementApiAddress());
          } else {
            LOG.warn(
                "Failed to create partition {} for topic {} on node {}",
                partitionId,
                pendingTopic.getTopicName(),
                nodeInfo.getManagementApiAddress(),
                error);
