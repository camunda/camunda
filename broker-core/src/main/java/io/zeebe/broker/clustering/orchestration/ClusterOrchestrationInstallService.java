...
public class ClusterOrchestrationInstallService implements Service<Void> {
  ...
  private final Injector<ControlMessageHandlerManager> controlMessageHandlerManagerInjector =
      new Injector<>();
  private final Injector<ServerTransport> transportInjector = new Injector<>();

  private final ServiceGroupReference<Partition> systemLeaderGroupReference =
  ...
  private void installClusterOrchestrationServices(
    ...
    compositeInstall
        .createService(ID_GENERATOR_SERVICE_NAME, idGenerator)
        .dependency(
            serverTransport(CLIENT_API_SERVER_NAME), idGenerator.getClientApiTransportInjector())
        .dependency(
            STREAM_PROCESSOR_SERVICE_FACTORY,
            idGenerator.getStreamProcessorServiceFactoryInjector())
    ...
    compositeInstall
        .createService(TOPIC_CREATION_SERVICE_NAME, topicCreationService)
        ...
        .dependency(
            ...
            topicCreationService.getManagementClientApiInjector())
        .install();
  ...
  private void removeClusterOrchestrationServices(
      final ServiceName<Partition> partitionServiceName, final Partition partition) {
