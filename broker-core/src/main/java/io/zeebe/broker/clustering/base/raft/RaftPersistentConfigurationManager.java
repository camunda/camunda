...
public class RaftPersistentConfigurationManager extends Actor
{
    ...
    private void readConfigurations(String dataDirectoryName, int offset)
    {
        ...
        for (File partitionDirectory : partitionDirectories)
        {
            ...
            if (configFile.exists())
            {
                ...
                configurations.add(new RaftPersistentConfiguration(configFile, logDirectory, snapshotsDirectory));
        ...
        List<SocketAddress> members)
    {
        ...
        actor.run(() ->
        {
            ...
            {
                final String partitionName = String.format("%s-%d", BufferUtil.bufferAsString(topicName), partitionId);

                final int assignedDataDirOffset = assignDataDirectory();
                final String assignedDataDirectoryName = dataConfiguration.getDirectories()[assignedDataDirOffset];
                ...
                try
                {
                    ...
                    final RaftPersistentConfiguration storage = new RaftPersistentConfiguration(metafile, logDirectory, snapshotDirectory);

                    storage.setTopicName(topicName)
