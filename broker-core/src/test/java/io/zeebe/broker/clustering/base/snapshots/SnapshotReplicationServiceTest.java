...
public class SnapshotReplicationServiceTest
{
    ...
    // TODO: currently it's manually synchronized to the private retry interval in SnapshotReplicationService
    ...
    public void shouldRetryIfLeaderNotInTopologyYet() throws Exception
    {
        ...
        // when
        final ListSnapshotsRequest sentRequest = (ListSnapshotsRequest) output.getLastRequest().getRequest();
    ...
    {
        // given
        final ErrorResponse errorResponse = new ErrorResponse().setCode(ErrorResponseCode.READ_ERROR).setData("could not read");
        final SnapshotMetadata[] snapshots = new SnapshotMetadata[]{
                createSnapshot("foo", 3L, "foo"),
                createSnapshot("bar", 6L, "bar")
        };
        ...
        // then
        assertThat(output.getLastRequest().getTemplateId()).isEqualTo(FetchSnapshotChunkRequestEncoder.TEMPLATE_ID);
    ...
    public void shouldReplicationSnapshot() throws Exception
    {
        ...
        // when
        FetchSnapshotChunkRequest request = (FetchSnapshotChunkRequest)output.getLastRequest().getRequest();
    ...
    private FetchSnapshotChunkResponse generateFetchSnapshotChunkResponse(final String contents, final FetchSnapshotChunkRequest request)
    {
    ...
    private ListSnapshotsResponse generateListSnapshotsResponse(final SnapshotMetadata[] snapshots)
    {
        ...
        {
            response.addSnapshot(snapshot.getName(), snapshot.getPosition(), snapshot.getChecksum(), snapshot.getSize());
        }

        return response;
    }

    private SnapshotMetadata createSnapshot(final String name, final long position, final String contents) throws Exception
    {
        final byte[] data = getBytes(contents);
    ...
    {
        return new NodeInfo(new SocketAddress(host, port),
