...
public class JobSubscriptionTest {
  ...
  public void shouldAddJobSubscription() {
    ...
    assertThat(jobStates)
        .containsExactly(
            JobIntent.CREATE, JobIntent.CREATED, JobIntent.ACTIVATE, JobIntent.ACTIVATED);
  }
  ...
  public void shouldContainSourceRecordPosition() {
    ...
    // then
    final SubscribedRecord activatedJobFromSubscription =
        apiRule.subscribedEvents().findAny().get();
    ...
    final SubscribedRecord activateJobCommand =
        testClient.receiveFirstJobCommand(JobIntent.ACTIVATE);
    ...
    assertThat(activatedJobFromSubscription.sourceRecordPosition())
        .isEqualTo(activateJobCommand.position());
  ...
  public void shouldNoLongerActivateJobsAfterRemoval() throws InterruptedException {
    ...
    final List<SubscribedRecord> receivedEvents =
        apiRule.subscribedEvents().limit(eventsAvailable).collect(Collectors.toList());

    assertThat(receivedEvents).hasSize(2);
    assertThat(receivedEvents)
        .allMatch(e -> e.subscriptionType() == SubscriptionType.TOPIC_SUBSCRIPTION);
    assertThat(receivedEvents)
        .extracting(r -> r.intent())
  ...
  public void shouldRejectSubscriptionWithZeroCredits() {
    ...
    assertThat(errorResponse.getErrorData())
        .isEqualTo("Cannot add job subscription. subscription credits must be greater than 0");
  }
  ...
  public void shouldRejectSubscriptionWithoutWorker() {
    ...
    assertThat(errorResponse.getErrorCode()).isEqualTo(ErrorCode.REQUEST_PROCESSING_FAILURE);
    assertThat(errorResponse.getErrorData())
        .isEqualTo("Cannot add job subscription. worker must not be empty");
  }
  ...
  public void shouldRejectSubscriptionWithExcessiveWorkerName() {
    ...
    assertThat(errorResponse.getErrorData())
        .isEqualTo(
            "Cannot add job subscription. length of worker must be less than or equal to 64");
  }
  ...
  public void shouldRejectSubscriptionWithZeroTimeout() {
    ...
    assertThat(errorResponse.getErrorCode()).isEqualTo(ErrorCode.REQUEST_PROCESSING_FAILURE);
    assertThat(errorResponse.getErrorData())
        .isEqualTo("Cannot add job subscription. timeout must be greater than 0");
  }
  ...
  public void shouldDistributeJobsInRoundRobinFashion() {
    ...
    final List<SubscribedRecord> receivedEvents =
        apiRule.subscribedEvents().limit(4).collect(Collectors.toList());

    final long firstReceivingSubscriber = receivedEvents.get(0).subscriberKey();
    final long secondReceivingSubscriber =
        firstReceivingSubscriber == subscriber1 ? subscriber2 : subscriber1;

    assertThat(receivedEvents)
        .extracting(e -> e.subscriberKey())
        .containsExactly(
  ...
  public void shouldCloseSubscriptionOnTransportChannelClose() throws InterruptedException {
    // given
    apiRule.openJobSubscription("foo").await();

    // when the transport channel is closed
    apiRule.interruptAllChannels();

    // then the subscription has been closed, so we can create a new job and activate it for a new
    // subscription
    ...
    final Optional<SubscribedRecord> jobEvent =
        apiRule
            ...
            .filter(
                (s) ->
                    s.subscriptionType() == SubscriptionType.JOB_SUBSCRIPTION
                        && s.key() == response.key())
            .findFirst();
  ...
  /**
   * This tries to provoke a race condition in JobSubscriptionManager with respect to stream
   * processor management. The concurrent behavior is:
   *
   * <ul>
   *   <li>Close subscription => closes job activation stream processor asynchronously because this
   *       is the last subscription for the job type
   ...
   *       is still registered with the closing stream processor. The result is a confirmed open
   *       subscription that receives no jobs.
   */
  @Test
  ...
  public void shouldContinueRoundRobinJobDistributionAfterClientChannelClose() {
    ...
    final List<SubscribedRecord> events =
        apiRule.subscribedEvents().limit(2).collect(Collectors.toList());
  ...
  public void shouldRejectCreditsEqualToZero() {
    ...
    assertThat(error.getErrorData())
        .isEqualTo("Cannot increase job subscription credits. Credits must be positive.");
  }
  ...
  public void shouldAddJobSubscriptionsForDifferentTypes() {
    ...
    final Function<String, Optional<SubscribedRecord>> findJobRecordForType =
        (jobType) ->
            jobEvents.stream().filter(j -> jobType.equals(j.value().get("type"))).findFirst();
  ...
  public void shouldActivateJobsUntilCreditsAreExhausted() {
    ...
    assertThat(jobEvents).extracting(s -> s.value().get("worker")).contains("bar");
  }
  ...
  public void shouldRejectRemovingNonExistingJobSubscription() {
    ...
    assertThat(errorResponse.getErrorCode()).isEqualTo(ErrorCode.REQUEST_PROCESSING_FAILURE);
    assertThat(errorResponse.getErrorData())
        .isEqualTo("Cannot remove job subscription. Subscription does not exist");
  }
}
