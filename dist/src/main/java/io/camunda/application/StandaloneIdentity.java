...
public class StandaloneIdentity {
  ...
  public static void main(final String[] args) {
    ...
    final var standaloneIdentityApplication =
        MainSupport.createDefaultApplicationBuilder()
            .sources(
                AuthenticationModuleConfiguration.class,
                WebappsModuleConfiguration.class,
                IdentityModuleConfiguration.class)
            .profiles(Profile.IDENTITY.getId(), Profile.STANDALONE.getId())
            .addCommandLineProperties(true)
            .properties(getDefaultProperties())
            .initializers(new WebappsConfigurationInitializer())
            .listeners(new ApplicationErrorListener())
            .build(args);

    standaloneIdentityApplication.run(args);
  }

  private static Map<String, Object> getDefaultProperties() {
    return new HashMap<>(getManagementProperties());
  }
