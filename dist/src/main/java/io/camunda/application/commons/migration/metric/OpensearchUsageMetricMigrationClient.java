...
public record OpensearchUsageMetricMigrationClient(OpenSearchClient client)
  ...
  @Override
  public UsageMetricsEntity getFirstUsageMetricEntity(final String index) throws IOException {
                                ...
                                bool ->
                                    bool.mustNot(
                                        mustNot ->
                                            mustNot.term(
                                                term ->
                                                    term.field(UsageMetricIndex.PARTITION_ID)
                                                        .value(v -> v.longValue(-1))))))
                    .sort(
                        s ->
                            s.field(
  ...
  public ReindexResult reindex(
    ...
    final var transformers = new OpensearchTransformers();
    final SearchTransfomer<SearchQuery, Query> transformer =
        transformers.getTransformer(SearchQuery.class);
    ...
    try {
      ...
      final var reindexRequest =
          new ReindexRequest.Builder()
              ...
              .source(s -> s.index(src).query(query))
