...
val openApiUpgradeReadinessOutputDir =
  "${project.layout.buildDirectory.get()}/generated/openapi-upgrade-readiness"
...
val openApiGenerateUpgradeReadiness =
  tasks.register<GenerateTask>("openApiGenerateUpgradeReadiness") {
    configureCommonOpenApiGeneration(
      fileTree("$projectDir/src/main/resources/api/cluster") { include("**/*.yaml", "**/*.yml") },
      "upgradeReadinessOpenApiSpecs",
    )
    inputSpec.set("$projectDir/src/main/resources/api/cluster/upgrade-readiness-api.yaml")
    outputDir.set(openApiUpgradeReadinessOutputDir)
    modelPackage.set("io.camunda.zeebe.management.cluster")
    ignoreFileOverride.set("$projectDir/src/main/resources/api/cluster/.openapi-generator-ignore")
  }
...
val generateGitProperties =
  tasks.register<WriteProperties>("generateGitProperties") {
    property("git.commit.id.abbrev", gitCommitId)
    destinationFile.set(layout.buildDirectory.file("generated/resources/git.properties"))
  }
...
val copyOpenApiYaml =
  tasks.register<Sync>("copyOpenApiYaml") {
    from("../zeebe/gateway-protocol/src/main/proto/v2") { include("**/*.yaml") }
...
val distTar =
  tasks.register<Tar>("distTar") {
    group = "build"
    description = "Create the Camunda distribution tar.gz archive"
...
val distZip =
  tasks.register<Zip>("distZip") {
    group = "build"
    description = "Create the Camunda distribution zip archive"
    dependsOn(assembleDist)
    archiveBaseName.set(distName)
    archiveVersion.set(distVersion)
    destinationDirectory.set(layout.buildDirectory.dir("distributions"))
...
dependencies {
  ...
  runtimeOnly(libs.software.amazon.awssdk.rds)
  ...
  implementation(project(":zeebe-restore-standalone"))
  ...
  implementation(project(":camunda-secret-store-file"))
  implementation(project(":camunda-secret-store-aws"))
  implementation(project(":camunda-secret-store-gcp"))
  implementation(project(":zeebe-rebalance"))
  ...
  implementation(libs.org.springframework.boot.spring.boot.micrometer.metrics)
  ...
  implementation(libs.org.springframework.boot.spring.boot.micrometer.observation)
  ...
  // Maven's provided JDBC drivers are available to test runtime as well as main compilation.
  ...
  testRuntimeOnly(libs.com.oracle.database.jdbc.ojdbc8)
}
...
tasks.named("assemble") { dependsOn(assembleDist) }
