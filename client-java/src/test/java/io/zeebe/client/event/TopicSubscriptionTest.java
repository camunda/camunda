...
public class TopicSubscriptionTest
{
    ...
    public void shouldOnlyAcknowledgeEventAndCloseSubscriptionAfterLastEventHasBeenHandled() throws InterruptedException, ExecutionException, TimeoutException
    {
        ...
        final TopicSubscriberGroup subscription = (TopicSubscriberGroup) clientRule.topicClient()
            .newSubscription()
            .name(SUBSCRIPTION_NAME)
            .recordHandler(handler)
            .startAtHeadOfTopic()
    ...
    public void shouldAllowReopeningSubscriptionAfterChannelClose() throws InterruptedException
    {
        ...
        final TopicSubscription firstSubscription = clientRule.topicClient()
            .newSubscription()
            .name(SUBSCRIPTION_NAME)
            .recordHandler(DO_NOTHING)
            .startAtHeadOfTopic()
        ...
        // when
        final TopicSubscription secondSubscription = clientRule.topicClient()
            .newSubscription()
            .name(SUBSCRIPTION_NAME)
            .recordHandler(DO_NOTHING)
            .startAtHeadOfTopic()
    ...
    public void shouldInvokeWorkflowInstanceHandlerForWorkflowInstanceEvent()
    {
        ...
        // when pushing two events
        broker.pushRecord(clientAddress, 123L, 1L, 1L, expectedEventTimestamp, RecordType.EVENT, ValueType.WORKFLOW_INSTANCE, WorkflowInstanceIntent.CREATED);
        broker.pushRecord(clientAddress, 123L, 1L, 2L, expectedEventTimestamp, RecordType.EVENT, ValueType.WORKFLOW_INSTANCE, WorkflowInstanceIntent.COMPLETED);
    ...
    public void shouldInvokeWorkflowInstanceHandlerForWorkflowInstanceCommand()
    {
        ...
        // when pushing two events
        broker.pushRecord(clientAddress, 123L, 1L, 1L, expectedEventTimestamp, RecordType.COMMAND, ValueType.WORKFLOW_INSTANCE, WorkflowInstanceIntent.CREATE);
        broker.pushRecord(clientAddress, 123L, 1L, 2L, expectedEventTimestamp, RecordType.COMMAND, ValueType.WORKFLOW_INSTANCE, WorkflowInstanceIntent.UPDATE_PAYLOAD);
    ...
    public void shouldInvokeIncidentEventHandlerForIncidentEvent()
    {
        ...
        // when pushing two events
        broker.pushRecord(clientAddress, 123L, 1L, 1L, expectedEventTimestamp, RecordType.EVENT, ValueType.INCIDENT, IncidentIntent.CREATED);
        broker.pushRecord(clientAddress, 123L, 1L, 2L, expectedEventTimestamp, RecordType.EVENT, ValueType.INCIDENT, IncidentIntent.DELETED);
    ...
    public void shouldInvokeIncidentCommandHandlerForIncidentCommand()
    {
        ...
        // when pushing two events
        broker.pushRecord(clientAddress, 123L, 1L, 1L, expectedEventTimestamp, RecordType.COMMAND, ValueType.INCIDENT, IncidentIntent.CREATE);
        broker.pushRecord(clientAddress, 123L, 1L, 2L, expectedEventTimestamp, RecordType.COMMAND, ValueType.INCIDENT, IncidentIntent.DELETE);
    ...
    public void shouldInvokeRaftEventHandlerForRaftEvent()
    {
        ...
        // when pushing two events
        broker.pushRecord(clientAddress, 123L, 1L, 1L, expectedEventTimestamp, RecordType.EVENT, ValueType.RAFT, Intent.UNKNOWN);
        broker.pushRecord(clientAddress, 123L, 1L, 2L, expectedEventTimestamp, RecordType.EVENT, ValueType.RAFT, Intent.UNKNOWN);
    ...
    public void shouldInvokeDefaultHandlerIfNoHandlerIsRegistered()
    {
        ...
        clientRule.topicClient().newSubscription()
            .name(SUBSCRIPTION_NAME)
    ...
    public void shouldIgnoreEventWithoutHandler()
    {
        ...
        final TopicSubscription subscription = clientRule.topicClient().newSubscription()
            .name(SUBSCRIPTION_NAME)
    ...
    public void shouldCloseSubscriptionWhileOpeningSubscriber()
    {
        ...
        final TopicSubscriptionBuilderImpl builder = (TopicSubscriptionBuilderImpl) clientRule.topicClient().newSubscription()
            .name("foo")
            .recordHandler(DO_NOTHING);
    ...
    public void shouldCloseClientAfterSubscriptionCloseIsCalled() throws Exception
    {
        ...
        final TopicSubscription foo = clientRule.topicClient()
            .newSubscription()
            .name(SUBSCRIPTION_NAME)
            .recordHandler(DO_NOTHING)
            .open();
