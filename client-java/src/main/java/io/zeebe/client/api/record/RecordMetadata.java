...
public interface RecordMetadata {
  ...
  /**
   * @return the unique position the record has in the partition. Records are ordered by position.
   */
  long getPosition();

  /**
   * Returns the position of the source record, which means the record that causes this current
   * record. It can be unset, if it is an command like workflow instance create, then the returned
   * value will be -1 otherwise the value will be >= 0.
   *
   * @return the unique position of the source record. Records are ordered by position.
   */
  ...
  /**
   * @return the timestamp of the record. This is the time at which the event was written to the
   *     log.
   */
  ...
  /** @return the type of the record (e.g. job, worklow, workflow instance, etc.) */
  ValueType getValueType();

  /**
   * @return either the event or the command name, depending if the record is an event or a command
   *     (rejection)
   */
  String getIntent();
