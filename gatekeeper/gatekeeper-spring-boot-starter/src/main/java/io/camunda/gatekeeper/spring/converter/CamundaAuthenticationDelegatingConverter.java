...
public final class CamundaAuthenticationDelegatingConverter
  ...
  private CamundaAuthenticationConverter<Authentication> getConverter(
    ...
    return converters.stream()
        ...
        .orElseThrow(
            () -> {
              final var message =
                  "Did not find a matching converter to convert a Spring Authentication '%s'"
                          .formatted(
                              Optional.ofNullable(authentication)
                                  .map(Authentication::getClass)
                                  .map(Class::getName)
                                  .orElse("null"))
                      + " to a Camunda Authentication";
              LOG.error(message);
              return new GatekeeperAuthenticationException(message);
            });
  }
}
