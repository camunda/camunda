...
public interface MessageStartProcessInstanceRequestRecordValue extends RecordValue, TenantOwned {
  ...
  /**
   * @return the deletion deadline (epoch millis) carried by this request; on {@code P_B} this is
   *     used directly as the dedup row's {@code deletionDeadline}. The value is the originating
   *     buffered message's {@code deadline} on {@code P_K} ({@code publishTime + ttl}), so the
   *     dedup row's expiry is bounded by the publisher's TTL rather than a separate engine
