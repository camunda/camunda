...
/** Unit tests for the variable type filtering in {@link CommonRecordFilter}. */
final class CommonRecordFilterVariableTypeTest {
  ...
  void shouldAcceptVariableWhenTypeMatchesInclusion() {
    ...
    final var filter =
        FilterBuilder.builder()
            ...
            .variableTypeInclusion(List.of("BOOLEAN"))
            .build();
  ...
  void shouldRejectVariableWhenTypeIsExcluded() {
    ...
    final var filter =
        FilterBuilder.builder()
            ...
            .eventEnabled(true)
            .allowValueType(ValueType.VARIABLE)
            .variableTypeExclusion(List.of("BOOLEAN"))
            .build();
  ...
  void shouldFilterVariablesByInferredType(
    ...
    final List<String> inclusionList;
    if (inclusionCsv != null && !inclusionCsv.isBlank()) {
      inclusionList = List.of(inclusionCsv.trim().split("\s*,\s*"));
    } else {
      inclusionList = List.of();
    }

    final List<String> exclusionList;
    if (exclusionCsv != null && !exclusionCsv.isBlank()) {
      exclusionList = List.of(exclusionCsv.trim().split("\s*,\s*"));
    } else {
    ...
    final var filter =
        FilterBuilder.builder()
            ...
            .eventEnabled(true)
            .allowValueType(ValueType.VARIABLE)
            .variableTypeInclusion(inclusionList)
            .variableTypeExclusion(exclusionList)
            .build();
  ...
  void shouldAcceptAllVariablesWhenInclusionAndExclusionAreEmpty() {
    ...
    final var filter =
        FilterBuilder.builder()
            ...
            .eventEnabled(true)
            .allowValueType(ValueType.VARIABLE)
            .variableTypeInclusion(List.of())
            .variableTypeExclusion(List.of())
            .build();
   ...
   * CommonFilterConfiguration} and {@link IndexConfig}, with configurable variable-type
   * inclusion/exclusion and basic record/value-type gating.
   */
  private static final class FilterBuilder {
    ...
    private List<String> inclusionTypes = List.of();
    private List<String> exclusionTypes = List.of();
    ...
    FilterBuilder variableTypeInclusion(final List<String> types) {
      inclusionTypes = types != null ? types : List.of();
      return this;
    }

    FilterBuilder variableTypeExclusion(final List<String> types) {
      exclusionTypes = types != null ? types : List.of();
      return this;
    }
    ...
    CommonRecordFilter build() {
      ...
      // Index config: provide non-null lists
      when(config.filterIndexConfig()).thenReturn(indexConfig);
      when(indexConfig.getVariableValueTypeInclusion()).thenReturn(inclusionTypes);
      when(indexConfig.getVariableValueTypeExclusion()).thenReturn(exclusionTypes);
      when(indexConfig.isOptimizeModeEnabled()).thenReturn(false);

      // Record type gating: only EVENT is relevant here
