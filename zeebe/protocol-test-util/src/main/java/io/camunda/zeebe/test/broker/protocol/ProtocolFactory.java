...
public final class ProtocolFactory {
  ...
  /**
   * Generates a record value using the immutable implementation class from protocol-impl instead of
   * the interface. This is useful for tests that need to work with the concrete implementation
   * classes.
   *
    ...
    try (final var scanResult =
      ...
      scanResult
          ...
          .forEach(
              implClass ->
                  randomizerRegistry.registerRandomizer(
                      implClass, () -> getRecordValueImpl(implClass)));
    }
  }

  private Object getRecordValueImpl(final Class<?> implClass) {
    try {
