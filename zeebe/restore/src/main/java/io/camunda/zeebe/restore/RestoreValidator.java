...
public class RestoreValidator {
  ...
  private static boolean checkPartitionDirectoryIsNotEmpty(
    ...
    final var partitionDirectory =
        RaftPartitionFactory.getPartitionDirectory(partitionMetadata, rootDataDirectory);
    return partitionDirectory.toFile().exists()
        && partitionDirectory.toFile().isDirectory()
        && partitionDirectory.toFile().listFiles() != null
        && partitionDirectory.toFile().listFiles().length > 0;
  }
