...
public final class ManifestManager {
  ...
  public Collection<BackupStatus> listBackupStatuses(final BackupIdentifierWildcard wildcard) {
    return listBackupStatuses(wildcard, ListOptions.all());
  }
  ...
  public List<BackupStatus> listBackupStatuses(
        ...
        page -> {
          for (final var blob : page.getValues()) {
            ...
            if (wildcard.matches(id)) {
              manifestBlobs.add(new ManifestBlob(id, blob));
            }
          }
        });

    final var statusFutures =
        options.select(manifestBlobs, ManifestBlob::id).stream()
            .map(
                manifestBlob ->
    ...
    return statusFutures.stream().map(CompletableFuture::join).toList();
  }
