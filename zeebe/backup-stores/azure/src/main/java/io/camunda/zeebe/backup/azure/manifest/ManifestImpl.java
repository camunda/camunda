...
import static io.camunda.zeebe.backup.azure.manifest.Manifest.StatusCode.COMPLETED;
import static io.camunda.zeebe.backup.azure.manifest.Manifest.StatusCode.FAILED;
import static io.camunda.zeebe.backup.azure.manifest.Manifest.StatusCode.IN_PROGRESS;
...
record ManifestImpl(
  ...
  ManifestImpl {
    if (failureReason != null && statusCode != FAILED) {
      throw new IllegalStateException(ERROR_MSG_WRONG_STATE.formatted(failureReason, statusCode));
    }
  }
