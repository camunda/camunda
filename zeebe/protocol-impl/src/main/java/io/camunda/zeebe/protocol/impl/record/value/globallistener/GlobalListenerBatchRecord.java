...
public final class GlobalListenerBatchRecord extends UnifiedRecordValue
  ...
  public List<GlobalListenerRecord> getExecutionListeners() {
    return listenersProp.stream()
        .filter(listener -> listener.getListenerType() == GlobalListenerType.EXECUTION)
        .toList();
  }

  /** Returns all listeners regardless of type, as concrete {@link GlobalListenerRecord}s. */
  @JsonIgnore
  public List<GlobalListenerRecord> getAllListenerRecords() {
    return listenersProp.stream().toList();
  }
