...
public class HistoryDeletionAuditLogTransformer
  ...
  public boolean supports(final Record<HistoryDeletionRecordValue> record) {
    ...
    final var resourceType = record.getValue().getResourceType();
    return resourceType == HistoryDeletionType.PROCESS_INSTANCE
        || resourceType == HistoryDeletionType.DECISION_INSTANCE;
  }
  ...
  public void transform(final Record<HistoryDeletionRecordValue> record, final AuditLogEntry log) {
    ...
    switch (value.getResourceType()) {
      case PROCESS_INSTANCE -> {
        log.setProcessInstanceKey(value.getResourceKey());
        if (value.getProcessId() != null && !value.getProcessId().isEmpty()) {
          log.setProcessDefinitionId(value.getProcessId());
        }
      }
      case DECISION_INSTANCE -> {
        log.setDecisionEvaluationKey(value.getResourceKey());
      ...
      default ->
          throw new IllegalStateException("Unsupported resource type: " + value.getResourceType());
    }
  }
