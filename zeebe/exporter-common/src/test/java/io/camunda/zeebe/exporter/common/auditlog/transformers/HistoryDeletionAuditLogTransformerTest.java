...
class HistoryDeletionAuditLogTransformerTest {
  ...
  void shouldScheduleCleanUp() {
    // given
    final HistoryDeletionRecordValue recordValue =
        new HistoryDeletionRecord().setResourceType(HistoryDeletionType.PROCESS_INSTANCE);
    final Record<HistoryDeletionRecordValue> record =
        factory.generateRecord(
            ValueType.HISTORY_DELETION,
            r -> r.withIntent(HistoryDeletionIntent.DELETED).withValue(recordValue));
  ...
  @Test
  void shouldSupportProcessInstanceDeletedIntent() {
    // given
    final HistoryDeletionRecordValue recordValue =
        new HistoryDeletionRecord().setResourceType(HistoryDeletionType.PROCESS_INSTANCE);
    final Record<HistoryDeletionRecordValue> record =
  ...
  @Test
  void shouldSupportDecisionInstanceDeletedIntent() {
    // given
    final HistoryDeletionRecordValue recordValue =
        new HistoryDeletionRecord().setResourceType(HistoryDeletionType.DECISION_INSTANCE);
    final Record<HistoryDeletionRecordValue> record =
        factory.generateRecord(
            ValueType.HISTORY_DELETION,
            r -> r.withIntent(HistoryDeletionIntent.DELETED).withValue(recordValue));
  ...
  void shouldNotSupportDeleteIntent() {
    ...
    final Record<HistoryDeletionRecordValue> record =
        factory.generateRecord(
            ValueType.HISTORY_DELETION,
            r -> r.withIntent(HistoryDeletionIntent.DELETE).withValue(recordValue));
  ...
  @Test
  void shouldNotSupportProcessDefinitionDeletion() {
    // given
    final HistoryDeletionRecordValue recordValue =
        new HistoryDeletionRecord().setResourceType(HistoryDeletionType.PROCESS_DEFINITION);
    final Record<HistoryDeletionRecordValue> record =
        factory.generateRecord(
            ValueType.HISTORY_DELETION,
            r -> r.withIntent(HistoryDeletionIntent.DELETED).withValue(recordValue));
  ...
  @Test
  void shouldNotSupportDecisionRequirementsDeletion() {
    // given
    final HistoryDeletionRecordValue recordValue =
        new HistoryDeletionRecord().setResourceType(HistoryDeletionType.DECISION_REQUIREMENTS);
    final Record<HistoryDeletionRecordValue> record =
        factory.generateRecord(
            ValueType.HISTORY_DELETION,
            r -> r.withIntent(HistoryDeletionIntent.DELETED).withValue(recordValue));
