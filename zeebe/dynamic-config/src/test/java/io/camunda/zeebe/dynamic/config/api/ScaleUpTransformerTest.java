...
public class ScaleUpTransformerTest {
  ...
  void shouldGenerateOperationForAllPartition1Members(
    ...
    scaleUpWithValidation(
        ...
        newPartitionCount,
        operations -> {
          assertThat(operations).hasSize(replicationFactor * 3); // 3 operation for each replica
          final var operationsByMember =
              operations.stream()
                  .collect(Collectors.groupingBy(ClusterConfigurationChangeOperation::memberId));
          assertThat(operationsByMember)
              .allSatisfy(
                  (memberId, operationsForMember) -> {
                    assertThat(operationsForMember).hasSize(3);
                    final var newPartitions =
                        partitionsInRange(currentPartitionCount + 1, desiredPartitionCount + 1);
                    assertThat(operationsForMember)
                        .isEqualTo(
                            List.of(
                                new StartPartitionScaleUp(memberId, desiredPartitionCount),
                                new AwaitRedistributionCompletion(
                                    memberId, desiredPartitionCount, newPartitions),
