...
final class EnterRecoveryApplierTest {
  ...
  @Test
  void shouldFailInitWhenMemberIsAlreadyRecovering() {
    // given
    final var applier = new EnterRecoveryApplier(MEMBER_ID, noopExecutor);
    final var config =
        ClusterConfiguration.init()
            .addMember(MEMBER_ID, MemberState.initializeAsActive(Map.of()).toRecovering());
  ...
  @Test
  void shouldReturnRecoveringTransformerOnInitWhenMemberIsActive() {
    // given
    final var applier = new EnterRecoveryApplier(MEMBER_ID, noopExecutor);
  ...
  @Test
  void shouldSetMemberStateToRecoveringAfterApplyingInitTransformer() {
    // given
    final var applier = new EnterRecoveryApplier(MEMBER_ID, noopExecutor);
    final var activeMember = MemberState.initializeAsActive(Map.of());
    final var config = ClusterConfiguration.init().addMember(MEMBER_ID, activeMember);
    final var transformer = applier.initMemberState(config).get();
