...
final class ClusterConfigurationJsonSerializerTest {
  ...
  void shouldDistinguishVariantsThatShareTheirComponents() {
    ...
    // then
    final var operations = json.at("/partitionGroups/default/pendingChanges/pendingOperations");
    assertThat(operations.get(0).at("/@type").asString()).isEqualTo("PartitionJoinOperation");
    assertThat(operations.get(1).at("/@type").asString()).isEqualTo("PartitionLeaveOperation");
  }
  ...
  void shouldKeepTheModelsShape() {
    ...
    assertThat(
            json.at("/partitionGroups/default/pendingChanges/pendingOperations/0/memberId")
                .asString())
        .isEqualTo("1");
