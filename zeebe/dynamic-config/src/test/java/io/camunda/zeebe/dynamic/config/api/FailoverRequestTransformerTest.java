...
final class FailoverRequestTransformerTest {
  ...
  void shouldForceRemoveZoneBrokersAndDropZoneFromConfig() {
    ...
    // when
    final var result = new FailoverRequestTransformer(ZONE_B).operations(currentTopology);
  ...
  void shouldReturnForced() {
    assertThat(new FailoverRequestTransformer(ZONE_B).isForced()).isTrue();
  }
  ...
  void shouldRejectWhenNoZoneAwareConfigIsPersisted() {
    ...
    // when
    final var result = new FailoverRequestTransformer(ZONE_A).operations(roundRobinTopology);
  ...
  void shouldRejectUnknownZone() {
    ...
    // when
    final var result = new FailoverRequestTransformer(ZONE_C).operations(currentTopology);
  ...
  void shouldRejectFailoverOfLastRemainingZone() {
    ...
    // when
    final var result = new FailoverRequestTransformer(ZONE_A).operations(currentTopology);
