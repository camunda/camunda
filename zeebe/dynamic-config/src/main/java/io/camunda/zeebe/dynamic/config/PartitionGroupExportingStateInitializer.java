...
/**
 * New-model counterpart of {@link ExportingStateInitializer}, seeding the legacy exporting state
 * into the local member's partitions in <em>every</em> partition group, instead of once for the
 * single default group.
 *
 * <p>Unlike {@link ExportingStateInitializer}, this does not have a post-restore branch: nothing
 * currently produces a restore change plan on {@link CurrentClusterConfiguration}, so that branch
