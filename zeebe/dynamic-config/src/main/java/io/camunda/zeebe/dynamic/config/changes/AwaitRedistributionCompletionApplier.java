...
public class AwaitRedistributionCompletionApplier implements ClusterOperationApplier {
  ...
  public Either<Exception, UnaryOperator<ClusterConfiguration>> init(
    ...
    // message correlation can be enabled only if the routing state is present
    // and it's not equal to ActivatePartitions
    //    if (operation.includeInMessageCorrelation()) {
    //      if (currentClusterConfiguration
    //          .routingState()
    //          .map(r -> r.requestHandling() instanceof ActivePartitions)
    //          .orElse(true)) {
    //        return Either.left(
    //            new IllegalStateException(
    //                "Cannot include partitions in message correlation when they are not all active
    // yet."));
    //      }}
