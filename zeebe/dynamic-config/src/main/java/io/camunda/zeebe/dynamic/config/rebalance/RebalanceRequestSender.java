...
/**
 * Forwards rebalance requests from the member an operator reached to the coordinator, which is the
 * lowest-id member of the cluster configuration.
 *
 * <p>A rebalance outlives any single round trip, so these requests only ever start, inspect or stop
 * one; none of them waits for a rebalance to finish.
 */
@NullMarked
