...
public final class LongPollingActivateJobsTest {
  ...
  private BrokerClient buildSinglePartitionBroker(
    ...
    clusterState.setPartitionLeader(Protocol.START_PARTITION_ID, 0, 1);
    clusterState.addPartitionIfAbsent(Protocol.START_PARTITION_ID);
    clusterState.setPartitionsCount(1);

    final var topologyManager = Mockito.mock(BrokerTopologyManager.class);
    Mockito.when(topologyManager.getTopology()).thenReturn(clusterState);

    final var firstCallDone = new AtomicBoolean(false);
    final var broker = Mockito.mock(BrokerClient.class);
    Mockito.when(broker.getTopologyManager()).thenReturn(topologyManager);
    Mockito.when(broker.sendRequest(any()))
        .thenAnswer(
            inv -> {
