...
public final class StubbedGateway {
  ...
  private static final class FakeOAuthCallCredentials extends CallCredentials {
    ...
    public void applyRequestMetadata(
      ...
      headers.put(key, Oidc.BEARER_PREFIX + generateToken());
  ...
  /**
   * A {@link JwtDecoder} that decodes a JWT without validating its signature. It's not generally
   * useful but works just well enough to decode the JWTs generated by {@link
   * FakeOAuthCallCredentials}.
   */
  private static final class FakeJwtDecoder implements JwtDecoder {
    ...
    public Jwt decode(final String token) {
      ...
      return new Jwt(
          ...
          Map.of("alg", "none"),
          convertClaims(jwt.getClaims()));
    }

    private static Map<String, Object> convertClaims(final Map<String, Claim> claims) {
      final var result = new HashMap<String, Object>(claims.size());
