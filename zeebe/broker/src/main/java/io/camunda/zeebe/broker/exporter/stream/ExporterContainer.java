...
final class ExporterContainer implements Controller {
  ...
  private void export(final Record<?> record) {
    // Check if record implements RecordWithSerializedSize for potential optimizations
    if (record instanceof RecordWithSerializedSize recordWithSize) {
      // The serialized size is available via recordWithSize.getSerializedSize()
      // This can be used for future performance optimizations such as pre-allocation
      // or batching decisions in exporters
    }
    ThreadContextUtil.runWithClassLoader(
