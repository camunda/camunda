...
public class CommandApiServiceImplTest {
  ...
  public void subscribesWhenBecomingLeader(final Role nonLeaderRole) {
    ...
    final var prepareFuture =
        commandApiService.prepareTransition(transitionContext, 1L, Role.LEADER);
    scheduler.workUntilDone();
    prepareFuture.join();

    // when - becomes LEADER for the partition
    final var transitionFuture = commandApiService.transitionTo(transitionContext, 1, Role.LEADER);
    scheduler.workUntilDone();
    ...
    final var prepareFollowerFuture =
        commandApiService.prepareTransition(transitionContext, 2, nonLeaderRole);
    scheduler.workUntilDone();
    ...
    final var transitionFollowerFuture =
        commandApiService.transitionTo(transitionContext, 2, nonLeaderRole);
