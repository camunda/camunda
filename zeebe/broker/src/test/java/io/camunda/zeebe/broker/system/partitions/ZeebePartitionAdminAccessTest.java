...
final class ZeebePartitionAdminAccessTest {
  ...
  /**
   * Unlike {@link MigrationStatus}, the actual algorithm lives on {@link ExporterDirector} itself
   * (it needs the partition's {@code LogStream}, not the {@code ZeebeDb}), so {@code
   * ZeebePartitionAdminAccess}'s own responsibility here is pure delegation -- a mocked {@link
   * ExporterDirector} is enough to verify it.
   */
  @Nested
