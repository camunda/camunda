...
final class ExportingMigrationStatusCalculatorTest {
  ...
  void shouldReportMigratedWhenNextRecordIsOnlyAPatchBehind() {
    // given - VersionCompatibilityCheck#check already guarantees a patch-only difference is
    // always Compatible.PatchUpgrade, never Incompatible; a patch release never changes the
    // on-disk/wire record format, so this is not a "previous version" backlog either
    final var status =
  ...
  void shouldReportMigratedWhenTheCurrentVersionHasAPreReleaseSuffix() {
    // given - a record's brokerVersion (VersionInfo) can never carry a "-SNAPSHOT"-style suffix,
    // but VersionUtil.getVersion() can on a non-release build; that asymmetry must not make an
    // otherwise-identical version look incompatible
    final var status =
