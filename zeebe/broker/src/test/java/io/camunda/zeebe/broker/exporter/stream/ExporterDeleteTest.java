...
public class ExporterDeleteTest {
  ...
  public void shoulddeleteExporter() {
    ...
    assertThat(rule.getDirector().getLowestPosition().join())
        .describedAs("The lowest position should be updated when one exporter is deleted")
        .isEqualTo(expectedLastExportedPosition);

    assertThat(exporters.get(EXPORTER_ID_1).getExportedRecords())
        .describedAs("Should not export to exporter-1 after deleting")
        .hasSizeLessThanOrEqualTo(1);
  }

  @Test
  public void shouldSucceedWhenDeletingAlreadyDeletedExporter() {
    // given
    rule.startExporterDirector(exporterDescriptors);
    rule.getDirector().deleteExporter(EXPORTER_ID_1).join();

    // when - then
    assertThat(rule.getDirector().deleteExporter(EXPORTER_ID_1))
        .succeedsWithin(Duration.ofSeconds(5));
  }

  @Test
  public void shouldSucceedWhenDeletingNonExistingExporter() {
    // given
    rule.startExporterDirector(exporterDescriptors);

    // when - then
    assertThat(rule.getDirector().deleteExporter("non-existing-exporter"))
        .succeedsWithin(Duration.ofSeconds(5));
  }

  @Test
  public void shouldSucceedDeletingWhenActorIsAlreadyClosed() {
    // give
    rule.startExporterDirector(exporterDescriptors);
