...
public class ExpressionLanguageMetricsTest {
  ...
  @Test
  void shouldWorkWithDefaultSimpleMeterRegistry() {
    // given - expression language with default SimpleMeterRegistry
    final var expressionLanguageWithDefaultRegistry =
        (ExpressionLanguage)
            new FeelExpressionLanguage(new TestFeelEngineClock(), new SimpleMeterRegistry());

    // when - parse and evaluate
    final var expression = expressionLanguageWithDefaultRegistry.parseExpression("=1 + 2");
    final var result =
        expressionLanguageWithDefaultRegistry.evaluateExpression(expression, EMPTY_CONTEXT);
