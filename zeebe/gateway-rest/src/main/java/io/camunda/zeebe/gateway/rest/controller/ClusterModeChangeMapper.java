    ...
    for (final Phase phase : requireNonNull(response.response()).phases()) {
      switch (phase) {
        case final PartitionGroupParallelPhase parallelPhase ->
            collectPerTenant(parallelPhase.groupOperations(), operationsPerTenant);
        // A graph phase carries the same operations, plus the edges between them. The edges say
        // what may run when, which this view does not report, so it reads the operations alone.
        case final PartitionGroupGraphPhase graphPhase ->
            collectPerTenant(graphPhase.groupOperations(), operationsPerTenant);
