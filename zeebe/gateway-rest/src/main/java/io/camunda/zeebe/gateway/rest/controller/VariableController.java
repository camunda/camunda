...
public class VariableController {
  ...
  public ResponseEntity<Object> searchVariables(
      ...
      @RequestParam(name = "includeFullValue", required = false, defaultValue = "false")
          final boolean includeFullValue) {
    return SearchQueryRequestMapper.toVariableQuery(query)
        .fold(RestErrorMapper::mapProblemToResponse, q -> search(q, includeFullValue));
  }

  private ResponseEntity<Object> search(final VariableQuery query, final boolean includeFullValue) {
    try {
      final var result =
          variableServices
      ...
      return ResponseEntity.ok(
          SearchQueryResponseMapper.toVariableSearchQueryResponse(result, includeFullValue));
