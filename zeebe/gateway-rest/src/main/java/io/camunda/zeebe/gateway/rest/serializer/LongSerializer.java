...
public class LongSerializer extends JsonSerializer<Long> {
  ...
  private boolean fieldNameEndsWithKey(final JsonGenerator gen) {
    ...
    return currentName != null && currentName.endsWith(KEY)
  ...
  public void serialize(
    ...
    if (!gatewayRestPackage(gen)) {
      writeNumber(value, gen);
      return;
    }
    if (!fieldNameEndsWithKey(gen)) {
      writeNumber(value, gen);
      return;
    }
    ...
    if (requestAttributes instanceof final ServletRequestAttributes r) {
      ...
      if (acceptHeader != null && acceptHeader.contains(RequestMapper.MEDIA_TYPE_KEYS_NUMBER)) {
        writeNumber(value, gen);
        return;
      }
      if (acceptHeader != null && acceptHeader.contains(RequestMapper.MEDIA_TYPE_KEYS_STRING)) {
        writeString(value, gen);
        return;
      }
    }
