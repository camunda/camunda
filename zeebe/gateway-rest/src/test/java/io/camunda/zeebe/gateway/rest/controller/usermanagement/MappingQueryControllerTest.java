...
public class MappingQueryControllerTest extends RestControllerTest {
  ...
  void getMappingShouldReturnOk() {
    ...
    when(mappingServices.getMapping(mapping.id())).thenReturn(mapping);

    // when
    webClient
        .get()
        .uri("%s/%s".formatted(MAPPING_BASE_URL, mapping.id()))
    ...
    // then
    verify(mappingServices, times(1)).getMapping(mapping.id());
  }
