...
public class BrokerRelocationRequest extends BrokerExecuteCommand<ScaleRecord> {

  private final ScaleRecord requestDto = new ScaleRecord();

  public BrokerRelocationRequest(final int oldPartitionCount, final int newPartitionCount) {
    super(ValueType.SCALE, ScaleIntent.RELOCATION_START);
    request.setPartitionId(1);
    requestDto.setRoutingInfo(oldPartitionCount, newPartitionCount);
  }
