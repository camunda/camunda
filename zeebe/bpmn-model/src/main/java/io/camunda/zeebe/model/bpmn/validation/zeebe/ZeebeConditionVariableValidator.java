...
public class ZeebeConditionVariableValidator
    implements ModelElementValidator<ZeebeConditionVariable> {

  private static final List<String> VALID_VARIABLE_EVENTS = Arrays.asList("create", "update");

  @Override
  public Class<ZeebeConditionVariable> getElementType() {
    return ZeebeConditionVariable.class;
  }

  @Override
  public void validate(
      final ZeebeConditionVariable element,
      ...
      for (final String event : events) {
        ...
        if (!trimmedEvent.isEmpty() && !VALID_VARIABLE_EVENTS.contains(trimmedEvent)) {
          validationResultCollector.addError(
              0,
              String.format(
                  "Variable event '%s' is not valid. Must be one of: %s",
