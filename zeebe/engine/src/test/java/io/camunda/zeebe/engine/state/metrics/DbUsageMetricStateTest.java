...
public class DbUsageMetricStateTest {
  ...
  public void shouldResetActiveBucket() {
    ...
    assertThat(actual.getTenantATUMap()).isEmpty();
  }
  ...
  @Test
  public void shouldRecordATUMetrics() {
    // given
    final var eventTime = InstantSource.system().millis();
    when(mockClock.millis()).thenReturn(eventTime);
    state.resetActiveBucket(1L);
    ...
    // when
    state.recordATUMetric(TenantOwned.DEFAULT_TENANT_IDENTIFIER, "assignee1");
    state.recordATUMetric(TenantOwned.DEFAULT_TENANT_IDENTIFIER, "assignee1");
    state.recordATUMetric(TenantOwned.DEFAULT_TENANT_IDENTIFIER, "assignee2");
    state.recordATUMetric("tenant1", "assignee1");
    state.recordATUMetric("tenant1", "assignee2");
    state.recordATUMetric("tenant1", "assignee3");
    state.recordATUMetric("tenant2", "assignee1");

    // then
    final var actual = state.getActiveBucket();
    assertThat(actual.getFromTime()).isEqualTo(1L);
    assertThat(actual.getToTime()).isEqualTo(1001L);
    assertThat(actual.getTenantATUMap())
        .containsExactlyInAnyOrderEntriesOf(
            Map.of(
