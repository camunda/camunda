...
import io.camunda.zeebe.engine.common.processing.deployment.model.validation.ProcessSignalStartEventSignalNameValidator;
...
public class ProcessSignalStartEventSignalNameValidatorTest {
  ...
  void shouldNotThrowNPEIfSignalNameIsNull() {
    // given
    final var model = Bpmn.createProcess().startEvent().signal((String) null).endEvent().done();
  ...
  final class WithMockedExpressionLanguage {
    ...
    private static final BpmnModelInstance MODEL =
        Bpmn.createProcess().startEvent().signal(TEST_EXPRESSION).endEvent().done();
    ...
    void setUp() {
      ...
      sutValidator = new ProcessSignalStartEventSignalNameValidator(mockExpressionLanguage);
    }
    ...
    void shouldAddErrorIfEvaluationDoesNotReturnString() {
      ...
      verify(mockResultCollector)
          .addError(
              0,
              "Expected constant expression of type String for signal name 'expression', but was NUMBER");
    }
  }
}
