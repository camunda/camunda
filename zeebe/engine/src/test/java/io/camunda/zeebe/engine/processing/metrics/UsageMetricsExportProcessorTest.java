...
class UsageMetricsExportProcessorTest {
  ...
  void shouldAppendExportedNoneEventWhenBucketNull() {
    // given
    when(state.getRollingBucket()).thenReturn(null);

    // when
    processor.processRecord(record);

    // then
    verify(state).getRollingBucket();
    verify(stateWriter)
        .appendFollowUpEvent(
            eq(1L), eq(UsageMetricIntent.EXPORTED), recordArgumentCaptor.capture());
    final var actual = recordArgumentCaptor.getValue();
    assertThat(actual.getIntervalType()).isEqualTo(IntervalType.ROLLING);
  ...
  void shouldAppendMultipleExportedEventsWhenRecordTooLong() {
    ...
    when(state.getRollingBucket())
        .thenReturn(
            new UsageMetricStateValue().setFromTime(1).setToTime(10).setTenantRPIMap(tenantRPIMap));
    ...
    assertThat(actual)
        ...
        .containsOnly(IntervalType.ROLLING);
