...
public final class MessageSuspensionCrossPartitionTest {
  ...
  /**
   * Verifies that a subscription still {@code OPENING} when the instance is suspended is driven to
   * the durable resume manifest instead of live-locking, even when its message-side CREATE
   * immediately correlates an already-buffered TTL message instead of sending a plain
