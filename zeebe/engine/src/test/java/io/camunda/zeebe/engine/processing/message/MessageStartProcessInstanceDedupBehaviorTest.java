 ...
 * dedup entry on a cache miss + success, the V3 PI-completion applier that transitions the entry to
 * {@code TOMBSTONE}, and the scheduled sweep that deletes past-deadline tombstones.
 *
...
public final class MessageStartProcessInstanceDedupBehaviorTest {
  ...
  @Test
  public void shouldReReplyStartedWithCachedKeyOnTombstoneHitWithinWindow() {
    // given a PI was started via REQUEST and has since completed (root PI ELEMENT_COMPLETED → V3
    // applier transitions the dedup entry to TOMBSTONE(now + tombstoneWindow))
    engine.deployment().withXmlResource(MESSAGE_START_PROCESS).deploy();
    final long subscriptionKey = waitForStartEventSubscriptionKey();
    writeRequest(subscriptionKey);
    final long firstPiKey = firstStartReplyPiKey();
    ...
    // when a retry arrives while the tombstone window is still open
    writeRequest(subscriptionKey);
    ...
    assertThat(
            startReplies
                .map(this::asRequest)
                .map(MessageStartProcessInstanceRequestRecordValue::getProcessInstanceKey))
        .as("tombstoned dedup entry keeps re-replying the cached PI key within its deadline")
        .containsExactly(firstPiKey, firstPiKey);
  }
