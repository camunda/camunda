...
final class BpmnIncidentBehaviorTest {
  ...
  void shouldNotCreateSecondIncidentForJobThatAlreadyHasOne() {
    // given - a job that already carries an unresolved incident, as one whose secret value
    // injection failed does: it keeps the incident and stays activatable
    ...
    // when - the job reaches an activation again and fails the same way
