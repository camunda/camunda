...
public final class SecretReferenceResolutionFailProcessorTest {
  ...
  private SecretReferenceResolutionFailProcessor processor;
  ...
  void setUp() {
    ...
    processor =
        new SecretReferenceResolutionFailProcessor(writers, keyGenerator, secretReferenceState);
  }
  ...
  @Test
  void shouldWriteResolutionFailedEvent() {
    // given
    ...
    verify(stateWriter)
        .appendFollowUpEvent(
            eq(500L), eq(SecretReferenceIntent.RESOLUTION_FAILED), eventCaptor.capture());
  ...
  @Test
  void shouldPropagateResolutionStateToFailedEvent() {
    ...
    // then - the reason is carried over to the event so exporters can surface it
    final var eventCaptor = ArgumentCaptor.forClass(SecretReferenceRecord.class);
    verify(stateWriter)
        .appendFollowUpEvent(
            eq(500L), eq(SecretReferenceIntent.RESOLUTION_FAILED), eventCaptor.capture());
    Assertions.assertThat(eventCaptor.getValue().getResolutionState())
        .isEqualTo(ResolutionState.STORE_UNAVAILABLE);
  }

  @Test
  void shouldWriteBatchCreateIncidentsCommandForWaitingJobs() {
    // given
    secretReferenceState.addPendingSecretReference(STORE_ID, SECRET_REF);
  ...
  void shouldCapBatchAtHundred() {
    // given - 105 jobs waiting on the failed secret reference
    secretReferenceState.addPendingSecretReference(STORE_ID, SECRET_REF);
  ...
  void shouldNotWriteBatchCommandWhenNoJobsWaiting() {
    // given - no jobs waiting on the failed secret reference
    secretReferenceState.addPendingSecretReference(STORE_ID, SECRET_REF);
    final var value =
        new SecretReferenceRecord().setStoreId(STORE_ID).setSecretReference(SECRET_REF);
  ...
  void shouldRejectWhenSecretReferenceNotPending() {
    // given - secret reference is NOT seeded as pending (never requested, or already failed)
    final var value =
        new SecretReferenceRecord().setStoreId(STORE_ID).setSecretReference(SECRET_REF);
  ...
  void shouldIncludeJobsStillWaitingOnOtherSecretsInBatch() {
    // given - job 1 waits on the failed secret reference AND on a second, still-pending secret.
    //         A failed secret can never resolve, so the job is doomed regardless of its other
    //         references: it must get an incident now rather than wait for the others.
    secretReferenceState.addPendingSecretReference(STORE_ID, SECRET_REF);
    secretReferenceState.addWaitingJob(STORE_ID, SECRET_REF, 1L);
