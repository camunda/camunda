...
public class AuthorizationKeys extends ObjectDbValue {
	private final ArrayProperty<LongValue> authorizationKeys = new ArrayProperty<>("authorizationKeys", LongValue::new);
	...
	public Set<Long> getAuthorizationKeys() {
		return StreamSupport.stream(authorizationKeys.spliterator(), false).map(LongValue::getValue)
