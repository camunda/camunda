...
/**
 * v1 applier: activates the (line, ordinal). The gatedField on the record is ignored at this
 * version — older binaries are not expected to know about it.
 */
public final class ClusterVersionAppliedV1Applier
    implements TypedEventApplier<ClusterVersionIntent, ClusterVersionRecord> {

  private final MutableClusterVersionState state;

  public ClusterVersionAppliedV1Applier(final MutableClusterVersionState state) {
    this.state = state;
  }
