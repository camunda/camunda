...
final class JobCreatedApplierV2 implements TypedEventApplier<JobIntent, JobRecord> {

  private final MutableElementInstanceState elementInstanceState;
  private final MutableJobState jobState;
  private final MutableJobMetricsState jobMetricsState;

  JobCreatedApplierV2(final MutableProcessingState state) {
  ...
  public void applyState(final long key, final JobRecord value) {
    ...
    jobMetricsState.incrementMetric(
        value.getType(), value.getTenantId(), value.getWorker(), JobMetricsExportState.CREATED);
  }
