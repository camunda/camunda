...
public class PersistedCommandDistribution extends ObjectDbValue {
	...
	private final ObjectProperty<UnifiedRecordValue> commandValueProperty = new ObjectProperty<>("commandValue",
			new UnifiedRecordValue(10));
	private final ObjectProperty<AuthInfo> authInfoProperty = new ObjectProperty<>("authInfo", new AuthInfo());

	public PersistedCommandDistribution() {
		super(5);
		declareProperty(queueIdProperty).declareProperty(valueTypeProperty).declareProperty(intentProperty)
				.declareProperty(commandValueProperty).declareProperty(authInfoProperty);
	}

	public PersistedCommandDistribution wrap(final CommandDistributionRecord commandDistributionRecord) {
	...
	public Intent getIntent() {
		...
		if (intentValue < 0 || intentValue > Short.MAX_VALUE) {
			throw new IllegalStateException(String.format(
					"Expected to read the intent, but it's persisted value '%d' is not a short integer", intentValue));
		}
