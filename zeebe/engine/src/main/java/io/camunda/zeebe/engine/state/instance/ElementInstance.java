...
public final class ElementInstance extends ObjectDbValue {
	...
	private static final StringValue MULTI_INSTANCE_LOOP_COUNTER = new StringValue("multiInstanceLoopCounter");
	private static final StringValue INTERRUPTING_ELEMENT_ID = new StringValue("interruptingElementId");
	private static final StringValue INTERRUPTED_BY_RUNTIME_INSTRUCTION = new StringValue(
			"interruptedByRuntimeInstruction");
	private static final StringValue CALLED_CHILD_INSTANCE_KEY = new StringValue("calledChildInstanceKey");
	...
	private static final StringValue ACTIVE_SEQUENCE_FLOW_IDS = new StringValue("activeSequenceFlowIds");
	private static final StringValue USER_TASK_KEY = new StringValue("userTaskKey");
	private static final StringValue EXECUTION_LISTENER_INDEX = new StringValue("executionListenerIndex");
	private static final StringValue TASK_LISTENER_INDICES_RECORD = new StringValue("taskListenerIndicesRecord");
	private static final StringValue COMPLETION_CONDITION_FULFILLED = new StringValue("completionConditionFulfilled");
	...
	private final IntegerProperty childActivatedCountProp = new IntegerProperty(CHILD_ACTIVATED_COUNT, 0);
	private final IntegerProperty childCompletedCountProp = new IntegerProperty(CHILD_COMPLETED_COUNT, 0);
	private final IntegerProperty childTerminatedCountProp = new IntegerProperty(CHILD_TERMINATED_COUNT, 0);
	private final LongProperty jobKeyProp = new LongProperty(JOB_KEY, 0L);
	private final IntegerProperty multiInstanceLoopCounterProp = new IntegerProperty(MULTI_INSTANCE_LOOP_COUNTER, 0);
	private final StringProperty interruptingEventKeyProp = new StringProperty(INTERRUPTING_ELEMENT_ID, "");
	private final BooleanProperty interruptedByRuntimeInstructionProp = new BooleanProperty(
			INTERRUPTED_BY_RUNTIME_INSTRUCTION, false);
	private final LongProperty calledChildInstanceKeyProp = new LongProperty(CALLED_CHILD_INSTANCE_KEY, -1L);
	private final ObjectProperty<IndexedRecord> recordProp = new ObjectProperty<>(ELEMENT_RECORD, new IndexedRecord());
	private final IntegerProperty activeSequenceFlowsProp = new IntegerProperty(ACTIVE_SEQUENCE_FLOWS, 0);
	private final ArrayProperty<StringValue> activeSequenceFlowIdsProp = new ArrayProperty<>(ACTIVE_SEQUENCE_FLOW_IDS,
			StringValue::new);
	private final LongProperty userTaskKeyProp = new LongProperty(USER_TASK_KEY, -1L);
	private final IntegerProperty executionListenerIndexProp = new IntegerProperty(EXECUTION_LISTENER_INDEX, 0);
	private final ObjectProperty<TaskListenerIndicesRecord> taskListenerIndicesRecordProp = new ObjectProperty<>(
			TASK_LISTENER_INDICES_RECORD, new TaskListenerIndicesRecord());
	private final IntegerProperty processDepth = new IntegerProperty(PROCESS_DEPTH, 1);
	private final BooleanProperty completionConditionFulfilledProp = new BooleanProperty(COMPLETION_CONDITION_FULFILLED,
			false);

	/**
	 * Expresses the current depth of the process instance in the called process
	 * tree.
	 *
	 * <p>
	 * A root process instance has depth 1. Each child instance has the depth of its
	 * parent incremented by 1.
	 *
	 * @since 8.3.22, 8.4.18, 8.5.17, 8.6.12, and 8.7
	 * @apiNote This property is added in 8.7 and backported to 8.6.12, 8.5.17,
	 *          8.4.18, and 8.3.22. Any child process instances created before 8.7
	 *          (or any of these patches) will have a depth of 1 rather than a
	 *          correct value. Child instances created before the property existed
	 *          will have a depth of 1 + the depth of the parent instance.
	 *          Therefore, child instances created on or after the property was
	 *          added that are part of a root process instance created prior to the
	 *          property existed, will not have a correct depth.
	 */
	public ElementInstance() {
		super(18);
		declareProperty(parentKeyProp).declareProperty(childCountProp).declareProperty(childActivatedCountProp)
				.declareProperty(childCompletedCountProp).declareProperty(childTerminatedCountProp)
				.declareProperty(jobKeyProp).declareProperty(multiInstanceLoopCounterProp)
				.declareProperty(interruptingEventKeyProp).declareProperty(calledChildInstanceKeyProp)
				.declareProperty(recordProp).declareProperty(activeSequenceFlowsProp)
				.declareProperty(activeSequenceFlowIdsProp).declareProperty(userTaskKeyProp)
				.declareProperty(executionListenerIndexProp).declareProperty(taskListenerIndicesRecordProp)
				.declareProperty(processDepth).declareProperty(interruptedByRuntimeInstructionProp)
				.declareProperty(completionConditionFulfilledProp);
	}

	public ElementInstance(final long key, final ElementInstance parent, final ProcessInstanceIntent state,
	...
	public ElementInstance(final long key, final ProcessInstanceIntent state, final ProcessInstanceRecord value) {
	...
	/**
	 * Returns a list of currently active sequence flow ids. If the same sequence
	 * flow is active multiple times, it will appear in the list multiple times.
	 * I.e. this can be used to track virtual sequence flow instances. Virtual,
	 * because there are no sequence flow instances in Zeebe.
	 *
	 * <p>
	 * Warning, this method should not be used for process instances created before
	 * 8.6. It may provide incorrect information for such process instances.
	 *
	 * @since 8.6
	 */
