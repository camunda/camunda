...
public class ResourceTransformer implements DeploymentResourceTransformer {
        ...
        .flatMap(
            resource ->
                checkForDuplicateResourceId(resource.id, deploymentResource, deployment)
                    .map(
                        noDuplicates -> {
                          final ResourceMetadataRecord resourceMetadataRecord = deployment.resourceMetadata().add();
                          appendMetadataToResourceRecord(resourceMetadataRecord, resource, deploymentResource, deployment);
                          return null;
                        }));
  }
            ...
            metadata -> {
              ...
              if (metadata.isDuplicate()) {
                ...
                metadata
                    .setResourceKey(key)
                    .setVersion(
                        resourceState.getNextResourceVersion(metadata.getResourceId(), metadata.getTenantId()))
                    .setDuplicate(false)
  ...
  private void appendMetadataToResourceRecord(
    ...
    final DirectBuffer checksum = checksumGenerator.checksum(deploymentResource.getResourceBuffer());
            ...
            latestResource -> {
              final boolean isDuplicate =
                  latestResource.getChecksum().equals(resourceMetadataRecord.getChecksumBuffer())
                      && latestResource.getResourceName().equals(resourceMetadataRecord.getResourceNameBuffer());

              if (isDuplicate) {
                final int latestVersion = latestResource.getVersion();
                resourceMetadataRecord
                    .setResourceKey(latestResource.getFormKey())
