 ...
 */
package io.camunda.zeebe.engine.common.processing.bpmn;

import io.camunda.zeebe.engine.common.processing.bpmn.behavior.BpmnInclusiveGatewayBehavior;
...
public final class ProcessInstanceStateTransitionGuard {
  ...
  private Either<String, ?> hasActiveProcessInstance(final BpmnElementContext context) {
    ...
    if (processInstance.isInterruptedByRuntimeInstruction()) {
      return Either.left(
          String.format(
              "Expected process instance to be active but was interrupted by an event with id '%s'.",
