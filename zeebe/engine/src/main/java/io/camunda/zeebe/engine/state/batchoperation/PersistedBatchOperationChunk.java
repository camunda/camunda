...
/**
 * Represents a chunk of itemKeys of a batch operation that is persisted in the
 * database.
 */
public class PersistedBatchOperationChunk extends ObjectDbValue {

	/**
	 * The key of the chunk. This key os unique just for the batch operation it
	 * belongs to.
	 */
	...
	/**
	 * The itemKeys of the batch operation chunk. This is an array of itemKeys that
	 * are part of the batch operation.
	 */
	private final ArrayProperty<LongValue> itemKeysProp = new ArrayProperty<>("itemKeys", LongValue::new);
	...
	public PersistedBatchOperationChunk removeItemKeys(final Set<Long> itemKeys) {
		final var newKeys = itemKeysProp.stream().map(LongValue::getValue).filter(k -> !itemKeys.contains(k)).toList();
