...
public class BatchOperationExecutionScheduler implements StreamProcessorLifecycleAware {
  ...
  private void executeBatchOperation(
    ...
    try{
      ...
      LOG.error(
          ...
          e);
      appendFailedCommand(taskResultBuilder, batchOperation, BatchOperationErrorType.QUERY_FAILED, e);
      return;
    }
    ...
    try {
      ...
      LOG.error(
          ...
          batchOperation.getKey(),
          e);
      appendFailedCommand(taskResultBuilder, batchOperation, BatchOperationErrorType.APPEND_CHUNKS_FAILED, e);
    }
  }

  private void appendChunks(final PersistedBatchOperation batchOperation,
      final TaskResultBuilder taskResultBuilder, final Set<Item> keys) {
    if (!keys.isEmpty()) {
      LOG.debug(
  ...
  private void appendFailedCommand(
    ...
    error.setPartition(partitionId);
    error.setStacktrace(exception.getMessage());
