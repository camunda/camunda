 ...
 * {@code RESUMING}).
 *
 * <p>Overriding is optional: when {@link #suspensionBehavior} returns {@code null} the gate applies
 * its origin-based default (external commands are rejected, internal commands are buffered). A
 * processor overrides only to deviate from that default (e.g. always PROCESS or always REJECT).
 *
...
public interface SuspensionAware<T extends UnifiedRecordValue> {
   ...
   * @return the {@link SuspensionBehavior} to apply, or {@code null} to use the gate's origin-based
   *     default
   */
  default @Nullable SuspensionBehavior suspensionBehavior(final TypedRecord<T> record) {
    return null;
  }
