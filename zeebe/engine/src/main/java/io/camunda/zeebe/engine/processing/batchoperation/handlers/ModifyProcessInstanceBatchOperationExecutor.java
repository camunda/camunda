...
public class ModifyProcessInstanceBatchOperationExecutor implements BatchOperationExecutor {
  ...
  public void execute(final long itemKey, final PersistedBatchOperation batchOperation) {
    ...
    final var processInstance = elementInstanceState.getInstance(itemKey);
    if (processInstance == null) {
      LOGGER.warn(
          "Unable to modify process instance with key {} because it does not exist", itemKey);
      return;
    }
    ...
    final var elementInstances = new ArrayDeque<>(elementInstanceState.getChildren(itemKey));
    while (!elementInstances.isEmpty()) {
      ...
      if (moveInstructions.containsKey(elementInstance.getValue().getElementId())) {
        command.addActivateInstruction(
        ...
        command.addTerminateInstruction(
    ...
    LOGGER.info("Modifying process instance with key '{}'", itemKey);
    commandWriter.appendFollowUpCommand(
        itemKey, ProcessInstanceModificationIntent.MODIFY, command, batchOperation.getKey());
  }
}
