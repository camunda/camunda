 ...
 */
public class BatchOperationEntityKeyProvider {

  private static final Logger LOG = LoggerFactory.getLogger(BatchOperationEntityKeyProvider.class);
  ...
  public BatchOperationEntityKeyProvider(final SearchClientsProxy searchClientsProxy) {
  ...
  private <F extends FilterBase> Set<Long> fetchEntityKeys(
      final KeyPageFetcher<F> keyPageFetcher, final F filter, final Supplier<Boolean> shouldAbort) {
    ...
    while (true) {
      ...
      final var result = keyPageFetcher.fetchKeys(filter, searchValues);
   ...
   */
  private record KeyPage(List<Long> keys, Object[] lastSortValues, long total) {}
   ...
   */
  private interface KeyPageFetcher<F extends FilterBase> {
