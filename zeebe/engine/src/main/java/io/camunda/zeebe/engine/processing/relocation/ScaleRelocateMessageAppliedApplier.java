...
public class ScaleRelocateMessageAppliedApplier
    implements TypedEventApplier<ScaleIntent, ScaleRecord> {
  private final MutableMessageState messageState;

  public ScaleRelocateMessageAppliedApplier(final MutableMessageState messageState) {
    this.messageState = messageState;
  }

  @Override
  public void applyState(final long key, final ScaleRecord value) {
    messageState.put(key, value.getMessageRecord());
    for (final var process : value.getCorrelatedProcesses()) {
      messageState.putMessageCorrelation(key, process);
    }
  }
}
