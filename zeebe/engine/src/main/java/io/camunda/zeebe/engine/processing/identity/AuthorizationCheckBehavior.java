...
public final class AuthorizationCheckBehavior {
  ...
  public Either<Rejection, Void> isAuthorized(final AuthorizationRequest request) {
    ...
    if (username.isPresent()) {
      final var userAuthorized =
          isEntityAuthorized(request, EntityType.USER, Stream.of(username.get()));
    ...
    final var mappingAuthorized =
        isEntityAuthorized(
            request,
            EntityType.MAPPING,
            getPersistedMappings(request.command).map(PersistedMapping::getMappingId));
  ...
  private Either<Rejection, Void> isEntityAuthorized(
      ...
      final Stream<String> entityIds) {
    if (multiTenancyEnabled) {
      if (entityIds.noneMatch(
          entityId ->
              getAuthorizedTenantIds(entityType, entityId).anyMatch(request.tenantId::equals))) {
        ...
        entityIds.flatMap(
            entityId ->
                getAuthorizedResourceIdentifiers(
                    entityType, entityId, request.getResourceType(), request.getPermissionType()));
