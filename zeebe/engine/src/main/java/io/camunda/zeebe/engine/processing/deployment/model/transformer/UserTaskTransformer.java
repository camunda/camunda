...
import io.camunda.zeebe.engine.ListenersOrderConfiguration;
...
public final class UserTaskTransformer implements ModelElementTransformer<UserTask> {
  ...
  private void transformTaskListeners(
    ...
    globalListenersConfiguration
        .map(ListenersOrderConfiguration::beforeLocal)
        .ifPresent(
            listeners ->
                listeners.stream()
    ...
    globalListenersConfiguration
        .map(ListenersOrderConfiguration::afterLocal)
        .ifPresent(
            listeners ->
                listeners.stream()
                    .map(l -> toTaskListenerModel(l, userTaskProperties))
