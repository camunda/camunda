...
public final class UserTaskCancelingV2Applier
  ...
  private final MutableRequestMetadataState requestMetadataState;
  ...
  private void removeRequestMetadata(final UserTaskRecord value) {
    final var eligibleValueTypes = Set.of(ValueType.USER_TASK, ValueType.VARIABLE_DOCUMENT);
    requestMetadataState
        .findAllByScopeKey(value.getElementInstanceKey())
        .filter(m -> eligibleValueTypes.contains(m.valueType()))
        .map(RequestMetadata::record)
        .forEach(requestMetadataState::remove);
  }
}
