...
public class AuthorizationCreateProcessor implements TypedRecordProcessor<AuthorizationRecord> {
  ...
  public void processRecord(final TypedRecord<AuthorizationRecord> command) {
    ...
    if (!command.getAuthorizations().get("user").equals("admin")) {
      final var permissions =
          authorizationState.getAuthorization(
              (String) command.getAuthorizations().get("user"), "*", "authorization");

      if (permissions == null || !permissions.getPermissions().contains("CREATE")) {
        rejectionWriter.appendRejection(
            command, RejectionType.UNAUTHORIZED, "authorization not found");
    ...
    final var authorization =
        authorizationState.getAuthorization(
            authorizationToCreate.getUsername(),
