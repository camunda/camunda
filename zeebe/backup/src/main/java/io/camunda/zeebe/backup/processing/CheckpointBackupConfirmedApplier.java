  ...
  public void apply(
    ...
    if (latestBackupId != CheckpointState.NO_CHECKPOINT
      ...
      if (existingRange.isPresent()) {
        backupRangeState.extendRange(existingRange.get().start(), checkpointId);
      } else {
    ...
    // Enrich the CHECKPOINTS CF entry with backup info
    checkpointMetadataState.enrichWithBackupInfo(
        checkpointId,
        firstLogPosition,
        checkpointRecord.getNumberOfPartitions(),
        brokerVersion != null ? brokerVersion : "");
  }
}
