...
class BackupServiceImplTest {
  ...
  @Test
  void shouldWriteDeleteBackupCommandToLog() {
    // given
  ...
  @Test
  void shouldFailDeleteBackupWhenLogWriteFails() {
    // given
    final int partitionId = 1;
    final long checkpointId = 2L;
    when(logStreamWriter.tryWrite(any(), any(LogAppendEntry.class)))
    ...
    // when
    final var result = backupService.deleteBackup(partitionId, checkpointId, concurrencyControl);
  ...
  @Test
  void shouldNotInteractWithBackupStoreOnDeleteBackup() {
    // given
    final int partitionId = 1;
