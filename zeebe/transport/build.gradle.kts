...
tasks.named<ProcessResources>("processResources") { exclude("stream-protocol.xml") }

// Copy stream-protocol.xml to a dedicated SBE input directory so XInclude paths resolve
// correctly. It must sit exactly three levels under build/ so the '../../../protocol' href
// resolves to the build/protocol symlink. Do NOT copy into build/classes/java/main: a foreign
// file there gives compileJava a non-exclusive output dir, which disables its build caching.
val copyStreamProtocol =
  tasks.register<Copy>("copyStreamProtocol") {
    from("src/main/resources/stream-protocol.xml")
    into(layout.buildDirectory.dir("sbe/input/main"))
