...
class SubscriptionFactoryContextHeadersTest {
  ...
  void shouldResolveHeadersIndependentlyOfAuth() throws Exception {
    // given — OAuth auth plus org/cluster/tenant from the deployment context
    final Config config =
        new Config()
    ...
    final var deploymentContext = new DeploymentContext("org-123", "cluster-ctx", "default");
    ...
    assertThat(extractHeaders(subscription))
        .containsExactlyInAnyOrderEntriesOf(
            Map.of(
                ...
                ContextHeaders.X_PHYSICAL_TENANT_ID, "default"));
    subscription.close();
  }

  @Test
  void shouldUseConfiguredClusterIdAndContextPhysicalTenant() throws Exception {
    // given — Self-Managed style: operator clusterId, api key auth, tenant from context
    final Config config =
        new Config().setUrl("http://example.com").setApiKey("key").setClusterId("sm-cluster");
    final var deploymentContext = new DeploymentContext(null, "", "tenant-a");
    ...
    assertThat(extractHeaders(subscription))
        .containsExactlyInAnyOrderEntriesOf(
            Map.of(
                ContextHeaders.X_CLUSTER_ID, "sm-cluster",
