...
final class AppIntegrationsExporterContextHeadersIT {
  ...
  @Test
  void shouldSendContextHeadersCapturedFromContextAndEnvironment() {
    // given — org id from the environment, cluster id and physical tenant from the context
    ...
    final ExporterTestContext context =
        new ExporterTestContext().setClusterId("cluster-from-ctx").setPhysicalTenantId("tenant-a");
    ...
    wireMock.verify(
        ...
        postRequestedFor(urlEqualTo("/"))
            ...
            .withHeader(ContextHeaders.X_CLUSTER_ID, equalTo("cluster-from-ctx"))
            .withHeader(ContextHeaders.X_PHYSICAL_TENANT_ID, equalTo("tenant-a")));
  }
