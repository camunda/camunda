...
class DecisionInstanceHistoryDeletionOperationHandlerTest {
  ...
  private DecisionInstanceHistoryDeletionOperationHandler handler;

  @BeforeEach
  void setUp() {
    handler = new DecisionInstanceHistoryDeletionOperationHandler(indexName, batchOperationCache);
  ...
  void shouldExtractCorrectProcessInstanceKey() {
    final var record = createSuccessRecord();
    assertThat(handler.getProcessInstanceKey(record)).isEqualTo(-1L);
  }
}
