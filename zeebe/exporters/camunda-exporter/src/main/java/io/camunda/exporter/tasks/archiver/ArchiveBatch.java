...
public interface ArchiveBatch {
  ...
  record ProcessInstanceBatchSizes(
    ...
    List<ArchiveBatch> splitBatch(final ArchiveBatch batch, final long maxDocumentsPerBatch) {
      ...
      final List<ArchiveBatch> batches = new ArrayList<>();
    ...
    private void splitBatchByKeys(
        ...
        final List<ArchiveBatch> batches) {
      ...
      while (it.hasNext()) {
        ...
        final long currentBatchDocs = currentBatch.currentDocCount();
        if (currentBatchDocs != 0L && (currentBatchDocs + docCount) > maxDocumentsPerBatch) {
          batches.add(currentBatch.newBatch());
        }
