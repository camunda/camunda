...
public final class RecordIndexRouter {
  ...
  /**
   * For each release line, the first version whose records let OpenSearch route documents by their
   * id; records of older versions keep the legacy partition id routing, see {@link
  ...
  /**
   * Returns the routing for this record, or {@code null} to let OpenSearch route the document by
   ...
   * (typically a small number, e.g. 1, 2, 3), which OpenSearch hashes into a handful of shards with
   * collisions. Some shards then stay empty while others hold several partitions' worth of data,
   * creating hotspots. The document id is unique per record (see {@link #idFor(Record)}), so
   * routing by it spreads records evenly across all shards; returning {@code null} achieves that
   * without storing a redundant routing value, as OpenSearch falls back to hashing the id.
   *
