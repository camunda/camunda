...
public final class RecordIndexRouter {
   ...
   * keeps OpenSearch from hashing several of them onto the same shard. In the incident, two of the
   * three shards of zeebe-record_job held several million documents each while the third was empty.
   * Instead, {@link #balancedRoutingFor(int)} searches for a value that lands on the shard this
   * partition is meant to occupy, so consecutive partitions occupy consecutive shards.
   *
  ...
  /**
   * Returns a routing value that OpenSearch assigns to the shard reserved for this partition, which
   * is the partition id wrapped around the number of shards. Only the partition id decides the
   * value, so every record of a partition shares a shard.
   *
   * <p>The value is found by appending a salt to the partition id and counting up until the hash
   * lands on that shard; {@link HashUtil#getShardForRouting(String, int)} mirrors how OpenSearch
