...
import io.camunda.zeebe.exporter.RecordIndexRouter.ShardCountResolver;
...
final class IndexShardCounts implements ShardCountResolver {
  ...
  IndexShardCounts(final ElasticsearchClient client, final IndexConfiguration config) {
    this.client = client;
    this.config = config;
  }
