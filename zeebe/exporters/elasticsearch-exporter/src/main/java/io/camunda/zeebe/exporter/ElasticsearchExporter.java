...
public class ElasticsearchExporter implements Exporter {
  ...
  public void export(final Record<?> record) {
    // we would call here the initializer and pass the client.
    // do we need to pass the configuration as well!
    if (!indexTemplatesCreated) {
      new ElasticsearchExporterInitializer(configuration, client).initializeSchema();
    }
