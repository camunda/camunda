...
final class RecordIndexRouter {
   ...
   * as there are partitions (typically a small number, e.g. 1, 2, 3), which Elasticsearch hashes
   * into a handful of shards with collisions. This leaves some shards empty while others hold
   * several partitions' worth of data, distributing documents unevenly and creating hotspots.
   * Routing by the high-cardinality document id spreads records evenly across all shards. Because
   * the id is stable per record, repeated writes of the same record still resolve to the same
   * shard, preserving update idempotency.
   */
