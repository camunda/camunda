 ...
 * show that the response is read correctly, hence a real Elasticsearch.
 */
@Testcontainers
final class IndexShardCountsIT {
  ...
  static void beforeAll() {
    final var clientConfig = new ElasticsearchExporterConfiguration();
    clientConfig.url = "http://" + CONTAINER.getHttpHostAddress();
    client = ElasticsearchClientFactory.of(clientConfig);
  }
  ...
  void shouldNotResolveANumberWhenNoneIsConfigured() {
    // given - the exporter leaves the number of shards to Elasticsearch
