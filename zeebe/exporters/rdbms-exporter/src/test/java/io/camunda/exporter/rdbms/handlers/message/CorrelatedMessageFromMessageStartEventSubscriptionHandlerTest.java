...
final class CorrelatedMessageFromMessageStartEventSubscriptionHandlerTest {

  private final ProtocolFactory factory = new ProtocolFactory();

  private final CorrelatedMessageWriter correlatedMessageWriter =
      mock(CorrelatedMessageWriter.class);
  private final CorrelatedMessageFromMessageStartEventSubscriptionExportHandler underTest =
      new CorrelatedMessageFromMessageStartEventSubscriptionExportHandler(correlatedMessageWriter);
  ...
  public void shouldUpdateEntityWithAllAttributes() {
    ...
    // then
    final var itemCaptor = ArgumentCaptor.forClass(CorrelatedMessageDbModel.class);
    verify(correlatedMessageWriter).create(itemCaptor.capture());
    final CorrelatedMessageDbModel entity = itemCaptor.getValue();
