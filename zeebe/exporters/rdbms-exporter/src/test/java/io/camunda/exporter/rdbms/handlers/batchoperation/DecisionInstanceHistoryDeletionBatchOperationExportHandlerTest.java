...
class DecisionInstanceHistoryDeletionBatchOperationExportHandlerTest {
  ...
  private DecisionInstanceHistoryDeletionBatchOperationExportHandler handler;

  @BeforeEach
  void setUp() {
    handler =
        new DecisionInstanceHistoryDeletionBatchOperationExportHandler(
            batchOperationWriter, batchOperationCache, BatchOperationType.DELETE_DECISION_INSTANCE);
  ...
  void shouldExtractCorrectProcessInstanceKey() {
    final var record = createSuccessRecord();
    assertThat(handler.getProcessInstanceKey(record)).isEmpty();
  }
}
