...
class JobBatchOperationExportHandlerTest {
  ...
  private JobBatchOperationExportHandler handler;

  @BeforeEach
  void setUp() {
    handler = new JobBatchOperationExportHandler(batchOperationWriter, batchOperationCache);
  ...
  private Record<JobRecordValue> createSuccessRecord() {
    return factory.generateRecord(
        ValueType.JOB,
        b -> b.withIntent(JobIntent.UPDATED).withBatchOperationReference(batchOperationKey));
  }

  private Record<JobRecordValue> createFailureRecord() {
    return factory.generateRecord(
        ValueType.JOB,
        b ->
            b.withRejectionType(RejectionType.PROCESSING_ERROR)
                .withIntent(JobIntent.UPDATE)
  ...
  @Test
  void shouldNotTreatPriorityUpdatedAsCompleted() {
    // given - a PRIORITY_UPDATED record is a field-specific follow-up event that precedes the
    // terminal UPDATED event on the same job update; treating it as a second completion signal
    // caused the RDBMS completed-items counter to be incremented twice per job.
    final Record<JobRecordValue> record =
        factory.generateRecord(
            ValueType.JOB,
            b ->
                b.withIntent(JobIntent.PRIORITY_UPDATED)
                    .withBatchOperationReference(batchOperationKey));

    // when / then
    assertThat(handler.isCompleted(record)).isFalse();
    assertThat(handler.canExport(record)).isFalse();
    verify(batchOperationWriter, never()).updateItem(any());
  }

  @Test
  void shouldExtractCorrectItemKey() {
    final var record = createSuccessRecord();
    assertThat(handler.getItemKey(record)).isEqualTo(record.getKey());
  }
