...
public class ProcessInstanceExportHandler
  ...
  private static final Set<Intent> FLOW_NODE_INTENT = Set.of(
  ...
  public ProcessInstanceExportHandler(final FlowNodeInstanceWriter flowNodeInstanceWriter,
  ...
  private void exportFlowNode(final Record<ProcessInstanceRecordValue> record) {
    ...
    if (record.getIntent() == ProcessInstanceIntent.ELEMENT_ACTIVATING) {
      final var flowNode = new FlowNodeInstanceDbModelBuilder()
      ...
      flowNodeInstanceWriter.update(record.getKey(), FlowNodeState.COMPLETED,
          DateUtil.toOffsetDateTime(Instant.ofEpochMilli(record.getTimestamp())));
    } else if (record.getIntent() == ProcessInstanceIntent.ELEMENT_TERMINATED) {
      flowNodeInstanceWriter.update(record.getKey(), FlowNodeState.TERMINATED,
          DateUtil.toOffsetDateTime(Instant.ofEpochMilli(record.getTimestamp())));
