...
import io.camunda.zeebe.topology.api.ClusterConfigurationRequestFailedException.ConcurrentModificationException;
import io.camunda.zeebe.topology.api.ErrorResponse.ErrorCode;
import io.camunda.zeebe.topology.serializer.ClusterConfigurationRequestsSerializer;
...
public final class ClusterConfigurationRequestServer implements AutoCloseable {
  ...
  private void registerTopologyCancelHandler() {
    communicationService.replyTo(
        ...
        request -> mapClusterTopologyResponse(clusterConfigurationManagementApi.cancelTopologyChange(request)),
        this::encodeClusterTopologyResponse);
  }
  ...
  private static <T> Either<ErrorResponse, T> mapError(final Throwable throwable) {
    ...
    return switch (throwable.getCause()) {
      case final ClusterConfigurationRequestFailedException.OperationNotAllowed operationNotAllowed ->
          Either.left(
