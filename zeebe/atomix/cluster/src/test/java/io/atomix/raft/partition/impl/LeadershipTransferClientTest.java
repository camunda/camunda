...
/**
 * The coordinator reaches a partition's leader without replicating the partition, so the subjects
 * it addresses have to be the ones that partition's {@link RaftServerCommunicator} answers on.
 * These put the two side by side over a real cluster: a leader wired up exactly as a {@link
 * RaftPartitionServer} wires one, and a client that has only the partition group and id to go on.
 */
