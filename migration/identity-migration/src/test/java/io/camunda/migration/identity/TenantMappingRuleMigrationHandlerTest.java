...
public class TenantMappingRuleMigrationHandlerTest {
  ...
  void stopWhenNoMoreRecords() {
    ...
    // then
    verify(managementIdentityClient, times(2)).fetchTenantMappingRules(any(), anyInt());
    verify(tenantService, times(4)).fetchOrCreateTenant(any(), any());
    verify(tenantService, times(4)).assignMemberToTenant(any(), any());
  ...
  void setErrorWhenMappingRuleCreationFailed() {
    ...
    // then
    verify(managementIdentityClient, times(2)).fetchTenantMappingRules(any(), anyInt());
    verify(tenantService, never()).fetchOrCreateTenant(any(), any());
    verify(mappingService, times(2)).findOrCreateMapping(any(), any(), any());
  ...
  private void givenMappingRules() {
    when(managementIdentityClient.fetchTenantMappingRules(any(), anyInt()))
        .thenReturn(
            List.of(
