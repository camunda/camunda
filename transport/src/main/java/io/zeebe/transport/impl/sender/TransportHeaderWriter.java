...
public class TransportHeaderWriter
{
    ...
    static
    {
        REQUEST_HEADER_LENGTH = DataFrameDescriptor.HEADER_LENGTH +
            TransportHeaderDescriptor.HEADER_LENGTH +
            RequestResponseHeaderDescriptor.HEADER_LENGTH;

        MESSAGE_HEADER_LENGTH = DataFrameDescriptor.HEADER_LENGTH +
            TransportHeaderDescriptor.HEADER_LENGTH;

        STREAM_ID_OFFSET = DataFrameDescriptor.STREAM_ID_OFFSET;

        REQUEST_ID_OFFSET = DataFrameDescriptor.HEADER_LENGTH +
            TransportHeaderDescriptor.HEADER_LENGTH +
    ...
    public void wrapRequest(MutableDirectBuffer buffer, BufferWriter messageWriter)
    {
        ...
        bufferView.putShort(DataFrameDescriptor.HEADER_LENGTH + TransportHeaderDescriptor.PROTOCOL_ID_OFFSET, TransportHeaderDescriptor.REQUEST_RESPONSE);
    ...
    public void wrapMessage(MutableDirectBuffer buffer, BufferWriter messageWriter, int remoteStreamId)
    {
        ...
        bufferView.putShort(DataFrameDescriptor.HEADER_LENGTH + TransportHeaderDescriptor.PROTOCOL_ID_OFFSET, TransportHeaderDescriptor.FULL_DUPLEX_SINGLE_MESSAGE);
