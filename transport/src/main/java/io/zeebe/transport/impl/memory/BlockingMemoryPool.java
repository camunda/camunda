 ...
 *<p>
 * Current implementation does not actually "pool" or "recycle" the memory, it leaves that
 * to GC.
 *<p>
 * It does however put a fixed size on how much memory can be in use at any given point in time.
 * When no more memory is available, it blocks for the specified time until one of the following
 * events occurs:
 * a) the maxBlocktime is reached. In this case the allocation times out and no memory can be allocated.
 * b) memory becomes available and can be allocated
 *<p>
 * The main usecase for this pool is the zeebe client where it is desired behavior to block client
 * threads on memory backpressure
 */
        ...
        try
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    LOG.trace("Waiting for {}ms for memory to be reclaimed.", waitTime);

                    try
                    {
                        memoryReclaimed.await(waitTime, TimeUnit.MILLISECONDS);
                    }
