...
public class RequestWithRetriesTest
{
    ...
    protected Queue<ActorFuture<ClientRequest>> pendingRequests = new ArrayDeque<>();
    ...
    public void shouldEchoMessages() throws Exception
    {
        ...
        final int maxNumRequestsInFlight = MAX_CONCURRENT_REQUESTS;
        ...
        {
            while (numRequestsSent < numRequests && numRequestsSent - numResponsesReceived < maxNumRequestsInFlight)
            {
                sendRequest(clientTransport, remote, numRequestsSent);
                numRequestsSent++;
            }

            final ActorFuture<ClientRequest> nextPendingRequest = pendingRequests.peek();

            if (nextPendingRequest != null && nextPendingRequest.isDone())
            {
                final ClientRequest request = nextPendingRequest.join();
    ...
    protected void sendRequest(ClientTransport client, RemoteAddress remote, int payload)
    {
        messageBuffer.putInt(0, payload);
        bufferWriter.wrap(messageBuffer, 0, messageBuffer.capacity());
        final ActorFuture<ClientRequest> request = client.getOutput().sendRequestWithRetry(remote, bufferWriter);
