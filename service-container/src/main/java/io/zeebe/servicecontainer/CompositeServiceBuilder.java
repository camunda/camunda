 ...
 * one of the services fails to install, all services are removed again).
 */
...
public class CompositeServiceBuilder
{
    ...
    public CompositeServiceBuilder(ServiceName<Void> name, ServiceContainer container, ServiceName<?>... additionalDependencies)
    {
        ...
        final ServiceBuilder<Void> installServiceBuilder = container.createService(compositeServiceName, new CompositeService());
    ...
    private final class CompositeInstallCompletion<S> extends Actor
    {
        ...
        CompositeInstallCompletion(CompletableActorFuture<S> future,
        ...
        protected void onActorStarted()
        {
            ...
            {
                actor.runOnCompletion(futures.getValue(), (val, t) ->
                {
                    if (t != null)
                    {
                        actor.runOnCompletion(container.removeService(compositeServiceName), (v, e) ->
                        {
                    ...
                    else
                    {
                        ...
                        {
                            future.completeWith((CompletableActorFuture<S>) installFutures.get(returnedServiceName));
                            actor.close();
                        }
                    }
                });
