...
public class PartitionHolder {
  ...
  protected Set<Integer> extractCurrentNodePartitions(Set<Integer> partitionIds) {
    Integer[] configuredIds = operateProperties.getImportProperties().getPartitionIds();
    if (configuredIds != null && configuredIds.length > 0) {
      partitionIds.retainAll(Arrays.asList(configuredIds));
    } else if (operateProperties.getImportProperties().getNodeCount() != null &&
              operateProperties.getImportProperties().getCurrentNodeId() != null){
      Integer nodeCount = operateProperties.getImportProperties().getNodeCount();
      Integer nodeId = operateProperties.getImportProperties().getCurrentNodeId();
      Integer partitionCount = partitionIds.size();
      Integer start = ((int) partitionCount / nodeCount) * nodeId + 1;
      Integer end;
      //last node
      if (nodeId.equals(nodeCount - 1)) {
        end = partitionCount;
      } else {
        end = start + partitionCount / nodeCount - 1;
      }
      partitionIds = new HashSet<>(CollectionUtil.fromTo(start, end));
    }
    return partitionIds;
  }
