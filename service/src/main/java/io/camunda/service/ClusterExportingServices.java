 ...
 *
 * <p>The change is submitted as a single atomic plan covering every physical tenant (see {@link
 * ExportingStateController#clusterWide()}), not a fan-out of one request per tenant, so there is no
 * partial-application state to reason about: the coordinator either admits and applies the whole
 * plan or rejects it outright.
 */
@NullMarked
