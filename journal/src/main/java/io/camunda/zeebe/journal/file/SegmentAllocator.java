...
interface SegmentAllocator {
   ...
   * channel.
   *
   * @param descriptor an open descriptor to the file to pre-allocate
   ...
   */
  void allocate(final FileDescriptor descriptor, FileChannel channel, final long segmentSize)
      throws IOException;
  ...
  static SegmentAllocator fill() {
    return (descriptor, channel, size) -> IoUtil.fill(channel, 0, size, (byte) 0);
  }
