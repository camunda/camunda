...
while IFS= read -r entry; do
  ...
  for (( bp_idx=0; bp_idx<bp_count; bp_idx++ )); do
    ...
    existing_comment_data=$(gh api "repos/${repo}/issues/${bp_num}/comments" \
      --jq "[.[] | select(.body | contains(\"${COMMENT_MARKER}\"))] | if length > 0 then .[0] else null end" 2>/dev/null || echo "null")
...
<sub>🤖 Auto-generated by the stale backport tracker.</sub>"
    ...
    existing_orig_comment=$(gh api "repos/${repo}/issues/${orig_num}/comments" \
      --jq "[.[] | select(.body | contains(\"${COMMENT_MARKER}\"))] | length" 2>/dev/null || echo "0")
