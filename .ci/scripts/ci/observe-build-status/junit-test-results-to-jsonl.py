...
def parse_test_files(file_paths):
    for file_path in file_paths:
        is_flaky = 'FLAKY.xml' in file_path
...
def output_results(results):
    for result in results:
        json_line = json.dumps(result, separators=(',', ':'))
        print(json_line)


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 junit-test-results-to-jsonl.py file1.xml [file2.xml ...]", file=sys.stderr)
        sys.exit(1)
