...
/**
 * Since the operator fields in the VariableValueFilter are reserved keywords in mybatis (OGNL expression language),
 * we need our own representation of it. (VariableValueDbFilter)
 */
public record VariableDbFilter(
  ...
  static VariableValueDbFilter map(final VariableValueFilter valueFilter) {
    ...
    return new VariableValueDbFilter(
        valueFilter.name(),
        operator,
        value,
        numericNeeded
