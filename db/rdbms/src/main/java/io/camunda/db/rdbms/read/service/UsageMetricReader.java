...
public class UsageMetricReader {
  ...
  public UsageMetricStatisticsEntity usageMetricStatistics(final UsageMetricsFilter filter) {
    ...
    if (filter.tenants()) {
      final var res = usageMetricMapper.usageMetricTenantsStatistics(filter);
      return UsageMetricEntityMapper.toEntity(res);
    } else {
      final var res2 = usageMetricMapper.usageMetricStatistics(filter);
      return new UsageMetricStatisticsEntity(
          ofNullable(res2.rpi()).orElse(0L),
          ofNullable(res2.edi()).orElse(0L),
          ofNullable(res2.at()).orElse(0L),
          null);
    }
  }
}
