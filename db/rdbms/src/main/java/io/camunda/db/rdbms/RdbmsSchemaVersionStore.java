...
public class RdbmsSchemaVersionStore {
  ...
  /**
   * The timeout {@link #boundedStatement} applies. A seam for tests that need contention to settle
   * in longer than this many seconds without slowing down every other caller of this class: an
   * empirical, per-vendor check of whether a driver honors a query timeout under row-lock
   * contention doesn't depend on the exact value, only on one being enforced, so a test can
   * override this to shrink the wait instead of waiting out {@link #STATEMENT_TIMEOUT_SECONDS}.
   */
  @VisibleForTesting
  protected int statementTimeoutSeconds() {
    return STATEMENT_TIMEOUT_SECONDS;
  }
   ...
   */
  private PreparedStatement boundedStatement(final Connection connection, final String sql)
      throws SQLException {
