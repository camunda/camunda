 ...
 * successfully loaded value or {@link #UNKNOWN_ROW_COUNT} if none has completed loading yet. Once a
 * value is due for refresh, Caffeine reloads it asynchronously on the given {@code executor} while
 * continuing to serve the previous value in the meantime. This matters because {@link
 * #getRowCount(String)} backs a Prometheus gauge - a slow or unreachable database must never be
 * able to block whoever is reading this metric (e.g. a scrape request thread).
 */
public class RdbmsTableRowCountProvider {
   ...
   * @return the row count, or {@link #UNKNOWN_ROW_COUNT} if the table is unknown, its first load
   *     hasn't completed yet, or the load failed
   */
  public long getRowCount(final String tableName) {
    ...
    try {
      ...
      // Defensive only: fetchRowCount() already catches its own exceptions, so the future should
      // never complete exceptionally. Guards against the unexpected (e.g. executor rejection)
      // without ever letting an exception reach whoever is reading this metric.
