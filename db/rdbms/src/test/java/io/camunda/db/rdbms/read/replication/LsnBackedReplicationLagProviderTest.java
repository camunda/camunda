...
class LsnBackedReplicationLagProviderTest {
  ...
  void shouldMapLsnStatusesToLagStatusesDroppingLogStatus() {
    ...
    when(lsnProvider.getReplicationStatuses())
        .thenReturn(
            List.of(
                new ReplicationLsnStatus(10L, "replica-1", 1_000L),
                new ReplicationLsnStatus(20L, "replica-2", 2_000L)));
    ...
    assertThat(statuses)
        .containsExactly(
            new ReplicationLagStatus("replica-1", 1_000L),
            new ReplicationLagStatus("replica-2", 2_000L));
  }
