...
class HistoryCleanupServiceTest {
  ...
  void setUp() {
    ...
    when(processInstanceWriter.selectExpiredProcessInstances(anyInt(), any(), anyInt()))
  ...
  void testFirstCleanupHistory() {
    ...
    verify(processInstanceWriter).selectExpiredProcessInstances(PARTITION_ID, CLEANUP_DATE, 100);
