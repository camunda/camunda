...
class UserTaskDbReaderTest {
  ...
  void shouldReturnEmptyPageWhenPageSizeIsZero() {
    when(userTaskMapper.count(any())).thenReturn(21L);

    final UserTaskQuery query = UserTaskQuery.of(b -> b.page(p -> p.size(0)));
    final ResourceAccessChecks resourceAccessChecks =
        ResourceAccessChecks.of(AuthorizationCheck.disabled(), TenantCheck.disabled());

    final var result = userTaskDbReader.search(query, resourceAccessChecks);

    assertThat(result.total()).isEqualTo(21L);
    assertThat(result.items()).isEmpty();
    verify(userTaskMapper, times(0)).search(any());
  }
}
