...
class BackupManagerElasticsearchTest {
  ...
  @InjectMocks @Spy private BackupManagerElasticSearch backupManager;
  ...
  public void shouldWaitForSnapshotWithTimeout() {
    ...
    doReturn(List.of(snapshotInfo)).when(backupManager).findSnapshots(backupId);

    final boolean finished =
        backupManager.isSnapshotFinishedWithinTimeout(snapshotInfo.snapshotId().getName());

    assertFalse(finished);
    verify(backupManager, atLeast(5)).findSnapshots(backupId);
  }
  ...
  public void shouldWaitForSnapshotTillCompleted() throws IOException {
    ...
    for (int i = 0; i < numberOfSnapshots; i++) {
      ...
      when(snapshotInfo.snapshotId().getName()).thenReturn(metadata.get(i).buildSnapshotName());
      when(snapshotInfo.userMetadata())
          .thenReturn(MAPPER.convertValue(metadata.get(i), new TypeReference<>() {}));
    ...
    for (int i = 0; i < numberOfSnapshots; i++) {
      backupManager.executeSnapshotting(
          new CreateSnapshotRequest()
              ...
              .snapshot(metadata.get(i).buildSnapshotName())
              .userMetadata(MAPPER.convertValue(metadata.get(i), new TypeReference<>() {})));
    }
    verify(searchClient.snapshot(), times(numberOfSnapshots))
        .createAsync(any(), any(), captor.capture());
    captor.getValue().onFailure(new SocketTimeoutException());
    verify(backupManager, times(3)).findSnapshots(backupId);
