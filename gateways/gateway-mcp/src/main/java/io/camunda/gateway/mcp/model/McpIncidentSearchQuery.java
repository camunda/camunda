...
/**
 * MCP-specific incident search query facade. Wraps the MCP-owned filter with existing page/sort
 * types. Used with {@code @McpToolParamsUnwrapped} to present filter/page/sort as top-level tool
 * parameters.
 */
@JsonInclude(JsonInclude.Include.ALWAYS)
public record McpIncidentSearchQuery(
    @JsonProperty
        @JsonPropertyDescription("The incident search filters.")
        @Schema(requiredMode = RequiredMode.NOT_REQUIRED, defaultValue = "##default")
        McpIncidentFilter filter,
    @JsonProperty
