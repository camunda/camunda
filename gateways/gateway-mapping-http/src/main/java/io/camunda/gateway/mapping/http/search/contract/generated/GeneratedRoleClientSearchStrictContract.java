...
public record GeneratedRoleClientSearchStrictContract(
  ...
  public static java.util.List<GeneratedRoleClientStrictContract> coerceItems(final Object value) {
    ...
    for (final var item : listValue) {
      if (item == null) {
        result.add(null);
      } else if (item instanceof GeneratedRoleClientStrictContract strictItem) {
        result.add(strictItem);

      } else {
        throw new IllegalArgumentException(
            "items must contain only GeneratedRoleClientStrictContract items, but got "
  ...
  public static final class Builder implements ItemsStep, OptionalStep {
    ...
    public GeneratedRoleClientSearchStrictContract build() {
      return new GeneratedRoleClientSearchStrictContract(coerceItems(this.items));
    }
  }
