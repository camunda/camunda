...
public class ElasticsearchSchemaManager {
  ...
  public boolean schemaAlreadyExists() {
    try {
      final String indexName = indexDescriptors.get(0).getAlias();
      return esClient.indices().exists(new GetIndexRequest(indexName), RequestOptions.DEFAULT);
    } catch (IOException e) {
      final String message =
          String.format("Exception occurred, while checking schema existence: %s", e.getMessage());
  ...
  protected boolean putIndexTemplate(
