...
//                                            },
            ...
            // 43.Test
            {
                ...
                // expected result
                new String(Files.readAllBytes(Paths.get(MappingExtractParameterizedTest.class.getResource("largeChangedJsonDocument.json")
                                                                                             .toURI()))) },
            // 44.Test
            {
                // source
    ...
    {
        // given payload
        final byte[] sourceBytes = MSGPACK_MAPPER.writeValueAsBytes(JSON_MAPPER.readTree(sourcePayload));
        final DirectBuffer sourceDocument = new UnsafeBuffer(sourceBytes);

        final byte[] targetBytes = MSGPACK_MAPPER.writeValueAsBytes(JSON_MAPPER.readTree(targetPayload));
