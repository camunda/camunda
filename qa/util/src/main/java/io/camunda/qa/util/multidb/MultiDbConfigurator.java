...
public class MultiDbConfigurator {
  ...
  public void configureRDBMSSupport(
    ...
    configureRDBMSSupport(
        retentionEnabled, url, username, password, driverClass, generateTablePrefix(), true);
  }

  /**
   * Configures RDBMS secondary-storage support with an explicit table prefix and optional Liquibase
   * auto-DDL flag.
   *
   * <p>When {@code autoDdl} is {@code false}, the Spring property {@code
   * camunda.data.secondary-storage.rdbms.auto-ddl} is set to {@code false}, which causes the
   * application to use the {@code NoopSchemaManager} instead of running Liquibase migrations. This
   * is used by the Oracle test path where the schema is pre-initialized from a generated SQL
   * template to avoid expensive Liquibase precondition evaluation at test startup.
   *
   * @param retentionEnabled whether history retention is enabled
   * @param url JDBC URL
   * @param username database username
   * @param password database password
   * @param driverClass fully-qualified JDBC driver class name
   * @param tablePrefix per-test table prefix (must already be generated by the caller)
   * @param autoDdl when {@code true} Liquibase runs at startup; when {@code false} it is skipped
   */
  public void configureRDBMSSupport(
      final boolean retentionEnabled,
      final String url,
   ...
   */
  public static String generateTablePrefix() {
