  ...
  private HttpResponse<String> sendRequest(
    ...
    try {
      ...
      if (authentication != null) {
        final var basicAuth =
            "%s:%s".formatted(authentication.username(), authentication.password());
        final var basicAuthEncoded = Base64.getEncoder().encodeToString(basicAuth.getBytes());
        requestBuilder.header("Authorization", "Basic %s".formatted(basicAuthEncoded));
      }
