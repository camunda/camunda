...
class ClusterPage {
  ...
  async clickCreateNewClusterButton(): Promise<void> {
    try {
      ...
      await this.page.getByRole('link', {name: 'Clusters'}).click();
    }
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries - 1) {
          console.warn(
            `Cluster health check attempt ${attempt + 1} failed. Retrying...`,
          );
  ...
  async clickClusterOption(): Promise<void> {
    ...
    for (const clickTab of tabs) {
      try {
        ...
        await this.clusterVersionOption.click({timeout: 120000});
        return;
      } catch (error) {
