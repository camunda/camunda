...
class ClusterDetailsPage {
      ...
      try {
        if (retries === 0) {
          ...
          await this.page
            ...
            .click({timeout: 60000});
          await await this.createFirstClientButton.click({timeout: 60000});
        }
        return;
      } catch (error) {
  ...
  async clickSettingsTab(): Promise<void> {
    ...
    try {
      await expect(
        this.page.getByRole('heading', {name: 'cluster settings'}),
      ).toBeVisible({
        timeout: 120000,
      });
    } catch (error) {
    ...
    if (toggleTextElements.length >= 2) {
      ...
      if (toggleText === 'Disabled') {
        ...
        if (toggleSwitchElements.length >= 2) {
          ...
          await this.rbaEnableButton.click({timeout: 60000});
        } else {
    ...
    if (toggleTextElements.length >= 2) {
      ...
      if (toggleText === 'Enabled') {
        ...
        if (toggleSwitchElements.length >= 2) {
          ...
          console.log('Second toggle switch clicked.');
          await expect(this.dialog).toBeVisible({timeout: 60000});
          await this.rbaDisableButton.click({timeout: 60000});
        } else {
          console.log('Second toggle switch not found or less than 2.');
        }
      } else {
