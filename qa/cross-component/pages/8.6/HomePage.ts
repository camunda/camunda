...
class HomePage {
  ...
  async clickCamundaComponents(): Promise<void> {
    try {
      await this.camundaComponentsButton.click({timeout: 120000});
    } catch (error) {
      // If clicking camundaAppsButton fails, try camundaComponentsButton
      try {
        await this.camundaAppsButton.click({timeout: 120000});
      } catch (error) {
        // If clicking camundaComponentsButton also fails, click appSwitcher
        try {
          await this.appSwitcher.click({timeout: 120000});
        } catch (error) {
  ...
  async clickCamundaApps(): Promise<void> {
    try {
      ...
      try {
        ...
        // If clicking camundaComponentsButton also fails, click appSwitcher
        try {
          await this.appSwitcher.click({timeout: 90000});
        } catch (error) {
          // If all attempts fail, throw an error
          throw new Error('Failed to click any button');
        }
