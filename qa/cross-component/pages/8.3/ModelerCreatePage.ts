...
class ModelerCreatePage {
  ...
  async clickAppendTaskButton(): Promise<void> {
    try {
      await this.appendTaskButton.click({timeout: 120000, force: true});
    } catch (error) {
      const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));
      await sleep(10000);
      await this.appendTaskButton.click({timeout: 90000, force: true});
    }
  }

  async clickChangeTypeButton(): Promise<void> {
    await this.changeTypeButton.click({timeout: 120000});
  }
  ...
  async clickWebhookTab(): Promise<void> {
    await this.webhookTab.click({timeout: 90000});
  }
  ...
  async clickCreateEndEvent(): Promise<void> {
    await this.createEndEvent.click({timeout: 100000});
  }
  ...
  async clickEndEventCanvas(): Promise<void> {
    await this.endEventCanvas.click({timeout: 100000});
  }
