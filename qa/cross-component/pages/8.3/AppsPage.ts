...
class AppsPage {
  ...
  async clickAppSwitcherButton(): Promise<void> {
    try {
      await this.appSwitcherButton.click({timeout: 30000});
    } catch (error) {
      await this.camundaAppsButton.click({timeout: 30000});
    }
  }
  ...
  async clickTasklistLink(): Promise<void> {
    ...
    for (let retries = 0; retries < maxRetries; retries++) {
      try {
        if (retries === 0) {
          await this.tasklistLink.click({timeout: 30000});
        } else {
          await this.clickAppSwitcherButton();
          await this.tasklistLink.click({timeout: 30000});
        }
        return;
      } catch (error) {
  ...
  async clickOptimizeLink(): Promise<void> {
    ...
    for (let retries = 0; retries < maxRetries; retries++) {
      try {
        if (retries === 0) {
          await this.optimizeLink.click({timeout: 30000});
        } else {
          await this.camundaComponentsButton.click({timeout: 30000});
          await this.optimizeLink.click({timeout: 30000});
        }
