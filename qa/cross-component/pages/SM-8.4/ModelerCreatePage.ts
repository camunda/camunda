...
class ModelerCreatePage {
  ...
  readonly modelerCanvas: Locator;
  ...
  async clickStartEventElement(): Promise<void> {
    if (await this.changeTypeButton.isVisible()) {
      console.log('Start Event Already active');
    } else {
  ...
  async clickNewForm(diagramName: string): Promise<void> {
    try {
      await this.newForm.click({timeout: 60000});
    } catch {
      const modelerHomePage = new ModelerHomePage(this.page);
      ...
      await modelerHomePage.clickProjectBreadcrumb();
      await this.page.getByText(diagramName).first().click();
  ...
  async clickCanvas(): Promise<void> {
    await this.modelerCanvas.click();
  }
  ...
  async clickImplementTab(): Promise<void> {
