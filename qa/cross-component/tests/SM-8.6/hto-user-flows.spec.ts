...
import {deleteAllUserGroups} from '@pages/SM-8.6/UtlitiesPage';
...
test.describe('HTO User Flow Tests', () => {
  ...
  test('User Task Restrictions Enabled Flow - Candidate User', async ({
    ...
    await test.step('Navigate to Operate and Assert Process Instance', async () => {
      ...
      await operateProcessInstancePage.activeUserTaskIconVisibleAssertion(
        'state-overlay-candidateUserTask' + processName + '1' + '-active',
      );
      await operateProcessInstancePage.activeUserTaskIconVisibleAssertion(
        'state-overlay-candidateUserTask' + processName + '2' + '-active',
      );
      await operateProcessInstancePage.activeUserTaskIconNotVisibleAssertion(
        'state-overlay-candidateUserTask' + processName + '3' + '-active',
      );
    });
  });
  test('User Task Restrictions Enabled Flow - Candidate Group', async ({
    ...
    await test.step('Navigate to Operate and Assert Process Instance', async () => {
      ...
      await operateProcessInstancePage.activeUserTaskIconVisibleAssertion(
        'state-overlay-candidateGroupTask' + processName + '1' + '-active',
      );
      await operateProcessInstancePage.activeUserTaskIconNotVisibleAssertion(
        'state-overlay-candidateGroupTask' + processName + '2' + '-active',
      );
    });
  });
});
