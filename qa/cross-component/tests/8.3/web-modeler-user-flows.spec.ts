...
import {LoginPage} from '@pages/8.3/LoginPage';
import {ModelerHomePage} from '@pages/8.3/ModelerHomePage';
import {ConsoleOrganizationPage} from '@pages/8.3/ConsoleOrganizationPage';
import {HomePage} from '@pages/8.3/HomePage';
import {SettingsPage} from '@pages/8.3/SettingsPage';
import {ModelerCreatePage} from '@pages/8.3/ModelerCreatePage';
import {MailSlurp} from 'mailslurp-client';
import {clickInvitationLinkInEmail} from '@pages/8.3/UtilitiesPage';
...
test.describe('Web Modeler User Flow Tests', async () => {
  ...
  test('Form.js Integration with User Task and AI Generated Form', async ({
    ...
    await test.step('Create an AI Generated Form', async () => {
      ...
      await formJsPage.fillFormRequestInput('A form with 1 text field');
    ...
    await test.step('View Process Instance in Operate and complete User Task in Tasklist', async () => {
      ...
      const operateTab = await page.waitForEvent('popup', {timeout: 120000});
      ...
      const tasklistTab = await operateTab.waitForEvent('popup');
      ...
      await tasklistTabTaskPanelPage.openTask('User_Task_Process_With_AI_Form');
      await tasklistTabTaskDetailsPage.clickAssignToMeButton();
      await tasklistTabTaskDetailsPage.clickTextInput();
      await tasklistTabTaskDetailsPage.fillTextInput('AI Form Test Text');
  ...
  // test will be skipped until CI flaky behavior is resolved
  test.skip('User Roles User Flow', async ({
    ...
    await test.step('Change User Roles and Assert This Updates in Web Modeler', async () => {
      ...
      const consoleTabPromise = page.waitForEvent('popup', {timeout: 60000});
  ...
  test('Form.js Integration with AI generated Start form', async ({
    ...
    await test.step('Create BPMN Diagram with a AI generated Start Form and Start Process Instance', async () => {
      ...
      await modelerCreatePage.fillStartformPayload('AI-generated-Start-Forms');
      await modelerCreatePage.clickStartInstanceSubButton();
    });
  ...
  test('Invite New C8 User To Web Modeler Flow', async ({
    ...
    await test.step('Login as Project Admin', async () => {
