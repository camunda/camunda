...
public class InheritedSimpleMappingAuthorizationIT {
  ...
  @ClientDefinition
  private static final TestClient MAPPING_RULE_THROUGH_AUTHORIZED_GROUP = createTestClient();

  @ClientDefinition
  private static final TestClient MAPPING_RULE_THROUGH_UNAUTHORIZED_GROUP = createTestClient();

  @ClientDefinition
  private static final TestClient MAPPING_RULE_THROUGH_AUTHORIZED_ROLE = createTestClient();

  @ClientDefinition
  private static final TestClient MAPPING_RULE_THROUGH_UNAUTHORIZED_ROLE = createTestClient();

  @ClientDefinition
  private static final TestClient MAPPING_RULE_THROUGH_GROUP_THROUGH_AUTHORIZED_ROLE =
      createTestClient();

  @ClientDefinition
  private static final TestClient MAPPING_RULE_THROUGH_GROUP_THROUGH_UNAUTHORIZED_ROLE =
      createTestClient();
  ...
  private static final TestGroup UNAUTHORIZED_GROUP =
      TestGroup.withoutPermissions(
          ...
          List.of(
              new Membership(
                  MAPPING_RULE_THROUGH_UNAUTHORIZED_GROUP.clientId(), EntityType.CLIENT)));
  ...
  private static final TestGroup AUTHORIZED_GROUP =
      new TestGroup(
          ...
          List.of(
              new Membership(
                  MAPPING_RULE_THROUGH_AUTHORIZED_GROUP.clientId(), EntityType.MAPPING_RULE)));
  ...
  private static final TestGroup GROUP_THROUGH_AUTHORIZED_ROLE =
      TestGroup.withoutPermissions(
          ...
          List.of(
              new Membership(
                  MAPPING_RULE_THROUGH_GROUP_THROUGH_AUTHORIZED_ROLE.clientId(),
  ...
  //                  ResourceType.GROUP, PermissionType.READ, List.of(UNAUTHORIZED_GROUP.id()))),
  //          List.of(
  //              new Membership(MAPPING_RULE_THROUGH_AUTHORIZED_ROLE.id(),
  // EntityType.MAPPING_RULE),
  ...
  //          "unauthorized",
  //          List.of(
  //              new Membership(MAPPING_RULE_THROUGH_UNAUTHORIZED_ROLE.id(),
  // EntityType.MAPPING_RULE),
  //              new Membership(GROUP_THROUGH_UNAUTHORIZED_ROLE.id(), EntityType.GROUP)));

  @ParameterizedTest
  @MethodSource("provideAuthorizedMappingRules")
  void shouldBeAuthorizedToDeploy(final String simpleMappingUsername, @TempDir final Path tempDir) {
    // given
    try (final CamundaClient client = createClient(simpleMappingUsername, tempDir)) {
  ...
  //  @ParameterizedTest
  //  @MethodSource("provideUnauthorizedMappingRules")
  //  void shouldBeUnauthorizedToRead(final TestMappingRule mappingRule, @TempDir final Path
  // tempDir) {
  //    // given
  //    try (final CamundaClient client = createClient(mappingRule, tempDir)) {
  //
  ...
  private static Stream<String> provideAuthorizedMappingRules() {
    //    return Stream.of(
    ...
    //        Named.of(
    //            "#userThroughGroupThroughRole",
    // MAPPING_RULE_THROUGH_GROUP_THROUGH_AUTHORIZED_ROLE));
  ...
  private static Stream<Named<TestMappingRule>> provideUnauthorizedMappingRules() {
    ...
    //        Named.of(
    //            "#userThroughGroupThroughRole",
    // MAPPING_RULE_THROUGH_GROUP_THROUGH_UNAUTHORIZED_ROLE));
    return Stream.of();
  }
