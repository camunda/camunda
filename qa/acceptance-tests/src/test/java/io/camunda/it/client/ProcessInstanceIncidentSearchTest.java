...
class ProcessInstanceIncidentSearchTest {
  ...
  private static List<ProcessInstance> PROCESS_INSTANCES_SORTED_BY_KEY = new ArrayList<>();
  private static List<Incident> INCIDENTS_SORTED_BY_KEY = new ArrayList<>();
  ...
  static void beforeAll() {
    ...
    PROCESS_INSTANCES_SORTED_BY_KEY =
        camundaClient
  ...
  void shouldSortByIncidentKey() {
    ...
    final long processInstanceKey =
        PROCESS_INSTANCES_SORTED_BY_KEY.getFirst().getProcessInstanceKey();
    ...
    // then
    assertThat(incidentsResult.items().size()).isEqualTo(INCIDENTS_SORTED_BY_KEY.size());

    final var sortedIncidentsInDesc =
        INCIDENTS_SORTED_BY_KEY.stream()
  ...
  void shouldPaginateWithLimitAndOffset() {
    ...
    // then
    assertThat(response1.page().totalItems()).isEqualTo(INCIDENTS_SORTED_BY_KEY.size());
    assertThat(response1.items()).containsExactly(INCIDENTS_SORTED_BY_KEY.getFirst());

    assertThat(response2.page().totalItems()).isEqualTo(INCIDENTS_SORTED_BY_KEY.size());
    assertThat(response2.items()).containsExactly(INCIDENTS_SORTED_BY_KEY.get(1));
  }

  private static Stream<Arguments> provideProcessInstanceKeyAndExpectedAmountOfIncidents() {
    final var processInstance1 = PROCESS_INSTANCES_SORTED_BY_KEY.get(0);
    final var processInstance1Incidents = INCIDENTS_SORTED_BY_KEY;

    final var processInstance2 = PROCESS_INSTANCES_SORTED_BY_KEY.get(1);
    final var processInstance2Incidents = INCIDENTS_SORTED_BY_KEY;

    final var processInstance3 = PROCESS_INSTANCES_SORTED_BY_KEY.get(2);
    final var processInstance3Incidents = List.of(INCIDENTS_SORTED_BY_KEY.get(1));
