...
public class UserTaskIT {
    ...
    public void shouldCreateAndFindUserTaskWithoutCandidatesByKey(
        ...
        final UserTaskDbModel userTask = UserTaskFixtures
                .createRandomized(b -> b.candidateGroups(null).candidateUsers(null));
        createAndSaveUserTask(rdbmsService, userTask);
    ...
    public void shouldUpdateAndFindUserTaskByKey(final CamundaRdbmsTestApplication testApplication) {
        ...
        final UserTaskDbModel updatedModel = userTask.toBuilder().state(UserTaskState.COMPLETED)
                .completionDate(NOW.plusDays(2)).build();
    ...
    public void shouldUpdateAndFindUserTaskWithoutCandidatesByKey(
        ...
        final UserTaskDbModel updatedModel = userTask.toBuilder()
    ...
    public void shouldDeletingExistingCandidates(final CamundaRdbmsTestApplication testApplication) {
        ...
        final UserTaskDbModel updatedModel = userTask.toBuilder().candidateGroups(null).candidateUsers(null).build();
        final RdbmsWriter writer = rdbmsService.createWriter(1L);
        writer.getUserTaskWriter().update(updatedModel);
        writer.flush();
    ...
    public void shouldFindUserTaskByProcessInstanceVariableName(
        ...
        final VariableDbModel randomizedVariable = VariableFixtures
                .createRandomized(b -> b.processInstanceKey(userTask.processInstanceKey()));
        final RdbmsWriter writer = rdbmsService.createWriter(1L);
                ...
                .search(
                        new UserTaskQuery(
                                new UserTaskFilter.Builder()
                                        .localVariables(
                                                List.of(
                                                        new VariableValueFilter.Builder().name("localVariable")
                                                                .build()))
                                        .build(),
                                UserTaskSort.of(b -> b),
                                SearchQueryPage.of(b -> b.from(0).size(10))));
    ...
    public void shouldFindUserTaskByCreationDateGt(
        ...
        final UserTaskDbModel userTask = UserTaskFixtures.createRandomized(b -> b.creationDate(creationDate));
        createAndSaveUserTask(rdbmsService, userTask);
    ...
    public void shouldFindUserTaskByCompletionDateGte(
        ...
        final UserTaskDbModel userTask = UserTaskFixtures.createRandomized(b -> b.completionDate(completionDate));
        createAndSaveUserTask(rdbmsService, userTask);
    ...
    public void shouldFindUserTaskByCompletionDateLt(
        ...
        final var searchResultGte = rdbmsService
                .getUserTaskReader()
                .search(
                        new UserTaskQuery(
                                new UserTaskFilter.Builder()
    ...
    public void shouldFindUserTaskByFollowUpDateGt(
        ...
        final UserTaskDbModel userTask = UserTaskFixtures.createRandomized(b -> b.followUpDate(followUpDate));
        createAndSaveUserTask(rdbmsService, userTask);
    ...
    public void shouldFindWithSearchAfter(final CamundaRdbmsTestApplication testApplication) {
        ...
        final var searchResult = reader.search(
                UserTaskQuery.of(
                        b -> b.filter(f -> f.tenantIds("tenant-1337"))
                                .sort(sort)
        ...
        final var firstPage = reader.search(
                UserTaskQuery.of(
                        b -> b.filter(f -> f.tenantIds("tenant-1337")).sort(sort).page(p -> p.size(15))));

        final var nextPage = reader.search(
                UserTaskQuery.of(
                        b -> b.filter(f -> f.tenantIds("tenant-1337"))
                                .sort(sort)
    ...
    public void shouldCleanup(final CamundaRdbmsTestApplication testApplication) {
        ...
        final var definition = ProcessDefinitionFixtures.createAndSaveProcessDefinition(rdbmsWriter, b -> b);
        final var item1 = createAndSaveUserTask(
                rdbmsWriter, b -> b.processDefinitionKey(definition.processDefinitionKey()));
        final var item2 = createAndSaveUserTask(
                rdbmsWriter, b -> b.processDefinitionKey(definition.processDefinitionKey()));
        final var item3 = createAndSaveUserTask(
                rdbmsWriter, b -> b.processDefinitionKey(definition.processDefinitionKey()));
        ...
        final var searchResult = reader.search(
                UserTaskQuery.of(
                        b -> b.filter(f -> f.processDefinitionKeys(definition.processDefinitionKey()))
                                .sort(s -> s)
                                .page(p -> p.from(0).size(20))));
