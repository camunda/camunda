...
class UserTaskCandidateGroupAuthorizationIT {
  ...
  void shouldFindTaskWhenBpmnUsesGroupName(
    ...
    // and - user searches filtered by the migrated group ID
    final var resultFilteredByGroupId =
        camundaClient
    ...
    // and - user searches filtered by the legacy group name
    final var resultFilteredByGroupName =
        camundaClient
            .newUserTaskSearchRequest()
    ...
    assertThat(resultFilteredByGroupId.items())
        .extracting(UserTask::getBpmnProcessId)
        .contains(PROCESS_ID_WITH_GROUP_NAME);
    // however, candidate group filtering by group name isn't supported
    assertThat(resultFilteredByGroupName.items()).isEmpty();
  }
