...
class BatchOperationAuthorizationIT {
  ...
  private void waitForIncident(
      ...
      final int expectedIncidentCount) {
    Awaitility.await("should wait for incident to have size " + expectedIncidentCount)
        .atMost(Duration.ofSeconds(30))
        .pollInterval(Duration.ofMillis(100))
        .ignoreExceptions()
        .until(
            () -> {
              final var result =
                  camundaClient
                      ...
                      .send()
                      .join();
              return result.items().size() == expectedIncidentCount;
            });
  }
}
