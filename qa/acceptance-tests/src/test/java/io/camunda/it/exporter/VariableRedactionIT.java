...
/**
 * End-to-end coverage for the variable redaction proof of concept (product-hub #3805, PoC-2).
 *
 * <p>The declaration -- whether a variable's name matches the sensitive-variable pattern -- is
 * decided once in the engine, in {@code VariableBehavior}, and carried as metadata on the variable
 * record from then on. The actual redaction still happens once in the broker's exporter director,
 * before the record is handed to any exporter, so it must hold for whichever secondary storage this
 * run is configured with. Externally this behaves the same as PoC-1: only where the sensitivity
 * decision is made has moved. The counterpart assertion matters just as much: job activation reads
 * the engine's live state rather than secondary storage, so a worker must still receive the plain
 * value it needs to do its work.
 */
@MultiDbTest
