...
public class UsageMetricsIT {
  ...
  public void shouldAggregateMetricsWithTenant() {
    ...
    final var actual =
        usageMetricReader.usageMetricStatistics(
            new UsageMetricsFilter.Builder().tenants(true).build());
  ...
  public void shouldFilterMetricsByDate() {
    ...
    writeMetric(usageMetricWriter, EDI, now.minusMinutes(1), TENANT2, 1L);
    rdbmsWriter.flush();
    ...
    final var actual =
        usageMetricReader.usageMetricStatistics(
            new UsageMetricsFilter.Builder()
                // TODO why is minusSeconds(1) needed? It fails on oracle
