...
public class AuditLogResourceOperationsIT {
  ...
  void shouldTrackProcessInstanceHistoryDeletion(
    ...
    // wait for process instance to complete
    ...
    // when - delete the process instance history using batch operation
  ...
  @Test
  void shouldTrackProcessDefinitionHistoryDeletion(
      @Authenticated(DEFAULT_USERNAME) final CamundaClient client) {
    // given - a deployed process
    final var processId = "auditLogProcessDefDeleteTest";
    final var process =
    ...
    final var deployment =
        client
            .newDeployResourceCommand()
            .addProcessModel(process, "process-def-delete-test.bpmn")
            .tenantId(TENANT_A)
            .send()
            .join();
    ...
    client.newDeleteResourceCommand(processDefinitionKey).deleteHistory(true).send().join();
    ...
    assertThat(auditLogItems).isNotEmpty();
    // Find the audit log entry for PROCESS_DEFINITION deletion (entity description)
    final var historyDeletionLog =
        auditLogItems.stream()
            .filter(al -> "PROCESS_DEFINITION".equals(al.getEntityDescription()))
            .findFirst()
            .orElseThrow(
                () ->
                    new AssertionError(
                        "No history deletion audit log found for process definition"));

    assertThat(historyDeletionLog.getEntityKey()).isEqualTo(String.valueOf(processDefinitionKey));
    assertThat(historyDeletionLog.getEntityType()).isEqualTo(AuditLogEntityTypeEnum.RESOURCE);
    assertThat(historyDeletionLog.getOperationType()).isEqualTo(AuditLogOperationTypeEnum.DELETE);
    assertThat(historyDeletionLog.getEntityDescription()).isEqualTo("PROCESS_DEFINITION");
    assertThat(historyDeletionLog.getProcessDefinitionId()).isEqualTo(processId);
    assertThat(historyDeletionLog.getResult()).isEqualTo(AuditLogResultEnum.SUCCESS);
    assertThat(historyDeletionLog.getTenantId()).isEqualTo(TENANT_A);
  }

  @Test
  void shouldTrackDecisionRequirementsHistoryDeletion(
      @Authenticated(DEFAULT_USERNAME) final CamundaClient client) {
    // given - a deployed decision requirements
    final var deployment =
        client
            .newDeployResourceCommand()
    ...
    // when - delete the decision requirements with history
    client.newDeleteResourceCommand(decisionRequirementsKey).deleteHistory(true).send().join();
    ...
    assertThat(auditLogItems).isNotEmpty();
    // Find the audit log entry for DECISION_REQUIREMENTS deletion (entity description)
    final var historyDeletionLog =
        auditLogItems.stream()
            .filter(al -> "DECISION_REQUIREMENTS".equals(al.getEntityDescription()))
            .findFirst()
            .orElseThrow(
                () ->
                    new AssertionError(
                        "No history deletion audit log found for decision requirements"));

    assertThat(historyDeletionLog.getEntityKey())
        .isEqualTo(String.valueOf(decisionRequirementsKey));
    assertThat(historyDeletionLog.getEntityType()).isEqualTo(AuditLogEntityTypeEnum.RESOURCE);
    assertThat(historyDeletionLog.getOperationType()).isEqualTo(AuditLogOperationTypeEnum.DELETE);
    assertThat(historyDeletionLog.getEntityDescription()).isEqualTo("DECISION_REQUIREMENTS");
    assertThat(historyDeletionLog.getDecisionDefinitionId()).isEqualTo(decisionDefinitionId);
    assertThat(historyDeletionLog.getResult()).isEqualTo(AuditLogResultEnum.SUCCESS);
    assertThat(historyDeletionLog.getTenantId()).isEqualTo(TENANT_A);
  }
