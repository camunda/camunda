...
public class ProcessMigrationIT {
  ...
  void shouldMigrateProcess(final TestStandaloneBroker testBroker) {
    ...
    steps
        ...
        .userTaskExistAndMatch(
            f -> f.processInstanceKey(steps.processInstanceKey),
            f -> {
              assertThat(f).hasSize(1);
              assertThat(f.getFirst())
                  .satisfies(
                      i -> {
                        assertThat(i.getElementId()).isEqualTo("taskA2");
                        assertThat(i.getBpmnProcessId()).isEqualTo("migration-process_v2");
                        assertThat(i.getProcessDefinitionVersion()).isEqualTo(1);
                      });
            })
