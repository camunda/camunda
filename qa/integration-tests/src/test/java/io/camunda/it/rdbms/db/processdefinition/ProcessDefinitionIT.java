...
public class ProcessDefinitionIT {
  ...
  public void shouldSaveAndFindProcessDefinitionByKey(
    ...
    final ProcessDefinitionReader processDefinitionReader = rdbmsService.getProcessDefinitionReader();

    final var processDefinition = ProcessDefinitionFixtures.createRandomized(
        b -> b);
    createAndSaveProcessDefinition(rdbmsWriter, processDefinition);

    final var oInstance = processDefinitionReader.findOne(processDefinition.processDefinitionKey());
    assertThat(oInstance).isNotNull();
    assertThat(oInstance).isNotEmpty();

    final var instance = oInstance.get();
  ...
  public void shouldFindAllProcessDefinitionPaged(
    ...
    createAndSaveRandomProcessDefinitions(rdbmsWriter, processDefinitionId);

    final var searchResult =
        processDefinitionReader.search(
            new ProcessDefinitionDbQuery(
                new ProcessDefinitionFilter.Builder().processDefinitionIds(processDefinitionId)
                    .build(),
                ProcessDefinitionSort.of(b -> b),
  ...
  public void shouldFindProcessInstanceWithFullFilter(
    ...
    assertThat(searchResult.total()).isEqualTo(1);
    assertThat(searchResult.hits()).hasSize(1);
    assertThat(searchResult.hits().getFirst().key()).isEqualTo(
