...
@AutoCloseResources
@ZeebeIntegration
public class TasklistCompleteUserTaskIT {
  ...
  @Test
  public void shouldCompleteJobBasedUserTask() {
    // when
    final var response =
        tasklistRestClient
            .withAuthentication(TEST_USER_NAME, TEST_USER_PASSWORD)
  ...
  public static long awaitUserTaskBeingAvailable(final long processInstanceKey) {
    ...
    Awaitility.await("should create a job-based user task")
        ...
        .untilAsserted(
            () -> {
              final var result = tasklistRestClient.searchUserTasks(processInstanceQuery);
              assertThat(result.hits()).hasSize(1);
  ...
  public static void ensureUserTaskIsCompleted(final long userTaskKey) {
    final var query =
        and(
            SearchQueryBuilders.term(TaskTemplate.KEY, userTaskKey),
            SearchQueryBuilders.term(TaskTemplate.STATE, "COMPLETED"));
    Awaitility.await("user task should be completed")
        .atMost(Duration.ofSeconds(60))
        .ignoreExceptions() // Ignore exceptions and continue retrying
        .untilAsserted(
            () -> {
              final var result = tasklistRestClient.searchUserTasks(query);
              assertThat(result.hits()).hasSize(1);
            });
  }
  ...
  public static void ensureJobBasedUserTaskIsCompleted(final long userTaskKey) {
    ...
    final var finalQuery = and(userTaskQuery, stateQuery);
