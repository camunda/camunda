...
public class UserTaskExporterIT {
  ...
  void shouldExportUserTask(final TestStandaloneBroker testBroker) {
    ...
    assertThat(userTasks.getFirst().getProcessInstanceKey())
        .isEqualTo(Long.parseLong(processInstanceId));
  ...
  @Nested
  class UserTaskUpdateIT {

    @TestTemplate
    void shouldUpdateUserTask(final TestStandaloneBroker testBroker) {
      // given
      final var client = testBroker.newClientBuilder().build();
      final var dateTime = OffsetDateTime.now();
    ...
    @TestTemplate
    void shouldAssignUserTask(final TestStandaloneBroker testBroker) {
      // given
      final var client = testBroker.newClientBuilder().build();
    ...
    @TestTemplate
    void shouldUnassignUserTask(final TestStandaloneBroker testBroker) {
      // given
      final var client = testBroker.newClientBuilder().build();
  ...
  @Nested
  class UserTaskStateTransitionIT {

    @TestTemplate
    void shouldCompleteUserTask(final TestStandaloneBroker testBroker) {
      // given
      final var client = testBroker.newClientBuilder().build();
