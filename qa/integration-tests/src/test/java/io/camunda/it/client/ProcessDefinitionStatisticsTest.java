...
public class ProcessDefinitionStatisticsTest {
  ...
  private static void waitForProcessInstances(
    ...
    Awaitility.await("should receive data from ES")
        ...
        .untilAsserted(
            () ->
                assertThat(camundaClient.newProcessInstanceQuery().filter(fn).send().join().items())
                    .hasSize(count));
  }
  ...
  void shouldGetEmptyStatisticsWithoutMatch() {
    // when
    final var actual = camundaClient.newProcessDefinitionFlowNodeStatistics(1L).send().join();
  ...
  void shouldGetStatisticsAndFilterByProcessInstanceKey() {
    ...
    final var actual =
        camundaClient
            .newProcessDefinitionFlowNodeStatistics(processDefinitionKey)
  ...
  void shouldGetStatisticsAndFilterByEndDateExists() {
    ...
    createInstance(processDefinitionKey).getProcessInstanceKey();
    waitForProcessInstances(
        2, f -> f.processDefinitionKey(processDefinitionKey).state(ProcessInstanceState.ACTIVE));
  ...
  void shouldGetStatisticsAndFilterByStateActive() {
    ...
    createInstance(processDefinitionKey).getProcessInstanceKey();
    waitForProcessInstances(
        3, f -> f.processDefinitionKey(processDefinitionKey).state(ProcessInstanceState.ACTIVE));
  ...
  void shouldGetStatisticsForCompleted() {
    ...
    // when
    final var actual =
        camundaClient.newProcessDefinitionFlowNodeStatistics(processDefinitionKey).send().join();
