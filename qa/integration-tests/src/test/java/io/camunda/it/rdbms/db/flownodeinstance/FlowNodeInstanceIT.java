...
public class FlowNodeInstanceIT {
  ...
  public void shouldSaveAndFindFlowNodeInstanceByKey(
    ...
    final var actual = reader.findOne(flowNodeInstance.flowNodeInstanceKey());
  ...
  public void shouldFindAllFlowNodeInstancePaged(
    ...
    final var searchResult =
        reader.search(
            new FlowNodeInstanceDbQuery(
                new FlowNodeInstanceFilter.Builder().processDefinitionIds(
                        processDefinitionId)
                    .build(),
                FlowNodeInstanceSort.of(b -> b),
  ...
  public void shouldFindAllFlowNodeInstancePageValuesAreNull(
    ...
    final var searchResult =
        reader.search(
            new FlowNodeInstanceDbQuery(
                new FlowNodeInstanceFilter.Builder().processDefinitionIds(processDefinitionId)
                    .build(),
                FlowNodeInstanceSort.of(b -> b),
  ...
  private static void compareFlowNodeInstance(final FlowNodeInstanceEntity actual,
      final FlowNodeInstanceDbModel expected) {
    assertThat(actual).usingRecursiveComparison()
        .ignoringFields("startDate", "endDate", "incident", "incidentKey", "treePath", "scopeKey",
            "processDefinitionId", "bpmnProcessId", "flowNodeInstanceKey", "key")
        .isEqualTo(expected);
    ...
    assertThat(OffsetDateTime.parse(actual.startDate())).isCloseTo(expected.startDate(),
        new TemporalUnitWithinOffset(1, ChronoUnit.MILLIS));
    assertThat(OffsetDateTime.parse(actual.endDate())).isCloseTo(expected.endDate(),
        new TemporalUnitWithinOffset(1, ChronoUnit.MILLIS));
  }
}
