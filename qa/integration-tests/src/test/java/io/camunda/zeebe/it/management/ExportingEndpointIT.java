...
final class ExportingEndpointIT {
  ...
  private ExportingActuator getActuator() {
    return ExportingActuator.ofAddress(CLUSTER.availableGateway().monitoringAddress());
  }
  ...
  private void allPartitionsPausedExporting() {
    for (final var broker : CLUSTER.brokers().values()) {
      assertThat(PartitionsActuator.ofAddress(broker.monitoringAddress()).query().values())
          .allMatch(
