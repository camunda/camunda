...
public class DecisionInstanceDaoRfc3339SerializationIT extends OperateSearchAbstractIT {
  ...
  private String secondDecisionRfc3339EvaluationDate =  "2024-02-15T22:41:10.834+00:00";
  private String thirdDecisionEvaluationDate = "2024-01-15T22:40:10.834+0000";
  private String thirdDecisionRfc3339EvaluationDate = "2024-01-15T22:40:10.834+00:00";
  @Override
  protected void runAdditionalBeforeAllSetup() throws Exception {
    ...
    testSearchRepository.createOrUpdateDocumentFromObject(indexName, new DecisionInstanceEntity().setId("2251799813685262-1")
        .setKey(2251799813685262L).setState(io.camunda.operate.entities.dmn.DecisionInstanceState.EVALUATED)
    ...
    testSearchRepository.createOrUpdateDocumentFromObject(indexName, new DecisionInstanceEntity().setId("2251799813685262-3")
        .setKey(2251799813685262L).setState(io.camunda.operate.entities.dmn.DecisionInstanceState.EVALUATED)
        .setEvaluationDate(dateTimeFormatter.parseGeneralDateTime(thirdDecisionEvaluationDate))
        .setProcessDefinitionKey(FAKE_PROCESS_DEFINITION_KEY).setProcessInstanceKey(FAKE_PROCESS_INSTANCE_KEY).setDecisionId("invoiceAssignApprover")
        .setDecisionDefinitionId("2251799813685252").setDecisionName("Process Invoice").setDecisionVersion(1)
        .setDecisionType(DecisionType.DECISION_TABLE).setResult("\"day-to-day expense\"").setTenantId(DEFAULT_TENANT_ID));
  ...
  public void shouldFilterByEvaluationDate() {
    Results<DecisionInstance> decisionInstanceResults = dao.search(new Query<DecisionInstance>()
        .setFilter(new DecisionInstance().setEvaluationDate(firstDecisionRfc3339EvaluationDate)));

    assertThat(decisionInstanceResults.getTotal()).isEqualTo(1L);
    assertThat(decisionInstanceResults.getItems().get(0).getEvaluationDate()).isEqualTo(firstDecisionRfc3339EvaluationDate);
  ...
  public void shouldFilterByEvaluationDateWithDateMath() {
    Results<DecisionInstance> decisionInstanceResults = dao.search(new Query<DecisionInstance>()
        .setFilter(new DecisionInstance().setEvaluationDate(firstDecisionRfc3339EvaluationDate + "||/d")));

    assertThat(decisionInstanceResults.getTotal()).isEqualTo(2L);

    DecisionInstance checkDecision = decisionInstanceResults.getItems().stream().filter(
            item -> "2251799813685262-1".equals(item.getId()))
    ...
    checkDecision = decisionInstanceResults.getItems().stream().filter(
            item -> "2251799813685262-2".equals(item.getId()))
        .findFirst().orElse(null);
