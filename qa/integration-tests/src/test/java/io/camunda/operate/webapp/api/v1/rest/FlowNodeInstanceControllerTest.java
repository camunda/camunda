...
@SpringBootTest(
    classes = {
        ...
        FlowNodeInstanceController.class
    })
public class FlowNodeInstanceControllerTest {
  ...
  public void shouldAcceptQueryWithSearchAfterAndSize() throws Exception {
    assertPostToWithSucceed(URI + SEARCH, "{\"searchAfter\": [\"" + FlowNodeInstance.PROCESS_INSTANCE_KEY+"\"], \"size\": 7}");
    verify(flowNodeInstanceDao).search(new Query<FlowNodeInstance>().setSearchAfter(new Object[]{
        FlowNodeInstance.PROCESS_INSTANCE_KEY}).setSize(7));
  }

  @Test
  public void shouldAcceptQueryWithSizeAndSortSpec() throws Exception {
    assertPostToWithSucceed(URI + SEARCH,
        "{\"size\": 7, \"sort\": [ "+ getSortFor(FlowNodeInstance.START_DATE, Order.DESC)+" ] }");
  ...
  public void shouldAcceptQueryWithFilter() throws Exception {
    assertPostToWithSucceed(URI + SEARCH, "{\"filter\": { \""+ FlowNodeInstance.PROCESS_INSTANCE_KEY +"\": \"1\" } }");
    verify(flowNodeInstanceDao).search(new Query<FlowNodeInstance>()
  ...
  public void shouldReturnErrorMessageForListFailure() throws Exception {
    ...
    // given
    when(flowNodeInstanceDao.search(any(Query.class))).thenThrow(
  ...
  public void shouldReturnErrorMessageForByKeyFailure() throws Exception {
    ...
    // given
    when(flowNodeInstanceDao.byKey(any(Long.class))).thenThrow(
