...
public class VariableControllerTest {
  ...
  public void shouldAcceptQueryWithSearchAfterAndSize() throws Exception {
    assertPostToWithSucceed(URI + SEARCH, "{\"searchAfter\": [\"" + Variable.PROCESS_INSTANCE_KEY+"\"], \"size\": 7}");
    verify(variableDao).search(new Query<Variable>().setSearchAfter(new Object[]{
        Variable.PROCESS_INSTANCE_KEY}).setSize(7));
  }
  ...
  public void shouldAcceptQueryWithSizeAndSortSpec() throws Exception {
    ...
    verify(variableDao).search(new Query<Variable>()
        .setSize(7)
  ...
  public void shouldAcceptQueryWithFilter() throws Exception {
    assertPostToWithSucceed(URI + SEARCH, "{\"filter\": { \""+ Variable.NAME +"\": \"customerId\" } }");
    verify(variableDao).search(new Query<Variable>()
        .setFilter(new Variable().setName("customerId")));
  }
  ...
  public void shouldReturnErrorMessageForListFailure() throws Exception {
    ...
    // given
    when(variableDao.search(any(Query.class))).thenThrow(
  ...
  public void shouldReturnErrorMessageForByKeyFailure() throws Exception {
    ...
    // given
    when(variableDao.byKey(any(Long.class))).thenThrow(
