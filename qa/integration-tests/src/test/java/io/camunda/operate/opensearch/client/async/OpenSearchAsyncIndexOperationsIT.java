...
public class OpenSearchAsyncIndexOperationsIT extends AbstractOpenSearchOperationIT {
  ...
  public void shouldReindex() throws Exception {
    ...
    var deleteByQueryRequestBuilder = reindexRequestBuilder(userIndex.getFullQualifiedName(), query, dstIndex)
    ...
    var task = richOpenSearchClient.async().index().reindex(deleteByQueryRequestBuilder, Throwable::getMessage).get().task();

    // then
    var total = withThreadPoolTaskScheduler(scheduler ->
      {
        try {
          return richOpenSearchClient.async().task().totalImpactedByTask(task, scheduler).get();
        } catch (Exception e) {
    ...
    var user = richOpenSearchClient.doc().searchUnique(searchRequestBuilder(dstIndex).query(query), UserEntity.class, id);
