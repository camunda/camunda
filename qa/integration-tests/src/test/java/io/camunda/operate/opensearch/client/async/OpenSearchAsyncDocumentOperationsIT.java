...
public class OpenSearchAsyncDocumentOperationsIT extends AbstractOpenSearchOperationIT {
  ...
  public void shouldDelete() throws Exception {
    ...
    // when
    var deleteByQueryRequestBuilder = deleteByQueryRequestBuilder(userIndex.getFullQualifiedName())
      .query(stringTerms("userId", List.of("1")))
      .waitForCompletion(false)
      .slices((long) operateProperties.getOpensearch().getNumberOfShards())
      .conflicts(Conflicts.Proceed);

    var task = richOpenSearchClient.async().doc().delete(deleteByQueryRequestBuilder, Throwable::getMessage).get().task();
