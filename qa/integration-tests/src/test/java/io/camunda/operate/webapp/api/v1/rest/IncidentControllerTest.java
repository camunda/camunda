...
public class IncidentControllerTest {
  ...
  public void shouldAcceptQueryWithSearchAfterAndSize() throws Exception {
    assertPostToWithSucceed(URI + SEARCH, "{\"searchAfter\": [\"" + Incident.PROCESS_INSTANCE_KEY+"\"], \"size\": 7}");
    verify(incidentDao).search(new Query<Incident>().setSearchAfter(new Object[]{
        Incident.PROCESS_INSTANCE_KEY}).setSize(7));
  }
  ...
  public void shouldAcceptQueryWithSizeAndSortSpec() throws Exception {
    ...
    verify(incidentDao).search(new Query<Incident>()
        .setSize(7)
  ...
  public void shouldAcceptQueryWithFilter() throws Exception {
    assertPostToWithSucceed(URI + SEARCH, "{\"filter\": { \""+ Incident.PROCESS_INSTANCE_KEY +"\": \"1\" } }");
    verify(incidentDao).search(new Query<Incident>()
        .setFilter(new Incident().setProcessInstanceKey(1L)));
  }
  ...
  public void shouldReturnErrorMessageForListFailure() throws Exception {
    ...
    // given
    when(incidentDao.search(any(Query.class))).thenThrow(
  ...
  public void shouldReturnErrorMessageForByKeyFailure() throws Exception {
    ...
    // given
    when(incidentDao.byKey(any(Long.class))).thenThrow(
  ...
  public void shouldReturnErrorMessageForSortByMessage() throws Exception {
    final String expectedPartialJSONContent = "{\"status\":400,\"message\":\"Field 'message' can't be used as sort field\"";
    // then
    assertPostToWithFailed(URI + SEARCH,
         "  {\"sort\": [{"
        + "      \"field\": \"message\","
