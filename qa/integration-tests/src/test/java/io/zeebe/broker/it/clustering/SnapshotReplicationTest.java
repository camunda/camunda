...
public class SnapshotReplicationTest
{
    ...
    public void shouldReplicateSnapshotsFromLeaderForSystemTopic() throws Exception
    {
        ...
        final SocketAddress leaderAddress = clusteringRule.getLeaderAddressForPartition(topic.getPartitions().get(0).getId());
        final SocketAddress followerAddress = clusteringRule.getFollowerAddressForPartition(topic.getPartitions().get(0).getId());
    ...
    public void shouldReplicateMultipleSnapshots() throws Exception
    {
        ...
        final TestSnapshot[] snapshots = new TestSnapshot[] {
            new TestSnapshot("snap", 1L, "foo"),
            new TestSnapshot("snop", 1L, "bar")
        };
    ...
    public void shouldNotReplicateLogBlockIndex() throws Exception
    {
        ...
        final TestSnapshot[] snapshots = new TestSnapshot[] {
            new TestSnapshot(logBlockIndexName, 1L, "bar"),
            new TestSnapshot("znap", 1L, "foo")
        };
    ...
    private SnapshotStorage getSnapshotStorage(final Topic topic, final SocketAddress address) throws Exception
    {
        ...
        final ServiceName<Void> name = ServiceName.newServiceName(String.format("test.snapshotReplication.extractor.%s", logName), Void.class);
    ...
    private void assertReplicated(final SnapshotStorage storage, final TestSnapshot snapshot) throws Exception
    {
