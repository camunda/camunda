...
public class EmbeddedBrokerRule extends ExternalResource {
  ...
  protected void after() {
    ...
    if (allocatedMemoryInKb > 0) {
      LOG.warn(
          "There are still allocated direct buffers of a total size of {}kB.", allocatedMemoryInKb);
    }
  ...
  public <S> S getService(final ServiceName<S> serviceName) {
    ...
    final ServiceName<Object> accessorServiceName =
        ServiceName.newServiceName("serviceAccess" + serviceName.getName(), Object.class);
    try {
      serviceContainer
  ...
  public void purgeSnapshots(final List<String> dataDirectories) {
