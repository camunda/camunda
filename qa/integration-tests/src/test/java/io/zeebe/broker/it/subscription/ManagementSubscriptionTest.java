...
{
    private static final WorkflowDefinition WORKFLOW = Bpmn.createExecutableWorkflow("wf")
            .startEvent()
            .done();
    ...
    {
        // when
        final TopicSubscription subscription = client
    ...
    {
        // given
        final DeploymentEvent deploymentEvent = client
            .topicClient()
            .workflowClient()
            .newDeployCommand()
        ...
        assertThat(deploymentResource.getResource()).isEqualTo(Bpmn.convertToString(WORKFLOW).getBytes(StandardCharsets.UTF_8));
    }
    ...
    public void shouldReceiveTopicEvents()
    {
        ...
        // when
        final TopicEvent topicEvent = client
        ...
        assertThat(events).extracting(TopicEvent::getState).containsExactly(TopicState.CREATING, TopicState.CREATE_COMPLETE, TopicState.CREATED);
    ...
    public void shouldReceiveAllRecords()
    {
        ...
        assertThat(records).extracting(r -> r.getMetadata().getValueType()).containsOnly(ValueType.DEPLOYMENT, ValueType.TOPIC);
        assertThat(records).extracting(r -> r.getMetadata().getRecordType()).containsOnly(RecordType.EVENT, RecordType.COMMAND);
    }
