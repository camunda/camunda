 ...
 */
@Component
public class BasicWorkflowDataGenerator {

  private static final Logger logger = LoggerFactory.getLogger(BasicWorkflowDataGenerator.class);
  public static final String WORKFLOW_BPMN_PROCESS_ID = "basicProcess";
  ...
  private static final DateTimeFormatter ARCHIVER_DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyyMMdd").withZone(ZoneId.systemDefault());
  ...
  public void createData(TestContext testContext) throws Exception {
    ...
    try {
      ...
      completeTasks("task1", WORKFLOW_INSTANCE_COUNT);
  ...
  private boolean createOperation(Long workflowInstanceKey, OperationType operationType) {
    ...
    return operationResponse.getStatusCode().equals(HttpStatus.OK) && (Integer)operationResponse.getBody().get("count") > 0;
  }
