...
function planConstraintMutations(cons: Record<string,any>, type: string): { kind: string; value: any }[] {
  ...
  if (type === 'string') {
    ...
    if (typeof cons.pattern === 'string') out.push({ kind: 'patternMismatch', value: '__NO_MATCH__' });
...
function buildParams(path: string): Record<string,string> | undefined {
  const m = path.match(/\{([^}]+)}/g); if (!m) return undefined; const params: Record<string,string> = {}; for (const t of m) params[t.slice(1,-1)]='x'; return params; }
