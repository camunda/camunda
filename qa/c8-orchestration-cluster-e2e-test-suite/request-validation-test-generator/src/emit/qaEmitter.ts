...
interface EmitOpts { outDir: string; qaImportDepth: number; specCommit?: string; generationTimestamp?: string; }
...
export async function emitQaTests(scenarios: ValidationScenario[], opts: EmitOpts) {
  ...
  try {
    ...
    throw new Error('[emit] Failed to resolve Prettier config: '+(e instanceof Error ? e.message : String(e)));
  }
  if (!resolvedConfig) {
    ...
    console.warn('[emit] No Prettier config found. Using built-in fallback config { singleQuote:true, trailingComma:"all", bracketSpacing:false }');
  }
  for (const [file, list] of byFile.entries()) {
    ...
    try {
      formatted = await prettier.format(raw, { ...(resolvedConfig||{}), filepath: file });
    } catch (e) {
      throw new Error('[emit] Prettier formatting failed: '+(e instanceof Error ? e.message : String(e)));
    }
...
function buildFile(scenarios: ValidationScenario[], depth: number, specCommit?: string, ts?: string): string {
...
function renderScenario(s: ValidationScenario, title: string): string {
  ...
  const headersExpr = s.headersAuth ? (s.bodyEncoding === 'multipart' ? '{}' : 'jsonHeaders()') : '{}';
  const dataPart = s.bodyEncoding === 'multipart' && s.multipartForm ? ',\n      multipart: formData' : (s.requestBody ? ',\n      data: requestBody' : '');
...
function buildBaseTitle(s: ValidationScenario): string {
  ...
  // Specialization: param constraint violations (path/query) should surface the specific constraint kind.
    if (s.type === 'param-constraint-violation') {
      ...
      const locLabel = location === 'path' ? 'Path param' : location === 'query' ? 'Query param' : 'Param';
      return `${s.operationId} - ${locLabel} ${paramName} ${constraint} violation`;
  }
  switch (s.type) {
    case 'missing-required': return `${s.operationId} - Missing ${s.target}`;
    case 'missing-required-combo': return `${s.operationId} - Missing combo ${s.target}`;
    case 'param-missing': return `${s.operationId} - Missing param ${s.target}`;
    case 'type-mismatch': return `${s.operationId} - Param ${s.target} wrong type`;
    case 'param-type-mismatch': return `${s.operationId} - Param ${s.target} wrong type`;
    case 'body-top-type-mismatch': return `${s.operationId} - Body wrong top-level type`;
    case 'missing-body': return `${s.operationId} - Missing body`;
    case 'union': return `${s.operationId} - oneOf violation`;
    case 'oneof-ambiguous': return `${s.operationId} - oneOf ambiguous`;
    case 'oneof-multi-ambiguous': return `${s.operationId} - oneOf multi ambiguous`;
    case 'oneof-cross-bleed': return `${s.operationId} - oneOf cross bleed`;
    case 'oneof-none-match': return `${s.operationId} - oneOf none match`;
    case 'constraint-violation': return `${s.operationId} - Constraint violation ${s.target}`;
    case 'enum-violation': return `${s.operationId} - Enum violation ${s.target}`;
    case 'additional-prop': return `${s.operationId} - Additional prop ${s.target}`;
    case 'additional-prop-general': return `${s.operationId} - Additional prop ${s.target}`;
    case 'nested-additional-prop': return `${s.operationId} - Nested additional prop ${s.target}`;
    case 'unique-items-violation': return `${s.operationId} - uniqueItems violation ${s.target}`;
    case 'multiple-of-violation': return `${s.operationId} - multipleOf violation ${s.target}`;
    case 'format-invalid': return `${s.operationId} - format invalid ${s.target}`;
    case 'discriminator-mismatch': return `${s.operationId} - discriminator mismatch`;
    case 'discriminator-structure-mismatch': return `${s.operationId} - discriminator structure mismatch`;
    case 'allof-missing-required': return `${s.operationId} - allOf missing required`;
    case 'allof-conflict': return `${s.operationId} - allOf conflict`;
    default: return s.id; // Fallback is globally unique id
  }
}
