...
function resolveParamSchema(p: ParameterModel): ResolvedParamSchema | undefined {
  ...
  function merge(s: any){ // eslint-disable-line @typescript-eslint/no-explicit-any
...
function buildViolations(p: ParameterModel, r: ResolvedParamSchema): { kind: string; invalid: string }[] {
  const out: { kind:string; invalid:string }[] = [];
  // Pattern violation
  if (r.pattern) {
    const invalid = buildGuaranteedPatternMismatch(r.pattern, { pathSegmentSafe: p.in === 'path' });
...
function buildParams(path: string, overrides: Record<string,string>): Record<string,string> | undefined {
...
export function generateParamConstraintViolations(ops: OperationModel[], opts: Opts): ValidationScenario[] {
