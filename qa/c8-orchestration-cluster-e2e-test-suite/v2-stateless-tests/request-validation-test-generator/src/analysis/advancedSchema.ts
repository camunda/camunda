...
export function generateNestedAdditionalProps(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  const out: ValidationScenario[] = [];
  for (const op of ops) {
    if (opts.onlyOperations && !opts.onlyOperations.has(op.operationId)) continue;
  const walk = buildWalk(op); if (!walk || !walk.root) continue;
  const baseline = buildBaselineBody(op); if (!baseline) continue;
    let produced = 0;
    ...
    function dfs(node: any, path: string[]) {
      ...
      if (node !== walk!.root && (node as any).raw && node.type === 'object' && (node as any).raw.additionalProperties === false) {
      ...
      if (node.properties) for (const [k,v] of Object.entries<any>(node.properties)) dfs(v, [...path,k]);
...
export function generateUniqueItemsViolations(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  const out: ValidationScenario[] = [];
  for (const op of ops) {
    if (opts.onlyOperations && !opts.onlyOperations.has(op.operationId)) continue;
    const walk = buildWalk(op); if (!walk || !walk.root) continue;
...
export function generateMultipleOfViolations(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  const out: ValidationScenario[] = [];
  for (const op of ops) {
    if (opts.onlyOperations && !opts.onlyOperations.has(op.operationId)) continue;
    const walk = buildWalk(op); if (!walk || !walk.root) continue;
    ...
    for (const node of walk.byPointer.values()) {
      ...
      if ((node.type === 'integer' || node.type === 'number') && raw && typeof raw.multipleOf === 'number') {
        const path = findPath(walk.root!, node); if (!path) continue;
...
function applySet(obj: any, path: string[], value: any): boolean {
  let target = obj;
  for (let i=0;i<path.length-1;i++){ const seg=path[i]; if(!(seg in target)) return false; target=target[seg]; }
  const last = path[path.length-1]; if(!(last in target)) return false; target[last]=value; return true;
}
...
function findPath(root: any, node: any): string[] | undefined { let found: string[] | undefined; function dfs(cur:any,p:string[]) { if (cur===node) { found=p; return; } if (cur.properties) for (const [k,v] of Object.entries(cur.properties)) { dfs(v,[...p,k]); if(found) return;} if (cur.items) dfs(cur.items,[...p,'0']); } dfs(root,[]); return found; }
