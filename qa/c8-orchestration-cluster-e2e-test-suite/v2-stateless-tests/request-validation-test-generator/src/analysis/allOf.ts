...
export function generateAllOfMissingRequired(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  const out: ValidationScenario[] = [];
  for (const op of ops) {
    if (opts.onlyOperations && !opts.onlyOperations.has(op.operationId)) continue;
    const root = op.requestBodySchema; if (!root) continue;
    if (!Array.isArray(root.allOf)) continue;
    // Build baseline to anchor other requireds
    const baseline = buildBaselineBody(op); if (!baseline || typeof baseline !== 'object') continue;
    // Collect required sets per constituent
    const constituents = root.allOf.filter((c:any)=>c && c.type==='object' && Array.isArray(c.required));
    ...
    for (const c of constituents) {
      for (const r of c.required) {
        ...
        if (r in body) { delete (body as any)[r]; } else continue;
        out.push({
...
export function generateAllOfConflicts(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  ...
  for (const op of ops) {
    ...
    const root = op.requestBodySchema; if (!root) continue;
    if (!Array.isArray(root.allOf)) continue;
    const objectConstituents = root.allOf.filter((c:any)=>c && c.type==='object' && c.properties);
    ...
    const conflicts = Object.entries(typeMap).filter(([_, set]) => set.size > 1).map(([k])=>k);
