...
export function generateOneOfMultiAmbiguous(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  const out: ValidationScenario[] = [];
  for (const op of ops) {
    if (!op.rootOneOf || op.rootOneOf.length < 3) continue;
    if (opts.onlyOperations && !opts.onlyOperations.has(op.operationId)) continue;
    const variants = op.rootOneOf.filter(v=>v && v.type==='object' && Array.isArray(v.required));
    if (variants.length < 3) continue;
    const a = variants[0], b = variants[1], c = variants[2];
...
export function generateDiscriminatorStructureMismatch(ops: OperationModel[], opts: Opts): ValidationScenario[] {
  const out: ValidationScenario[] = [];
  for (const op of ops) {
    if (opts.onlyOperations && !opts.onlyOperations.has(op.operationId)) continue;
    const d = op.discriminator; if (!d) continue;
    const root = op.requestBodySchema; if (!root || !Array.isArray(root.oneOf)) continue;
