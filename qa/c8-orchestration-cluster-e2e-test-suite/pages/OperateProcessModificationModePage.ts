...
export class OperateProcessModificationModePage {
    ...
    constructor(page: Page) {
        ...
        this.cancelButtonPopup = page.getByTitle('Cancel selected instance in this flow node');
        this.cancelAllButtonPopup = page.getByTitle('Cancel all running flow node instances in this flow node');
        ...
        this.multipleInstancesAlert = page.getByText('Flow node has multiple instances. To select one, use the instance history tree below.');
     ...
     */
    async verifyModificationOverlay(flowNodeName: string, tokenChange: number): Promise<void> {
    ...
    async cancelOneTokenUsingHistory(flowNodeName: string): Promise<void> {
        ...
        const meow = this.history.getByTestId(/^tree-node-/).locator('[aria-current="true"]').first();
        await meow.click();
        await expect(this.page.getByTestId('popover').getByText('Selected running instances: 1')).toBeVisible();
