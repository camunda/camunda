...
test.describe.serial('Process Instance Migration', () => {
  ...
  test('Migrated process instances', async ({
    ...
    await test.step('Verify Exclusive gateway incident migration', async () => {
      // Navigate to Details tab first so the incidents panel re-renders fresh
      // with ExclusiveGateway2 selected, avoiding stale filter from the
      // previous BusinessRuleTask2 step.
