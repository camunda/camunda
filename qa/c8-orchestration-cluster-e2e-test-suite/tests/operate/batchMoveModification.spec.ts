...
test.describe('Process Instance Batch Modification', () => {
  ...
  test('Move Operation', async ({
    ...
    operateDiagramPage,
    operateBatchModificationModePage,
  }) => {
    ...
    await test.step('Start move modification', async () => {
      await operateBatchModificationModePage.startBatchMoveModification();
    });

    await test.step('Select target flow node', async () => {
      await operateDiagramPage.clickFlowNode('shipArticles');

      await operateBatchModificationModePage.expectModificationNotification(
        NUM_SELECTED_PROCESS_INSTANCES,
        'Check payment',
        'Ship Articles',
      );
    ...
    await test.step('Test undo functionality', async () => {
      await operateBatchModificationModePage.clickUndoButton();

      await expect(
        operateBatchModificationModePage.modificationNotification.filter({
          hasText: 'Modification scheduled',
        }),
      ).toBeHidden();

      await expect(
        operateBatchModificationModePage.batchModificationModeText,
      ).toBeVisible();
      await expect(
        operateBatchModificationModePage.applyModificationButton,
      ).toBeDisabled();
    });
    ...
    await test.step('Re-select target flow node and apply modification', async () => {
      ...
      await operateBatchModificationModePage.applyAndConfirmModification();
    });
  ...
  test('Exit Modal', async ({
    ...
    await test.step('Test navigation interruption and modal', async () => {
      ...
      await expect(
        operateBatchModificationModePage.exitModificationModeModal.dialog,
      ).toBeVisible();
      await expect(
        operateBatchModificationModePage.exitModificationModeModal.subtitle,
      ).toBeVisible();
    });

    await test.step('Test modal cancellation', async () => {
      await operateBatchModificationModePage.cancelExitModificationMode();
      await expect(
        operateBatchModificationModePage.exitModificationModeModal.dialog,
      ).toBeHidden();
      await expect(
    ...
    await test.step('Test modal confirmation and exit', async () => {
      ...
      await operateBatchModificationModePage.confirmExitModificationMode();
