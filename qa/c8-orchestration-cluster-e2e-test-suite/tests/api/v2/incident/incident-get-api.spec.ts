...
import { createTwoDifferentIncidentsInOneProcess, createTwoIncidentsInOneProcess} from '../../../../utils/requestHelpers/incident-requestHelpers';
...
test.describe.parallel('Search Incidents API Tests', () => {
  ...
  test('Get Incident Success',  async ({request}) => {
    ...
    for (const incidentKey of localState['incidentKeys'] as string[]) {
        ...
        expect(json.processInstanceKey).toBe(localState['processInstanceKey'] as string);
  ...
  test('Get Incidents Unauthorized', async ({request}) => {
    ...
    const firstIncidentKey = incidentKeys[0];
      ...
      await expect(async () => {
        const res = await request.get(buildUrl(`/incidents/${firstIncidentKey}`), {
          headers: {
    ...
    test('Get Incidents Invalid Value', async ({request}) => {
        await expect(async () => {
          ...
          const res = await request.get(buildUrl(`/incidents/${someInvalidValue}`), {
            headers: jsonHeaders(),
          });
