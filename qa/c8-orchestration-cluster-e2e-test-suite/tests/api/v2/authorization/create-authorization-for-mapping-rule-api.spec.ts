...
test.describe
  ...
  test('Create Authorization for Mapping Rule - 400 Bad Request - wrong value for resourceType', async ({
    ...
    await expect(async () => {
      ...
      await assertBadRequest(
        authRes,
        "Unexpected value 'WRONG_VALUE_FOR_TEST' for enum field 'resourceType'. Use any of the following values: [AUDIT_LOG, AUTHORIZATION, BACKUP, BATCH, CLUSTER_VARIABLE, COMPONENT, DECISION_DEFINITION, DECISION_REQUIREMENTS_DEFINITION, DOCUMENT, EXPORTER, EXPRESSION, GLOBAL_LISTENER, GROUP, MAPPING_RULE, MESSAGE, PROCESS_DEFINITION, RESOURCE, ROLE, SECRET, SYSTEM, TENANT, USER, USER_TASK]",
      );
    }).toPass(defaultAssertionOptions);
  });
