...
describe('<ConversationMessage />', () => {
  it('should open a markdown preview modal when expand is clicked', async () => {
    const {user} = render(
      <ConversationMessage actor="assistant" messages={['Hello **world**']} />,
    );
  ...
  it('should render markdown in the expand modal', async () => {
    const {user} = render(
      <ConversationMessage
        actor="user"
        messages={['Use `npm install` to set up']}
      />,
    );
  ...
  it('should close the modal when close button is clicked', async () => {
    const {user} = render(
      <ConversationMessage actor="system" messages={['System message']} />,
    );
  ...
  it('should show correct modal title for each actor', async () => {
    const {user, rerender} = render(
      <ConversationMessage actor="system" messages={['prompt']} />,
    );
    ...
    rerender(<ConversationMessage actor="user" messages={['msg']} />);
    await user.click(screen.getByRole('button', {name: 'Expand'}));
    expect(
  ...
  it('should default to preview mode in the modal', async () => {
    const {user} = render(
      <ConversationMessage actor="assistant" messages={['Hello **bold**']} />,
    );
  ...
  it('should show only source view for plain text without markdown', async () => {
    const {user} = render(
      <ConversationMessage actor="assistant" messages={['Just plain text']} />,
    );
