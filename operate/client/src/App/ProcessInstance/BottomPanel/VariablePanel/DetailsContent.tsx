...
const DetailsContent: React.FC = observer(() => {
  ...
  const businessObject = selectedElementId
    ...
    : null;
  const elementInstanceMetadata = resolvedElementInstance;
  ...
  const rows = useMemo(() => {
        ...
        columns: [
          {cellContent: 'Execution Duration'},
          {
            cellContent: startDate
              ? getExecutionDuration(startDate, endDate ?? null)
              : '—',
          },
        ],
      },
    ...
    if (job?.worker) {
      baseRows.push({
        key: 'job-worker',
        columns: [{cellContent: 'Job Worker'}, {cellContent: job.worker}],
      });
    }
    ...
    if (job?.jobKey) {
      baseRows.push({
        key: 'job-key',
        columns: [{cellContent: 'Job Key'}, {cellContent: job.jobKey}],
      });
    }
