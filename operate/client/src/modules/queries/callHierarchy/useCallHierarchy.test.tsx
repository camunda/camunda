...
describe('useCallHierarchy', () => {
  ...
  it('should fetch call hierarchy successfully', async () => {
    const mockData: GetProcessInstanceCallHierarchyResponseBody = {
      items: [
        {
          processInstanceKey: 'childInstance1',
          processDefinitionName: 'Child Process 1',
        },
        {
          processInstanceKey: 'childInstance2',
          processDefinitionName: 'Child Process 2',
        },
      ],
    };
    ...
    const {result} = renderHook(() => useCallHierarchy(), {
      wrapper: Wrapper,
    });
  ...
  it('should apply select function correctly', async () => {
    ...
    const select = (data: GetProcessInstanceCallHierarchyResponseBody) =>
      data.items.map((child) => child.processDefinitionName);

    const {result} = renderHook(() => useCallHierarchy(select), {
      wrapper: Wrapper,
    });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(result.current.data).toEqual(['Child Process 1', 'Child Process 2']);
  });
