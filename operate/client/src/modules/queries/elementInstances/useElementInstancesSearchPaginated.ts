...
type QueryOptions<T> = {
  ...
  enabled?: boolean;
  select?: (data: InfiniteData<QueryElementInstancesResponseBody>) => T;
};

const useElementInstancesSearchPaginated = <
  T = InfiniteData<QueryElementInstancesResponseBody>,
>(
  options: QueryOptions<T>,
) => {
  return useInfiniteQuery({
    queryKey: queryKeys.elementInstances.searchPaginated(options.payload),
...
export {useElementInstancesSearchPaginated, PAGE_LIMIT};
