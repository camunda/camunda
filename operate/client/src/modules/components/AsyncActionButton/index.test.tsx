...
describe('<AsyncActionButton />', () => {
  ...
  it('should call onError callback when status is error', () => {
    vi.useFakeTimers();
    const ON_ERROR_CALLBACK = vi.fn();

    render(
      <AsyncActionButton status="error" onError={ON_ERROR_CALLBACK}>
        button
      </AsyncActionButton>,
    );
    ...
    expect(ON_ERROR_CALLBACK).toHaveBeenCalled();

    vi.useRealTimers();
  });

  it('should hide button when isHidden is true', () => {
