...
test.describe('process instance page', () => {
  ...
  // Guards INSTANCE_HISTORY_MIN_WIDTH. The constant is a measured value, so a
  // wider title or an extra control in the header silently outgrows it - this
  // fails instead, and on Linux, where the font metrics differ from the macOS
  // ones the number was originally measured against.
  test('instance history header stays readable at the panel minimum width', async ({
    page,
    processInstancePage,
  }) => {
    ...
    const panel = processInstancePage.instanceHistory;
    await expect(panel).toBeVisible();

    const title = panel.getByRole('heading', {name: 'Instance History'});
    await expect(title).toBeVisible();

    // The stored 10% is far below the minimum, so the panel is pinned to it.
    const isTitleTruncated = await title.evaluate(
      (element) => element.scrollWidth > element.clientWidth,
    );
    expect(isTitleTruncated).toBe(false);

    // `Container` hides its overflow, so a clipped toggle is still "visible" to
    // Playwright - compare against the panel's right edge instead.
    const panelBox = await panel.boundingBox();
    expect(panelBox).not.toBeNull();

    for (const toggle of [
      processInstancePage.endDateToggle,
      processInstancePage.executionCountToggle,
    ]) {
