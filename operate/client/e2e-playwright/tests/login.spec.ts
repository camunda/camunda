...
test.describe('login page', () => {
  test('Log in with invalid user account', async ({loginPage, page}) => {
    ...
    await expect(page).toHaveURL(`/operate${Paths.login()}`);
  });
  ...
  test('Redirect to initial page after login', async ({loginPage, page}) => {
    await expect(page).toHaveURL(`/operate${Paths.login()}`);
    await page.goto(`/operate${Paths.processes()}?active=true&incidents=true`);
    await expect(page).toHaveURL(`/operate${Paths.login()}`);
    ...
    await expect(page).toHaveURL(
      `/operate${Paths.processes()}?active=true&incidents=true`,
    );
  });
});
