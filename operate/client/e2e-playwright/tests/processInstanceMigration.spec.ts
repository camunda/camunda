...
test.describe.skip('Process Instance Migration', () => {
  ...
  test('Auto Mapping @roundtrip', async ({
    ...
    await expect(page.getByLabel(/target element for/i)).toHaveCount(48);

    await expect(
      page.getByLabel(/target element for check payment/i),
    ).toHaveValue('checkPayment');
    await expect(
      page.getByLabel(/target element for ship articles/i),
    ).toHaveValue('shipArticles');
    await expect(
      page.getByLabel(/target element for request for payment/i),
    ).toHaveValue('requestForPayment');
    await expect(page.getByLabel(/target element for task a/i)).toHaveValue(
      'TaskA',
    );
    await expect(page.getByLabel(/target element for task b/i)).toHaveValue(
      'TaskB',
    );
    await expect(page.getByLabel(/target element for task c/i)).toHaveValue(
      'TaskC',
    );
    await expect(page.getByLabel(/target element for task d/i)).toHaveValue(
      'TaskD',
    );
    await expect(
      page.getByLabel(/target element for message interrupting/i),
    ).toHaveValue('MessageInterrupting');
    await expect(
      page.getByLabel(/target element for timer interrupting/i),
    ).toHaveValue('TimerInterrupting');
    await expect(
      page.getByLabel(/target element for message non-interrupting/i),
    ).toHaveValue('MessageNonInterrupting');
    await expect(
      page.getByLabel(/target element for timer non-interrupting/i),
    ).toHaveValue('TimerNonInterrupting');
    await expect(
      page.getByLabel(/target element for message intermediate catch$/i),
    ).toHaveValue('MessageIntermediateCatch');
    await expect(
      page.getByLabel(/target element for timer intermediate catch$/i),
    ).toHaveValue('TimerIntermediateCatch');
    await expect(
      page.getByLabel(/target element for message event sub process/i),
    ).toHaveValue('MessageEventSubProcess');
    await expect(
      page.getByLabel(/target element for timer event sub process/i),
    ).toHaveValue('TimerEventSubProcess');
    await expect(page.getByLabel(/target element for task e/i)).toHaveValue(
      'TaskE',
    );
    await expect(page.getByLabel(/target element for task f/i)).toHaveValue(
      'TaskF',
    );
    await expect(
      page.getByLabel(/target element for message receive task/i),
    ).toHaveValue('MessageReceiveTask');
    await expect(
      page.getByLabel(/target element for business rule task/i),
    ).toHaveValue('BusinessRuleTask');
    await expect(
      page.getByLabel(/target element for script task/i),
    ).toHaveValue('ScriptTask');
    await expect(page.getByLabel(/target element for send task/i)).toHaveValue(
      'SendTask',
    );
    await expect(
      page.getByLabel(/target element for timer start event/i),
    ).toHaveValue('TimerStartEvent');
    await expect(
      page.getByLabel(/target element for signal start event/i),
    ).toHaveValue('SignalStartEvent');
    await expect(
      page.getByLabel(/target element for signal boundary event/i),
    ).toHaveValue('SignalBoundaryEvent');
    await expect(
      page.getByLabel(/target element for signal intermediate catch/i),
    ).toHaveValue('SignalIntermediateCatch');
    await expect(
      page.getByLabel(/target element for signal event sub process/i),
    ).toHaveValue('SignalEventSubProcess');
    await expect(
      page.getByLabel(/target element for error event sub process/i),
    ).toHaveValue('ErrorEventSubProcess');
    await expect(
      page.getByLabel(/target element for error start event/i),
    ).toHaveValue('ErrorStartEvent');
    await expect(page.getByLabel(/target element for task g/i)).toHaveValue(
      'TaskG',
    );
    await expect(
      page.getByLabel(/target element for sub process/i),
    ).toHaveValue('SubProcess');
    await expect(
      page.getByLabel(/target element for multi instance sub process/i),
    ).toHaveValue('MultiInstanceSubProcess');
    await expect(
      page.getByLabel(/target element for multi instance task/i),
    ).toHaveValue('MultiInstanceTask');
    await expect(
      page.getByLabel(/target element for compensation task/i),
    ).toHaveValue('CompensationTask');
    await expect(
      page.getByLabel(/target element for compensation boundary event/i),
    ).toHaveValue('CompensationBoundaryEvent');
    await expect(
      page.getByLabel(/target element for message start event/i),
