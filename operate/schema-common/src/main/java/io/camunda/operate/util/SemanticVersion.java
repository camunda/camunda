...
public final class SemanticVersion implements Comparable<SemanticVersion> {
  ...
  public SemanticVersion(final String version) {
    ...
    versionParts = tokenize(normalizedVersion);
    displayText = versionParts.stream().map(Object::toString).collect(Collectors.joining("."));
  }
  ...
  public boolean isNewerThan(final SemanticVersion otherVersion) {
   ...
   * @return {@code true} if this major version is newer, {@code false} if it is not.
   */
  public boolean isMajorNewerThan(final SemanticVersion otherVersion) {
  ...
  private Integer getOrZero(final List<Integer> elements, final int i) {
  ...
  private List<Integer> tokenize(final String version) {
    ...
    try {
      for (final String part : splitPattern.split(version)) {
