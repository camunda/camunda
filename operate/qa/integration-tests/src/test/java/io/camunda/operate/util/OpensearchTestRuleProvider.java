...
public class OpensearchTestRuleProvider implements SearchTestRuleProvider {
  ...
  @Override
  public void blockIndexReads(final String index, final boolean block) throws IOException {
    final var indexSettingsRequest =
        new PutIndicesSettingsRequest.Builder()
            .settings(s -> s.index(i -> i.blocksRead(block)))
            .index(index)
            .build();
    richOpenSearchClient.index().putSettings(indexSettingsRequest);
  }

  @Override
  public <T> T getDocument(final String index, final String id, final Class<T> clazz) {
    return richOpenSearchClient.doc().getWithRetries(index, id, clazz).orElse(null);
  }
