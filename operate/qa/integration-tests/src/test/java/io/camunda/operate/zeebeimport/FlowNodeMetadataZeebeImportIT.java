...
public class FlowNodeMetadataZeebeImportIT extends OperateZeebeAbstractIT {
  ...
  public void shouldReturnOneInstanceMetadata() throws Exception {
    ...
    FlowNodeMetadataDto flowNodeMetadata =
        tester.getFlowNodeMetadataFromRest(String.valueOf(processInstanceKey), taskId, null, null);
  ...
  public void shouldReturnOneInstanceMetadataWithBreadcrumb() throws Exception {
    ...
    final BpmnModelInstance testProcess =
        Bpmn.createExecutableProcess(processId)
            ...
            .subProcess(
                subprocessId,
                s ->
                    s.multiInstance(m -> m.zeebeInputCollectionExpression("items").sequential())
                        .embeddedSubProcess()
                        .startEvent()
                        .serviceTask(taskId)
                        .zeebeJobType(taskId)
                        .serviceTask("someTask")
                        .zeebeJobType(taskId)
                        .endEvent())
            .endEvent()
            .done();
    ...
    // then
    assertBreadcrumb(
        flowNodeMetadata.getBreadcrumb(),
    ...
    final FlowNodeInstanceBreadcrumbEntryDto breadcrumb = flowNodeMetadata.getBreadcrumb().get(0);
    flowNodeMetadata =
        tester.getFlowNodeMetadataFromRest(
            String.valueOf(processInstanceKey),
            breadcrumb.getFlowNodeId(),
            breadcrumb.getFlowNodeType(),
            null);
  ...
  public void shouldReturnCalledProcessInstanceIdForMultiInstanceCallActivity() throws Exception {
    ...
    final ProcessInstanceForListViewEntity calledProcessInstance =
        processInstanceReader.getProcessInstanceByKey(Long.valueOf(calledProcessInstanceId1));
    assertThat(calledProcessInstance.getProcessDefinitionKey())
        .isEqualTo(calledProcessDefinitionKey);
  ...
  public void shouldReturnBreadcrumbForPeterCase() throws Exception {
    ...
    final List<FlowNodeInstanceDto> flowNodeInstances =
        tester.getFlowNodeInstanceOneListFromRest(String.valueOf(processInstanceKey));
    final List<FlowNodeInstanceDto> tasks =
        flowNodeInstances.stream()
            .filter(fni -> fni.getType().equals(SERVICE_TASK))
  ...
  public void testMetadataForFinishedProcess() throws Exception {
    ...
    final Long processInstanceKey =
        tester
            .deployProcess("processWithGateway.bpmn")
    ...
    assertMetadata(
        instances,
        EventSourceType.PROCESS_INSTANCE,
  ...
  /**
   * parentProcess -> calledProcess -> process (has incident) Getting the metadata from
   * parentProcess and calledProcess should return incident information from process (last one
   * called).
   *
   * @throws Exception
   */
  ...
  public void testSingleIncidentMetadataForCallActivity() throws Exception {
    ...
    final BpmnModelInstance testProcess2 =
        Bpmn.createExecutableProcess(calledProcess1Id)
            ...
            .done();
    final Long calledProcessDefinitionKey1 =
        tester.deployProcess(testProcess2, "testProcess2.bpmn").getProcessDefinitionKey();
    ...
    flowNodeMetadata =
        tester.getFlowNodeMetadataFromRest(processInstanceId, null, null, flowNodeInstanceId);
    ...
    final String calledProcessInstanceId1 =
        tester
            .getSingleProcessInstanceByBpmnProcessId(String.valueOf(calledProcessDefinitionKey1))
            .getId();
    flowNodeMetadata =
        tester.getFlowNodeMetadataFromRest(calledProcessInstanceId1, callActivity2Id, null, null);
  ...
  /**
   * parentProcess -> calledProcess (has incident) -> process (has incident) Getting the metadata
   * from parentProcess should return incident count = 2.
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * parentProcess -> calledProcess (Peter case: 2 instances of call activity) -> process (each of
   * two instances has incident) Getting the metadata from parentProcess should return incident
   * count = 2. Getting the metadata from calledProcess and flowNodeId should return instance count
   * = incident count = 2.
   *
   * @throws Exception
   */
  ...
  /**
   * parentProcess -> calledProcess (multi-instance: 2 instances of call activity) -> process (each
   * of two instances has incident) Getting the metadata from parentProcess should return incident
   * count = 2. Getting the metadata from calledProcess and flowNodeId should return instance count
   * = incident count = 2.
   *
   * @throws Exception
  ...
  public void testMessageData() throws Exception {
    ...
    // when
    tester.waitUntil().flowNodeIsActive("IntermediateCatchEvent_1ds9kwv");
  ...
  private void assertSingleIncident(
      final IncidentDto incident, final String bpmnProcessId, final String errorMsg) {
    assertThat(incident).isNotNull();
    assertThat(incident.getErrorType().getId()).isEqualTo(ErrorType.JOB_NO_RETRIES.name());
    assertThat(incident.getErrorMessage()).isEqualTo(errorMsg);
    final ProcessInstanceReferenceDto rootCauseInstance = incident.getRootCauseInstance();
    assertThat(rootCauseInstance).isNotNull();
    assertThat(rootCauseInstance.getProcessDefinitionName()).isEqualTo(bpmnProcessId);
    final ProcessInstanceForListViewEntity rootProcessInstance =
        processInstanceReader.getProcessInstanceByKey(
            Long.valueOf(rootCauseInstance.getInstanceId()));
  ...
  private void assertMetadata(
    ...
    final Optional<FlowNodeInstanceDto> flowNodeInstance =
        flowNodes.stream().filter(fni -> fni.getFlowNodeId().equals(flowNodeId)).findFirst();
    ...
    if (eventSourceType.equals(EventSourceType.INCIDENT)) {
      ...
      if (errorMessage != null) {
        assertThat(incident.getErrorMessage())
            .as(assertionName + ".incidentErrorMessage")
      ...
      assertThat(incident.getErrorType()).as(assertionName + ".incidentErrorType").isNotNull();
    }
  }
}
