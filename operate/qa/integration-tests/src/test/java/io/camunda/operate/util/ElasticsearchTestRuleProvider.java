...
public class ElasticsearchTestRuleProvider implements SearchTestRuleProvider {
  ...
  public void blockIndexReads(final String index, final boolean block) throws IOException {
    final var updateIndexRequest = new UpdateSettingsRequest(index);
    updateIndexRequest.settings(Settings.builder().put("index.blocks.read", block).build());
    esClient.indices().putSettings(updateIndexRequest, RequestOptions.DEFAULT);
  }

  @Override
  public <T> T getDocument(final String index, final String id, final Class<T> clazz)
      throws IOException {
    final var getRequest = new org.elasticsearch.action.get.GetRequest(index, id);
    final var response = esClient.get(getRequest, RequestOptions.DEFAULT);
    if (response.isExists()) {
      final var sourceAsString = response.getSourceAsString();
