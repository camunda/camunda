...
public class ElasticsearchConnectorHelper {

  public static final ElasticsearchConnectorHelper INSTANCE = new ElasticsearchConnectorHelper();
  private static final Logger LOGGER = LoggerFactory.getLogger(ElasticsearchConnectorHelper.class);

  public ElasticsearchClient createElasticsearchClient(final ElasticsearchProperties elsConfig) {
  ...
  private boolean checkHealth(
      final RestHighLevelClient esClient, final ElasticsearchProperties elsConfig) {
    try {
      return RetryOperation.<Boolean>newBuilder()
          .noOfRetry(50)
