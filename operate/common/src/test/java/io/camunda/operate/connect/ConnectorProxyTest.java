...
/**
 * Integration tests for proxy support in Operate's Elasticsearch and OpenSearch connectors.
 *
  ...
  private void pingDatabase(final DatabaseType databaseType, final ProxyProperties proxy)
    ...
    switch (databaseType) {
      case ELASTICSEARCH -> {
        ...
        final var esProperties = new OperateElasticsearchProperties();
      ...
      case OPENSEARCH -> {
        final var operateProperties = new OperateProperties();
        final var osProperties = new OperateOpensearchProperties();
        ...
        final var connector = new OpensearchConnector(operateProperties, new ObjectMapper());
