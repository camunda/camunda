...
public class OpensearchConnectorTest {
  ...
  void shouldResolveCredentialsForAllConfiguredUrls() throws Exception {
    opensearchProperties.setUrls(List.of("http://opensearch-1:9205", "http://opensearch-2:9206"));
    opensearchProperties.setUsername("user");
    opensearchProperties.setPassword("secret");
    final OpensearchConnector spyConnector = spy(opensearchConnector);
    doReturn(true).when(spyConnector).checkHealth(any(OpenSearchClient.class));
