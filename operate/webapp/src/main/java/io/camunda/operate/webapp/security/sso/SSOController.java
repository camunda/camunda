...
public class SSOController {
  ...
  public String login(final HttpServletRequest req, final HttpServletResponse res) {
    final String authorizeUrl = auth0Service.getAuthorizeUrl(
  ...
  public void loggedInCallback(final HttpServletRequest req, final HttpServletResponse res)
    ...
    try {
      final var authentication = auth0Service.authenticate(
          (javax.servlet.http.HttpServletRequest) req, (javax.servlet.http.HttpServletResponse) res);
    ...
    } catch (final Auth0ServiceException ase) {
  ...
  public void logout(final HttpServletRequest req, final HttpServletResponse res) throws IOException {
    logger.debug("logout user");
    cleanup(req);
    logoutFromAuth0(res, auth0Service.getRedirectURI((javax.servlet.http.HttpServletRequest) req, ROOT));
  }

  protected void clearContextAndRedirectToNoPermission(
      final HttpServletRequest req, final HttpServletResponse res, final Throwable t) throws IOException {
    logger.error("Error in authentication callback: ", t);
    cleanup(req);
    res.sendRedirect(auth0Service.getRedirectURI((javax.servlet.http.HttpServletRequest) req, NO_PERMISSION));
  }
  ...
  protected void logoutAndRedirectToNoPermissionPage(
    ...
    cleanup(req);
    logoutFromAuth0(res, auth0Service.getRedirectURI((javax.servlet.http.HttpServletRequest) req, NO_PERMISSION));
  }
  ...
  protected void logoutFromAuth0(final HttpServletResponse res, final String returnTo) throws IOException {
