...
import static io.camunda.authentication.OidcES256AlgorithmTest.CLIENT_ID;
import static io.camunda.authentication.OidcES256AlgorithmTest.CLIENT_SECRET;
...
@SpringBootTest(
    ...
    properties = {
      ...
      "camunda.security.authentication.oidc.id-token-algorithm=ES256",
...
public class OidcES256AlgorithmTest {
  ...
  // Test EC JWK used to sign ID tokens and exposed via JWKS
  private static ECKey ecJwk;
  ...
  /**
   * Test that the OIDC authentication flow works end-to-end when the IdP uses custom ES256-signed
   * ID token
   */
  @Test
  ...
  private static void stubIdpEndpoints() {
    ...
    final String jwksBody =
        JSONObjectUtils.toJSONString(new JWKSet(ecJwk.toPublicJWK()).toJSONObject());
    stubFor(
  ...
  private static String tokenResponse() {
    // Build a minimal ES256-signed ID token with kid matching the JWKS and the expected nonce hash
    ...
    final SignedJWT signedJWT =
        new SignedJWT(
            new JWSHeader.Builder(JWSAlgorithm.ES256).keyID(ecJwk.getKeyID()).build(), claims);
    try {
