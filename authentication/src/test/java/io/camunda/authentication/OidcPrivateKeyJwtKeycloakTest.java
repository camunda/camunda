...
class OidcPrivateKeyJwtKeycloakTest {
  ...
  // Test RSA JWK used to sign the IdP's ID tokens and exposed via JWKS. Unrelated to the
  // keystore-backed private key used for the client's own assertion signing.
  private static RSAKey idpRsaJwk;
  ...
  private static void stubJwks() {
    final String jwksBody =
        JSONObjectUtils.toJSONString(new JWKSet(idpRsaJwk.toPublicJWK()).toJSONObject());
    stubFor(
  ...
  public void shouldSendClientAssertionToTokenEndpointDuringRefreshToken() {
    ...
    // trigger OAuth2RefreshTokenFilter to refresh it using the stored refresh token
    final var authorizationRedirect = beginAuthenticationFlow();
    ...
    final var unresolvedException = result.getUnresolvedException();
    if (unresolvedException != null) {
      assertThat(unresolvedException).isNotInstanceOf(IllegalArgumentException.class);
    }
  ...
  private static String signedIdToken(final String nonceHash) {
    ...
    final SignedJWT signedJWT =
        new SignedJWT(
            new JWSHeader.Builder(JWSAlgorithm.RS256).keyID(idpRsaJwk.getKeyID()).build(), claims);
    try {
      signedJWT.sign(new RSASSASigner(idpRsaJwk.toPrivateKey()));
