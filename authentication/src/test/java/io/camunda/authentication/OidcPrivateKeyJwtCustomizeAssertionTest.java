...
@SpringBootTest(
    ...
    properties = {
      ...
      "camunda.security.authentication.oidc.assertionKeystore.kidDigestAlgorithm=sha1",
      "camunda.security.authentication.oidc.assertionKeystore.kidEncoding=hex",
      "camunda.security.authentication.oidc.assertionKeystore.kidCase=upper",
...
class OidcPrivateKeyJwtCustomizeAssertionTest {
  ...
  private void verifyClientAssertion() throws ParseException {
    ...
    final Function<String, Object> getClaim = c -> jwt.getPayload().toJSONObject().get(c);
    assertThat(getClaim.apply("iss")).isEqualTo(CLIENT_ID);
    assertThat(getClaim.apply("sub")).isEqualTo(CLIENT_ID);
    assertThat(getClaim.apply("aud")).isEqualTo(expectedTokenEndpointUrl);
    assertThat(getClaim.apply("jti")).isNotNull();
    assertThat(getClaim.apply("iat")) // issued in the past
        .is(
            new Condition<>(
                iat -> Instant.now().getEpochSecond() >= (Long) iat,
                "less than or equal to %d",
                Instant.now().getEpochSecond()));
    assertThat(getClaim.apply("exp")) // expires in the future
        .is(
            new Condition<>(
                exp -> Instant.now().getEpochSecond() < (Long) exp,
                "more than %d",
                Instant.now().getEpochSecond()));
