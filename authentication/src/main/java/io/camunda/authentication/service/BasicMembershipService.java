...
public class BasicMembershipService implements MembershipService {

  private final TenantServices tenantServices;
  private final RoleServices roleServices;
  private final GroupServices groupServices;

  public BasicMembershipService(
  ...
  public MembershipProvider createProvider(
    ...
    // BASIC auth has no token claims and never produces CLIENT principals; the converter calls
    // createProviderForUser(...), so claims and type aren't read here.
    return new Provider(principalId);
  }

  /**
   * Per-authentication provider that memoizes the {@code groups → roles → tenants} chain so
  ...
  private final class Provider implements MembershipProvider {
    ...
    Provider(final String username) {
