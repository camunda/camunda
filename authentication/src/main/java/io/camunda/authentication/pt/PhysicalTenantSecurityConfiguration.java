 ...
 * is available on the request when basic-auth processing runs inside the security chain.
 */
@Configuration
public class PhysicalTenantSecurityConfiguration {
   ...
   * available during Spring Security's basic-auth processing (which runs inside the security filter
   * chain, after {@code FilterRegistrationBean} filters at lower order values).
   */
  @Bean
  public FilterRegistrationBean<PhysicalTenantPreSecurityFilter>
      physicalTenantPreSecurityFilterRegistration(
          final CamundaSecurityScopeProvider physicalTenantScopeProvider) {
    final var tenantIds =
        physicalTenantScopeProvider.get().stream()
            .map(
                d -> {
                  final String base = d.basePath(); // e.g. /physical-tenants/tenanta
                  final int slash = base.lastIndexOf('/');
                  return slash >= 0 ? base.substring(slash + 1) : base;
                })
            .collect(Collectors.toUnmodifiableSet());

    final var filter = new PhysicalTenantPreSecurityFilter(tenantIds);
