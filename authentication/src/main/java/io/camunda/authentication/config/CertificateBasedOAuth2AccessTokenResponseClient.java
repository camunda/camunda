...
/**
 * Custom OAuth2AccessTokenResponseClient that supports certificate-based client assertion
 * for Microsoft Entra ID authentication following the specification:
...
public class CertificateBasedOAuth2AccessTokenResponseClient 
  ...
  public CertificateBasedOAuth2AccessTokenResponseClient(
    ...
    this.delegate.setRequestEntityConverter(new CertificateBasedRequestEntityConverter());
  }

  @Override
  public OAuth2AccessTokenResponse getTokenResponse(final OAuth2AuthorizationCodeGrantRequest authorizationGrantRequest) {
  ...
  /**
   * Custom converter that modifies the token request to include client assertion
  ...
  private class CertificateBasedRequestEntityConverter 
    ...
    @Override
    public RequestEntity<?> convert(final OAuth2AuthorizationCodeGrantRequest authorizationGrantRequest) {
      ...
      if (oidcConfiguration.isClientAssertionEnabled()) {
        ...
        if (entity.getBody() instanceof MultiValueMap) {
          ...
          MultiValueMap<String, String> originalParams = (MultiValueMap<String, String>) entity.getBody();
        ...
        final String tokenUri = authorizationGrantRequest.getClientRegistration().getProviderDetails().getTokenUri();
        final String clientAssertion = clientAssertionService.createClientAssertion(oidcConfiguration, tokenUri);
        
        formParameters.put("client_assertion_type", Collections.singletonList(CLIENT_ASSERTION_TYPE));
