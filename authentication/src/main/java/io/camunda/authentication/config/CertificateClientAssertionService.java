...
public class CertificateClientAssertionService {
  ...
  public String createClientAssertion(
    ...
    try {
      ...
      LOG.info("JWT Header: {}", signedJWT.getHeader().toString());
      LOG.info("JWT Claims: {}", signedJWT.getJWTClaimsSet().toString());
      LOG.info("JWT Algorithm: {}", signedJWT.getHeader().getAlgorithm());
      LOG.info("JWT Key ID: {}", signedJWT.getHeader().getKeyID());
