...
public class OidcTokenEndpointCustomizer
  ...
  private JWK createJwk(final String clientRegistrationId) {
    ...
    try {
      ...
      return new RSAKey.Builder(pub)
          ...
          .keyID(generateKid(cert, oidcConfig.getAssertionKeystore())) // default for Keycloak
   ...
   * the client.
   *
   * <p>The <code>kid</code> generation can be modified using the {@link
   * AssertionKeystoreConfiguration} to control if the certificate or it's public key is used, which
   * hashing algorithm will be used, what will be the encoding of the <code>kid</code> string, and
   * if the <code>kid</code> string will be in lowercase or uppercase (relevant for hex encoding).
   *
   ...
   */
  private String generateKid(final Certificate cert, final AssertionKeystoreConfiguration config) {
    ...
    final var kid =
        switch (config.getKidEncoding()) {
          ...
          case null -> Base64URL.encode(digest).toString();
        };
    LOG.debug("generated kid '{}' from keystore '{}'", kid, config.getPath());
    return kid;
  }
  ...
  private MessageDigest getKidDigestAlgorithmInstance(final KidDigestAlgorithm alg) {
    try {
      return switch (alg) {
        ...
        case null -> MessageDigest.getInstance("SHA-256");
      };
  ...
  private byte[] getKidSourceBytes(final Certificate cert, final KidSource source) {
    try {
      return switch (source) {
        ...
        case null -> cert.getPublicKey().getEncoded();
      };
