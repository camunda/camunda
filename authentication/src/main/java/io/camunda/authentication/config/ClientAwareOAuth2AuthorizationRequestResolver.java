...
public class ClientAwareOAuth2AuthorizationRequestResolver
  ...
  protected OAuth2AuthorizationRequestResolver createAuthorizationRequestResolver(
    ...
    Optional.ofNullable(oidcProvider)
        .map(OidcAuthenticationConfiguration::getAuthorizeRequest)
        .map(AuthorizeRequestConfiguration::getAdditionalParameters)
        .ifPresent(p -> resolver.setAuthorizationRequestCustomizer(c -> c.additionalParameters(p)));
