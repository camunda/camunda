...
This is **not** the same generic OC server-side `private_key_jwt` implementation described in the [Orchestration Cluster Identity architecture](./architecture.md#632-oidc-with-private_key_jwt-client-authentication). Management Identity currently uses the narrower Microsoft / Entra path from the Identity SDK (`MicrosoftAuthentication` + `CertSignedJwt`). Instead of OC's generic Spring Security assertion stack, it signs a certificate-backed JWT assertion and sends it to the Microsoft token endpoint.
