...
public interface NativeUserRepository extends JpaRepository<NativeUser, Long> {
  ...
  @Query(
      """
          select new io.camunda.identity.automation.usermanagement.CamundaUser(users.id, users.username, profiles.name, profiles.email, users.enabled) \
          from User users \
          left join NativeUser profiles on users.id = profiles.id""")
