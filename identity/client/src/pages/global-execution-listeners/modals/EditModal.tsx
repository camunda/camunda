...
import {
  ...
  LISTENER_EXECUTION_EVENT_TYPES,
...
const EditModal: FC<UseEntityModalProps<GlobalExecutionListener>> = ({
  ...
  const handleEventTypeChange = (
    ...
    const individualTypes = LISTENER_EXECUTION_EVENT_TYPES.filter(
      (opt) => opt !== "all",
    );

    if (selectedItems.includes("all") && !eventTypes.includes("all")) {
      setValue("eventTypes", [...LISTENER_EXECUTION_EVENT_TYPES]);
      return;
    }
    ...
    if (
      ...
      selectedItems.length < LISTENER_EXECUTION_EVENT_TYPES.length
    ) {
      setValue(
        "eventTypes",
  ...
  const onSubmit = async (data: CreateGlobalExecutionListenerRequestBody) => {
    const resolvedEventTypes = data.eventTypes.includes("all")
      ? ["all" as const]
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <MultiSelect
            ...
            items={[...LISTENER_EXECUTION_EVENT_TYPES]}
