...
/**
 * Dispatches incoming requests and messages to the right raft. This handler is called by the
 * transport {@link Receiver} agent.
 */
public class RaftApiMessageHandler
    implements ServerMessageHandler, ServerRequestHandler, Consumer<Runnable> {
  ...
  private final RequestWithPartitionIdDecoder requestWithPartitionIdDecoder =
      new RequestWithPartitionIdDecoder();
  ...
  public boolean onRequest(
    ...
    if (raft == null) {
      LOG.warn(
          "Dropping request from {}: no partition with id {} present.", remoteAddress, partitionId);
    } else {
  ...
  public boolean onMessage(
    ...
    if (raft == null) {
      LOG.warn(
          "Dropping message from {}: no partition with id {} present.", remoteAddress, partitionId);
      return true;
    } else {
  ...
  private int getPartitionId(DirectBuffer buffer, int offset) {
    ...
    requestWithPartitionIdDecoder.wrap(
