...
import static io.camunda.zeebe.client.impl.ZeebeClientEnvironmentVariables.OAUTH_ENV_CLIENT_ASSERTION_KEYSTORE_PATH;
...
public final class ZeebeClientBuilderImpl implements ZeebeClientBuilder, ZeebeClientConfiguration {
  ...
  private boolean shouldUseDefaultCredentialsProvider() {
    final boolean clientSecretOrCertificateProvided =
        Environment.system().get(OAUTH_ENV_CLIENT_SECRET) != null
            || Environment.system().get(OAUTH_ENV_CLIENT_ASSERTION_KEYSTORE_PATH) != null;
  ...
  private static void checkIfUriIsAbsolute(final URI address, final String propertyName) {
