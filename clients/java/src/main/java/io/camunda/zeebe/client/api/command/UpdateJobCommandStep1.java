...
public interface UpdateJobCommandStep1 {
   ...
   * when the command is processed. The timeout value will be added to the current time then.
   *
   * @param retries retries the retries of this job, if null it will be not updated
   * @param timeout the time the timeout of this job, if null it will be not updated
   * @return the builder for this command. Call {@link #send()} to complete the command and send it
   *     to the broker.
   */
