...
final class JobStreamerImpl implements JobStreamer {
  ...
  private JobStream streamControl;
  ...
  public void openStreamer(final Consumer<ActivatedJob> jobConsumer) {
    final StreamJobsCommandStep3 command = buildCommand(jobConsumer);
    open(command);
  }
  ...
  private StreamJobsCommandStep3 buildCommand(final Consumer<ActivatedJob> jobConsumer) {
    StreamJobsCommandStep3 command =
        jobClient
            ...
            .jobType(jobType)
            .listener(new Listener(jobConsumer))
  ...
  private final class Listener implements StreamJobsListener {
    private final Consumer<ActivatedJob> jobConsumer;

    private Listener(final Consumer<ActivatedJob> jobConsumer) {
      this.jobConsumer = jobConsumer;
    }

    @Override
    public void onJob(final ActivatedJob job) {
