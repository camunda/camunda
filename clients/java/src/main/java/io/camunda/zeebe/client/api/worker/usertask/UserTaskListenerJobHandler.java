...
public interface UserTaskListenerJobHandler {
   ...
   * received.
   *
   * <p>In case the task listener job handler throws an exception the job is failed and the job
   * retries are automatically decremented by one. The failed job will contain the exception
   ...
   */
  void handle(UserTaskListenerJob job) throws Exception;
}
