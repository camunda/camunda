...
public final class JobWorkerBuilderImpl
  ...
  // Prototype for #59734: fraction of a streaming worker's capacity reserved for its poll path. See
  // reservedPollCapacity below. To be settled by the cluster benchmark before this becomes a real
  // default.
  ...
  // Prototype for #59734. A streaming worker's push path reaches a freed slot in microseconds while
  // its poll path only reacts a network round-trip later, so under sustained push the poll — the
  // only path that drains the ACTIVATABLE backlog and recovers timed-out jobs — is starved of
  // slots.
  // Keeping a small share of the capacity reachable only by the poll guarantees the backlog a way
  // in. The share is the floor of a fixed fraction, so a worker reserves nothing until it has
  // enough slots to spare a whole one — a small worker would otherwise hand the push path far too
  // little. This value is a prototype knob to be settled by the cluster benchmark in #59734.
  private static int reservedPollCapacity(final int maxJobsActive) {
