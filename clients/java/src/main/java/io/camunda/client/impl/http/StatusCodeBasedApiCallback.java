...
/**
 * An implementation of {@link FutureCallback} that completes a {@link CompletableFuture} based on
 * the HTTP status code of the {@link ApiResponse}. The callback uses a {@link
 * StatusCodeTransformer} to transform successful status codes into response values, and handles
 * error status codes by completing the future exceptionally with appropriate exceptions.
 *
 * @param <RespT> the type of the response value produced by the callback on success
 */
final class StatusCodeBasedApiCallback<RespT> implements FutureCallback<ApiResponse<Void>> {
  ...
  private final StatusCodeTransformer<RespT> transformer;
  ...
  public StatusCodeBasedApiCallback(
  ...
  @Override
  public void completed(final ApiResponse<Void> result) {
    final ApiEntity<Void> body = result.entity();
  ...
  private void handleErrorResponse(
      final ApiEntity<Void> body, final int code, final String reason) {
  ...
  private void completeResponse(final int code, final String reason) {
    try {
      response.complete(transformer.transform(code));
