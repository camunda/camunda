...
/**
 * Custom Jackson deserializer for {@link StringFilterProperty} that supports both plain string
 * values and object form. A plain string is treated as an exact match ({@code $eq}).
 */
public class StringFilterPropertyDeserializer extends StdDeserializer<StringFilterProperty> {
  ...
  public StringFilterProperty deserialize(final JsonParser p, final DeserializationContext ctxt)
    ...
    // For object form, delegate to AdvancedStringFilter which has the same structure
    // but avoids recursive deserialization
