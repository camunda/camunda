...
public interface EvaluateConditionalCommandStep1
  ...
  /**
   * Skip specifying a process definition key for the evaluation.
   *
   * <p>This method allows the evaluation to be performed across all process definitions with
   * conditional start events. When used, all matching conditional start events across all process
   * definitions will be evaluated.
   *
   * @return the builder for this command, continuing to the next step without a process definition
   *     key.
   */
  EvaluateConditionalCommandStep2 withoutProcessDefinitionKey();
