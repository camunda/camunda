...
final class ActivationDeadlineTest {
  ...
  private final Supplier<ActivationDeadline> deadlines =
      ActivationDeadline.startingOnReceipt(clock, JOB_TIMEOUT);

  @Test
  void shouldNotHavePassedRightAway() {
    // given
    final ActivationDeadline deadline = deadlines.get();
  ...
  void shouldMeasureEachJobFromItsOwnArrival() {
    ...
    final ActivationDeadline first = deadlines.get();

    // when the next job arrives one whole timeout later
    clock.advance(JOB_TIMEOUT);
    final ActivationDeadline second = deadlines.get();
  ...
  void shouldAcceptATimeoutTooLongToMeasureInNanoseconds() {
    ...
    // when
    final Supplier<ActivationDeadline> longDeadlines =
        ActivationDeadline.startingOnReceipt(clock, beyondNanos);

    // then taking a deadline does not throw, where converting the timeout to nanoseconds would
    assertThat(longDeadlines.get().hasPassed()).isFalse();
  }
  ...
  void shouldTreatATimeoutTooLongToMeasureAsOneThatNeverPasses() {
    // given
    final Duration beyondNanos = ActivationDeadline.LONGEST_MEASURABLE_TIMEOUT.plusDays(1);
    final ActivationDeadline deadline =
        ActivationDeadline.startingOnReceipt(clock, beyondNanos).get();

    // when a century goes by
    clock.advance(Duration.ofDays(36_500));

    // then the job is still the worker's to run, rather than the capped timeout wrapping round
