 ...
 *
 * <p>Active only under the {@code secrets} Spring profile and only when {@link
...
public class SecretsDriver implements CommandLineRunner {
   ...
   */
  private void registerLatencyTimers() {
  ...
  private void issueResolve() {
    ...
    final CompletionStage<?> future =
        client
            ...
            .requestTimeout(config.getRequestTimeout())
            .send();
    recordOnComplete(future, ENDPOINT_RESOLVE, startTime);
  }
  ...
  private void issueList() {
    ...
    recordOnComplete(future, ENDPOINT_LIST, startTime);
  }

  private void recordOnComplete(
      final CompletionStage<?> future, final String endpoint, final long startTime) {
    future.whenComplete(
        (result, error) -> {
          final long durationNanos = System.nanoTime() - startTime;
          final String outcome = error == null ? OUTCOME_SUCCESS : OUTCOME_ERROR;
          latencyTimers
   ...
   */
  private List<String> buildResolveBatch() {
    final int batchSize = config.getEffectiveBatchSize();
