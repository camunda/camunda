...
public class OptimizeReportApiClient implements AutoCloseable {
  ...
  // ---------------------------------------------------------------------------
  // Authentication (Keycloak Client Credentials Grant)
  // ---------------------------------------------------------------------------

  public void authenticateWithClientCredentials() throws Exception {
    ...
    if (response.statusCode() != 200) {
      throw new RuntimeException(
          String.format(
              "Client credentials token request failed with status %d: %s",
    ...
    LOG.debug("Successfully authenticated via client credentials grant");
  }
