...
class SuspensionMeterTest {
  ...
  void shouldBuildTargetVariablesWithRequestedFanOut() {
    // given / when
    final var variables = SuspensionMeter.buildTargetVariables(500, 500, "i0");

    // then - the multi-instance collections drive the fan-out size
    assertThat(variables).containsOnlyKeys("jobs", "subs");
  ...
  void shouldPrefixSubscriptionKeysSoTheyAreUniqueAndDistinct() {
    // given / when
    final var variables = SuspensionMeter.buildTargetVariables(0, 3, "i7");
