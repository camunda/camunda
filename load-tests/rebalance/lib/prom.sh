...
# How many partitions a cancellation left untouched. Read from the terminal-
# outcomes counter rather than the partition-state gauge: that gauge only ever
# reports a partition's progress (pending/transferring/completed), not what a
# cancellation made of it, so CANCELLED is counted the same way as any other
# result - and, being a counter, only the delta since a baseline means anything.
CANCELLED_QUERY='sum(zeebe_cluster_rebalance_partition_duration_seconds_count{namespace="$NAMESPACE", result="CANCELLED"})'
...
# resolved without asking anyone (ALREADY_LEADER).
result_query() {
