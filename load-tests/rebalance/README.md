...
| Cluster-admin credentials      | See below — differs by the cluster's authentication method                                                              |

The suite drives the production surface, `/cluster/v2/rebalance` on the REST Gateway port (8080),
not the actuator/monitoring port (9600) everything else here uses — the API this suite validates is
the one clients use, not the actuator surface an earlier iteration of this branch targeted. The
endpoint sits behind the cluster-admin security chain, which does **not** accept an Orchestration
Cluster user's credentials, and which of two mutually-exclusive chains is even registered depends
on the cluster's `security.authentication.method`:

- **`basic`**: set `CLUSTER_ADMIN_USER`/`CLUSTER_ADMIN_PASSWORD` in the environment, matching a user
  configured under `camunda.security.cluster-admin.basic.users`.
- **`oidc`** (the default for a load-test cluster): set `CLUSTER_ADMIN_BEARER_TOKEN` to a bearer
  token whose `client_id`/group/claim matches `camunda.security.cluster-admin.oidc.*`. This is
  **deny-all until configured** — not "disabled" — so on a fresh cluster nobody can call the endpoint
  until at least one matcher is set, e.g.:
  ```bash
  kubectl set env statefulset/camunda -n "$NS" CAMUNDA_SECURITY_CLUSTERADMIN_OIDC_CLIENTS=orchestration
  kubectl rollout status statefulset/camunda -n "$NS"
  # then mint a token for that same client (secret is orchestration-security-authentication-oidc-secret
  # in the camunda-credentials secret) via its client-credentials grant against Keycloak's token-uri.
  ```

Leaving both env vars unset only works against a cluster with the cluster-admin chain fully
permissive, which a normal load-test cluster is not.
...
## Checking the dashboard on its own

A panel that renders empty looks the same whether nothing happened, the metric is not published, or
the query is wrong. `check-panels.sh` runs every query behind a dashboard row and reports how many
series each panel would draw, so the difference is answerable:

```bash
kubectl port-forward -n monitoring svc/kube-prometheus-stack-prometheus 9090:9090 &
./check-panels.sh -n <ns>                        # the Rebalancing row, as of now
./check-panels.sh -n <ns> --row Raft --range 1h  # any other row, over any window
```

`run.sh` runs it over the whole run's window at the end and puts the result in the summary, so a run
that produced no readable dashboard says so.
...
| `zeebe_cluster_rebalance_partition_duration_seconds`                     | the coordinator       | Time on one partition, tagged with what became of it — the fastest read on why a rebalance did what it did. Counts partitions resolved without asking anyone under `ALREADY_LEADER`                                                                                                                                                                                                                                                                                 |
| `zeebe_cluster_rebalance_partition_state`                                | the coordinator       | Where the last rebalance got to with each partition: 1 pending, 2 transferring, 3 completed. Deliberately **left standing** once a rebalance ends, because a rebalance is often over inside a single scrape and there would otherwise be nothing to see; the next rebalance replaces it, and it goes away when the coordinator moves. So the check between rebalances is that no partition sits in a live state (`<= 2`), not that the series is gone. What a completed partition actually resolved to (transferred, skipped, failed, cancelled, ...) is only in `zeebe_cluster_rebalance_partition_duration_seconds`'s `result` tag, not in this gauge |
