...
| `zeebe_cluster_rebalance_partition_state`                                | the coordinator       | Where the last rebalance got to with each partition: 1 pending, 2 transferring, 3 transferred, 4 skipped, 5 failed. Deliberately **left standing** once a rebalance ends, because a rebalance is often over inside a single scrape and there would otherwise be nothing to see; the next rebalance replaces it, and it goes away when the coordinator moves. So the check between rebalances is that no partition sits in a live state, not that the series is gone |
