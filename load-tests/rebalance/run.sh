...
LAGGING_BROKER=""
# For minting fresh cluster-admin OIDC tokens (see lib/cluster.sh's
# rest_auth_args). The Keycloak service name matches the load-test namespace by
# convention; only used when CLUSTER_ADMIN_CLIENT_ID is set.
KEYCLOAK_NAMESPACE=keycloak-operator
KEYCLOAK_SERVICE=""
KEYCLOAK_PORT=18080
KEYCLOAK_PF_PORT=28080
...
The rebalance endpoint (POST/GET/DELETE /cluster/v2/rebalance) sits behind the
cluster-admin security chain, which does not accept an Orchestration Cluster
user's credentials. Set CLUSTER_ADMIN_USER and CLUSTER_ADMIN_PASSWORD in the
environment if the cluster enforces it; leave them unset only against a
cluster with cluster-admin authentication disabled.
EOF
}
...
KEYCLOAK_SERVICE="${KEYCLOAK_SERVICE:-$NS}"
...
preflight() {
  ...
  local body state running
  body=$(reb_get 2>/dev/null || echo '{}')
  state=$(jq -r '.state // "unreachable"' <<<"$body")
  running=$(jq -r '.runningRebalance != null' <<<"$body")
  [[ "$state" != "unreachable" ]] || die "the rebalance endpoint is unreachable"
  [[ "$running" != "true" ]] || die "a rebalance is already running; expected none before starting"
