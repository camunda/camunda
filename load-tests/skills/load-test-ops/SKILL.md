...
definitions, SLO targets, and Prometheus queries, see `load-tests/docs/metrics.md` on `main` — that
file hasn't been ported to this branch yet, so query the headline metrics via
[`load-tests/docs/scripts/queries.yaml`](https://github.com/camunda/camunda/blob/main/load-tests/docs/scripts/queries.yaml)
or the Grafana dashboards directly instead (see [Analyze metrics](#analyze-metrics)). For the canonical schema of every
...
PromQL queries lives in `load-tests/docs/metrics.md` on `main` (not yet ported to this branch —
use the Prometheus/Grafana links below instead). The Prometheus
...
For metric names and PromQL queries, see `load-tests/docs/metrics.md` on `main` (not yet ported to
this branch).
...
When the headline metrics aren't enough, query the full set from `load-tests/docs/metrics.md` on
`main` (not yet ported to this branch) directly against Prometheus. Open a
...
**Metrics reference** — Prometheus queries, SLO targets per variant, and threshold definitions: `load-tests/docs/metrics.md` on `main` (not yet ported to this branch).
