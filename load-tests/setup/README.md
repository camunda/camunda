...
The load test configuration is done via the `camunda-platform-values.yaml` file that was copied to your namespace folder during setup. This file contains optimized settings for running load tests with a Camunda cluster on GKE.

You can also customize the load test behavior by passing additional parameters to the `make install-load-test` command.

#### Use different Camunda Snapshot

If you want to use your own or a different Camunda snapshot, you can do the following:
...
Then run the Makefile commands with the `IMAGE_TAG` variable:

```bash
make install IMAGE_TAG=SNAPSHOT-2024-01-15-abcd1234
```

### How to run a load test

After you have set up your load test namespace and made changes to your configuration, you can start your load test with:
...
This will:
1. Install the Camunda Platform helm chart with `camunda`, `elastic` and other components
2. Install the load test helm chart with `starter` and `worker` applications
3. Set up a leader balancing cronjob

You can also install components separately:
```bash
# Install only the platform
make install-platform

# Install only the load test
make install-load-test
```

### How to update a running load test

To update an existing load test deployment:

```bash
# Update both platform and load test
make update

# Update only the platform
make update-platform

# Update only the load test
...
Alternatively, you can clean up the helm releases while keeping the namespace:
...
To do this, use the `*-stable` Makefile targets:

```shell
# Install both platform and load test on stable VMs
make install-stable

# Install only the platform on stable VMs
