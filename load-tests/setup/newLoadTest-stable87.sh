#!/bin/bash
# Scaffold script for stable/8.7 load tests.
# Uses configuration from stable-8.7/ instead of default/.
# This script lives on main so that 8.7 setup changes don't require backports.
...
usage() {
  cat <<'EOF'
Usage: newLoadTest-stable87.sh <namespace> [secondaryStorage] [ttl_days] [enable_optimize] [enable_webapps] [enable_single_zone]
...
Examples:
  ./newLoadTest-stable87.sh demo
  ./newLoadTest-stable87.sh perf elasticsearch 3 true true
...
# Scaffold the namespace folder from the stable-8.7 config directory.
mkdir -p "$namespace"
...
# If the secret already exists in the cluster, preserve all existing values.
