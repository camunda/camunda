...
| `agent-visibility` | 20          | 160          | Ad-hoc sub-process, fixed 4-round tool-calling schedule (8 flow-node completions per PI: 4 orchestrator rounds + 4 tool calls). Run once with the treatment env vars off (baseline) and once on (see `load-tests/README.md`) for A/B comparison — no throughput SLO of its own, used for relative-overhead measurement. Baseline sustains 20 PI/s cleanly (drain time ~1-2s after creation stops, confirmed across multiple runs). Treatment (`agent-instance-simulation-enabled=true`) creates at the same 20 PI/s, but the added synchronous `AgentInstance` CREATE/UPDATE calls push orchestrator handler mean latency from ~0.1ms to ~150-165ms and drain time to ~70-99s — the orchestrator's `execution-threads=10` becomes the throughput ceiling once handling blocks on a network round-trip. This is the expected, measured cost of agent-instance visibility, not a regression. |
