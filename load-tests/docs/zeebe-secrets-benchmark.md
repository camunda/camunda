...
(`SPRING_PROFILES_ACTIVE=secrets`) instead of the process starter/workers. The driver issues
...
Because `orchestration.secretStore` is not in the pinned released chart yet, the `zeebe-secrets` scenario
requires the platform chart to be sourced from git `main`. Set `LOAD_TEST_PLATFORM_CHART_GIT_REF`
when scaffolding (see below); `newLoadTest.sh` then clones `camunda-platform-helm` at that ref and
builds its dependencies, instead of pulling the released artifact. All other scenarios are
unaffected.
...
select the branch, name the test, and choose the `zeebe-secrets` scenario, which drives
mixed resolve + list traffic by default. To instead run the
...
| resolve store-miss                 | 1.0            | 20          | 0.0              | 100                 | false    | Worst-case backend (file store) read cost                                                                                                |
...
For the **concurrency ramp**, step the offered load (`rate` and/or `threads`) upward across runs —
e.g. 1 → 5 → 10 → 25 → 50 → 100 parallel — and record where latency rises sharply, throughput
plateaus, or error rate climbs.

`referencePoolSize` for the store-miss run must not exceed `zeebeSecretsBenchmark.count` (the number of
seeded secret files, default 100). To force store reads with a pool larger than the cache, raise the
secret cache pressure instead — reduce `camunda.secrets.cache.max-size` below `referencePoolSize`,
or set `warmup=false` so first-touch reads are measured.
...
Requests completed per second, split by endpoint.

- **Unit:** req/s
- **Measurement:** rate

```promql
sum(rate(zeebe_secrets_requests_submitted_total{namespace=~"$namespace"}[$__rate_interval])) by (endpoint)
```
