...
For a **finite** (default 10min) run that posts a comparable, baseline-checked metrics table back as a PR comment and auto-cleans the namespace, use [**quicker-max**](https://github.com/camunda/camunda/labels/quicker-max) (saturating throughput). It runs in its own namespace and can coexist with the `benchmark` label. See [`quicker-benchmark/`](quicker-benchmark/README.md) for details.
