...
// TODO: this is a copy of Operate's SoftHashMap that avoids using inefficient
// thread-safe classes; thread-safety is not needed because we have a single exporter
// owning the map; probably this class should be replaced entirely because
// it has an unpredictable memory consumption
/**
 ...
 */
public class OperateElasticsearchSoftHashMap<K, V> implements Map<K, V> {
   ...
   * @see #SoftHashMap(int)
   */
  public OperateElasticsearchSoftHashMap() {
  ...
  public OperateElasticsearchSoftHashMap(int retentionSize) {
    super();
    RETENTION_SIZE = Math.max(0, retentionSize);
    queue = new ReferenceQueue<>();
    map = new ConcurrentHashMap<>();
    strongReferences = new LinkedList<>();
  }
   ...
   */
  public OperateElasticsearchSoftHashMap(Map<K, V> source) {
    this(DEFAULT_RETENTION_SIZE);
    putAll(source);
  }
   ...
   *     referenced (retained), preventing them from being eagerly garbage collected by the JVM.
   */
  public OperateElasticsearchSoftHashMap(Map<K, V> source, int retentionSize) {
    this(retentionSize);
    putAll(source);
  }
