 ...
 * of two is used. The max index size is {@link HashIndex#MAX_TABLE_SIZE} as the index stores
 ...
 *
 */
public abstract class HashIndex<K extends IndexKeyHandler, V extends IndexValueHandler>
{
    private static final int KEY_HANDLER_IDX = 0;
    private static final int VALUE_HANDLER_IDX = 1;

    public static final int DEFAULT_IDX_SIZE = 32;
     ...
     */
    public static final int OPTIMAL_BUCKET_COUNT = DEFAULT_BLOCK_COUNT;

    /**
     * Deprecated since the hash table can grow dynamic now. It is used only for the start hash table size.
     */
    public static final int OPTIMAL_INDEX_SIZE = DEFAULT_IDX_SIZE;
    ...
    protected final HashTableBuckets dataBuffer;

    protected int maxTableSize;
    protected int tableSize;
    protected int mask;
    protected final int maxBlockLength;

    protected final AtomicBoolean isClosed = new AtomicBoolean(false);


    public HashIndex(int maxKeyLength, int maxValueLength)
    {
        this(DEFAULT_IDX_SIZE, DEFAULT_BLOCK_COUNT, maxKeyLength, maxValueLength);
    }
     ...
     *
     * <p>
     * The index can store `X` entries, which is at maximum equal to `tableSize * maxBlockLength`.
     ...
     * </pre>
     *
     * @param tableSize is the count of buckets, which should been used by the hash index
     * @param blockCount the count of blocks in a bucket
     * @param maxKeyLength the max length of a key
     * @param maxValueLength the max length of a value
     */
    public HashIndex(int tableSize, int blockCount, int maxKeyLength, int maxValueLength)
    {
        ...
        this.tableSize = ensureIndexSizeIsPowerOfTwo(tableSize);
        this.mask = this.tableSize - 1;

        this.hashTable = new HashTable(this.tableSize);
        maxBlockLength =
            addExact(BLOCK_DATA_OFFSET,
                     multiplyExact(blockCount, getRecordLength(maxKeyLength, maxValueLength)));
        this.dataBuffer = new HashTableBuckets(maxBlockLength, maxKeyLength);
    ...
    public long size()
    {
        return hashTable.getLength() + dataBuffer.getLength();
    }
    ...
    private int ensureIndexSizeIsPowerOfTwo(final int indexSize)
    {
        ...
        {
            LOG.warn("Supplied hash table size {} is not a power of two. Using next power of two {} instead.", indexSize, powerOfTwo);
        }
    ...
    private void init()
    {
        ...
        {
            hashTable.setBlockAddress(idx, blockOffset);
        }
    }
    ...
    private <K extends IndexKeyHandler> K createKeyHandlerInstance(int maxKeyLength)
    {
    ...
    {
        this.maxTableSize = ensureIndexSizeIsPowerOfTwo(maxTableSize);
    }
    ...
    protected boolean put()
    {
        ...
        {
            final long blockOffset = hashTable.getBlockAddress(blockId);
    ...
    private void splitBlock(long filledBlockOffset)
    {
        ...
        if (newBlockId >= tableSize)
        {
            // TODO: implement overflow!
            // if load factor is less then 0.7 we use overflow of buckets
            ...
            {
                throw new RuntimeException("Index Full. Cannot resize the hash table to size: " + newTableSize +
        ...
        for (int i = newBlockId; i < tableSize; i += indexDiff)
        {
            hashTable.setBlockAddress(i, newBlockOffset);
        }
    }
    ...
    public HashTableBuckets getDataBuffer()
    {
        return dataBuffer;
    }
