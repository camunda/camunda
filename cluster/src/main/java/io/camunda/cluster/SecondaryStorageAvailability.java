...
/**
 * Reports the availability of a physical tenant's secondary storage, consulted for request-time
 ...
 * otherwise unusable. {@link #isAvailable(String)} is {@code true} exactly when the tenant is not
 * degraded.
 *
 ...
 *
 * <p>Implementations currently derive availability purely from schema-initialization state. Richer
 ...
 */
@NullMarked
public interface SecondaryStorageAvailability {

  /**
   * Reports every physical tenant's secondary storage as available. Used when there is no
   * degradation signal to consult (e.g. {@code database.type=none}) and in tests.
   */
  SecondaryStorageAvailability ALWAYS_AVAILABLE =
   ...
   * @return {@code true} if the physical tenant's secondary storage is available (not degraded);
   *     {@code false} for a degraded physical tenant, or for an unknown physical tenant id (defense
   *     in depth — unknown tenant ids are already rejected upstream by the security chain)
   */
  boolean isAvailable(String physicalTenantId);

  /**
   * @return {@code true} if at least one known physical tenant's secondary storage is available.
   *     Used to decide node readiness: the node stays ready as long as it can serve at least one
   *     physical tenant.
   */
