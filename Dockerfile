...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    ...
    apt-get -qq update && \
    apt-get install -y --no-install-recommends tini=0.19.0-1 ca-certificates gnupg wget && \
...
COPY --from=init --chown=1000:0 /zeebe/gosu ${ZB_HOME}/bin/
...
ENTRYPOINT ["gosu", "zeebe:zeebe", "tini", "--", "/usr/local/bin/startup.sh"]
