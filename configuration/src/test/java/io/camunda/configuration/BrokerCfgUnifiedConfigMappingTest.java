...
class BrokerCfgUnifiedConfigMappingTest {
  ...
  /**
   * Properties that are deliberately not mapped from unified configuration, with the rationale for
   * each. Every entry here must be justified below; this set exists so that a future property added
   * to {@link BrokerCfg} still fails this test by default, forcing a conscious decision instead of
   * a silent gap (see https://github.com/camunda/camunda/issues/50097).
   */
  private static final Set<String> INTENTIONALLY_UNMAPPED_PROPERTIES =
      Set.of(
          ...
          "cluster.nodeVersion",
          // Defined but not consumed anywhere in the broker (no code reads
          // ExperimentalCfg#isContinuousBackups()). Exposing a unified property for a flag that
          // currently has no effect would be misleading; revisit once the continuous-backup feature
          // this was reserved for is actually implemented.
          "experimental.continuousBackups",
          // Deprecated since 8.2.0 for removal (see ExperimentalCfg#isDisableExplicitRaftFlush),
          // superseded by the already-mapped camunda.cluster.raft.flush.*.
          "experimental.disableExplicitRaftFlush",
          // The embedded gateway's cluster/membership configuration (EmbeddedGatewayCfg#getCluster)
          // is derived from the broker's own network/cluster settings in
          // EmbeddedGatewayCfg#init(BrokerCfg, String) so that the embedded gateway always joins
          // the
          // broker's own atomix cluster. Adding independent unified-config keys for it would let
          // the
          // embedded gateway's view of the cluster diverge from the broker's own, which is not a
          // supported configuration. The legacy zeebe.broker.gateway.cluster.* properties remain
          // for
          // advanced/back-compat overrides only.
          "gateway.cluster.advertisedHost",
          "gateway.cluster.advertisedPort",
          "gateway.cluster.clusterName",
          "gateway.cluster.host",
          "gateway.cluster.initialContactPoints",
          "gateway.cluster.memberId",
          "gateway.cluster.membership.broadcastDisputes",
          "gateway.cluster.membership.broadcastUpdates",
          "gateway.cluster.membership.failureTimeout",
          "gateway.cluster.membership.gossipFanout",
          "gateway.cluster.membership.gossipInterval",
          "gateway.cluster.membership.notifySuspect",
          "gateway.cluster.membership.probeInterval",
          "gateway.cluster.membership.probeTimeout",
          "gateway.cluster.membership.suspectProbes",
          "gateway.cluster.membership.syncInterval",
  ...
  void shouldHaveUnifiedConfigMappingForAllBrokerCfgProperties() throws IOException {
    ...
    // Remove properties that are deliberately not mapped (see the rationale documented on
    // INTENTIONALLY_UNMAPPED_PROPERTIES)
    unmappedProperties.removeAll(INTENTIONALLY_UNMAPPED_PROPERTIES);
   ...
   * @param visited Classes on the current recursion path (to avoid infinite recursion on
   *     self-referential types). Scoped to the current path rather than the whole tree: the same
   *     class can legitimately appear at multiple, unrelated property paths (e.g. {@code LimitCfg}
   *     backs both {@code backpressure} and {@code flowControl.request}), and a tree-wide memo
   *     would silently and non-deterministically drop whichever path's properties lost the race.
   */
