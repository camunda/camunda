...
public class ClusterGatewayMembershipTest {
  @Nested
  @TestPropertySource(
      properties = {
  ...
  class WithOnlyUnifiedConfigSet {
    ...
    void shouldSetMembershipFromNew() {
      assertThat(gatewayCfg.getCluster().getMembership())
  ...
  @TestPropertySource(
      properties = {
        ...
        "zeebe.broker.cluster.membership.gossipInterval=501ms",
        "zeebe.broker.cluster.membership.gossipFanout=5",
        "zeebe.broker.cluster.membership.probeInterval=2001ms",
        "zeebe.broker.cluster.membership.probeTimeout=201ms",
        "zeebe.broker.cluster.membership.suspectProbes=6",
        "zeebe.broker.cluster.membership.failureTimeout=20001ms",
        "zeebe.broker.cluster.membership.syncInterval=21001ms"
      })
  class WithOnlyLegacyBrokerMembershipSet {
    final GatewayBasedProperties gatewayCfg;

    WithOnlyLegacyBrokerMembershipSet(@Autowired final GatewayBasedProperties gatewayCfg) {
      this.gatewayCfg = gatewayCfg;
    }

    @Test
    void shouldNotSetFromLegacyBrokerMembership() {
      assertThat(gatewayCfg.getCluster().getMembership())
  ...
  @TestPropertySource(
      properties = {
        ...
        "zeebe.gateway.cluster.membership.gossipInterval=502ms",
        "zeebe.gateway.cluster.membership.gossipFanout=7",
        "zeebe.gateway.cluster.membership.probeInterval=2002ms",
        "zeebe.gateway.cluster.membership.probeTimeout=202ms",
        "zeebe.gateway.cluster.membership.suspectProbes=8",
        "zeebe.gateway.cluster.membership.failureTimeout=20002ms",
        "zeebe.gateway.cluster.membership.syncInterval=21002ms"
      })
  class WithOnlyLegacyGatewayMembershipSet {
    final GatewayBasedProperties gatewayCfg;

    WithOnlyLegacyGatewayMembershipSet(@Autowired final GatewayBasedProperties gatewayCfg) {
      this.gatewayCfg = gatewayCfg;
    }

    @Test
    void shouldSetFromLegacyGatewayMembership() {
      assertThat(gatewayCfg.getCluster().getMembership())
          .returns(true, MembershipCfg::isBroadcastUpdates)
          .returns(false, MembershipCfg::isBroadcastDisputes)
          .returns(true, MembershipCfg::isNotifySuspect)
