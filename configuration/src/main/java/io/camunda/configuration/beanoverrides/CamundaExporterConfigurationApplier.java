...
public final class CamundaExporterConfigurationApplier {
  ...
  public static void applyConnect(
    ...
    getDocumentBasedDatabase(unifiedConfiguration)
        .ifPresent(
            source -> {
    ...
    getDocumentBasedDatabase(unifiedConfiguration)
        .ifPresent(
            source -> {
              ...
              target.setWaitPeriodBeforeArchiving(
                  source.getHistory().getWaitPeriodBeforeArchiving());
        ...
        .ifPresent(
            source -> {
              ...
              target.setMemoryLimit(
                  Math.toIntExact(source.getBulk().getMemoryLimit().toMegabytes()));
            });
  }
    ...
    getDocumentBasedDatabase(unifiedConfiguration)
        .ifPresent(
            source -> {
              ...
              exporterConfiguration
                  .getProcessCache()
                  .setMaxCacheSize(source.getProcessCache().getMaxSize());
              exporterConfiguration
                  .getDecisionRequirementsCache()
                  .setMaxCacheSize(source.getDecisionRequirementsCache().getMaxSize());
              exporterConfiguration
                  .getFormCache()
                  .setMaxCacheSize(source.getFormCache().getMaxSize());
            });
  ...
  private static Optional<DocumentBasedSecondaryStorageDatabase> getDocumentBasedDatabase(
      final UnifiedConfiguration unifiedConfiguration) {
