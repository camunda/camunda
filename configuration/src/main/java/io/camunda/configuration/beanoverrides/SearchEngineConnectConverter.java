...
/** Converts a {@link Camunda} configuration into a {@link SearchEngineConnectProperties}. */
public class SearchEngineConnectConverter {

  private static final Logger LOGGER = LoggerFactory.getLogger(SearchEngineConnectConverter.class);

  public static SearchEngineConnectProperties convert(final Camunda camunda) {
    ...
    populateFromSecurity(camunda, override);
    populateFromInterceptorPlugins(camunda, override);
  ...
  private static void populateFromDocumentBasedSecondaryStorageDatabase(
  ...
  private static void populateFromSecurity(
      final Camunda camunda, final SearchEngineConnectProperties override) {
    final SecondaryStorageSecurity security = resolveSecurity(camunda, override);
    if (security == null) {
      return;
    }
  ...
  private static SecondaryStorageSecurity resolveSecurity(
      final Camunda camunda, final SearchEngineConnectProperties override) {
    final SecondaryStorage secondaryStorage = camunda.getData().getSecondaryStorage();
  ...
  private static void populateFromInterceptorPlugins(
      final Camunda camunda, final SearchEngineConnectProperties override) {
    final List<InterceptorPlugin> interceptorPlugins = resolveInterceptorPlugin(camunda, override);
  ...
  private static List<InterceptorPlugin> resolveInterceptorPlugin(
      final Camunda camunda, final SearchEngineConnectProperties override) {
    final SecondaryStorage secondaryStorage = camunda.getData().getSecondaryStorage();
