...
import io.camunda.configuration.processor.CamundaLegacyPropertiesMapper.MappedPropertySource.Fail;
import io.camunda.configuration.processor.CamundaLegacyPropertiesMapper.MappedPropertySource.Success;
...
public class CamundaLegacyPropertiesMapper {
  private final DeferredLog log;
  private final List<CamundaLegacyPropertiesMapping> mappings;

  public CamundaLegacyPropertiesMapper(
      final DeferredLog log, final List<CamundaLegacyPropertiesMapping> mappings) {
    this.log = log;
    this.mappings = mappings;
  }

  public void mapProperties(final ConfigurableEnvironment environment) {
    final Set<String> profiles =
        Arrays.stream(environment.getActiveProfiles()).collect(Collectors.toSet());
    log.debug(String.format("Detected profiles: %s", String.join(", ", profiles)));
  ...
  private Stream<? extends MappedPropertySource> detectPropertyValue(
    ...
    if (property.newProperty().endsWith("*")) {
      return detectWildcardProperties(environment, property, profiles, mode);
    }
    ...
    for (final Set<LegacyProperty> validPropertyNames : appliedLegacyProperties) {
      if (!haveMatchingValues(
        ...
        final Set<String> validPropertyNameSet =
            findPropertiesWithValue(validPropertyNames, environment).stream()
        ...
        return validPropertyNameSet.stream()
            .map(legacyProperty -> new Fail(legacyProperty, errorMessage));
      }
                  ...
                  legacyProperty ->
                      new Fail(
                          ...
                          notSupportedErrorMessage(
                              property.newProperty(), legacyPropertiesWithValue)));
        }
        log.warn(
            String.format(
  ...
  private static String notSupportedErrorMessage(
      final String newProperty, final Set<LegacyProperty> legacyProperties) {
    return String.format(
        "The following legacy configuration properties are no longer supported and must be removed in favor of '%s': %s",
        newProperty,
        String.join(
            ", ", legacyProperties.stream().map(LegacyProperty::name).collect(Collectors.toSet())));
  }
          ...
          .filter(
              p ->
                  normalizedPropertyNames.keySet().stream()
                      .anyMatch(
                          npn ->
                              npn.startsWith(normalize(composePrefix(p.name()), false) + ".")
                                  || npn.startsWith(
                                      normalize(composePrefix(p.name()), false) + "[")))
          .map(
              p ->
                  switch (mode) {
                    case supported, supportedOnlyIfValuesMatch ->
                        new Success(
                            p.name(),
                            normalizedPropertyNames.keySet().stream()
                                .filter(
                                    np -> np.startsWith(normalize(composePrefix(p.name()), false)))
                                .filter(
                                    np ->
                                        !environment.containsProperty(
                                            normalize(
                                                composeNewPropertyName(
                                                    p, np, property.newProperty()),
                                                true)))
                                .filter(
                                    np ->
                    ...
                    case notSupported ->
                        new Fail(
                            p.name(),
                            notSupportedErrorMessage(property.newProperty(), containedProperties));
                  });
    }
    return Stream.of();
  }
  ...
  sealed interface MappedPropertySource {
    ...
    record Success(String sourceName, Map<String, Object> properties, boolean skip)
