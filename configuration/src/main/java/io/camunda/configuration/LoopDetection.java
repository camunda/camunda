...
public class LoopDetection {
   ...
   * element type (e.g. {@code SERVICE_TASK}) and applies to each child activation. The surrounding
   * {@code MULTI_INSTANCE_BODY} is a distinct element type with its own threshold (the global
   * default unless overridden). A pre-spawn batch check on parallel multi-instance bodies resolves
   * its threshold from {@code MULTI_INSTANCE_BODY}, so setting only {@code SERVICE_TASK=0} does not
   * disable that check — a large collection can still be caught via the {@code MULTI_INSTANCE_BODY}
   * threshold. To fully disable detection for such a task, also set {@code MULTI_INSTANCE_BODY=0}.
   */
