...
class ResourceAccessChecksTest {
  ...
  @Nested
  class GetAuthorizedResourceIds {

    @Test
    void shouldReturnEmptyListWhenAuthorizationDisabled() {
      // given
      final var checks = ResourceAccessChecks.disabled();

      // when
      final var result = checks.getAuthorizedResourceIds();
    ...
    @Test
    void shouldReturnEmptyListWhenNoResourceIdsProvided() {
      // given
      final var authorization = Authorization.of(b -> b.processDefinition().readUserTask());
      final var checks =
          ResourceAccessChecks.of(
    ...
    @Test
    void shouldReturnEmptyListWhenNullAuthorizationCondition() {
      // given
      final var checks =
          ResourceAccessChecks.of(
              AuthorizationCheck.enabled((AuthorizationCondition) null), TenantCheck.disabled());
    ...
    @Test
    void shouldReturnResourceIdsForSingleAuthorization() {
      // given
      final var authorization =
          Authorization.of(
              builder ->
                  builder
                      .processDefinition()
                      .readUserTask()
                      .resourceIds(List.of("pd-1", "pd-2", "pd-3")));
      final var checks =
          ResourceAccessChecks.of(
              AuthorizationCheck.enabled(authorization), TenantCheck.disabled());
      ...
      // then
      assertThat(result).containsExactly("pd-1", "pd-2", "pd-3");
    }

    @Test
    void shouldReturnOnlyFirstAuthorizationResourceIdsForCompositeCondition() {
      // given - composite condition with multiple authorizations
      final var first =
          Authorization.of(
              builder ->
                  builder.processDefinition().readUserTask().resourceIds(List.of("pd-1", "pd-2")));
      ...
      // then - only first authorization's IDs are returned
      assertThat(result).containsExactly("pd-1", "pd-2");
    }

    @Test
    void shouldReturnEmptyListWhenFirstAuthorizationHasNoIds() {
      // given - first authorization without IDs, second with IDs
      final var withoutIds =
          Authorization.of(builder -> builder.processDefinition().readUserTask());
      final var withIds =
          Authorization.of(
              builder -> builder.userTask().read().resourceIds(List.of("ut-1", "ut-2")));
      ...
      // then - empty because first authorization has no IDs
      assertThat(result).isEmpty();
    }
  }
