...
class UpdateTest {
  ...
  private void updateZeebe(final UpdateTestCase testCase, final boolean withSnapshot) {
    ...
    if (withSnapshot) {
      ...
      EitherAssert.assertThat(forceTakeSnapshot())
  ...
  // to ensure we will take a snapshot, we publish a dummy message so something is processed
  private Either<Throwable, Map<String, PartitionStatus>> forceTakeSnapshot() {
    state
        .client()
        .newPublishMessageCommand()
        .messageName("takeSnapshot")
        .correlationKey("ignored")
        .send()
        .join(5, TimeUnit.SECONDS);

    return state.getPartitionsActuatorClient().takeSnapshot();
  }
  ...
  private void assertSnapshotAvailable() {
    ...
    assertThat(partitionStatus)
        .as("partitions query did not return status for partition 1")
        .isNotNull();
    assertThat(partitionStatus.snapshotId)
        .as("partitions 1 should have a snapshot ID, but was %s", partitionStatus)
        .isNotBlank();
  }
