...
public class OidcTokenEndpointCustomizer
  ...
  public void customize(final OAuth2LoginConfigurer<HttpSecurity>.TokenEndpointConfig config) {
    final boolean hasKeystore = isAssertionKeystoreConfigured();
    final boolean hasResource = isResourceConfigured();

    // Only replace the default token response client when there are custom parameters to add.
    // The default client properly handles standard OIDC token responses including id_token
    // parsing. Replacing it unnecessarily can break additionalParameters handling.
    if (!hasKeystore && !hasResource) {
      return;
    }
    ...
    tokenResponseClient.addParametersConverter(createResourceParameterConverter());
  ...
  private boolean isResourceConfigured() {
    return oidcAuthenticationConfigurationRepository
        .getOidcAuthenticationConfigurations()
        .values()
        .stream()
        .anyMatch(c -> c.getResource() != null && !c.getResource().isEmpty());
  }

  private boolean isAssertionKeystoreConfigured() {
    return oidcAuthenticationConfigurationRepository
        .getOidcAuthenticationConfigurations()
        .values()
        .stream()
        .anyMatch(
            c -> {
              final var keystore = c.getAssertion().getKeystore();
              return keystore.getPath() != null && !keystore.getPath().isBlank();
            });
  }
