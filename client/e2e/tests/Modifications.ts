...
(IS_MODIFICATION_MODE_ENABLED ? test : test.skip)(
  'Should apply/remove add variable modifications',
  async (t) => {
    await t.expect(screen.queryByTestId('foo').exists).ok();
    await t.click(
      screen.getByRole('button', {
    ...
    await t
      .expect(
        ProcessInstancePage.getNewVariableNameFieldValue('newVariables[0]')
      )
      .eql('test2-edited');

    await t
      .expect(
        ProcessInstancePage.getNewVariableNameFieldValue('newVariables[1]')
      )
      .eql('test3');
