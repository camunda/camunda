...
const InstancesContainer = observer(
  class InstancesContainer extends Component {
      ...
      this.subscriptions = {
        ...
        REFRESH_AFTER_OPERATION: ({state, response, error}) => {
          if (state === LOADING_STATE.LOADED) {
            ...
            this.setState({
              workflowInstances:
                response[LOAD_LIST_INSTANCES].workflowInstances,
    ...
    fetchNewData(filter, prevState) {
      const {groupedWorkflows} = this.state;
      const {diagramModel} = instancesDiagram.state;
      ...
      if (didWorkflowChange && isEmpty(filter.workflow)) {
        instancesDiagram.resetDiagramModel();
      }
    ...
    fetchWorkflowXML(groupedWorkflows, workflow, version) {
      ...
      const workflowId = getWorkflowByVersion(
        groupedWorkflows[workflow],
        version
      ).id;

      instancesDiagram.fetchWorkflowXml(workflowId);
      this.setState({statistics: []}, this.fetchStatistics());
    }
    ...
    sanitizeFilter = (filter, groupedWorkflows) => {
      // (1) empty filter
      if (!filter) {
