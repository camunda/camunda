...
import OldTypeahead from './OldTypeahead';
import {Dropdown, Input} from 'components';

import {shallow} from 'enzyme';

it('should display available options if dropdown is open', () => {
  const node = shallow(<OldTypeahead values={['foo', 'bar']} />);
...
it('should only display entries that match the Typeahead value', () => {
  const node = shallow(<OldTypeahead values={['varFoo', 'varBar', 'varFoobar']} />);
...
it('should only display entries that match the OldTypeahead value, even if there is a formatter', () => {
  const node = shallow(
    <OldTypeahead values={['varFoo', 'varBar', 'varFoobar']} formatter={v => ({text: v + v})} />
  );
...
it('should call the provided onSelect method when a selection is done', () => {
  const spy = jest.fn();
  const node = shallow(<OldTypeahead values={['foo', 'bar']} onSelect={spy} />);
...
it('should reset the query to the latest committed state when the input field blurs', () => {
  ...
  node.setState({
    ...
    query: 'I typed something in the OldTypeahead'
  });
...
it('should disable the input field when disabled prop is set to true', () => {
  const node = shallow(<OldTypeahead values={['foo', 'bar']} disabled={true} />);
...
it('should show a no results message if no values are provided', () => {
  const node = shallow(<OldTypeahead values={[]} noValuesMessage="No reports have been created" />);
...
it('should show the initial value if provided on mount', () => {
  const node = shallow(<OldTypeahead initialValue="foo" values={['bar']} />);

  expect(node.find(Input)).toHaveValue('foo');
});

it('should show no results options if there are not results', () => {
  const node = shallow(<OldTypeahead values={['a']} />);
...
it('should show an info message if there are more result than the shown', async () => {
  const node = shallow(<OldTypeahead values={() => ({result: ['a'], total: 25})} />);
...
it('should call getValue to filter and render the data when available', async () => {
  ...
  const node = shallow(<OldTypeahead values={spy} />);
