...
it('should use the provided type for the ChartRenderer', () => {
  ...
  shallow(<Chart {...props} data={{foo: 123}} type="visualization_type" />);
...
it('should use the special targetLine type when target values are enabled on a line chart', () => {
  ...
  shallow(
    <Chart
      ...
      data={{foo: 123}}
      type="line"
      targetValue={{active: true, countChart: {isBelow: true, value: 1}}}
    />
  );
...
it('should change type for the ChartRenderer if props were updated', () => {
  ...
  const chart = shallow(<Chart {...props} data={{foo: 123}} type="visualization_type" />);
