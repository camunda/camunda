...
func TestCamundaCmdWithKeystoreSettings(t *testing.T) {
	...
	expectedJavaOpts := "JAVA_OPTS= -Dserver.ssl.keystore=file:" + settings.keystore + " -Dserver.ssl.enabled=true" + " -Dserver.ssl.key-password=" + settings.keystorePassword + " -Dspring.profiles.active=operate,tasklist,broker,identity,consolidated-auth -Dcamunda.security.authentication.method=basic"
