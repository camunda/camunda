...
func TestCamundaCmdWithKeystoreSettings(t *testing.T) {
	...
	expectedJavaOpts := "JAVA_OPTS= -Dserver.ssl.keystore=file:" + settings.Keystore + " -Dserver.ssl.enabled=true" + " -Dserver.ssl.key-password=" + settings.KeystorePassword + " -Dcamunda.security.authentication.unprotected-api=true" + " -Dspring.profiles.active=operate,tasklist,broker,identity,consolidated-auth"
