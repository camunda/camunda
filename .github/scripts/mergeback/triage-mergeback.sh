...
#   source_missing     - a commit touching this path is on RELEASE but NOT on TARGET (git cherry
#                        patch-id '+'). THE ALARM: may be missing from target.
#   version_build_gap  - a version/build file carrying a REAL missing dep change (also actionable).
#   source_drift       - release-side changes already on target by patch-id. Text drift only.
#
...
# Emit {sha,author,subject} for <sha>. NUL-delimited git fields split on NUL, so a '|' (or any
# char) in author/subject can't mis-split; the trailing %x00 absorbs git's end-of-entry newline.
commit_detail_json() {
  git log -1 --format='%h%x00%an%x00%s%x00' "$1" 2>/dev/null \
    | jq -Rsc 'split("\u0000") | {sha: .[0], author: .[1], subject: .[2]}'
}
...
for p in "${paths[@]}"; do
  ...
  if is_version_build "$p"; then
    ...
    while IFS= read -r sha; do
      ...
      if commit_touches "$sha" "$p"; then
        vb_hits+="$(commit_detail_json "$sha")"$'\n'
      fi
    done <<< "$missing_shas"
    if [ -n "$vb_hits" ]; then
      version_build_gap+=("$p")
  ...
  # Source path: find missing-on-target commits touching it.
  hits=""
  while IFS= read -r sha; do
    [ -z "$sha" ] && continue
    if commit_touches "$sha" "$p"; then
      hits+="$(commit_detail_json "$sha")"$'\n'
    fi
  done <<< "$missing_shas"
  if [ -n "$hits" ]; then
    source_missing+=("$p")
