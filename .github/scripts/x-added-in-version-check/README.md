...
The authoritative version map and endpoint map are generated by
`build-bundler-version-map.mjs`, vendored in this directory from
[camunda/return-of-api-added-in-analysis](https://github.com/camunda/return-of-api-added-in-analysis)
(see git history for provenance). This directory consumes both and reports any
drift.
...
```
build-bundler-version-map.mjs # vendored from return-of-api-added-in-analysis; computes
                               # the version map by diffing property/operation presence
                               # across released baselines + the in-development version
build-artefacts.mjs       # runs build-bundler-version-map.mjs and copies its
...
| `LATEST_BRANCH`     | _(none)_                        | Forwarded to `build-bundler-version-map.mjs`; identifies the in-development ref of `camunda/camunda` to bundle as the next (unreleased) version. Accepts a branch name or a commit SHA. CI sets it to the PR head SHA (PR check) or the resolved branch ref (scheduled run) |
