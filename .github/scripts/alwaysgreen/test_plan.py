...
# ---------------------------------------------------------------------------
# How long a no-fix verdict binds
# ---------------------------------------------------------------------------

#: Distinct from the `NOW` the PR-lock section defines further down: both are
#: module-level, so a shared name would silently hand this section that clock.
VERDICT_NOW = datetime(2026, 9, 1, 12, 0, tzinfo=timezone.utc)
SETTLED = {"prs": [], "product_bugs": [{"issue_number": 55864}]}
UNSETTLED = {"category": "not-determined", "prs": []}


def _binds(meta, created, **kw):
    return plan.verdict_still_binds(
        meta,
        created,
        VERDICT_NOW,
        settled_days=kw.get("settled_days", 7),
        unsettled_hours=kw.get("unsettled_hours", 6),
    )


def test_a_filed_issue_settles_the_verdict():
    assert plan.verdict_is_settled(SETTLED)
    assert not plan.verdict_is_settled(UNSETTLED)
    assert not plan.verdict_is_settled({"prs": [], "product_bugs": []})
    assert not plan.verdict_is_settled(None)


def test_category_alone_does_not_settle_a_verdict():
    # `category` is free text the agent writes; a rephrasing must not silently
    # change how long a verdict binds. Only a filed issue is structural.
    assert not plan.verdict_is_settled({"prs": [], "category": "product-bug"})


def test_unsettled_verdict_expires_in_hours():
    # Run 33464059381: the SaaS Modeler returned 500 on /api/internal/login, the
    # agent filed nothing because the fault was another team's service, and both
    # smoke tests stayed suppressed for seven days after the outage cleared.
    assert _binds(UNSETTLED, "2026-09-01T09:00:00Z")
    assert not _binds(UNSETTLED, "2026-09-01T02:50:00Z")


def test_settled_verdict_keeps_the_long_window():
    assert _binds(SETTLED, "2026-08-28T02:50:00Z")
    assert not _binds(SETTLED, "2026-08-20T02:50:00Z")


def test_missing_or_unparseable_timestamp_keeps_the_verdict():
    # Matches the `ok` bias in discover's key lookups: an unproven state
    # suppresses rather than risking a duplicate agent on the same failure.
    for created in (None, "", "   ", "not-a-date"):
        assert _binds(UNSETTLED, created), created


def test_a_zero_window_disables_that_half_of_the_rule():
    assert not _binds(UNSETTLED, "2026-09-01T11:59:00Z", unsettled_hours=0)
    assert not _binds(SETTLED, "2026-09-01T11:59:00Z", settled_days=0)
    # The other branch is unaffected by the disabled one.
    assert _binds(SETTLED, "2026-09-01T11:59:00Z", unsettled_hours=0)


def test_naive_now_does_not_raise_against_an_offset_aware_timestamp():
    assert plan.verdict_still_binds(
        UNSETTLED,
