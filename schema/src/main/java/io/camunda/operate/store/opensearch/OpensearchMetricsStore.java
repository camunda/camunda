...
public class OpensearchMetricsStore implements MetricsStore {
  ...
  private AggregationResult searchWithAggregation(SearchRequest.Builder requestBuilder, String aggregationName) {
    ...
    try {
      aggregate = richOpenSearchClient.doc().search(requestBuilder, Object.class).aggregations().get(aggregationName);
    ...
    final List<RichOpenSearchClient.AggregationValue> values = buckets.stream()
      .map(bucket -> new RichOpenSearchClient.AggregationValue(bucket.key(), bucket.docCount()))
      .toList();
  ...
  public Long retrieveProcessInstanceCount(OffsetDateTime startTime, OffsetDateTime endTime) {
    ...
    var searchRequestBuilder = searchRequestBuilder(metricIndex.getFullQualifiedName())
      .query(
        and(
      ...
      .aggregations(PROCESS_INSTANCES_AGG_NAME, termAggregation(VALUE, limit)._toAggregation());
  ...
  public Long retrieveDecisionInstanceCount(final OffsetDateTime startTime,
    ...
    var searchRequestBuilder = searchRequestBuilder(metricIndex.getFullQualifiedName())
      ...
      .aggregations(DECISION_INSTANCES_AGG_NAME, termAggregation(VALUE, limit)._toAggregation());
