...
public class OpenSearchPipelineOperations extends OpenSearchRetryOperation {
  ...
  public boolean addPipelineWithRetries(String name, String definition) {
    try {
      var processorsJSONNodes = objectMapper.readTree(new StringReader(definition)).get("processors");
      ...
      return executeWithRetries("AddPipeline " + name,
            () -> openSearchClient.ingest().putPipeline(p -> p.id(name).processors(processors)).acknowledged());
    } catch (Exception e) {
  ...
  private Processor readProcessorFromJSON(String processorText) {
    try (JsonParser jsonParser = JsonProvider.provider().createParser(new StringReader(processorText))) {
