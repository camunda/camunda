...
public class RunnableOrderingTests
{
    ...
    {
        // given
        final ConcurrentQueueChannel<Object> ch = new ConcurrentQueueChannel<>(new ConcurrentLinkedQueue<>());
        ch.add(new Object());
        ch.add(new Object());
            ...
            protected void onActorStarted()
            {
                actor.run(() ->
                {
                    actor.consume(ch, () ->
                    {
                        ch.poll();
                        actions.add(THREE);
                        actor.run(runnable(FOUR)); // this is done before the consumer fired for the second time
                    });
                    actions.add(ONE);
                });
                actor.run(runnable(TWO));
    ...
    public void conditionTest()
    {
        ...
        final ActionRecordingActor actor = new ActionRecordingActor()
        {
            ...
            protected void onActorStarted()
            {
                actor.run(() ->
                {
                    final ActorCondition condition = actor.onCondition("cond", () ->
                    {
                        actions.add(THREE);
                        actor.run(runnable(FOUR)); // this is done before the condition is fired for the second time
                    });
    ...
    public void timerTest()
    {
        ...
        final ActionRecordingActor actor = new ActionRecordingActor()
        {
            ...
            protected void onActorStarted()
            {
                actor.run(() ->
                {
                    actor.runAtFixedRate(Duration.ofMillis(10), () ->
                    {
                        actions.add(THREE);
                        actor.run(runnable(FOUR)); // this is done before the timer is fired for the second time
                    });
                    actions.add(ONE);
                });
                actor.run(runnable(TWO));
