...
/**
 * Logic and state for the priority scheduling. Each {@link ActorThread}
 * maintains a local instance if this class.
 */
public class PriorityScheduler implements TaskScheduler
{
    private static final int TIME_SLICES_PER_SECOND = 100;
    private static final long TIME_SLICE_LENTH_NS = TimeUnit.MILLISECONDS.toNanos(1000 / TIME_SLICES_PER_SECOND);
    ...
    /**
     * @param getTaskFn
     *            function which can be used to get a task by a given priority
     *            class.
     * @param quotas
     *            the quotas by priority class. Must be an array of doubles,
     *            assigning a quota to the priority denoted by the index in the
     *            array (if quotas[0] = 0.3, then the quota for the priority
     *            class '0' is 0.3). A quota must be a number between [0..1],
     *            such that the sum of all quotas in the array is exactly 1. The
     *            quota multiplied by 100 corresponds to the number of time
    ...
    /**
     * calculates and returns the next task to execute or null if no such task
     * can be determined.
     */
    @Override
