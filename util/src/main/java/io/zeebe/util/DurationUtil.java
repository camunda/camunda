...
{
    /**
     * Input format expected to be [value][unit], where:
     *      - value is a number {@link java.lang.Float#parseFloat}
    ...
    public static Duration parse(String durationString)
    {
        ...
        final String matchedValue = durationString.substring(0, durationString.length() - matchedUnit.length());
        ...
        switch (matchedUnit)
        {
            ...
            case "h":
                ...
                break;
            default:
                final String errorMessage = String.format("unknown unit %s; must be one of: ms, s, m, h", matchedUnit);
