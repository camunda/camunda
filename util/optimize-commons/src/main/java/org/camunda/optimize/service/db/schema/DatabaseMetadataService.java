...
public abstract class DatabaseMetadataService<CLIENT extends DatabaseClient> {
  ...
  public void initMetadataIfMissing(final CLIENT dbClient) {
    ...
    upsertMetadataWithScript(
        ...
        currentOptimizeProfile
            .map(
                profile ->
                    createInitInstallationIdAndProfileScriptIfMissing(newInstallationId, profile))
  ...
  protected ScriptData createInitInstallationIdAndProfileScriptIfMissing(
      final String newInstallationId, final OptimizeProfile currentOptimizeProfile) {
  ...
  protected ScriptData generateUpdateScript(
      String newInstallationId, String newSchemaVersion, OptimizeProfile newOptimizeProfile) {
    ...
    if (newOptimizeProfile != null) {
      ...
      scriptString +=
          "ctx._source."
              + MetadataDto.Fields.optimizeProfile.name()
              + " = params.newOptimizeProfile;\n";
    }
  ...
  private ScriptData generateUpdateScript(
      String newInstallationId, OptimizeProfile newOptimizeProfile) {
  ...
  private void validateConfiguredOptimizeProfile(final CLIENT dbClient) {
    getCurrentOptimizeProfile()
        .ifPresentOrElse(
            currentOptimizeProfile ->
                getPersistedOptimizeProfile((dbClient))
                    .filter(
                        persistedOptProfile -> !persistedOptProfile.equals(currentOptimizeProfile))
                    .ifPresent(
                        persistedOptProfile -> {
                          final String errorMessage =
                              String.format(
                                  ...
                                  persistedOptProfile, currentOptimizeProfile);
  ...
  private Optional<OptimizeProfile> getCurrentOptimizeProfile() {
    return Optional.ofNullable(environment)
        .map(env -> ConfigurationService.getOptimizeProfile(environment));
  }
}
