...
import static io.camunda.optimize.service.db.DatabaseConstants.*
    ...
    void upgradeAndVerifySchemaIntegrityTest() {
        ...
        try {
            ...
            new File(getOptimizeUpdateLogPath()).eachLine { line ->
                ...
                // we also ignore warns about legacy index templates until we've upgraded the ES client
                def matcherWarn = line =~ /WARN(?!.*snapshot_in_progress_exception)(?!.*Legacy index templates are deprecated)/
