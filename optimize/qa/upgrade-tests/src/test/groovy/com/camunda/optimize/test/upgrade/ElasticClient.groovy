...
class ElasticClient {
    ...
    public static final String DEFAULT_OPTIMIZE_INDEX_PATTERN =
            "${OPTIMIZE_INDEX_PREFIX}-*,-${OPTIMIZE_INDEX_PREFIX}-${DatabaseConstants.UPDATE_LOG_ENTRY_INDEX_NAME}*"

    String name
    RestHighLevelClient client;
    Boolean readonlySnapshotRepository

    ElasticClient(String name, int port = 9200, String host = "localhost", Boolean readonlySnapshotRepository = false) {
        this.name = name
    ...
    def deleteSnapshotRepository() {
        log.info("Removing snapshot repository on ${name} Elasticsearch...")
        client.snapshot().deleteRepository(
                new DeleteRepositoryRequest().name(SNAPSHOT_REPOSITORY_NAME),
                RequestOptions.DEFAULT
        )
        log.info("Done removing snapshot repository on ${name} Elasticsearch!")
    }

    def createSnapshotOfOptimizeIndices(Boolean waitForCompletion = true) {
        ...
        createSnapshotRequest.setJsonEntity("{\"indices\":\"optimize-*\",\n\"include_global_state\":true}");
