 ...
 *   --agent-rate   Fraction of instances with an agent instance [0.0–1.0] (default: 0.3)
 * </pre>
 */
  ...
  public static void main(final String[] args) {
    ...
    for (int i = 0; i < args.length; i++) {
      switch (args[i]) {
        ...
        case "--agent-rate" -> agentRate = Double.parseDouble(args[++i]);
  ...
  private static long queryMaxPosition(final ElasticsearchClient esClient, final String prefix) {
    final List<String> indexes =
        List.of(
            ...
            prefix + "-" + ZEEBE_AGENT_INSTANCE_INDEX_NAME);
    long maxPosition = 0L;
