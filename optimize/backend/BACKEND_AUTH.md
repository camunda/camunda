...
When `jwtAuthForApiEnabled` is `true`, Optimize adds `ApiBearerTokenAuthenticationFilter`
before the cookie filter in the Spring Security chain. On every request that carries an
`Authorization: Bearer` header the filter:

1. Validates the JWT signature, expiry, and audience against the configured Keycloak JWK Set URI.
2. On success — places a `JwtAuthenticationToken` in the `SecurityContextHolder`; the cookie
   filter is skipped.
3. On failure — immediately returns `401 Unauthorized`; the request is **not** forwarded to the
   cookie filter (a bad bearer header is never silently ignored).
...
cookie-only authentication. The Bearer header will be silently ignored.
