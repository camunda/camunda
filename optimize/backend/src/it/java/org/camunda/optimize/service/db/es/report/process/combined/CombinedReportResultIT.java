...
public class CombinedReportResultIT extends AbstractProcessDefinitionIT {
  ...
  public void dateReports_staticIntervals_sameResultsAsSingleReportEvaluation(
      final Pair<AggregateByDateUnit, List<SingleProcessReportDefinitionRequestDto>>
          combinableReportsWithUnit) {
    // given
    ...
    final List<String> reportIds =
        combinableReportsWithUnit.getValue().stream()
            .map(reportClient::createSingleProcessReport)
            .toList();
    final List<ReportResultResponseDto<List<MapResultEntryDto>>> singleReportResults =
        reportIds.stream()
    ...
    // then the combined combinedResult evaluation yields the same results as the single report
    // evaluations
  ...
  public void dateReports_automaticInterval_combinedReportsHaveSameBucketRanges(
    ...
    startAndEndProcessInstancesWithGivenRuntime(
        4, Duration.of(60, ChronoUnit.SECONDS), startOfFirstInstance);
    importAllEngineEntitiesFromScratch();
  ...
  public void dateVariableReports_staticIntervals_sameResultsAsSingleReportEvaluation(
    ...
    ProcessReportDataDto reportData1 =
        createDateVariableReport(def.getKey(), def.getVersionAsString());
    ...
    List<SingleProcessReportDefinitionRequestDto> reportDefs =
        Arrays.asList(
            new SingleProcessReportDefinitionRequestDto(reportData1),
            new SingleProcessReportDefinitionRequestDto(reportData2));

    final List<String> reportIds =
        reportDefs.stream().map(reportClient::createSingleProcessReport).toList();
    final List<ReportResultResponseDto<List<MapResultEntryDto>>> singleReportResults =
        reportIds.stream()
            .map(reportId -> reportClient.evaluateMapReportById(reportId).getResult())
            .collect(Collectors.toList());
  ...
  public void correctCombinedInstanceCount_differentProcessDefinitions() {
    ...
    startAndEndProcessInstancesWithGivenRuntime(
        4, mapToChronoUnit(AggregateByDateUnit.DAY).getDuration(), OffsetDateTime.now());

    // and report for second definition (with no instances in it)
    final SingleProcessReportDefinitionRequestDto singleReport2 =
        createReport(PROC_INST_FREQ_GROUP_BY_END_DATE, AggregateByDateUnit.DAY);
    singleReport2.getData().setProcessDefinitionKey("runningInstanceDef");
    ProcessDefinitionEngineDto runningInstanceDef =
    ...
    final SingleProcessReportDefinitionRequestDto singleReport3 =
        createReport(PROC_INST_FREQ_GROUP_BY_START_DATE, AggregateByDateUnit.DAY);
    ...
    // when
    final List<String> reportIds =
        Arrays.asList(singleReport1, singleReport2, singleReport3).stream()
            .map(reportClient::createSingleProcessReport)
            .toList();
    final CombinedProcessReportResultDataDto<?> combinedResult =
        reportClient.saveAndEvaluateCombinedReport(reportIds);
  ...
  public void correctCombinedInstanceCount_sameDefinition_distinctSingleReportInstances() {
    ...
    List<ProcessInstanceEngineDto> instances =
        startAndEndProcessInstancesWithGivenRuntime(
            4, mapToChronoUnit(AggregateByDateUnit.DAY).getDuration(), OffsetDateTime.now());
    engineDatabaseExtension.changeProcessInstanceState(instances.get(0).getId(), SUSPENDED_STATE);
    importAllEngineEntitiesFromScratch();

    // when
    final List<String> reportIds =
        Arrays.asList(singleReport1, singleReport2).stream()
            .map(reportClient::createSingleProcessReport)
            .toList();
    final CombinedProcessReportResultDataDto<?> combinedResult =
        reportClient.saveAndEvaluateCombinedReport(reportIds);
  ...
  private void assertSameBucketKeys(final CombinedProcessReportResultDataDto<?> result) {
    ...
    List<String> bucketKeys1 =
        ((List<MapResultEntryDto>) singleReportResults.get(0).getResult().getFirstMeasureData())
            .stream().map(MapResultEntryDto::getKey).toList();
    List<String> bucketKeys2 =
        ((List<MapResultEntryDto>) singleReportResults.get(1).getResult().getFirstMeasureData())
            .stream().map(MapResultEntryDto::getKey).toList();
    assertThat(bucketKeys1).isEqualTo(bucketKeys2);
  }

  private static Stream<Pair<AggregateByDateUnit, List<SingleProcessReportDefinitionRequestDto>>>
      staticIntervalDateReportCombinationsPerUnit() {
    return staticAggregateByDateUnits()
        .flatMap(
            unit -> {
              final SingleProcessReportDefinitionRequestDto runningDateReport =
                  createReport(PROC_INST_FREQ_GROUP_BY_RUNNING_DATE, unit);

              final SingleProcessReportDefinitionRequestDto startDateReport =
                  createReport(PROC_INST_FREQ_GROUP_BY_START_DATE, unit);

              final SingleProcessReportDefinitionRequestDto endDateReport =
                  createReport(PROC_INST_FREQ_GROUP_BY_END_DATE, unit);
  ...
  private static Stream<List<SingleProcessReportDefinitionRequestDto>>
      automaticIntervalDateReportCombinations() {
    final SingleProcessReportDefinitionRequestDto runningDateReport =
        createReport(PROC_INST_FREQ_GROUP_BY_RUNNING_DATE, AggregateByDateUnit.AUTOMATIC);

    final SingleProcessReportDefinitionRequestDto startDateReport =
        createReport(PROC_INST_FREQ_GROUP_BY_START_DATE, AggregateByDateUnit.AUTOMATIC);

    final SingleProcessReportDefinitionRequestDto endDateReport =
        createReport(PROC_INST_FREQ_GROUP_BY_END_DATE, AggregateByDateUnit.AUTOMATIC);

    final SingleProcessReportDefinitionRequestDto emptyRunningDateReport =
        createReport(
            PROC_INST_FREQ_GROUP_BY_RUNNING_DATE,
            AggregateByDateUnit.AUTOMATIC,
            createSuspendedInstancesOnlyFilter());

    final SingleProcessReportDefinitionRequestDto emptyStartDateReport =
        createReport(
            PROC_INST_FREQ_GROUP_BY_START_DATE,
            AggregateByDateUnit.AUTOMATIC,
            createSuspendedInstancesOnlyFilter());
  ...
  private CombinedProcessReportResultDataDto<?> getCombinedReportResult(
    ...
    return reportClient.saveAndEvaluateCombinedReport(
        reports.stream().map(reportClient::createSingleProcessReport).collect(toList()));
  }

  private ProcessReportDataDto createDateVariableReport(
  ...
  private static SingleProcessReportDefinitionRequestDto createReport(
      final ProcessReportDataType reportDataType, final AggregateByDateUnit unit) {
  ...
  private static SingleProcessReportDefinitionRequestDto createReport(
    ...
    ProcessReportDataDto runningReportData =
