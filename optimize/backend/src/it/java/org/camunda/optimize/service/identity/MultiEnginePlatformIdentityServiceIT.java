...
public class MultiEnginePlatformIdentityServiceIT extends AbstractMultiEngineIT {
  ...
  protected final LogCapturer logCapturer =
      LogCapturer.create().forLevel(Level.WARN).captureForType(PlatformIdentityService.class);
  ...
  public void fetchUserFromConnectedEngineWhenOtherEnginesAreDown() {
            ...
            request()
                .withPath(
                    engineIntegrationExtension.getEnginePath()
                        + createGetUserByIdPath(otherEngineUser)))
    ...
    logCapturer.assertContains(
        String.format(
            "Failed fetching identity with id %s from engine %s.",
            otherEngineUser, DEFAULT_ENGINE_ALIAS));
    // second engine was called to return the user
    secondEngineMock.verify(
        request()
            .withPath(
                secondaryEngineIntegrationExtension.getEnginePath()
                    + createGetUserByIdPath(otherEngineUser)),
        once());
  }
