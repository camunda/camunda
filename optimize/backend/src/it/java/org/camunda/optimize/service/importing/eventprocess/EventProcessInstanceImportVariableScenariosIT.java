...
public class EventProcessInstanceImportVariableScenariosIT extends AbstractEventProcessIT {
  ...
  @Test
  public void
      instancesAreGeneratedWithCorrectVariablesFromCamundaEventImportSource_tracedByBusinessKey() {
    // given
    Map<VariableType, Map<String, Object>> variableTypeToVariableMap =
        createVariableTypeToVariableMap();
    final ProcessInstanceEngineDto processInstanceEngineDto =
        deployAndStartProcessWithVariables(convertToVariables(variableTypeToVariableMap));
  ...
  @Test
  public void
      instancesAreGeneratedWithCorrectVariablesFromCamundaEventImportSource_multipleImportBatches() {
    // given
    Map<VariableType, Map<String, Object>> variableTypeToVariableMap =
        createVariableTypeToVariableMap();
    final ProcessInstanceEngineDto processInstanceEngineDto =
  ...
  @Test
  public void
      instancesAreGeneratedWithCorrectVariablesFromCamundaEventImportSource_tracedByVariable() {
    // given
    Map<VariableType, Map<String, Object>> variableTypeToVariableMap =
        createVariableTypeToVariableMap();
  ...
  public void instancesAreGeneratedWithCorrectVariables_camundaAndExternalEventImportSource() {
    ...
    createAndPublishEventMapping(
        mappingsForEventProcess, Arrays.asList(camundaEventSource, externalEventSource));
        ...
        .satisfies(
            processInstanceDto -> {
              assertProcessInstance(
                  ...
                  processInstanceEngineDto.getBusinessKey(),
                  Arrays.asList(BPMN_START_EVENT_ID, USER_TASK_ID_ONE, BPMN_END_EVENT_ID));
              List<SimpleProcessVariableDto> expectedVariabes =
                  extractExpectedEngineVariables(variableTypeToVariableMap);
              expectedVariabes.add(expectedExternalEventVariable());
              assertThat(processInstanceDto.getVariables())
                  .containsExactlyInAnyOrderElementsOf(expectedVariabes);
            });
  }
  ...
  private SimpleProcessVariableDto expectedExternalEventVariable() {
    return new SimpleProcessVariableDto(
        VARIABLE_ID,
        VARIABLE_ID,
        VARIABLE_VALUE.getClass().getSimpleName(),
        Collections.singletonList(VARIABLE_VALUE),
        1);
  }

  private Map<String, Object> convertToVariables(
      final Map<VariableType, Map<String, Object>> variableTypeToVariableMap) {
    return variableTypeToVariableMap.values().stream()
        .flatMap(map -> map.entrySet().stream())
        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
  }

  private void assertEngineVariables(
  ...
  private void assertVariables(
      ProcessInstanceDto processInstanceDto, List<SimpleProcessVariableDto> expectVariableArray) {
    assertThat(processInstanceDto.getVariables())
        .containsExactlyInAnyOrderElementsOf(expectVariableArray);
  }

  private List<SimpleProcessVariableDto> extractExpectedEngineVariables(
      final Map<VariableType, Map<String, Object>> variableTypeToVariableMap) {
                    ...
                    .map(
                        variableProperties ->
                            new SimpleProcessVariableDto(
                                variableProperties.getKey(),
                                variableProperties.getKey(),
                                entry.getKey().getId(),
                                Collections.singletonList(variableProperties.getValue().toString()),
                                1)))
  ...
  private Map<VariableType, Map<String, Object>> createVariableTypeToVariableMap() {
    ...
    variableTypeToVariableMap.put(
        VariableType.STRING, Maps.newHashMap("string key", "some string value"));
