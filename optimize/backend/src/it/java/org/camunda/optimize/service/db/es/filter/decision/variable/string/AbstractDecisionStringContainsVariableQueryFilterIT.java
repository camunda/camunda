...
public abstract class AbstractDecisionStringContainsVariableQueryFilterIT
    extends AbstractDecisionStringVariableQueryFilterIT {

  @Test
  public void containsFilter_possibleMatchingScenarios() {
    // given
    final DecisionDefinitionEngineDto decisionDefinitionDto =
        deployInputEqualsOutputDecisionDefinition();
    final String exactMatch = "ketchup";
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(exactMatch));
    final String matchesTheBeginning = "ketchupMustard";
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), createInputs(matchesTheBeginning));
    final String matchesTheEnd = "mustardKetchup";
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(matchesTheEnd));
    final String matchesInBetween = "mustardKetchupMayonnaise";
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), createInputs(matchesInBetween));
    final String caseInsensitiveMatch = "(asokndf249814kETchUpa;rinioanbrair01-34";
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), createInputs(caseInsensitiveMatch));
    // no match 1
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs("noMatch"));
    // no match 2
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(null));
    ...
    ReportResultResponseDto<List<RawDataDecisionInstanceDto>> result =
        evaluateReportWithFilter(decisionDefinitionDto, containsFilter);
  ...
  public void containsFilter_matchesNoValue() {
    ...
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(noMatch1));
    final String noMatch2 = "mustard";
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(noMatch2));
    importAllEngineEntitiesFromScratch();

    // when
    final List<DecisionFilterDto<?>> filter = createContainsFilterForValues("ketchup");
    ReportResultResponseDto<List<RawDataDecisionInstanceDto>> result =
        evaluateReportWithFilter(decisionDefinitionDto, filter);
  ...
  public void containsFilter_otherVariableWithMatchingValue() {
    ...
    inputs.put(
        INPUT_SEASON_ID,
        new InputVariableEntry(INPUT_SEASON_ID, "Something", VariableType.STRING, "ketchup"));
  ...
  public void containsFilter_nullValue() {
    ...
    final DecisionDefinitionEngineDto decisionDefinitionDto1 =
        deployInputEqualsOutputDecisionDefinition();
    startDecisionInstanceWithInputVars(decisionDefinitionDto1.getId(), createInputs(null));
    final DecisionDefinitionEngineDto decisionDefinitionDto2 =
        deployInputEqualsOutputDecisionDefinition();
    startDecisionInstanceWithInputVars(decisionDefinitionDto2.getId(), createInputs("ketchup"));
    importAllEngineEntitiesFromScratch();
  ...
  public void containsFilter_combineSeveralValues() {
    ...
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(ketchupMatch));
    final String mustMatch = "mustard";
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(mustMatch));
    final String nullMatch = null;
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs(nullMatch));
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), createInputs("mayonnaise"));
    importAllEngineEntitiesFromScratch();

    // when
    final List<DecisionFilterDto<?>> filter =
        createContainsFilterForValues("ketchup", null, "must");
    ReportResultResponseDto<List<RawDataDecisionInstanceDto>> result =
        evaluateReportWithFilter(decisionDefinitionDto, filter);
  ...
  public void containsFilter_worksWithVeryLongValues() {
    ...
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), createInputs(longMatchWhichUsesWildcardQuery));
  ...
  public void containsFilter_missingValueIsNotAllowed() {
    ...
    final DecisionReportDataDto reportWithAllVersionSet =
        createReportWithAllVersionSet(decisionDefinitionDto);
  ...
  public void containsFilter_variableWithDifferentTypeIsIgnored() {
    ...
    final HashMap<String, InputVariableEntry> inputWithStringMatch =
        new HashMap<String, InputVariableEntry>() {
          {
            put(
                STRING_INPUT_ID,
                new InputVariableEntry(STRING_INPUT_ID, "input", VariableType.STRING, shouldMatch));
            put(
                INTEGER_INPUT_ID,
                new InputVariableEntry(INTEGER_INPUT_ID, "input", VariableType.INTEGER, 0));
          }
        };
    startDecisionInstanceWithInputVars(decisionDefinitionDto.getId(), inputWithStringMatch);
    final HashMap<String, InputVariableEntry> inputsWithIntegerCouldMatchButIsDifferentType =
        new HashMap<String, InputVariableEntry>() {
          {
            put(
                STRING_INPUT_ID,
                new InputVariableEntry(STRING_INPUT_ID, "input", VariableType.STRING, "123"));
            put(
                INTEGER_INPUT_ID,
                new InputVariableEntry(INTEGER_INPUT_ID, "input", VariableType.INTEGER, 100));
          }
        };
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), inputsWithIntegerCouldMatchButIsDifferentType);
    importAllEngineEntitiesFromScratch();
  ...
  protected abstract void assertThatResultContainsVariables(
      final ReportResultResponseDto<List<RawDataDecisionInstanceDto>> result,
      final String... shouldMatch);

  protected abstract List<DecisionFilterDto<?>> createContainsFilterForValues(
