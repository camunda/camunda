...
public class MultiEngineImportIT extends AbstractMultiEngineIT {
  ...
  public void tenantProcessesAreOnlyExcludedFromTheCorrectEngine() {
    ...
    ProcessDefinitionEngineDto shallBePresent3 =
        deployProcessDefinitionWithTenantAndEngine(
            firstTenantId, secondaryEngineIntegrationExtension);

    deployProcessDefinitionWithTenantAndEngine(
        excludedTenantId, secondaryEngineIntegrationExtension);

    // Workaround to solve odd behavior from the configuredEnginesMap key being different from the
    // getEngineName()
    ...
    embeddedOptimizeExtension
        ...
        .get(workaroundMapping.get(secondaryEngineIntegrationExtension.getEngineName()))
    ...
    assertThat(storedDefinitions)
        ...
        .containsExactlyInAnyOrder(
            Tuple.tuple(
                workaroundMapping.get(engineIntegrationExtension.getEngineName()),
                shallBePresent1.getId()),
            Tuple.tuple(
                workaroundMapping.get(engineIntegrationExtension.getEngineName()),
                shallBePresent2.getId()),
            Tuple.tuple(
                workaroundMapping.get(secondaryEngineIntegrationExtension.getEngineName()),
                shallBePresent3.getId()));
  }
  ...
  public void afterRestartOfOptimizeRightImportIndexIsUsed() {
    ...
    // then
    final List<TimestampBasedImportIndexDto> timestampIndexes =
        databaseIntegrationTestExtension.getAllDocumentsOfIndexAs(
  ...
  private ProcessDefinitionEngineDto deployProcessDefinitionWithTenantAndEngine(
      String tenantId, EngineIntegrationExtension engine) {
  ...
  private void deployAllPossibleEngineDataForAllEngines() {
    ...
    final List<ProcessInstanceEngineDto> processInstancesToSuspend =
        deployAndStartUserTaskProcessForAllEngines();
    engineIntegrationExtension.suspendProcessInstanceByInstanceId(
        processInstancesToSuspend.get(0).getId());
    secondaryEngineIntegrationExtension.suspendProcessInstanceByInstanceId(
        processInstancesToSuspend.get(1).getId());
    ...
    secondaryEngineIntegrationExtension.failExternalTasks(
        processInstanceEngineDto.getBusinessKey());
  ...
  private void assertProcessInstanceImportResults(
      final List<ProcessInstanceDto> processInstances, final String... expectedDefinitionKeys) {
