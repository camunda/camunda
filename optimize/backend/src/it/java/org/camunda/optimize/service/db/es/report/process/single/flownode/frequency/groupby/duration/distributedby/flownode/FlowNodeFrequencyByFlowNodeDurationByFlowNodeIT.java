 ...
 */
package org.camunda.optimize.service.db.es.report.process.single.flownode.frequency.groupby.duration.distributedby.flownode;
...
import org.camunda.optimize.service.db.es.report.process.single.ModelElementFrequencyByModelElementDurationByModelElementIT;
...
public class FlowNodeFrequencyByFlowNodeDurationByFlowNodeIT
  ...
  private static final ImmutableList<String> FLOW_NODES =
      ImmutableList.of(END_EVENT, START_EVENT, USER_TASK_1);
  private static final ImmutableList<String> FLOW_NODES_2 =
      ImmutableList.of(END_EVENT + 2, START_EVENT + 2, USER_TASK_2);
  ...
  public void multipleProcessInstances_runningInstanceDurationIsCalculated() {
    ...
    startProcessInstanceCompleteTaskAndModifyDuration(
        definition.getId(), completedActivityInstanceDurations);
  ...
  public void viewLevelFlowNodeDurationFilterOnlyIncludesFlowNodesMatchingFilter() {
    ...
    assertThat(
            MapResultUtil.getDataEntryForKey(
                result.getFirstMeasureData(), createDurationBucketKey(1000)))
        .isPresent();
    assertThat(
            MapResultUtil.getDataEntryForKey(
                result.getFirstMeasureData(), createDurationBucketKey(5000)))
        .isPresent();
    assertThat(
            MapResultUtil.getDataEntryForKey(
                result.getFirstMeasureData(), createDurationBucketKey(10000)))
        .isNotPresent();
            ...
            bucket -> {
              if (bucket.getKey().equals(createDurationBucketKey(1000))
                ...
                assertThat(bucket.getValue())
                    ...
                    .hasSize(3)
                    .contains(
                        Tuple.tuple(START_EVENT, null),
                        Tuple.tuple(USER_TASK_1, 1.),
                        Tuple.tuple(END_EVENT, null));
              } else {
                assertThat(bucket.getValue())
                    .extracting(MapResultEntryDto::getKey, MapResultEntryDto::getValue)
                    .hasSize(3)
