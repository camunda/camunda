...
public class KpiEvaluationSchedulerServiceIT extends AbstractPlatformIT {
  ...
  public void noResultsSavedWhenNoKpiReportExists() {
    ...
    assertThat(processOverviewDtos)
        .singleElement()
        .satisfies(element -> assertThat(element.getLastKpiEvaluationResults()).isEmpty());
  }
  ...
  public void kpiResultsSavedWhenKpiReportExists() {
    ...
    assertThat(processOverviewDtos)
        ...
        .satisfies(
            element ->
                assertThat(element.getLastKpiEvaluationResults())
  ...
  public void existingProcessOverviewDocumentGetsUpdatedWithKpiEvaluationInformation() {
    ...
    final ProcessDefinitionEngineDto secondDef =
        deploySimpleProcessDefinition(anotherDefinitionKey);
    importAllEngineEntitiesFromScratch();
    // setting the owner in order to create a document in process overview index
    processOverviewClient.updateProcess(
        firstDef.getKey(), DEFAULT_USERNAME, new ProcessDigestRequestDto());
    processOverviewClient.updateProcess(
        secondDef.getKey(), DEFAULT_USERNAME, new ProcessDigestRequestDto());
  ...
  public void kpiResultRemovedWhenReportIsNoLongerKpiReport() {
    ...
    // when
    final ProcessReportDataDto updatedReportData =
        createProcessReportDataDto(
            PROCESS_DEFINITION_KEY, false, ProcessReportDataType.PROC_INST_FREQ_GROUP_BY_NONE);
  ...
  public void kpiResultSavedWhenEvaluationResultIsNone() {
    ...
    final ProcessReportDataDto reportDataDto =
        TemplatedProcessReportDataBuilder.createReportData()
            ...
            .definitions(List.of(new ReportDataDefinitionDto(PROCESS_DEFINITION_KEY)))
            .build();
    reportDataDto.setFilter(ProcessFilterBuilder.filter().withDeletedIncident().add().buildList());
  ...
  public void kpiReportGetsUpdatedToNonSingleNumberReport() {
    ...
    final ProcessReportDataDto updatedReportData =
        createProcessReportDataDto(
            PROCESS_DEFINITION_KEY, true, ProcessReportDataType.PROC_INST_DUR_GROUP_BY_START_DATE);
    SingleProcessReportDefinitionRequestDto singleProcessReportDefinitionRequestDto =
        new SingleProcessReportDefinitionRequestDto();
    singleProcessReportDefinitionRequestDto.setData(updatedReportData);
    reportClient.updateSingleProcessReport(reportId, singleProcessReportDefinitionRequestDto);
  ...
  private String createKpiReport(final String definitionKey) {
    ProcessReportDataDto processReportDataDto =
        createProcessReportDataDto(
            definitionKey, true, ProcessReportDataType.PROC_INST_FREQ_GROUP_BY_NONE);
    return reportClient.createSingleProcessReport(processReportDataDto);
  }

  private ProcessReportDataDto createProcessReportDataDto(
      final String definitionKey, final boolean isKpi, final ProcessReportDataType reportDataType) {
    final ProcessReportDataDto reportDataDto =
        TemplatedProcessReportDataBuilder.createReportData()
  ...
  private String createAndPublishEventBasedProcess() {
    ...
    final EventProcessMappingDto simpleEventProcessMappingDto =
        buildSimpleEventProcessMappingDto(startedEvent, finishedEvent);
  ...
  private EventProcessMappingDto buildSimpleEventProcessMappingDto(
      final EventMappingDto startEventMapping, final EventMappingDto endEventMapping) {
    final Map<String, EventMappingDto> eventMappings =
        Map.of(BPMN_START_EVENT_ID, startEventMapping, BPMN_END_EVENT_ID, endEventMapping);
