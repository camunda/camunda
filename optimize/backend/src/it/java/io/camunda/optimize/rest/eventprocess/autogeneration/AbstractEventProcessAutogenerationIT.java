...
public abstract class AbstractEventProcessAutogenerationIT extends AbstractEventProcessIT {
  ...
  protected void assertProcessMappingConfiguration(
    ...
    assertThat(eventProcessMapping.getEventSources())
        .extracting(
            source -> source.getConfiguration().getEventScope(), EventSourceEntryDto::getSourceType)
        .containsExactlyInAnyOrderElementsOf(
            externalSources.stream()
                .map(
                    source ->
                        Tuple.tuple(
                            source.getConfiguration().getEventScope(), source.getSourceType()))
  ...
  protected Gateway getGatewayWithId(final Collection<Gateway> gateways, final String gatewayId) {
    return gateways.stream().filter(gateway -> gateway.getId().equals(gatewayId)).findFirst().get();
  }
  ...
  protected void ingestEventAndProcessTraces(final List<CloudEventRequestDto> eventsToIngest) {
