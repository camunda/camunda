...
import static io.camunda.optimize.test.optimize.UiConfigurationClient.TEST_WEBHOOK_URL_INVALID_PORT;
...
import io.camunda.optimize.service.alert.SyncListener;
...
public abstract class AbstractAlertIT extends AbstractPlatformIT {
  ...
  protected OffsetDateTime getNextReminderExecutionTime(String id) throws SchedulerException {
    Date nextTimeReminderIsExecuted =
        embeddedOptimizeExtension
            .getAlertService()
            .getScheduler()
  ...
  protected AlertCreationRequestDto setupBasicProcessAlertAsUser(
      final String definitionKey, final String user, final String password) {
    String collectionId = collectionClient.createNewCollection(user, password);
    final CollectionScopeEntryDto scopeEntry =
        new CollectionScopeEntryDto(PROCESS, definitionKey, DEFAULT_TENANTS);
    collectionClient.addScopeEntryToCollectionAsUser(collectionId, scopeEntry, user, password);
    String reportId =
        createNumberReportForCollection(definitionKey, collectionId, PROCESS, user, password);
    return alertClient.createSimpleAlert(reportId);
  }

  protected String createNewProcessReportAsUser(
  ...
  private String createNewProcessReportAsUser(
      final String user,
  ...
  private String createNewDecisionReportAsUser(
      final String user,
      final String password,
      final String collectionId,
      final DecisionDefinitionEngineDto decisionDefinitionDto) {
    SingleDecisionReportDefinitionRequestDto decReport =
        getDecisionNumberReportDefinitionDto(collectionId, decisionDefinitionDto);
    return embeddedOptimizeExtension
        .getRequestExecutor()
        .withUserAuthentication(user, password)
  ...
  protected String createAndStoreDurationNumberReportInNewCollection(
  ...
  protected String createAndStoreDurationNumberReport(
      final String collectionId,
  ...
  private String createAndStoreDurationNumberReportAsUser(
      final String collectionId,
      final String processDefinitionKey,
      final String processDefinitionVersion,
      final String user,
    ...
    SingleProcessReportDefinitionRequestDto singleProcessReportDefinitionDto =
        getDurationReportDefinitionDto(
            collectionId, processDefinitionKey, processDefinitionVersion);
    return createNewProcessReportAsUser(user, password, singleProcessReportDefinitionDto);
  }

  protected String createNumberReportForCollection(
      final String collectionId, final DefinitionType definitionType) {
    return createNumberReportForCollection(
        DEFAULT_DEFINITION_KEY, collectionId, definitionType, DEFAULT_USERNAME, DEFAULT_PASSWORD);
  }

  private String createNumberReportForCollection(
    ...
    switch (definitionType) {
      case PROCESS:
        ProcessDefinitionEngineDto processDefinition =
            deployAndStartSimpleServiceTaskProcess(definitionKey);
        importAllEngineEntitiesFromScratch();
      ...
      case DECISION:
        DecisionDefinitionEngineDto decisionDefinitionDto =
            deployAndStartSimpleDecisionDefinition(definitionKey);
        importAllEngineEntitiesFromScratch();
  ...
  private SingleDecisionReportDefinitionRequestDto getDecisionNumberReportDefinitionDto(
      String collectionId, DecisionDefinitionEngineDto decisionDefinitionDto) {
  ...
  protected SingleProcessReportDefinitionRequestDto getProcessNumberReportDefinitionDto(
      String collectionId, ProcessDefinitionEngineDto processDefinition) {
    return getProcessNumberReportDefinitionDto(
        collectionId, processDefinition.getKey(), String.valueOf(processDefinition.getVersion()));
  }

  protected SingleProcessReportDefinitionRequestDto getProcessNumberReportDefinitionDto(
      String collectionId, String processDefinitionKey, String processDefinitionVersion) {
  ...
  private SingleProcessReportDefinitionRequestDto getDurationReportDefinitionDto(
      String collectionId, String processDefinitionKey, String processDefinitionVersion) {
    ProcessReportDataDto reportData =
        TemplatedProcessReportDataBuilder.createReportData()
            .setProcessDefinitionKey(processDefinitionKey)
