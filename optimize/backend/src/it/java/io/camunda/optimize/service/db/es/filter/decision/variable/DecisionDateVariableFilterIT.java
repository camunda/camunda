...
import static io.camunda.optimize.test.util.decision.DecisionFilterUtilHelper.createDateInputVariableFilter;
import static io.camunda.optimize.test.util.decision.DecisionFilterUtilHelper.createFixedDateInputVariableFilter;
import static io.camunda.optimize.test.util.decision.DecisionFilterUtilHelper.createRelativeDateInputVariableFilter;
import static io.camunda.optimize.test.util.decision.DecisionFilterUtilHelper.createRollingDateInputVariableFilter;
...
public class DecisionDateVariableFilterIT extends AbstractDecisionDefinitionIT {
  ...
  public void dateFilterSupportsNullValueInclusionAndExclusion(
    ...
    reportData.setFilter(
        Lists.newArrayList(createDateInputVariableFilter(inputClauseId, dateFilterSupplier.get())));
  ...
  public void resultFilterByRollingDateInputVariable() {
    ...
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), createInputsWithDate(100.0, toDateString(now.minusDays(2))));
    startDecisionInstanceWithInputVars(
        decisionDefinitionDto.getId(), createInputsWithDate(100.0, toDateString(now.minusDays(3))));
