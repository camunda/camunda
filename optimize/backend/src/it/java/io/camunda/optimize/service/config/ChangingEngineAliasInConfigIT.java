 ...
 */
package io.camunda.optimize.service.config;
...
public class ChangingEngineAliasInConfigIT extends AbstractMultiEngineIT {
  ...
  public void changeEngineAlias_processReportEvaluation() {
    ...
    // then the report evaluation should be fine even if it would include a definition from a non
    // existing engine alias
    final ReportResultResponseDto<List<RawDataProcessInstanceDto>> result =
        reportClient.evaluateRawReport(createRawDataReport(PROCESS_KEY_1)).getResult();
  ...
  public void changeEngineAlias_decisionReportEvaluation() {
    ...
    engineIntegrationExtension.deployAndStartDecisionDefinition(
        createSimpleDmnModel(DECISION_KEY_1), null);
    ...
    final ReportResultResponseDto<List<RawDataDecisionInstanceDto>> result =
        reportClient
            .evaluateDecisionRawReport(createRawDecisionDataReport(DECISION_KEY_1))
            .getResult();
    assertThat(result.getInstanceCount()).isEqualTo(2L);
  }
  ...
  public void changeEngineAlias_branchAnalysis() {
    ...
    engineIntegrationExtension.deployAndStartProcessWithVariables(
        BpmnModels.getSimpleGatewayProcess(PROCESS_KEY_1), ImmutableMap.of("goToTask1", true));
    ...
    // then the analysis should be fine even if it would include a definition from a non existing
    // engine alias
  ...
  public void changeEngineAlias_outlierAnalysis() {
    ...
    engineIntegrationExtension.deployProcessAndGetId(
        BpmnModels.getTwoServiceTasksProcess(PROCESS_KEY_1));
    final String procDefId =
        secondaryEngineIntegrationExtension.deployProcessAndGetId(
            BpmnModels.getTwoServiceTasksProcess(PROCESS_KEY_1));
    final OutlierDistributionClient outlierDistributionClient =
    ...
    // then the analysis should be fine even if it would include a definition from a non existing
    // engine alias
    final Map<String, FindingsDto> outlierTest =
        analysisClient.getFlowNodeOutliers(
            PROCESS_KEY_1, Collections.singletonList(ALL_VERSIONS), DEFAULT_TENANT_IDS);
