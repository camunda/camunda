...
public abstract class AbstractProcessInstanceFrequencyByProcessInstanceDateReportEvaluationIT
  ...
  protected abstract void changeProcessInstanceDate(
      final String processInstanceId, final OffsetDateTime newDate) throws SQLException;
  ...
  protected ProcessDefinitionEngineDto deployTwoRunningAndOneCompletedUserTaskProcesses(
    ...
    engineDatabaseExtension.changeProcessInstanceStartDate(
        processInstance2.getId(), now.minusDays(1));
    final ProcessInstanceEngineDto processInstance3 =
        engineIntegrationExtension.startProcessInstance(processDefinition.getId());
    engineDatabaseExtension.changeProcessInstanceStartDate(
        processInstance3.getId(), now.minusDays(2));
    return processDefinition;
  }

  protected abstract void updateProcessInstanceDates(Map<String, OffsetDateTime> newIdToDates)
      throws SQLException;
  ...
  public void processInstancesStartedAtSameIntervalAreGroupedTogether() throws Exception {
    ...
    final Optional<MapResultEntryDto> todayEntry =
        resultData.stream().filter(e -> expectedStringToday.equals(e.getKey())).findFirst();
    ...
    final Optional<MapResultEntryDto> yesterdayEntry =
        resultData.stream().filter(e -> expectedStringYesterday.equals(e.getKey())).findFirst();
    assertThat(yesterdayEntry)
        .isPresent()
        .get()
  ...
  public void emptyIntervalBetweenTwoProcessInstances() throws Exception {
    ...
    final Optional<MapResultEntryDto> dayBeforeYesterdayEntry =
        resultData.stream()
            .filter(e -> expectedStringDayBeforeYesterday.equals(e.getKey()))
            .findFirst();
    assertThat(dayBeforeYesterdayEntry)
        .isPresent()
        .get()
        .extracting(MapResultEntryDto::getValue)
        .isEqualTo(1.);
  ...
  private ProcessReportDataDto createGroupByStartDateReport(
  ...
  private void assertStartDateResultMap(
  ...
  private void updateProcessInstancesTime(
