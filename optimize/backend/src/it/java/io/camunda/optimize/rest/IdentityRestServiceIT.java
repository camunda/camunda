...
import static io.camunda.optimize.dto.optimize.rest.AuthorizationType.CSV_EXPORT;
import static io.camunda.optimize.dto.optimize.rest.AuthorizationType.ENTITY_EDITOR;
...
import io.camunda.optimize.rest.providers.GenericExceptionMapper;
...
public class IdentityRestServiceIT extends AbstractPlatformIT {
  ...
  public void searchForUser() {
    ...
    embeddedOptimizeExtension
        .getIdentityService()
        .addIdentity(new UserDto("otherId", "Bilbo", "Baggins", "bilbo.baggins@camunda.com"));
  ...
  public void searchForUserWithRoles() {
    // given
    final UserDto userIdentity =
        new UserDto("testUser", "Frodo", "Baggins", "frodo.baggins@camunda.com", List.of("myRole"));
    embeddedOptimizeExtension.getIdentityService().addIdentity(userIdentity);
    embeddedOptimizeExtension
        .getIdentityService()
        .addIdentity(new UserDto("otherId", "Bilbo", "Baggins", "bilbo.baggins@camunda.com"));
  ...
  public void searchForGroup() {
    ...
    embeddedOptimizeExtension
        .getIdentityService()
        .addIdentity(new GroupDto("orcs", "The Orcs", 1000L));
    ...
    // then
    assertThat(searchResult)
        .isEqualTo(new IdentitySearchResultResponseDto(Lists.newArrayList(groupIdentity)));
  }
  ...
  public void searchForGroupAndUser() {
    ...
    embeddedOptimizeExtension
        .getIdentityService()
        .addIdentity(new UserDto("otherUser", "Frodo", "NotAHobbit", "not.a.hobbit@camunda.com"));

    // when
    final IdentitySearchResultResponseDto searchResult =
        identityClient.searchForIdentity("baggins");

    // then
    assertThat(searchResult)
        // user is first as name and email contains baggins
        .isEqualTo(
            new IdentitySearchResultResponseDto(Lists.newArrayList(userIdentity, groupIdentity)));
  }
  ...
  public void searchForUser_excludeGroupsFromResults() {
    ...
    // when
    final IdentitySearchResultResponseDto searchResult =
        identityClient.searchForIdentity("baggins", true);
  ...
  public void searchForGroupAndUser_userMetaDataDisabled() {
    ...
    assertThat(searchResult.getResult())
        // user does not contain meta-data, group name is still there though (as not considered
        // metadata)
  ...
  public void emptySearchStringReturnsAlphanumericSortingList() {
    ...
    // then
    assertThat(searchResult)
        // user is first as name and email contains baggins, empty user is second because name == id
        // and is `testUser2`
        // group is last because it starts with `Th`
        .isEqualTo(
            new IdentitySearchResultResponseDto(
                Lists.newArrayList(userIdentity, emptyMetaDataUserIdentity, groupIdentity)));
  }
  ...
  public void getIdentityById_presentInCache(
      final IdentityWithMetadataResponseDto expectedIdentity) {
    // given
    embeddedOptimizeExtension.getIdentityService().addIdentity(expectedIdentity);

    // when
    final IdentityWithMetadataResponseDto identity =
        identityClient.getIdentityById(expectedIdentity.getId());
  ...
  @ParameterizedTest
  @MethodSource("identities")
  public void getIdentityById_notPresentInCache(
      final IdentityWithMetadataResponseDto expectedIdentity) {
    // given
    switch (expectedIdentity.getType()) {
      case USER:
        ...
        assertThat(
                embeddedOptimizeExtension
                    .getUserIdentityCache()
                    .getUserIdentityById(expectedIdentity.getId()))
            .isEmpty();
        break;
      case GROUP:
        ...
        assertThat(
                embeddedOptimizeExtension
                    .getUserIdentityCache()
                    .getGroupIdentityById(expectedIdentity.getId()))
            .isEmpty();
        break;
      default:
  ...
  public void getIdentityById_notPresentInCache_engineFetchFail(
      ...
      case USER:
        ...
        assertThat(
                embeddedOptimizeExtension
                    ...
                    .getUserIdentityById(expectedIdentity.getId()))
            .isEmpty();
        engineFetchRequestMatcher =
            request()
                .withPath(
                    engineIntegrationExtension.getEnginePath()
                        + "/user/"
                        + expectedIdentity.getId()
                        + "/profile");
        break;
      case GROUP:
        authorizationClient.createGroupAndGrantOptimizeAccess(
  ...
  @ParameterizedTest
  @MethodSource("identities")
  public void getIdentityById_notPresentInCache_userMetaDataDisabled(
      final IdentityWithMetadataResponseDto expectedIdentity) {
    // given
    embeddedOptimizeExtension
        .getConfigurationService()
    ...
    assertThat(identity)
        .hasNoNullFieldsOrPropertiesExcept(
            ...
            UserDto.Fields.firstName,
            UserDto.Fields.lastName,
            UserDto.Fields.roles);
  ...
  @MethodSource("identities")
  public void
      getIdentityById_notPresentInCache_postFetchFailsIfIdentityNotAuthorizedToAccessOptimize(
          final IdentityWithMetadataResponseDto expectedIdentity) {
    // given
    switch (expectedIdentity.getType()) {
      case USER:
  ...
  public void getIdentityById_failOnInvalidId() {
    ...
    assertThat(errorResponseDto.getErrorCode())
        .isEqualTo(GenericExceptionMapper.NOT_FOUND_ERROR_CODE);
  }
  ...
  public void getCurrentUserIdentity() {
    ...
    // when
    final UserResponseDto currentUserDto =
        identityClient.getCurrentUserIdentity(KERMIT_USER, KERMIT_USER);

    // then
    final UserResponseDto expectedUser =
        new UserResponseDto(
            new UserDto(
                KERMIT_USER,
                DEFAULT_FIRSTNAME,
  ...
  public void getCurrentUserIdentityForSuperUserWithEditAuthorization(
  ...
  public void getCurrentUserIdentityWithRolesIfPresent() {
    ...
    authorizationClient.addKermitUserAndGrantAccessToOptimize();
    embeddedOptimizeExtension.getUserIdentityCache().synchronizeIdentities();
    final UserDto currentUserIdentity =
        embeddedOptimizeExtension
            .getUserIdentityCache()
            .getUserIdentityById(KERMIT_USER)
            .orElseThrow();
  ...
  public void getCurrentUserIdentity_userMetaDataDeactivated() {
    ...
    assertThat(currentUserDto)
        .isEqualTo(
            new UserResponseDto(new UserDto(KERMIT_USER), List.of(CSV_EXPORT, ENTITY_EDITOR)));
    assertThat(currentUserDto.getUserDto().getName()).isEqualTo(KERMIT_USER);
  }

  @ParameterizedTest
  @EnumSource(SuperUserType.class)
  public void getCurrentUserIdentity_withSuperUserAndSuperGroupAuthorizations(
