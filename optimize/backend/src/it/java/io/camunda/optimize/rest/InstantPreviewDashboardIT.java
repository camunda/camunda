...
// import static
// io.camunda.optimize.dto.optimize.query.dashboard.InstantDashboardDataDto.INSTANT_DASHBOARD_DEFAULT_TEMPLATE;
// import static io.camunda.optimize.rest.RestTestConstants.DEFAULT_USERNAME;
// import static
// io.camunda.optimize.service.dashboard.InstantPreviewDashboardService.INSTANT_PREVIEW_DASHBOARD_TEMPLATES_PATH;
// import static
// io.camunda.optimize.service.dashboard.InstantPreviewDashboardService.TYPE_IMAGE_VALUE;
// import static
// io.camunda.optimize.service.dashboard.InstantPreviewDashboardService.getChecksumCRC32;
// import static
...
//   private static final Map<String, List<String>> EXPECTED_TILE_STRINGS =
// createExpectedTileString();
//
//   @Test
...
//
//     // when
//     // now fiddle with the stored hash in the database so that the code thinks a change has
// happened
...
//
//     // then
//     // Since the entry had been de-validated, I expect that a new dashboard with a new ID has
// been
//     // created
...
//     DashboardDefinitionRestDto originalDashboard =
//         dashboardClient.getInstantPreviewDashboard(PROCESS_DEF_KEY, TEMPLATE_3_FILENAME);
//     // We keep track of the report IDs that belong to this dashboard, this will be important
// later
//     Set<String> originalReportIds = originalDashboard.getTileIds();
//
//     // when
...
//         .getInstantPreviewDashboardService()
//         .deleteInstantPreviewDashboardsAndEntitiesForChangedTemplates();
//     // Now get the dashboard again. Since the entry was still valid, I expect the same old
// dashboard
//     // with the same ID
//     DashboardDefinitionRestDto newDashboard =
//         dashboardClient.getInstantPreviewDashboard(PROCESS_DEF_KEY, TEMPLATE_3_FILENAME);
...
//               // when
//               Response response =
//                   reportClient.evaluateReportAsUserRawResponse(reportId, KERMIT_USER,
// KERMIT_USER);
//               // then
//               // Kermit has no access to the process definition, therefore he shall not be able
// to
...
//               // Moreover we need to make sure that although he can evaluate it, he is only a
// viewer
...
//     final Response reportDeleteResponse = reportClient.deleteReport(instantReportId.get(),
// false);
//
//     // then
...
//         new SingleProcessReportDefinitionExportDto(new
...
//           // getInstanceCountWithoutFilters() is the same as getInstanceCount() if the filter is
// of
//           // type
//           // InstanceEndDateFilterDto, so excluding that one. This is an intentional behaviour
// from
//           // the filter
...
//                   Tuple.tuple(
//                       FIRST_DEF_KEY, List.of(ALL_VERSIONS), List.of(FIRST_TENANT,
// SECOND_TENANT)));
...
//                           (Map<String, Object>) tile.getConfiguration();
//                       InstantPreviewDashboardService.findAndConvertTileContent(
//                           textTileConfiguration, TYPE_IMAGE_VALUE, this::assertImagePresent,
// "src");
//                     }
//                   });
//         });
...
//         getClass()
//             .getClassLoader()
//             .getResourceAsStream(INSTANT_PREVIEW_DASHBOARD_TEMPLATES_PATH +
...
//                 "Dieses Dashboard bietet eine Übersicht der Metriken und Anwendungsfälle, die mit
// Optimize abgedeckt werden können.",
//                 "Melde Geschäftskennzahlen",
//                 "Aggregiere und gruppiere Prozessausführungsdaten in Tagen, Wochen und Monaten um
...
//                 "Nutze alle gesammelten Prozessausführungsdaten, um Probleme zu untersuchen oder
// neue Prozessverbesserungen einzuleiten.",
//                 "Nutze die Optimize Ausreißeranalyse, um Verbesserungspotenzial im Prozess zu
// finden",
//                 "Echtzeitnahe Überwachung der Prozess Gesundheit",
//                 "Überwache Prozessausführungsdaten der letzten Minuten, Stunden und Tage
// echtzeitnah, um korrektiv eingreifen zu können.",
//                 "Erstellen Sie eine Kopie dieses Dashboards und passen Sie es Ihren Bedürfnissen
// an",
//                 "Aktuell laufend",
//                 "Dieser Bericht zeigt den Fortschritt aller laufenden Prozesse an.",
//                 "In den letzten 7 Tagen eingegangene",
//                 "Dieser Bericht zeigt die in den letzten 7 Tagen eingegangenen Anfragen die einen
// Prozess gestartet haben.",
...
//                 "Dieser Bericht zeigt wie stark der Prozess in den letzten Monaten genutzt
// wurde.",
//                 "Flaschenhälse der in den letzten 6 Monaten beendeten Prozesse",
//                 "Die Heatmap hebt die Prozessknoten hervor, die im Durchschnitt die meiste Zeit
// bis zur Fertigstellung benötigen."),
//         "en",
//             List.of(
//                 "Instant process dashboard",
//                 "This dashboard provides a competitive overview of metrics and use cases that can
// be covered with Optimize. It is "
//                     + "available for any process as soon as a process is deployed.",
//                 "Report business metrics",
//                 "Aggregate and group the process execution data into days, weeks, and months to
// monitor and report business metrics and"
//                     + " key performance indicators (KPI).",
//                 "Subscribe to an email digest to obtain a weekly summary of your process metrics
// and KPIs",
//                 "Investigate problems and find improvements",
//                 "Use all gathered process execution data to investigate problems or kick off new
// process improvements.",
//                 "Use Optimize outlier analysis to find improvement potential in your process",
//                 "Monitor process health in near real-time",
//                 "See process execution data of the last minutes, hours, and days in near
// real-time to take corrective actions if needed.",
...
//                 "This report provides an overview of the utilization of the process in the last
// months. ",
//                 "Bottlenecks in the process ended in the last 6 months",
//                 "The heatmap highlights the flow nodes that consume on average the most time to
// be completed."));
//   }
//
...
//             Set.of(
//                 "% SLA Met",
//                 "Which process steps take too much time? (To do: Add target values for these
// process steps)",
...
//                 new LocalizedReportData(
//                     "Processes ended grouped by month",
//                     "This report provides an overview of the utilization of the process in the
// last months. ",
//                     "Process instance Count",
//                     "End date"),
//                 new LocalizedReportData(
...
//                 new LocalizedReportData(
//                     "Bottlenecks in the process ended in the last 6 months",
//                     "The heatmap highlights the flow nodes that consume on average the most time
// to be completed.",
//                     "Flow node Duration",
//                     "Flow nodes"))),
//         Arguments.of(
...
//                 new LocalizedReportData(
//                     "In den letzten 7 Tagen eingegangene",
//                     "Dieser Bericht zeigt die in den letzten 7 Tagen eingegangenen Anfragen die
// einen Prozess gestartet haben.",
//                     "Prozessinstanz Anzahl",
//                     "Keiner"),
//                 new LocalizedReportData(
//                     "In den letzten 7 Tagen beendete Prozesse",
//                     "Dieser Bericht zeigt die Anzahl der in den letzten 7 Tagen beendeten
// Prozesse.",
//                     "Prozessinstanz Anzahl",
//                     "Keiner"),
//                 new LocalizedReportData(
...
//                 new LocalizedReportData(
//                     "Flaschenhälse der in den letzten 6 Monaten beendeten Prozesse",
//                     "Die Heatmap hebt die Prozessknoten hervor, die im Durchschnitt die meiste
// Zeit bis zur Fertigstellung benötigen.",
//                     "Prozessknoten Dauer",
//                     "Prozessknoten"))));
//   }
//
