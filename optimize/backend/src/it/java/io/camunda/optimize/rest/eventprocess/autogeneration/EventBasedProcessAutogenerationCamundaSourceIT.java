...
public class EventBasedProcessAutogenerationCamundaSourceIT
  ...
  public void createFromCamundaSource_startEndEvents_multipleStartMultipleEndEvents() {
    ...
    assertCorrectMappingsAndContainsEvents(
        ...
        generatedInstance,
        Arrays.asList(
            expectedStartEvent1, expectedStartEvent2, expectedEndEvent1, expectedEndEvent2));
    ...
    assertNodeConnection(
        idOf(expectedStartEvent1), START_EVENT, convergingId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        idOf(expectedStartEvent2), START_EVENT, convergingId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        convergingId, EXCLUSIVE_GATEWAY, divergingId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        divergingId, EXCLUSIVE_GATEWAY, idOf(expectedEndEvent1), END_EVENT, generatedInstance);
    assertNodeConnection(
        divergingId, EXCLUSIVE_GATEWAY, idOf(expectedEndEvent2), END_EVENT, generatedInstance);
  ...
  public void createFromCamundaSource_startEndEvents_multipleStartNoEndEvents() {
    ...
    // then the mappings contain the correct events and are all in the model
    assertCorrectMappingsAndContainsEvents(
        mappings, generatedInstance, Arrays.asList(expectedStartEvent1, expectedStartEvent2));
    ...
    assertNodeConnection(
        idOf(expectedStartEvent1), START_EVENT, gatewayId, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        idOf(expectedStartEvent2), START_EVENT, gatewayId, EXCLUSIVE_GATEWAY, generatedInstance);
