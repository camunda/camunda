...
import static io.camunda.optimize.test.util.decision.DecisionTypeRef.BOOLEAN;
import static io.camunda.optimize.test.util.decision.DecisionTypeRef.STRING;
import static io.camunda.optimize.test.util.decision.DecisionTypeRef.values;
...
public abstract class DecisionVariableNameRetrievalIT extends AbstractDecisionDefinitionIT {
  ...
  public void getVariableNames() {
    ...
    DecisionDefinitionEngineDto decisionDefinitionDto =
        deployDecisionsWithStringVarNames(of("var1", "var2"));

    importAllEngineEntitiesFromScratch();

    // when
    List<DecisionVariableNameResponseDto> variableResponse =
        getVariableNames(decisionDefinitionDto);
    ...
    assertThat(variableResponse)
        ...
        .extracting(
            DecisionVariableNameResponseDto::getName, DecisionVariableNameResponseDto::getType)
        .containsExactly(
            Tuple.tuple("var1", VariableType.STRING), Tuple.tuple("var2", VariableType.STRING));
  }
  ...
  public void getVariableNames_multipleDefinitions() {
    ...
    DecisionDefinitionEngineDto decisionDefinitionDto2 =
        deployDecisionsWithStringVarNames(of("var2", "var3", "var4"));

    importAllEngineEntitiesFromScratch();

    // when
    List<DecisionVariableNameResponseDto> variableResponse =
        getVariableNames(Arrays.asList(decisionDefinitionDto1, decisionDefinitionDto2));
  ...
  public void getVariableNamesSingleBucketFilteredBySingleTenant() {
    ...
    deployAndStartMultiTenantDecision(newArrayList(null, tenantId1, tenantId2));
    importAllEngineEntitiesFromScratch();
    ...
    List<DecisionVariableNameResponseDto> variableResponse =
        getVariableNames(variableNameRequestDto);
  ...
  public void getVariableNamesForMultipleDefinitionVersions_onlyLastVersionIsConsidered() {
    // given
    DecisionDefinitionEngineDto decisionDefinitionDto =
        deployDecisionsWithStringVarNames(of("bar"));
    DecisionDefinitionEngineDto decisionDefinitionDto2 =
        deployDecisionsWithStringVarNames(of("foo1", "foo2"));
  ...
  public void getMoreThan10Variables() {
    ...
    DecisionDefinitionEngineDto decisionDefinition =
        deployDecisionsWithStringVarNames(varListWithMoreThan10Entries);
    importAllEngineEntitiesFromScratch();
  ...
  public void getVariablesForAllVersions_onlyLastVersionIsConsidered() {
    ...
    // when
    List<DecisionVariableNameResponseDto> variableResponse =
        getVariableNames(decisionDefinition.getKey(), ALL_VERSIONS);
  ...
  public void getVariableNamesForLatestVersions() {
    ...
    // when
    List<DecisionVariableNameResponseDto> variableResponse =
        getVariableNames(decisionDefinition.getKey(), LATEST_VERSION);
  ...
  public void noVariablesFromAnotherDecisionDefinition() {
    // given
    DecisionDefinitionEngineDto decisionDefinition =
        deployDecisionsWithStringVarNames(of("expectedVar"));
  ...
  public void variablesAreSortedAccordingTheirOccurrence() {
    ...
    List<String> actualVars =
        variableResponse.stream()
            .map(DecisionVariableNameResponseDto::getName)
  ...
  public void variableWithSameNameAndDifferentType() {
    // given
    DecisionDefinitionEngineDto decisionDefinition =
        deployDecisionsWithVarNames(of("var", "var"), of(STRING, BOOLEAN));
    importAllEngineEntitiesFromScratch();
    ...
    assertThat(variableResponse)
        ...
        .extracting(
            DecisionVariableNameResponseDto::getName, DecisionVariableNameResponseDto::getType)
        .containsExactly(
            Tuple.tuple("var", VariableType.STRING), Tuple.tuple("var", VariableType.BOOLEAN));
  }

  protected abstract DecisionDefinitionEngineDto deployDecisionsWithVarNames(
      List<String> varNames, List<DecisionTypeRef> types);

  protected abstract List<DecisionVariableNameResponseDto> getVariableNames(
  ...
  protected abstract List<DecisionVariableNameResponseDto> getVariableNames(
      List<DecisionDefinitionEngineDto> decisionDefinitions);
