...
public class AnalysisAuthorizationIT extends AbstractPlatformIT {
  ...
  public void branchAnalysisWithoutAuthorization() {
    ...
    // when
    BranchAnalysisRequestDto analysisDto =
        analysisClient.createAnalysisDto(
            ...
            END_EVENT_ID);
    Response response =
        analysisClient.getProcessDefinitionCorrelationRawResponseWithoutAuth(analysisDto);
  ...
  public void branchAnalysis_authorizedTenant() {
    ...
    final ProcessDefinitionEngineDto processDefinition =
        deploySimpleGatewayProcessDefinition(tenantId);
    authorizationClient.addKermitUserAndGrantAccessToOptimize();
    authorizationClient.addGlobalAuthorizationForResource(RESOURCE_TYPE_PROCESS_DEFINITION);
  ...
  public void branchAnalysis_partiallyUnauthorizedTenants() {
    ...
    final ProcessDefinitionEngineDto processDefinition1 =
        deploySimpleGatewayProcessDefinition(tenantId1);
    final ProcessDefinitionEngineDto processDefinition2 =
        deploySimpleGatewayProcessDefinition(tenantId2);
    authorizationClient.addKermitUserAndGrantAccessToOptimize();
    authorizationClient.addGlobalAuthorizationForResource(RESOURCE_TYPE_PROCESS_DEFINITION);
    authorizationClient.grantSingleResourceAuthorizationsForUser(
    ...
    // when
    Response response =
        executeBranchAnalysisAsKermit(processDefinition1, Lists.newArrayList(tenantId1, tenantId2));
  ...
  public void branchAnalysis_notAuthorizedToProcessDefinition() {
    ...
    final Response response =
        executeBranchAnalysisAsKermit(processDefinition, Lists.newArrayList(tenantId));

    // then
    assertThat(response.getStatus()).isEqualTo(Response.Status.FORBIDDEN.getStatusCode());
  }

  private Response executeBranchAnalysisAsKermit(
      final ProcessDefinitionEngineDto processDefinition) {
