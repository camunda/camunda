...
public class ExecutedFlowNodeViewLevelFilterIT extends AbstractFilterIT {
  ...
  // This stream does not include endDate reports as these scenarios are covered by the equivalent
  // startDate report.
  ...
  public void notInExecutedFlowNodeFilterWorks_viewLevelFilteringAlsoAffectsInstanceCount(
    ...
    // then the instance with only start and userTask1 is filtered out and only the end event bucket
    // is in the result
    assertThat(resultDto.getInstanceCount()).isEqualTo(1);
    assertThat(resultDto.getInstanceCountWithoutFilters()).isEqualTo(2);
  }
