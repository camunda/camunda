 ...
 */
package io.camunda.optimize.service.db.es.report.process.single.processinstance.duration.groupby.variable.distributedby.none;
...
import static io.camunda.optimize.service.util.ProcessReportDataType.PROC_INST_DUR_GROUP_BY_VARIABLE;
import static io.camunda.optimize.service.util.ProcessReportDataType.PROC_INST_DUR_GROUP_BY_VARIABLE_WITH_PART;
...
import static io.camunda.optimize.test.util.DurationAggregationUtil.getAggregationTypesAsListForProcessParts;
import static io.camunda.optimize.util.BpmnModels.END_LOOP;
import static io.camunda.optimize.util.BpmnModels.START_LOOP;
...
public class ProcessInstanceDurationByVariableWithProcessPartReportEvaluationIT
  ...
  public void reportEvaluationForOneProcess() {
    ...
    final Double calculatedResult =
        MapResultUtil.getEntryForKey(result.getFirstMeasureData(), DEFAULT_VARIABLE_VALUE)
            .get()
            .getValue();
  ...
  public void evaluateReportForMultipleEvents() {
    ...
    ProcessInstanceEngineDto processInstanceDto =
        engineIntegrationExtension.startProcessInstance(processEngineDto.getId(), variables);
    ...
    assertThat(
            MapResultUtil.getEntryForKey(result.getFirstMeasureData(), DEFAULT_VARIABLE_VALUE + 2)
                .get()
                .getValue())
    ...
    evaluationResponse
        ...
        .forEach(
            measureResult -> {
              ...
              assertThat(
                      MapResultUtil.getEntryForKey(resultData, DEFAULT_VARIABLE_VALUE)
                          .get()
                          .getValue())
                  .isEqualTo(
                      databaseIntegrationTestExtension
                          .calculateExpectedValueGivenDurations(1000., 9000., 2000.)
  ...
  public void takeCorrectActivityOccurrences() {
    ...
    ProcessInstanceEngineDto processInstanceDto =
        deployAndStartLoopingProcess(
            Maps.newHashMap(DEFAULT_VARIABLE_NAME, DEFAULT_VARIABLE_VALUE));
    ...
    assertThat(
            MapResultUtil.getEntryForKey(resultDto.getFirstMeasureData(), DEFAULT_VARIABLE_VALUE))
        .isPresent()
        .get()
        .extracting(MapResultEntryDto::getValue)
  ...
  public void otherProcessDefinitionsDoNoAffectResult() {
    ...
    engineDatabaseExtension.changeAllFlowNodeEndDates(
        procInstDto.getId(), startDate.plusSeconds(2));
    importAllEngineEntitiesFromScratch();
  ...
  public void filterInReportWorks() {
    ...
    engineDatabaseExtension.changeAllFlowNodeStartDates(processInstanceDto.getId(), startDate);
    engineDatabaseExtension.changeAllFlowNodeEndDates(
        processInstanceDto.getId(), startDate.plusSeconds(4));
    importAllEngineEntitiesFromScratch();
  ...
  public void variableTypeIsImportant() {
    ...
    engineDatabaseExtension.changeAllFlowNodeEndDates(
        processInstanceDto2.getId(), startDate.plusSeconds(2));
    importAllEngineEntitiesFromScratch();
    ...
    assertThat(MapResultUtil.getEntryForKey(resultDto.getFirstMeasureData(), "1").get().getValue())
        .isEqualTo(1000.);
    assertThat(
            MapResultUtil.getEntryForKey(resultDto.getFirstMeasureData(), MISSING_VARIABLE_KEY)
                .get()
                .getValue())
        .isEqualTo(2000.);
  }
  ...
  public void otherVariablesDoNotDistortTheResult() {
    ...
    engineDatabaseExtension.changeAllFlowNodeStartDates(processInstanceDto2.getId(), startDate);
    engineDatabaseExtension.changeAllFlowNodeEndDates(
        processInstanceDto2.getId(), startDate.plusSeconds(5));
    importAllEngineEntitiesFromScratch();
    ...
    assertThat(resultDto.getFirstMeasureData()).hasSize(2);
    assertThat(
            MapResultUtil.getEntryForKey(resultDto.getFirstMeasureData(), "bar1").get().getValue())
        .isEqualTo(1000.);
    assertThat(
            MapResultUtil.getEntryForKey(resultDto.getFirstMeasureData(), MISSING_VARIABLE_KEY)
                .get()
  ...
  public void groupByDateVariableWorksForAllStaticUnits(final AggregateByDateUnit unit) {
    ...
    for (int i = 0; i < numberOfInstances; i++) {
      ...
      engineDatabaseExtension.changeAllFlowNodeEndDates(
          instance.getId(), dateVariableValue.plusSeconds(1));
    }
    ...
    for (int i = 0; i < numberOfInstances; i++) {
      final String expectedBucketKey =
          formatter.format(
              truncateToStartOfUnit(dateVariableValue.plus(i, chronoUnit), chronoUnit));
  ...
  public void groupByDateVariableWorksForAutomaticInterval() {
    ...
    // buckets span across all variable values (buckets are in descending order, so the first bucket
    // is based on the
  ...
  private void startThreeProcessInstances(
    ...
    endDatesToUpdate.put(
        processInstanceDto.getId(), activityStartDate.plusSeconds(activityDurationsInSec.get(0)));
    endDatesToUpdate.put(
        processInstanceDto2.getId(), activityStartDate.plusSeconds(activityDurationsInSec.get(1)));
    endDatesToUpdate.put(
        processInstanceDto3.getId(), activityStartDate.plusSeconds(activityDurationsInSec.get(2)));
