 ...
 */
package io.camunda.optimize.service.db.es.report.process.single.flownode.frequency.groupby.flownode.distributedby.process;

import static io.camunda.optimize.dto.optimize.ReportConstants.ALL_VERSIONS;
import static io.camunda.optimize.service.util.ProcessReportDataType.FLOW_NODE_FREQ_GROUP_BY_FLOW_NODE_BY_PROCESS;
...
public class FlowNodeFrequencyByFlowNodeByProcessReportEvaluationIT extends AbstractPlatformIT {
        ...
        .containsExactly(
            List.of(
                createHyperMapResult(
                    END_EVENT, new MapResultEntryDto(processIdentifier, 1.0, processDisplayName)),
                createHyperMapResult(
                    START_EVENT, new MapResultEntryDto(processIdentifier, 1.0, processDisplayName)),
                createHyperMapResult(
                    USER_TASK_1,
  ...
  public void reportEvaluationWithSingleProcessDefinitionSourceWithAllInstancesRemovedByFilter() {
    ...
    assertThat(result.getMeasures())
        ...
        .containsExactly(
            List.of(
                createHyperMapResult(
                    END_EVENT, new MapResultEntryDto(processIdentifier, null, processDisplayName)),
                createHyperMapResult(
                    START_EVENT,
  ...
  public void reportEvaluationWithMultipleProcessDefinitionSources_oneWithoutInstanceStarted() {
    ...
    // We deploy this to create an index, but it is not part of the report definition so should be
    // excluded
    ...
    ReportDataDefinitionDto firstDefinition =
        new ReportDataDefinitionDto(
            firstIdentifier, firstDefinitionInstance.getProcessDefinitionKey(), firstDisplayName);
    ReportDataDefinitionDto secondDefinition =
        new ReportDataDefinitionDto(
