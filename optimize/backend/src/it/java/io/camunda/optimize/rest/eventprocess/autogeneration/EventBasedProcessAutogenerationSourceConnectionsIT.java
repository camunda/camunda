...
import static io.camunda.optimize.service.util.EventModelBuilderUtil.generateConnectionGatewayIdForDefinitionKey;
...
public class EventBasedProcessAutogenerationSourceConnectionsIT
  ...
  public void createFromTwoCamundaStartEndEventSources_multipleStartEndEvents() {
    ...
    // 4 additional gateways exist in the source models, plus a single gateway to connect two source
    // models
    ...
    // The previous source is responsible for creating the converging connecting gateway so the ID
    // uses its
    // definition key
    ...
    // The next source is responsible for creating the diverging connecting gateway so the ID uses
    // its definition key
    final String connectingId2 =
        generateConnectionGatewayIdForDefinitionKey(
            Diverging, secondCamundaSource.getConfiguration().getProcessDefinitionKey());
    assertNodeConnection(
        idOf(firstStart1), START_EVENT, convergingId1, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        idOf(firstStart2), START_EVENT, convergingId1, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        convergingId1, EXCLUSIVE_GATEWAY, divergingId1, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        divergingId1, EXCLUSIVE_GATEWAY, idOf(firstEnd1), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        divergingId1, EXCLUSIVE_GATEWAY, idOf(firstEnd2), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(firstEnd1), INTERMEDIATE_EVENT, connectingId1, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        idOf(firstEnd2), INTERMEDIATE_EVENT, connectingId1, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        connectingId1, EXCLUSIVE_GATEWAY, connectingId2, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
        connectingId2,
        EXCLUSIVE_GATEWAY,
        idOf(secondStart1),
    ...
    assertNodeConnection(
        ...
        generatedInstance);
    assertNodeConnection(
        convergingId2, EXCLUSIVE_GATEWAY, divergingId2, EXCLUSIVE_GATEWAY, generatedInstance);
    assertNodeConnection(
  ...
  public void
    ...
    // then the model elements are of the correct type and connected to expected nodes correctly
    assertNodeConnection(
        idOf(firstStart), START_EVENT, idOf(secondStart), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(secondStart), INTERMEDIATE_EVENT, idOf(secondEnd), END_EVENT, generatedInstance);
    assertNodeConnection(idOf(secondEnd), END_EVENT, null, null, generatedInstance);
  ...
  public void
    ...
    // the second and third sources have instances that can be correlated together by another
    // business key
  ...
  public void
    ...
    // the first and third sources have instances that can be correlated together by another
    // business key
    final String secondBusinessKey = "businessKey2";
    engineIntegrationExtension.startProcessInstance(
        firstInstanceEngineDto.getDefinitionId(), Collections.emptyMap(), secondBusinessKey);
    importAllEngineEntitiesFromLastIndex();
  ...
  public void createFromCamundaAndExternalSources_correlatableInstanceForAllSources() {
    ...
    // We supply the sources in a different order than we expect them to appear, to make sure the
    // order determination
    // is being applied
    final List<EventSourceEntryDto<?>> sources =
        Arrays.asList(
            secondCamundaSource, firstCamundaSource, createExternalEventAllGroupsSourceEntry());
    final EventProcessMappingCreateRequestDto createRequestDto =
        buildAutogenerateCreateRequestDto(sources);
    ...
    assertNodeConnection(
        idOf(EVENT_B), INTERMEDIATE_EVENT, idOf(firstStart), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
        idOf(firstStart),
        INTERMEDIATE_EVENT,
        idOf(firstEnd),
  ...
  public void createFromTwoCamundaAndExternalSources_externalSourceNotCorrelatable() {
    ...
    final List<EventSourceEntryDto<?>> sources =
        Arrays.asList(
            secondCamundaSource, createExternalEventAllGroupsSourceEntry(), firstCamundaSource);
    final EventProcessMappingCreateRequestDto createRequestDto =
        buildAutogenerateCreateRequestDto(sources);
    ...
    assertNodeConnection(
        ...
        generatedInstance);
    assertNodeConnection(
        idOf(secondEnd), INTERMEDIATE_EVENT, idOf(EVENT_A), INTERMEDIATE_EVENT, generatedInstance);
    assertNodeConnection(
  ...
  public void
    ...
    BpmnModelInstance thirdModelInstance =
        singleStartSingleEndModel(PROCESS_ID_3, START_EVENT_ID_3, END_EVENT_ID_3);
    // This uses a different business key so can't be correlated by instance and should be placed at
    // the end
    final CamundaEventSourceEntryDto thirdCamundaSource =
        deployDefinitionWithInstanceAndCreateEventSource(
    ...
    assertNodeConnection(
        ...
        generatedInstance);
    assertNodeConnection(
        idOf(thirdStart), INTERMEDIATE_EVENT, idOf(thirdEnd), END_EVENT, generatedInstance);
