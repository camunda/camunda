...
// import static
// io.camunda.optimize.service.dashboard.ManagementDashboardService.CURRENTLY_IN_PROGRESS_NAME_LOCALIZATION_CODE;
// import static
// io.camunda.optimize.service.dashboard.ManagementDashboardService.ENDED_IN_LAST_SIX_MONTHS_NAME_LOCALIZATION_CODE;
// import static
// io.camunda.optimize.service.dashboard.ManagementDashboardService.STARTED_IN_LAST_SIX_MONTHS_NAME_LOCALIZATION_CODE;
...
//     // all available definitions are included in the report definition as it is a management
// report
...
//   @Test
//   public void
//       savedManagementReportCanBeEvaluatedAndIncludesAllProcessesEvenIfProcessHasNoInstanceData()
// {
//     // given
//     engineIntegrationExtension.deployAndStartProcess(getSingleUserTaskDiagram(FIRST_DEF_KEY));
//     engineIntegrationExtension.deployAndStartProcess(getSingleUserTaskDiagram(SECOND_DEF_KEY));
...
//         getSingleUserTaskDiagram(defKeyNoInstanceData));
...
//     importAllEngineEntitiesFromScratch();
//     final Map<String, SingleProcessReportDefinitionRequestDto> mgmtReportsByName =
//         getAllManagementReports().stream()
//             .collect(Collectors.toMap(ReportDefinitionDto::getName, Function.identity()));
//
//     // then
//     assertThat(
//             reportClient.evaluateNumberReportById(
//                 mgmtReportsByName.get(CURRENTLY_IN_PROGRESS_NAME_LOCALIZATION_CODE).getId()))
//         .satisfies(
//             response -> {
...
//
// mgmtReportsByName.get(STARTED_IN_LAST_SIX_MONTHS_NAME_LOCALIZATION_CODE).getId()))
//         .satisfies(
//             response -> {
//               assertThat(response.getReportDefinition().getData().getDefinitions())
//                   .extracting(ReportDataDefinitionDto::getKey)
...
//     assertThat(
//             reportClient.evaluateNumberReportById(
//                 mgmtReportsByName.get(ENDED_IN_LAST_SIX_MONTHS_NAME_LOCALIZATION_CODE).getId()))
//         .satisfies(
//             response -> {
//               assertThat(response.getReportDefinition().getData().getDefinitions())
//                   .extracting(ReportDataDefinitionDto::getKey)
...
//         KERMIT_USER, authorizedProcess.getProcessDefinitionKey(),
...
//         .containsExactlyInAnyOrder(
//             Tuple.tuple(
//                 authorizedProcess.getProcessDefinitionKey(),
...
//     authorizationClient.addKermitUserAndGrantAccessToOptimize();
//     authorizationClient.grantSingleResourceAuthorizationsForUser(
//         KERMIT_USER, processInstance.getProcessDefinitionKey(),
// RESOURCE_TYPE_PROCESS_DEFINITION);
//     authorizationClient.grantSingleResourceAuthorizationsForUser(
...
//   public void managementReportNamesAndDescriptionsAreLocalized(
//       final String locale, final Set<LocalizedReportData> expectedReportNamesAndDescriptions) {
//     // given
//     engineIntegrationExtension.deployAndStartProcess(getSingleUserTaskDiagram(FIRST_DEF_KEY));
//     importAllEngineEntitiesFromScratch();
//
//     // when
//     final Set<LocalizedReportData> localizedReportNamesAndDescriptions =
//         getAllManagementReports().stream()
//             .map(
//                 report -> {
...
//                   final SingleProcessReportDefinitionRequestDto responseDef =
//                       reportResponse.getReportDefinition();
//                   final SingleReportConfigurationDto reportConfig =
//                       responseDef.getData().getConfiguration();
...
//
//     // then
//     assertThat(localizedReportNamesAndDescriptions)
//         .containsExactlyInAnyOrderElementsOf(expectedReportNamesAndDescriptions);
//   }
//
//   private AuthorizedProcessReportEvaluationResponseDto<List<MapResultEntryDto>>
//       evaluateReportAsKermit(final String reportId) {
//     return evaluateReportAsUser(reportId, KERMIT_USER, KERMIT_USER);
//   }
//
//   private AuthorizedProcessReportEvaluationResponseDto<List<MapResultEntryDto>>
//       evaluateReportAsUser(final String reportId, final String username, final String password) {
...
//     return getAllManagementReports().stream()
...
//         null, FIRST_DEF_KEY, Collections.emptyList());
//   }
//
//   private BpmnModelInstance createSimpleProcessWithName(
//       final String processDefKey, final String processName) {
...
//                     "This report provides insight into the overall adoption of process
// orchestration.",
...
//                     "Dieser Bericht gibt Aufschluss über den Status der Einführung der
