...
import static io.camunda.optimize.service.importing.CustomerOnboardingDataImportIT.CUSTOMER_ONBOARDING_DEFINITION_FILE_NAME;
import static io.camunda.optimize.service.importing.CustomerOnboardingDataImportIT.CUSTOMER_ONBOARDING_PROCESS_INSTANCES;
import static io.camunda.optimize.service.onboarding.OnboardingEmailNotificationService.DASHBOARD_LINK_TEMPLATE;
import static io.camunda.optimize.service.onboarding.OnboardingEmailNotificationService.EMAIL_SUBJECT;
...
public class OnboardingSchedulerServiceIT extends AbstractPlatformIT {
  ...
  @RegisterExtension
  public final LogCapturer notificationServiceLogs =
      LogCapturer.create().captureForType(OnboardingEmailNotificationService.class);

  @RegisterExtension
  public final LogCapturer schedulerServiceLogs =
      LogCapturer.create().captureForType(OnboardingSchedulerService.class);
  ...
  public void processOnboardingSchedulingIsEnabledByDefault() {
    ...
    assertThat(
            embeddedOptimizeExtension
                ...
                .getBean(OnboardingSchedulerService.class)
                .isScheduledToRun())
        .isTrue();
  }
  ...
  public void triggerNotificationForANewProcess() {
    ...
    onboardingSchedulerService.setPanelNotificationHandler(
        processTriggeredOnboardingPanelNotification::add);
  ...
  public void doNotTriggerNotificationWhenProcessAlreadyOnboarded() {
    ...
    onboardingSchedulerService.setPanelNotificationHandler(
        processTriggeredOnboardingPanelNotification::add);
    final ProcessDefinitionEngineDto processOne =
  ...
  public void eachProcessGetsItsOwnTriggerNotification() {
    ...
    assertThat(processTriggeredOnboardingPanelNotification)
  ...
  public void aNewProcessDoesntInterfereWithAnOldProcess() {
    ...
    // First instance was completed and notified, now let's start with the first instance of the 2nd
    // process
    ...
    engineIntegrationExtension.startProcessInstance(
        processOne.getId()); // New instance for the already onboarded process
  ...
  public void checkingIntervalIsRespected() {
    ...
    importAllEngineEntitiesFromScratch();
    // There are new processes to be onboarded, but we will not see them within the next 2 seconds,
    // as our checking
    // interval is 30min
    // wait 2 seconds
    ...
    // I need to set it directly, as the settings from the ConfigurationService would be capped at
    // the 60s minimum
    ...
    Awaitility.given()
        ...
        .untilAsserted(
            () ->
                assertThat(processTriggeredOnboardingEmail)
                    .containsExactlyInAnyOrderElementsOf(
                        processTriggeredOnboardingPanelNotification)
                    .containsExactlyInAnyOrder(processKey1, processKey2));
  }
  ...
  public void oldOnboardedProcessesAreNotOnboardedTwice() {
    ...
    final ProcessDefinitionEngineDto processOne =
        deployAndStartSimpleServiceTaskProcess(processKey1);
    final ProcessDefinitionEngineDto processTwo =
        deployAndStartSimpleServiceTaskProcess(processKey2);
    importAllEngineEntitiesFromScratch();
    assertDefinitionHasOnboardedStateForDefinition(false, processKey1);
    assertDefinitionHasOnboardedStateForDefinition(false, processKey2);
  ...
  public void doNotTriggerAnythingIfServiceIsDeactivated() {
    ...
    onboardingSchedulerService
        .init(); // Reinitialize the service to simulate the first start when Optimize is booted
    deployAndStartSimpleServiceTaskProcess(processKey1);
    importAllEngineEntitiesFromScratch();
  ...
  public void emailNotificationIsSentCorrectly() {
    ...
    String expectedEmailText =
        String.format(
                "You have started your first process instance for the %s process!", processKey)
            .replaceAll("[\\n\t ]", "");
    String expectedDashboardLink =
        String.format(DASHBOARD_LINK_TEMPLATE, "", processKey).replaceAll("[\\n\t ]", "");
    String expectedGreeting =
        String.format(
                "Hi %s %s,",
                kermitUser.getProfile().getFirstName(), kermitUser.getProfile().getLastName())
            .replaceAll("[\\n\t ]", "");
    String emailBodyWithoutEmptySpaces =
        GreenMailUtil.getBody(emails[0]).replaceAll("[\\n\r\t ]", "");
    ...
    assertThat(emails[0].getRecipients(Message.RecipientType.TO)[0])
        .hasToString(kermitUser.getProfile().getEmail());
  }
  ...
  public void emailNotificationDoesNotSendForOwnerlessProcesses() {
    ...
    notificationServiceLogs.assertContains(
        String.format(
  ...
  public void demoOnboardingDataDoesNotTriggerNotifications() {
    ...
    // It is crucial that the next statement comes after the import of onboarding data, because
    // during that import the
  ...
  private void setupEmailAlerting(
  ...
  private ProcessDefinitionEngineDto deployAndStartSimpleServiceTaskProcess(
  ...
  private ProcessDefinitionEngineDto deployUserTaskProcess(String definitionKey) {
    return engineIntegrationExtension.deployProcessAndGetProcessDefinition(
        BpmnModels.getSingleUserTaskDiagram(definitionKey));
  }

  private void restartOnboardingSchedulingService(
      final OnboardingSchedulerService onboardingSchedulerService) {
    onboardingSchedulerService.stopOnboardingScheduling();
    embeddedOptimizeExtension
        .getConfigurationService()
        .getOnboarding()
        .setEnableOnboardingEmails(true);
  ...
  private EngineUserDto createKermitUserDtoWithEmail(final String email) {
    final UserProfileDto kermitProfile =
  ...
  private static void assertDefinitionHasOnboardedStateForDefinition(
      final boolean onboardedState, final String defKey) {
    assertThat(databaseIntegrationTestExtension.getAllProcessDefinitions())
