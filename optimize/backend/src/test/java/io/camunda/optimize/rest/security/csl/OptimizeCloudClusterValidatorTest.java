...
class OptimizeCloudClusterValidatorTest {
  ...
  @Test
  void shouldAcceptMatchingClusterId() {
    assertThat(validator.validate(jwtWithClusterId("cluster-1")).hasErrors()).isFalse();
  }

  @Test
  void shouldRejectMismatchedClusterId() {
    assertThat(validator.validate(jwtWithClusterId("other-cluster")).hasErrors()).isTrue();
  }

  @Test
  void shouldAcceptWhenClusterClaimAbsent() {
    // Lenient on absence: the interactive login id_token does not always carry the claim.
  ...
  private static Jwt jwtWithClusterId(final String clusterId) {
    return baseJwt().claim(OptimizeCloudClusterValidator.CLUSTER_ID_CLAIM, clusterId).build();
  }
