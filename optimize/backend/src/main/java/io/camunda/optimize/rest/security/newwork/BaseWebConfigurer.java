...
import static io.camunda.optimize.rest.security.newwork.OptimizeURIs.API;
import static io.camunda.optimize.rest.security.newwork.OptimizeURIs.AUTH_WHITELIST;
import static io.camunda.optimize.rest.security.newwork.OptimizeURIs.COOKIE_JSESSIONID;
...
import static io.camunda.optimize.rest.security.newwork.OptimizeURIs.PUBLIC_API;
import static io.camunda.optimize.rest.security.newwork.OptimizeURIs.RESPONSE_CHARACTER_ENCODING;
import static io.camunda.optimize.rest.security.newwork.OptimizeURIs.X_CSRF_TOKEN;
...
public abstract class BaseWebConfigurer {
  ...
  protected OptimizeProperties OptimizeProperties;
  OptimizeProfileService errorMessageService;
  ...
  public BaseWebConfigurer(
    ...
    webSecurityProperties = OptimizeProperties.getWebSecurity();
  }
  ...
  protected void applySecurityHeadersSettings(final HttpSecurity http) throws Exception {
    final WebSecurityProperties webSecurityConfig = OptimizeProperties.getWebSecurity();
  ...
  protected String getContentSecurityPolicy() {
    /*
    if (OptimizeProperties.getCloud().getClusterId() == null) {
    ...
    return (webSecurityProperties.getContentSecurityPolicy() == null)
        ...
        : webSecurityProperties.getContentSecurityPolicy();
     */
    return webSecurityProperties.DEFAULT_SM_SECURITY_POLICY;
  }
  ...
  private void defaultFilterSettings(final HttpSecurity http) throws Exception {
    if (OptimizeProperties.isCsrfPreventionEnabled()) {
