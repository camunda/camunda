...
/**
 * CCSaaS webapp login gate, wired into CSL's OIDC webapp chain through its {@code
 * ObjectProvider<OidcUserService>} hook ({@code userInfoEndpoint().oidcUserService(...)}). See <a
 ...
 * TokenValidatorFactory} override in {@link OptimizeCloudSecurityConfiguration}; this class only
 * gates the interactive webapp login.
 */
public final class OptimizeCloudOidcUserService extends OidcUserService {
  ...
  public OidcUser loadUser(final OidcUserRequest userRequest) throws OAuth2AuthenticationException {
    ...
    if (!hasRequiredOrgRole(user)) {
      LOG.debug(
          "Denying login for subject [{}]: not in organization [{}] with an allowed role {}",
          user.getSubject(),
          organizationId,
          allowedRoles);
