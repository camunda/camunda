...
import io.camunda.optimize.service.security.AbstractCachingAuthorizationService;
import io.camunda.optimize.service.security.ApplicationAuthorizationService;
import io.camunda.optimize.service.security.EngineAuthorizations;
import io.camunda.optimize.service.security.ResolvedResourceTypeAuthorizations;
...
public class EngineDefinitionAuthorizationService
  ...
  public EngineDefinitionAuthorizationService(
  ...
  protected Map<String, EngineAuthorizations> fetchAuthorizationsForUserId(final String userId) {
    final List<String> authorizedEngines =
            ...
            engineContext -> {
              if (authorizedEngines.contains(engineContext.getEngineAlias())) {
                result.put(
                    ...
                    fetchEngineAuthorizationsForUser(userId, engineContext));
              } else {
                result.put(
                    engineContext.getEngineAlias(),
                    new EngineAuthorizations(engineContext.getEngineAlias()));
              }
            });
    return result;
  }
        ...
        .forEach(
            engineContext -> {
              if (authorizedEngines.contains(engineContext.getEngineAlias())) {
                result.put(
                    engineContext.getEngineAlias(),
                    fetchEngineAuthorizationsForGroup(groupId, engineContext));
              } else {
                result.put(
                    engineContext.getEngineAlias(),
                    new EngineAuthorizations(engineContext.getEngineAlias()));
  ...
  public boolean isAuthorizedToSeeProcessDefinition(
      final String identityId,
      final IdentityType identityType,
      final String definitionKey,
      final List<String> tenantIds) {
    return isAuthorizedToSeeDefinition(
        identityId, identityType, definitionKey, DefinitionType.PROCESS, tenantIds);
  }

  public boolean isUserAuthorizedToSeeProcessDefinition(
      final String userId,
  ...
  public boolean isUserAuthorizedToSeeDecisionDefinition(
      final String identityId,
  ...
  public boolean isAuthorizedToSeeDecisionDefinition(
      final String identityId,
      final IdentityType identityType,
      final String definitionKey,
      final List<String> tenantIds) {
    return isAuthorizedToSeeDefinition(
        identityId, identityType, definitionKey, DefinitionType.DECISION, tenantIds);
  }

  public List<String> filterAuthorizedTenantsForDefinition(
      final String identityId,
      final IdentityType identityType,
      final String definitionKey,
      final DefinitionType definitionType,
      final List<String> tenantIds,
      final Set<String> engines) {
    final Map<String, Set<String>> tenantsPerEngine =
        resolveTenantAndEnginePairs(tenantIds).stream()
            // if a list of engines is provided only consider them, if not any engine can grant
            // access
            .filter(
                tenantAndEnginePair ->
                    engines.isEmpty() || engines.contains(tenantAndEnginePair.getEngine()))
            .collect(
                groupingBy(
  ...
  private boolean isAuthorizedToSeeDefinition(
      final String identityId,
      final IdentityType identityType,
      final String definitionKey,
      final DefinitionType definitionType,
  ...
  private boolean isAuthorizedToSeeDefinition(
    ...
    // empty tenant list or null should be replaced with the actual not defined tenant singleton
    // list
  ...
  private Set<String> getDefinitionEngines(
  ...
  private boolean isAuthorizedToSeeFullyQualifiedDefinition(
      final String identityId,
      final IdentityType identityType,
      final String definitionKey,
      final DefinitionType definitionType,
      final String tenantId,
      final String engineAlias) {
    if (!tenantAuthorizationService.isAuthorizedToSeeTenant(
        identityId, identityType, tenantId, engineAlias)) {
      return false;
    }

    final Map<String, EngineAuthorizations> authorizationsByEngine =
    ...
    final ResolvedResourceTypeAuthorizations resourceAuthorizations =
        Optional.ofNullable(authorizationsByEngine.get(engineAlias))
            .map(
                groupedEngineAuthorizations ->
                    resolveResourceAuthorizations(
                        groupedEngineAuthorizations,
                        RELEVANT_PERMISSIONS,
                        mapToResourceType(definitionType)))
            .filter(
                resolvedAuthorizations ->
                    resolvedAuthorizations.isAuthorizedToAccessResource(definitionKey))
            .orElseGet(ResolvedResourceTypeAuthorizations::new);

    return resourceAuthorizations.isAuthorizedToAccessResource(definitionKey);
  }

  private static EngineAuthorizations fetchEngineAuthorizationsForUser(
      final String userId, final EngineContext engineContext) {
    final List<AuthorizationDto> allAuthorizationsForUser =
        ImmutableList.<AuthorizationDto>builder()
  ...
  private static EngineAuthorizations fetchEngineAuthorizationsForGroup(
    ...
    return mapToEngineAuthorizations(
        engineContext.getEngineAlias(), allAuthorizations, Collections.singletonList(groupId));
  }
  ...
  private List<TenantAndEnginePair> resolveTenantAndEnginePairs(final List<String> tenantIds) {
    ...
    return engineContextFactory.getConfiguredEngines().stream()
        .flatMap(
            engineContext ->
                tenantService.getTenantsByEngine(engineContext.getEngineAlias()).stream()
                    .filter(tenantDto -> tenantIds.contains(tenantDto.getId()))
                    .map(
                        tenantDto ->
                            new TenantAndEnginePair(
                                tenantDto.getId(), engineContext.getEngineAlias())))
        .toList();
  }
