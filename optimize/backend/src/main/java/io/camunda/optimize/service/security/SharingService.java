...
public class SharingService implements ReportReferencingService, DashboardReferencingService {
  ...
  private void addReportInformation(final DashboardShareRestDto createSharingDto,
      final String userId) {
  ...
  private void validateAndCheckAuthorization(final ReportShareRestDto reportShare,
      final String userId) {
  ...
  public Optional<DashboardDefinitionRestDto> evaluateDashboard(final String shareId) {
    final Optional<DashboardShareRestDto> sharedDashboard = sharingReader.findDashboardShare(
        shareId);
