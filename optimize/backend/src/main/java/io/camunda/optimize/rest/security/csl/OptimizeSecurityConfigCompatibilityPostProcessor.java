 ...
 *
 * <p>Reads Optimize's existing auth/security configuration and emits the equivalent
 * {@code camunda.security.*} properties so operators are not forced to migrate their config when
...
public final class OptimizeSecurityConfigCompatibilityPostProcessor
  ...
  public void postProcessEnvironment(
    ...
    mapIfPresent(env, derived, "CAMUNDA_OPTIMIZE_IDENTITY_CLIENTID",
        "camunda.security.authentication.oidc.client-id");
    mapIfPresent(env, derived, "CAMUNDA_OPTIMIZE_IDENTITY_CLIENTSECRET",
        "camunda.security.authentication.oidc.client-secret");
    mapIfPresent(env, derived, "CAMUNDA_OPTIMIZE_IDENTITY_AUDIENCE",
        "camunda.security.authentication.oidc.audiences");

    // --- Public API JWT (api.jwtSetUri / api.audience). ---
    mapIfPresent(env, derived, "SPRING_SECURITY_OAUTH2_RESOURCESERVER_JWT_JWK_SET_URI",
        "camunda.security.authentication.oidc.jwk-set-uri");
    mapIfPresent(env, derived, "CAMUNDA_OPTIMIZE_API_AUDIENCE",
        "camunda.security.authentication.oidc.audiences");
    ...
    // endpoints, diagnosticsEnabled -> oidc.diagnostics, CCSaaS Auth0 registration. See CONFIG-COMPAT.md.

    warnObsolete(env, "CAMUNDA_OPTIMIZE_SECURITY_AUTH_TOKEN_SECRET",
        "self-signed token secret is unused with server-side sessions");
    warnObsolete(env, "CAMUNDA_OPTIMIZE_SECURITY_AUTH_COOKIE_MAX_SIZE",
        "cookie splitting is gone; the session-id cookie is small");
    warnObsolete(env, "OPTIMIZE_API_ACCESS_TOKEN",
    ...
    if (!derived.isEmpty()) {
      ...
      env.getPropertySources()
          .addLast(new MapPropertySource("optimizeCslCompatibility", derived));
