...
public final class OptimizeSecurityConfigCompatibilityPostProcessor
  ...
  private static final String PUBLIC_API_JWK_SET_URI_ENV_VAR =
      "SPRING_SECURITY_OAUTH2_RESOURCESERVER_JWT_JWK_SET_URI";
  private static final String PUBLIC_API_JWK_SET_URI = "api.jwtSetUri";
  ...
  //
  // Both spellings are collected rather than one winning, the way bridgeAudiences treats its own
  // sources: the CSL property is List-valued, so it can hold both, and dropping one would silently
  // ignore the value an operator added while migrating off the environment variable, with the
  // deprecation warning naming only the spelling that did take effect.
  //
  ...
  private void bridgePublicApiJwkSetUri(
      final ConfigurableEnvironment env, final Map<String, Object> derived) {
    final Set<String> jwkSetUris = new LinkedHashSet<>();
    addPublicApiJwkSetUri(env, jwkSetUris, PUBLIC_API_JWK_SET_URI_ENV_VAR);
    addPublicApiJwkSetUri(env, jwkSetUris, PUBLIC_API_JWK_SET_URI);
    if (!jwkSetUris.isEmpty()) {
      derived.putIfAbsent(OIDC_PREFIX + "additional-jwk-set-uris", String.join(",", jwkSetUris));
    }
  }

  private void addPublicApiJwkSetUri(
      final ConfigurableEnvironment env, final Set<String> jwkSetUris, final String legacyKey) {
    final String value = env.getProperty(legacyKey);
    if (!isBlank(value)) {
      jwkSetUris.add(value.trim());
      warnDeprecated(legacyKey, OIDC_PREFIX + "additional-jwk-set-uris");
    }
  }
