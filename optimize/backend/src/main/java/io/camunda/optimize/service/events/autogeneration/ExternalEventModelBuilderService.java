...
import static io.camunda.optimize.service.util.EventModelBuilderUtil.addInclusiveGateway;
import static io.camunda.optimize.service.util.EventModelBuilderUtil.addIntermediateEvent;
import static io.camunda.optimize.service.util.EventModelBuilderUtil.addParallelGateway;
import static io.camunda.optimize.service.util.EventModelBuilderUtil.addStartEvent;
import static io.camunda.optimize.service.util.EventModelBuilderUtil.generateGatewayIdForNode;
...
public class ExternalEventModelBuilderService {
  ...
  public AbstractFlowNodeBuilder<?, ?> createOrExtendModelWithExternalEventSource(
    ...
    for (EventTypeDto rootNode : startEvents) {
      ...
      if (currentBuilder == null) {
        nextBuilder =
            addStartEvent(rootNode, nodeId, processBuilder, startEvents.indexOf(rootNode));
      } else {
    ...
    if (builderToReturn != null && !isFinalSourceInSeries) {
      builderToReturn =
          prepareModelBuilderForNextSource(
              builderToReturn,
  ...
  private AbstractFlowNodeBuilder<?, ?> depthTraverseGraph(
      final EventTypeDto previouslyAddedNode,
    ...
    for (EventTypeDto nodeToAdd : nodesToAdd) {
      ...
      if (precedingDivergingGatewayId.isPresent()
        ...
        currentNodeBuilder =
            addGateway(
                currentNodeBuilder, previouslyAddedNode, Diverging, graphDto, sampleTraceLists);
      }
      if (graphDto.getEndEvents().contains(nodeToAdd)) {
        currentNodeBuilder =
            addOrConnectToEndNode(
                graphDto,
                sampleTraceLists,
                currentNodeBuilder,
                mappings,
        ...
        if (!nodesExistInParallel(nodeToAdd, previouslyAddedNode, graphDto, sampleTraceLists)) {
          currentNodeBuilder =
              addOrConnectToIntermediateEventNode(
                  graphDto,
                  sampleTraceLists,
                  currentNodeBuilder,
                  mappings,
  ...
  private void depthTraverseOrConnect(
      final AutogenerationEventGraphDto graphDto,
      final List<List<EventTypeDto>> sampleTraceLists,
    ...
    final List<EventTypeDto> succeedingEvents =
        graphDto.getAdjacentEventTypesDtoMap().get(nodeToAdd).getSucceedingEvents();
    final List<EventTypeDto> nonParallel =
        succeedingEvents.stream()
            .filter(
                nextNode -> !nodesExistInParallel(nextNode, nodeToAdd, graphDto, sampleTraceLists))
    ...
    if (!nonParallel.isEmpty()) {
      ...
      final Optional<EventTypeDto> nodeToConnect =
          findAlreadyModelledNextNodeId(currentNodeBuilder, succeedingEvents);
      if (nodeToConnect.isPresent()) {
        ...
        log.debug(
            "Cannot connect node {} to its next node as it could not be found",
            generateNodeId(nodeToAdd));
      }
    }
  }

  private AbstractFlowNodeBuilder<?, ?> addOrConnectToEndNode(
      final AutogenerationEventGraphDto graphDto,
      final List<List<EventTypeDto>> sampleTraceLists,
      AbstractFlowNodeBuilder<?, ?> currentNodeBuilder,
      final Map<String, EventMappingDto> mappings,
  ...
  private AbstractFlowNodeBuilder<?, ?> addOrConnectToIntermediateEventNode(
      final AutogenerationEventGraphDto graphDto,
      final List<List<EventTypeDto>> sampleTraceLists,
      AbstractFlowNodeBuilder<?, ?> currentNodeBuilder,
      final Map<String, EventMappingDto> mappings,
    ...
    if (!nodeAlreadyAddedToModel(nodeId, currentNodeBuilder)) {
      ...
      mappings.put(nodeId, EventMappingDto.builder().start(nodeToAdd).build());
      depthTraverseOrConnect(
          graphDto, sampleTraceLists, nextBuilder, mappings, nodeToAdd, isFinalSourceInSeries);
    } else {
  ...
  private void connectToExistingNode(
      final AbstractFlowNodeBuilder<?, ?> currentNodeBuilder, final EventTypeDto nodeToConnectTo) {
  ...
  private AbstractFlowNodeBuilder<?, ?> addGateway(
    ...
    final AutogenerationAdjacentEventTypesDto adjacentEventTypesDto =
        graphDto.getAdjacentEventTypesDtoMap().get(nodeToAdd);
    ...
    final Map<Integer, Long> traceCountsByAdjacentEventOccurrence =
        extractEventTraceCountByAdjacentEventOccurrence(
            nodeToAdd, gatewayDirection, adjacentEventsToLookup, sampleTraceLists);

    final boolean isParallelGateway =
        traceCountsByAdjacentEventOccurrence.get(adjacentEventsToLookup.size()) != null
            && traceCountsByAdjacentEventOccurrence.size() == 1;
    final boolean isExclusiveGateway =
        traceCountsByAdjacentEventOccurrence.get(1) != null
            && traceCountsByAdjacentEventOccurrence.size() == 1;
  ...
  private Map<Integer, Long> extractEventTraceCountByAdjacentEventOccurrence(
      final EventTypeDto nodeToAdd,
      final GatewayDirection gatewayDirection,
      final List<EventTypeDto> adjacentEventsToLookup,
      final List<List<EventTypeDto>> sampleTraceLists) {
    return sampleTraceLists.stream()
        .filter(eventTypeTrace -> eventTypeTrace.contains(nodeToAdd))
        .map(
            eventTrace -> {
              if (Diverging.equals(gatewayDirection)) {
                // if diverging, we only care about the occurrences of adjacent events after the
                // last occurring target event
                return eventTrace.subList(eventTrace.lastIndexOf(nodeToAdd), eventTrace.size());
              } else {
                // if converging, we only care about the occurrences of adjacent events before the
                // first occurring target
                // event
  ...
  private boolean nodeSucceedsConvergingGateway(
      final EventTypeDto node, final AutogenerationEventGraphDto graphDto) {
    return getPrecedingNodes(node, graphDto).size() > 1;
  }

  private Optional<String> getPrecedingDivergingGatewayId(
      final EventTypeDto node, final AutogenerationEventGraphDto graphDto) {
    final List<Map.Entry<EventTypeDto, AutogenerationAdjacentEventTypesDto>> precedingNodes =
        getPrecedingNodes(node, graphDto);
    if (precedingNodes.size() == 1
        && precedingNodes.get(0).getValue().getSucceedingEvents().size() > 1) {
      return Optional.of(generateGatewayIdForNode(precedingNodes.get(0).getKey(), Diverging));
    }
    return Optional.empty();
  }

  private List<Map.Entry<EventTypeDto, AutogenerationAdjacentEventTypesDto>> getPrecedingNodes(
      final EventTypeDto node, final AutogenerationEventGraphDto graphDto) {
    return graphDto.getAdjacentEventTypesDtoMap().entrySet().stream()
  ...
  private boolean nodesExistInParallel(
      final EventTypeDto nodeToAdd,
      final EventTypeDto alreadyAddedNode,
      final AutogenerationEventGraphDto graphDto,
      final List<List<EventTypeDto>> sampleTraceLists) {
    // if a node is adjacent to a gateway, we can try to determine whether it is part of a parallel
    // path to the
    ...
    final boolean startOrEnd =
        Stream.concat(graphDto.getStartEvents().stream(), graphDto.getEndEvents().stream())
    ...
    final List<List<EventTypeDto>> containsBothEvents =
        sampleTraceLists.stream()
            .filter(
                eventTrace -> eventTrace.containsAll(Arrays.asList(nodeToAdd, alreadyAddedNode)))
            .collect(Collectors.toList());
    final boolean aBeforeB =
        containsBothEvents.stream()
            .anyMatch(trace -> trace.indexOf(nodeToAdd) == trace.indexOf(alreadyAddedNode) + 1);
    final boolean bBeforeA =
        containsBothEvents.stream()
    ...
    return getPrecedingDivergingGatewayId(nodeToAdd, graphDto)
        .map(
            gatewayId -> {
              final Optional<String> existingGatewayId =
                  getPrecedingDivergingGatewayId(alreadyAddedNode, graphDto);
              return existingGatewayId.isPresent() && existingGatewayId.get().equals(gatewayId);
            })
        .orElse(false);
  }

  private boolean canGenerateModelFromGraph(
      final AutogenerationEventGraphDto autogenerationEventGraphDto) {
