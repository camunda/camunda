...
public class IdentityAuthorizationService
  ...
  public boolean isUserAuthorizedToSeeIdentity(
      final String userId,
      final IdentityType requestedIdentityType,
      final String requestedIdentityId) {
    return isAuthorizedToSeeIdentity(
        IdentityType.USER, userId, requestedIdentityType, requestedIdentityId);
  }

  private boolean isAuthorizedToSeeIdentity(
  ...
  @Override
  protected Map<IdentityType, ResolvedResourceTypeAuthorizations> fetchAuthorizationsForUserId(
      final String userId) {
    final Map<IdentityType, ResolvedResourceTypeAuthorizations> result =
        new EnumMap<>(IdentityType.class);
    result.put(IdentityType.USER, fetchIdentityAuthorizationsForUserId(IdentityType.USER, userId));
    result.put(
        IdentityType.GROUP, fetchIdentityAuthorizationsForUserId(IdentityType.GROUP, userId));
    return result;
  }

  @Override
  protected Map<IdentityType, ResolvedResourceTypeAuthorizations> fetchAuthorizationsForGroupId(
      final String groupId) {
    final Map<IdentityType, ResolvedResourceTypeAuthorizations> result =
        new EnumMap<>(IdentityType.class);
    result.put(
        IdentityType.USER, fetchIdentityAuthorizationsForGroupId(IdentityType.USER, groupId));
    result.put(
        IdentityType.GROUP, fetchIdentityAuthorizationsForGroupId(IdentityType.GROUP, groupId));
    return result;
  }
  ...
  private ResolvedResourceTypeAuthorizations fetchIdentityAuthorizationsForUserId(
    ...
    for (String engineAlias : engineAliases) {
      engineContextFactory
          ...
          .ifPresent(
              engineContext ->
                  authorizations.merge(
                      resolveUserAuthorizations(identitytype, userId, engineContext)));
    }
    ...
    for (String engineAlias : engineAliases) {
      engineContextFactory
          ...
          .ifPresent(
              engineContext ->
                  authorizations.merge(
                      resolveGroupAuthorizations(identitytype, groupId, engineContext)));
    }
    return authorizations;
  }

  private ResolvedResourceTypeAuthorizations resolveUserAuthorizations(
      final IdentityType identitytype, final String userId, final EngineContext engineContext) {
    final List<AuthorizationDto> allAuthorizations =
        getAllIdentityAuthorizationsForUser(engineContext, identitytype, userId);
  ...
  private ResolvedResourceTypeAuthorizations resolveGroupAuthorizations(
      final IdentityType identitytype, final String groupId, final EngineContext engineContext) {
    final List<AuthorizationDto> allAuthorizations =
        getAllIdentityAuthorizations(engineContext, identitytype);
  ...
  private List<AuthorizationDto> getAllIdentityAuthorizations(
      final EngineContext engineContext, final IdentityType identityType) {
  ...
  private List<AuthorizationDto> getAllIdentityAuthorizationsForUser(
      final EngineContext engineContext, final IdentityType identityType, final String userId) {
    if (IdentityType.USER.equals(identityType)) {
