 ...
 * surface — see camunda/camunda#63372. Independent of, and composes with, PR #63400's session-path
 * {@code OptimizeComponentAccessFilter}: the two act on disjoint authentication types (login
 * session vs. bearer token) and neither depends on the other's beans.
 */
@Configuration
...
public class OptimizeBearerPermissionConfiguration {
  ...
  @Bean
  public OidcBearerPrincipalClassifier oidcBearerPrincipalClassifier(
      final CamundaSecurityLibraryProperties cslProperties) {
    return new OidcBearerPrincipalClassifier(cslProperties.getAuthentication().getOidc());
  }
