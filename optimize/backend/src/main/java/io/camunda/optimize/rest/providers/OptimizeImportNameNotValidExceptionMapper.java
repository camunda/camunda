...
@Provider
@Slf4j
public class OptimizeImportNameNotValidExceptionMapper
    implements ExceptionMapper<OptimizeImportNameNotValidException> {
  public static final String ERROR_CODE = "importNameInvalid";

  private final LocalizationService localizationService;

  public OptimizeImportNameNotValidExceptionMapper(
      @Context final LocalizationService localizationService) {
    this.localizationService = localizationService;
  }

  @Override
  public Response toResponse(final OptimizeImportNameNotValidException exception) {
    log.info("Mapping OptimizeImportNameNotValidException");

    return Response.status(Response.Status.BAD_REQUEST)
        .type(MediaType.APPLICATION_JSON_TYPE)
        .entity(getNameNotValidResponseDto(exception))
        .build();
  }

  private ErrorResponseDto getNameNotValidResponseDto(
      OptimizeImportNameNotValidException exception) {
