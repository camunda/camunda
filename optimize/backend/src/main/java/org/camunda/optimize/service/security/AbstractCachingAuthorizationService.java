...
public abstract class AbstractCachingAuthorizationService<T>
  ...
  public AbstractCachingAuthorizationService(
  ...
  private void initAuthorizationsCache() {
    ...
    groupAuthorizationLoadingCache =
        Caffeine.newBuilder()
            .maximumSize(CACHE_MAXIMUM_SIZE)
            .expireAfterAccess(
                configurationService.getAuthConfiguration().getTokenLifeTimeMinutes(),
  ...
  public static ResolvedResourceTypeAuthorizations resolveUserResourceAuthorizations(
      final String engine,
  ...
  public static ResolvedResourceTypeAuthorizations resolveResourceAuthorizations(
      final String engine,
  ...
  public static ResolvedResourceTypeAuthorizations resolveResourceAuthorizations(
      final EngineAuthorizations resourceAuthorizations,
      final List<String> relevantPermissions,
      final int resourceType) {
    final ResolvedResourceTypeAuthorizations authorizations =
        new ResolvedResourceTypeAuthorizations();
    ...
    resourceAuthorizations
        ...
        .forEach(
            authorization ->
                addGloballyAuthorizedResources(
                    authorization, relevantPermissions, authorizations, resourceType));

    // group authorizations
    addResourceAuthorizations(
        resourceAuthorizations.getGroupAuthorizations(),
        relevantPermissions,
        authorizations,
        resourceType);
    ...
    // user authorizations
    addResourceAuthorizations(
        resourceAuthorizations.getUserAuthorizations(),
        relevantPermissions,
        authorizations,
        resourceType);
  ...
  public static EngineAuthorizations mapToEngineAuthorizations(
      final String engine, final List<AuthorizationDto> engineAuthorizationsForUser) {
  ...
  private static List<AuthorizationDto> extractGlobalAuthorizations(
      final List<AuthorizationDto> allAuthorizations) {
    return allAuthorizations.stream()
        .filter(authorization -> authorization.getType() == AUTHORIZATION_TYPE_GLOBAL)
        .collect(Collectors.toList());
  }

  private static List<AuthorizationDto> extractAuthorizationsForGroups(
      final List<String> groupsIdsOfUser, final List<AuthorizationDto> allAuthorizations) {
    return allAuthorizations.stream()
        .filter(a -> groupsIdsOfUser.contains(a.getGroupId()))
        .collect(Collectors.toList());
  }

  private static List<AuthorizationDto> extractAllGroupAuthorizations(
      final List<AuthorizationDto> authorizations) {
    return authorizations.stream().filter(a -> a.getGroupId() != null).collect(Collectors.toList());
  }

  private static List<AuthorizationDto> extractAllUserAuthorizations(
      final List<AuthorizationDto> allAuthorizationsOfUser) {
    return allAuthorizationsOfUser.stream()
        .filter(a -> a.getUserId() != null)
        .collect(Collectors.toList());
  }

  private static void addResourceAuthorizations(
    ...
    groupAuthorizations.forEach(
        authDto ->
            removeRevokedAuthorizations(
                authDto, relevantPermissions, resolvedAuthorizations, resourceType));
    groupAuthorizations.forEach(
        authDto ->
            addGrantedAuthorizations(
                authDto, relevantPermissions, resolvedAuthorizations, resourceType));
    groupAuthorizations.forEach(
        authDto ->
            removeAuthorizationForProhibitedResource(
                authDto, relevantPermissions, resolvedAuthorizations, resourceType));
    groupAuthorizations.forEach(
        authDto ->
            addAuthorizationForResources(
                authDto, relevantPermissions, resolvedAuthorizations, resourceType));
  }

  private static void addGloballyAuthorizedResources(
  ...
  private static void addGrantedAuthorizations(
      final AuthorizationDto authorization,
      final List<String> relevantPermissions,
      final ResolvedResourceTypeAuthorizations resourceAuthorizations,
      final int resourceType) {
  ...
  private static void addAuthorizationForResources(
      final AuthorizationDto authorization,
      final List<String> relevantPermissions,
      final ResolvedResourceTypeAuthorizations resourceAuthorizations,
      final int resourceType) {
  ...
  private static void removeRevokedAuthorizations(
      final AuthorizationDto authorization,
      final List<String> relevantPermissions,
      final ResolvedResourceTypeAuthorizations resourceAuthorizations,
      final int resourceType) {
  ...
  private static void removeAuthorizationForProhibitedResource(
      final AuthorizationDto authorization,
      final List<String> relevantPermissions,
      final ResolvedResourceTypeAuthorizations resourceAuthorizations,
      final int resourceType) {
  ...
  private static boolean hasRelevantPermissions(
      final AuthorizationDto authorization, final List<String> relevantPermissions) {
