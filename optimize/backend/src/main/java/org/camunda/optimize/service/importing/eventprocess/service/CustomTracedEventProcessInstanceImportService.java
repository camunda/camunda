...
@AllArgsConstructor
@Slf4j
public class CustomTracedEventProcessInstanceImportService
    implements ImportService<CamundaActivityEventDto> {
  ...
  public void executeImport(
    ...
    final List<EventDto> filteredEvents =
        filterForConfiguredTenantsAndVersions(camundaActivities).stream()
  ...
  private List<EventDto> correlateCamundaEvents(final List<EventDto> eventDtosToImport) {
    ...
    final Map<String, List<VariableUpdateInstanceDto>> processInstanceToVariableUpdates =
        variableUpdateInstanceReader
            .getVariableInstanceUpdatesForProcessInstanceIds(processInstanceIds)
            .stream()
            .distinct()
    ...
    if (eventSourceConfig.isTracedByBusinessKey()) {
      Map<String, String> instanceIdToBusinessKeys =
          businessKeyReader.getBusinessKeysForProcessInstanceIds(processInstanceIds).stream()
              ...
              .collect(
                  Collectors.toMap(
                      BusinessKeyDto::getProcessInstanceId, BusinessKeyDto::getBusinessKey));
      correlatedEvents =
          eventDtosToImport.stream()
              .filter(eventDto -> instanceIdToBusinessKeys.get(eventDto.getTraceId()) != null)
              .peek(
                  eventDto ->
                      eventDto.setTraceId(instanceIdToBusinessKeys.get(eventDto.getTraceId())))
              .collect(Collectors.toList());
    } else {
      correlatedEvents =
          eventDtosToImport.stream()
                            ...
                            .filter(
                                variableUpdateInstanceDto ->
                                    variableUpdateInstanceDto
                                        ...
                                        .equalsIgnoreCase(eventSourceConfig.getTraceVariable()))
                            .distinct()
                            .min(Comparator.comparing(VariableUpdateInstanceDto::getTimestamp))
                            .get();
                    // If there are multiple values in this variable, we always take the first
                    eventDto.setTraceId(
                        firstUpdate.getValue().stream().sorted().findFirst().orElse(null));
                    if (firstUpdate.getValue().size() > 1) {
                      log.info(
                          "Variable {} has multiple values, setting traceID to first value in list.",
                          firstUpdate.getName());
                    }
                  })
              .collect(Collectors.toList());
  ...
  private Object extractVariablesDataForEvent(
    ...
    return variableUpdateInstanceDtos.stream()
        .collect(groupingBy(VariableUpdateInstanceDto::getName))
        .entrySet()
        .stream()
        .peek(
            entry ->
                entry
                    .getValue()
                    .sort(Comparator.comparing(VariableUpdateInstanceDto::getTimestamp).reversed()))
        .collect(
            Collectors.toMap(
                Map.Entry::getKey, entry -> getTypedVariable(entry.getValue().get(0))));
  }
