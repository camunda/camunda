...
public class CamundaPlatformTenantAuthorizationService
  ...
  public CamundaPlatformTenantAuthorizationService(
  ...
  public boolean isAuthorizedToSeeTenant(
    ...
    if (tenantId == null || tenantId.isEmpty()) {
      ...
      final Stream<ResolvedResourceTypeAuthorizations> relevantEngineAuthorizations =
          Optional.ofNullable(getCachedAuthorizationsForId(identityId, identityType))
              .map(
                  authorizationsByDataSource -> {
                    if (dataSourceName == null) {
                      // no specific data source, return all
                      return authorizationsByDataSource.values().stream();
                    } else {
                      return Optional.ofNullable(authorizationsByDataSource.get(dataSourceName))
                          .stream();
                    }
                  })
  ...
  @Override
  protected Map<String, ResolvedResourceTypeAuthorizations> fetchAuthorizationsForUserId(
      final String userId) {
            ...
            engineAlias ->
                engineContextFactory
                    ...
                    .ifPresent(
                        engineContext ->
                            result.put(
                                engineAlias, resolveUserAuthorizations(userId, engineContext))));
    return result;
  }

  @Override
  protected Map<String, ResolvedResourceTypeAuthorizations> fetchAuthorizationsForGroupId(
      final String groupId) {
    final Map<String, ResolvedResourceTypeAuthorizations> result = new HashMap<>();
    applicationAuthorizationService
        .getAuthorizedEnginesForGroup(groupId)
        .forEach(
            engineAlias ->
                engineContextFactory
                    .getConfiguredEngineByAlias(engineAlias)
                    .ifPresent(
                        engineContext ->
                            result.put(
                                engineAlias, resolveGroupAuthorizations(groupId, engineContext))));
    return result;
  }

  private ResolvedResourceTypeAuthorizations resolveUserAuthorizations(
      final String userId, final EngineContext engineContext) {
    final List<AuthorizationDto> allAuthorizationsOfUser =
        engineContext.getAllTenantAuthorizationsForUser(userId);
  ...
  private ResolvedResourceTypeAuthorizations resolveGroupAuthorizations(
  ...
  private void addEnginesDefaultTenantAuthorizationForUser(
      final String username,
      final EngineContext engineContext,
      final List<AuthorizationDto> allAuthorizations) {
    final String enginesDefaultTenantId =
        defaultTenantIdByEngine.get(engineContext.getEngineAlias());
    if (enginesDefaultTenantId != null) {
      allAuthorizations.add(
  ...
  private void addEnginesDefaultTenantAuthorizationForGroup(
      final String groupId,
      final EngineContext engineContext,
      final List<AuthorizationDto> allAuthorizations) {
    final String enginesDefaultTenantId =
