...
/**
 * Icons come from the design system's registry rather than from the crumb data: C3
 * types its `icon` for Carbon's sizing contract, and the registry is the agreed glyph
 * per context across the Camunda apps.
 */
...
function mapBreadcrumb(breadcrumb: BreadcrumbDescriptor): NavBreadcrumbDescriptor {
  return {
    ...
    linkProps:
      breadcrumb.key === 'app' ? (breadcrumb.linkProps ?? {to: '/'}) : breadcrumb.linkProps,
    ...
    dropdownItems: breadcrumb.dropdownItems?.map((item) => ({
      key: item.key,
      label: item.label,
      icon: breadcrumb.key === 'app' ? appIcon(item.key) : crumbIcon(breadcrumb.key),
...
/**
 * The org / cluster / app context chain for the header. C3 stays the data source —
 * the design system ships the rendering half only.
 */
export default function useBreadcrumbs() {
  ...
  return (
    <NavBreadcrumbSwitcher
      ...
      aria-label={t('navigation.breadcrumb').toString()}
      className="HeaderV2-breadcrumb"
    />
  );
}
