...
export default function MultiValueInput({
  ...
  return (
    <div className="MultiValueInput">
      ...
      <div
        className={classnames('textInput', `${prefix}--text-input`, `${prefix}--text-input__field-wrapper`, {
      ...
      >
        ...
        <input {...props} aria-labelledby={titleText ? id : undefined} className="textInput" />
      </div>
      {helperText && !invalid && <div className={`${prefix}--form__helper-text`}>{helperText}</div>}
      {invalid && invalidText && <div className={`${prefix}--form-requirement`}>{invalidText}</div>}
    </div>
  );
}
...
function RemovableTag({
  ...
  return (
    <div
      className={classnames('RemovableTag', `${prefix}--tag`, `${prefix}--tag--filter`, {
