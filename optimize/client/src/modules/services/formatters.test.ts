...
const exampleDurationReport = {
  ...
  data: {
    ...
    visualization: 'table',
    configuration: {sorting: null, groupByDateVariableUnit: 'day'},
  },
  result: {
    instanceCount: 100,
    data: [
...
describe('automatic interval selection', () => {
  ...
  } as unknown as SingleProcessReportData;
...
describe('createDurationFormattingOptions', () => {
  ...
  it('should show nice ticks for duration formats on the y axis', () => {
    ...
    // @ts-expect-error: I couldn't find a way to properly type this
...
describe('formatLabel', () => {
  it('should shorten to long string label', () => {
    ...
    expect(result?.toString().length).toBe(53);
    expect(result?.toString().slice(50)).toBe('...');
  });
