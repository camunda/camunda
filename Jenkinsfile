...
def static ZEEBE_TASKLIST_DOCKER_IMAGE() { return "registry.camunda.cloud/team-hto/tasklist" }
            ...
            container('maven') {
              configFileProvider([configFile(fileId: 'maven-nexus-settings', variable: 'MAVEN_SETTINGS_XML')]) {
                ...
                sh '''
                  ...
                  mvn verify -s $MAVEN_SETTINGS_XML -P -docker-es,-docker-os,skipFrontendBuild -B -T$LIMITS_CPU --fail-at-end -Dcamunda.tasklist.database=elasticsearch
                  '''
              }
            }
          }
