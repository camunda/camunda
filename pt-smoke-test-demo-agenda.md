...
> Local prep notes, not part of the harness deliverable. Branch: `pt-webapp-routes-demo`
> (rebased onto current `main`). Exercises main's PT code
> ([`authentication/.../pt/`](authentication/src/main/java/io/camunda/authentication/pt), CSL `0.1.0-alpha34`),
> plus PT-prefixed webapp serving (#55682) and per-PT durable web sessions (#55777), all merged.

## Goal

Show that per-physical-tenant identity isolation works end-to-end across **both surfaces**:

- **API** — how OC tells CSL what chains to build, what one realistic setup looks like, and
  per-tenant token isolation proven live.
- **Webapp** — the same PT prefix dispatches to the Operate/Tasklist UIs, with a PT-scoped login
  session, so each tenant has its own isolated web entry point.

Secondary storage is in-memory H2 (RDBMS), one auto-provisioned database per tenant (#52027) — no
Elasticsearch, no external datastore.
...
structure; secrets, per-PT H2 storage, logging, init omitted). The app still loads the split files:
...
| Layer | What | Where |
|---|---|---|
| **Contract (CSL)** | `CamundaSecurityScopeProvider` SPI → emits one `ScopedSecurityDescriptor` per scope | `camunda-security-library-api` jar (alpha34) |
...
```bash
# Terminal 1 — 3 Keycloak realms (:8081 default, :8082 tenanta, :8083 tenantb); no datastore
./pt-smoke-test-idp.sh

# Terminal 2 — OC under the D overlay (per-PT in-memory H2; webapps served)
./pt-smoke-test-oc.sh pt-smoke-test,pt-smoke-test-two-tenants

# Terminal 3 — the API PASS/FAIL assertion matrix (this is the live moment)
./pt-smoke-test-api-two-tenants.sh

# Terminal 3 (webapp surface) — PT-prefixed route dispatching
./pt-smoke-test-webapp.sh
```

**Webapp visual beat (optional, high-impact):** open
`http://localhost:8080/physical-tenants/tenanta/operate`, log in as `bob`/`bob`, and land back in
Operate under the tenant prefix — a PT-scoped session (`camunda-session-physical-tenants-tenanta`),
separate from `/physical-tenants/default/operate` (`alice`/`alice`).

### ⚠️ Pre-warm before the meeting
The infra boot is the slow, risky part: 3 Keycloak containers (first run pulls `keycloak:26.2`,
~500 MB) plus the first OC build (npm frontend build). **Boot Terminals 1 & 2 ahead of time** so
that, live, you only run Terminal 3 (the matrix with the satisfying PASS output, and the browser
beat). Removes nearly all live-demo risk. No datastore to pre-warm — storage is in-memory H2.

### Pre-flight checklist
- [ ] Docker running; `keycloak:26.2` image pulled (no Elasticsearch)
- [ ] Free ports: 8080 (OC), 8081/8082/8083 (Keycloak default/tenanta/tenantb), 9600 (OC mgmt)
- [ ] Java 21 on `PATH`; `curl`, `jq` available
- [ ] Terminal 1 shows `=== PT smoke-test local IdPs ready ===`
- [ ] Terminal 2 shows `Tomcat started on port 8080` (logs tee to `/tmp/oc.log`)
- [ ] Dry-run Terminal 3 once before the meeting (API matrix exit 0 / all PASS; webapp routing PASS)
