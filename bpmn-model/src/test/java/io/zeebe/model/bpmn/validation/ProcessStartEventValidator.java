...
/** @author Daniel Meyer */
public class ProcessStartEventValidator implements ModelElementValidator<Process> {
  ...
  public void validate(Process process, ValidationResultCollector validationResultCollector) {
    ...
    final int startEventCount = startEvents.size();

    if (startEventCount != 1) {
      validationResultCollector.addError(
          10,
          String.format("Process does not have exactly one start event. Got %d.", startEventCount));
    }
  }
}
