...
import {TextInput as BaseTextInput, TextArea as BaseTextArea, IconButton as BaseIconButton} from '@carbon/react';
...
const TextInput: typeof BaseTextInput = styled(BaseTextInput)`
	input {
		${({invalid}) =>
			invalid
				? css`
						padding-right: calc(${ICON_WIDTH}px + ${ICON_WIDTH}px + ${RIGHT_SPACING}px);
					`
				: css`
...
const TextArea: typeof BaseTextArea = styled(BaseTextArea)`
	textarea {
		${({invalid}) =>
			invalid
				? css`
						padding-right: calc(${ICON_WIDTH}px + ${ICON_WIDTH}px + ${SCROLLBAR_WIDTH}px);
					`
				: css`
						padding-right: calc(${ICON_WIDTH}px + ${SCROLLBAR_WIDTH}px + ${RIGHT_SPACING}px);
					`}
	}
`;
