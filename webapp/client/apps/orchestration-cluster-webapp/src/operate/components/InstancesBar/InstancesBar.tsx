...
const InstancesBar: React.FC<Props> = ({label, activeInstancesCount, incidentsCount, size, className}) => {
	...
	return (
		<div className={className}>
			<Wrapper $size={size}>
				<IncidentsCount data-testid="incident-instances-badge" $hasIncidents={hasIncidents}>
					{incidentsCount}
				</IncidentsCount>
				{label && (
					<Label $size={label.size} $isRed={label.type === 'incident' && size === 'medium'}>
						{label.text}
					</Label>
				)}
			...
			{showIncidentsBar && (
				<BarContainer>
					...
					<IncidentsBar $size={size} style={{width: `${incidentsBarRatio}%`}} />
				</BarContainer>
			)}
		</div>
	);
