...
import {HistoryItemDetailsModal} from '#/tasklist/modules/task-details-history/shadcn.components/HistoryItemDetailsModal';
import {useCallback} from 'react';

export const Route = createFileRoute('/shadcn/_auth/tasklist/_tasks/$userTaskKey/history/$auditLogKey')({
	...
	loader: async ({context: {queryClient}, params: {userTaskKey, auditLogKey}, location}) => {
		...
		try {
			...
			toast.error(t('tasklist.taskDetailsHistoryAuditLogErrorTitle'));
	...
	component: function HistoryItemDetailsModalRoute() {
		...
		const handleClose = useCallback(() => {
			void navigate({to: '/shadcn/tasklist/$userTaskKey/history', params: {userTaskKey}, search});
