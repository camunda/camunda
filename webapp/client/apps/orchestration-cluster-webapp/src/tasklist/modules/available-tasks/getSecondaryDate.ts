...
import {formatISODate} from '#/tasklist/modules/dates/formatDateRelative';
import {isBefore} from 'date-fns';

type FormattedDate = NonNullable<ReturnType<typeof formatISODate>>;

type SecondaryDate = {
	completionDate?: FormattedDate;
	followUpDate?: FormattedDate;
	overDueDate?: FormattedDate;
...
function getSecondaryDate({
	...
	const isFollowupBeforeDueDate = dueDate && followUpDate && isBefore(followUpDate.date, dueDate.date);

	if (sortBy === 'creation' && completionDate) {
		return {completionDate};
	} else if (sortBy === 'creation' && followUpDate && !isOverdue && isFollowupBeforeDueDate) {
		return {followUpDate};
