...
import {advancedStringFilterCodec, encodeFilterOperation, splitEncodedFilterOperation} from './advancedStringFilter';
...
describe('advancedStringFilterCodec.decode', () => {
	...
	it('should decode multiple chained filter entries', () => {
		expect(advancedStringFilterCodec.decode('eq_order-123___exists_true')).toEqual({
			$eq: 'order-123',
			$exists: true,
		});
	});
	...
	it('should fail on invalid inputs', () => {
		expect(advancedStringFilterCodec.safeDecode('order-123').success).toBe(false);
		expect(advancedStringFilterCodec.safeDecode('foo_value').success).toBe(false);
		expect(advancedStringFilterCodec.safeDecode('exists_maybe').success).toBe(false);
...
describe('advancedStringFilterCodec.encode', () => {
	it('should encode filters with basic conditions', () => {
		expect(advancedStringFilterCodec.encode({$eq: 'order-123'})).toBe('eq_order-123');
		expect(advancedStringFilterCodec.encode({$neq: 'order-123'})).toBe('neq_order-123');
		expect(advancedStringFilterCodec.encode({$exists: true})).toBe('exists_true');
	});

	it('should encode $like filter by stripping the wildcards added on decode', () => {
		expect(advancedStringFilterCodec.encode({$like: '*ord*er*'})).toBe('like_ord*er');
	});

	it('should encode list filters', () => {
		expect(advancedStringFilterCodec.encode({$in: ['a', 'b', 'c']})).toBe('in_a b c');
	});
