...
import {addDays, differenceInMinutes, format, isSameDay, isSameWeek, isSameYear, parseISO, subDays} from 'date-fns';
...
function formatISODate(dateString: string | null | undefined) {
...
function formatDateTime(time: Date, now?: Date) {
	...
	const speech = format(time, getFormat({resolution, isSpeech: true, includeTime: true}), {
		locale: getCurrentDateLocale(),
	});
	const absoluteText = format(time, getFormat({resolution: {type: 'years'}, includeTime: true}), {
		locale: getCurrentDateLocale(),
	});
	return {
		date: time,
...
export {formatISODate, formatISODateTime};
