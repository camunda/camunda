...
import {Field, useField, type FieldInputProps, type FieldRenderProps} from 'react-final-form';
...
const DelayedErrorField: React.FC<Props> = ({children, addExtraDelay, name, ...props}) => {
	const {
		meta: {error},
	} = useField(name);
	...
	useEffect(() => {
		...
		if (addExtraDelay) {
			timeoutId = setTimeout(() => setDelayedError(error), 500);
		} else {
	...
	return (
		<Field name={name} {...props}>
			{({input, meta}) => children({input, meta: {...meta, error: delayedError}})}
		</Field>
	);
};
