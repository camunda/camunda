...
describe('<HistoryTable />', () => {
	it('should show the task history entries', async () => {
		const screen = await renderWithRouter(
			...
			{path: '/shadcn/tasklist/$userTaskKey/history', initialEntry: '/shadcn/tasklist/2251799813685281/history'},
		);
	...
	it('should open a history entry from the details action', async () => {
		...
		await expect
			...
			.toBe('/shadcn/tasklist/2251799813685281/history/create-log');
	});
	...
	it('should preserve the history search params when opening details', async () => {
		const {router, ...screen} = await renderWithRouter(
			...
			{
				path: '/shadcn/tasklist/$userTaskKey/history',
				initialEntry: '/shadcn/tasklist/2251799813685281/history?sort=actorId+asc',
			},
		);
	...
	it('should sort by date when the user selects the date column', async () => {
		const {router, ...screen} = await renderWithRouter(
			...
			{
				path: '/shadcn/tasklist/$userTaskKey/history',
				initialEntry: '/shadcn/tasklist/2251799813685281/history?sort=timestamp+asc',
			},
		);
