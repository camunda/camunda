...
const DOCUMENT_ENDPOINT = decodeURIComponent(endpoints.getDocument({documentId: DOCUMENT_ID_PLACEHOLDER}).url);
...
class CamundaDocumentEndpointBuilder {
	buildUrl(options: BuildUrlOptions): string {
		...
		const finalUrl = new URL(DOCUMENT_ENDPOINT.replace(DOCUMENT_ID_PLACEHOLDER, documentId));
...
class FormManager {
	...
	render = async (options: {
		...
		if (schema !== null && (this.#schema !== schema || !isEqual(this.#data, data))) {
			try {
