...
function getLabel(formManager: FormManager, expressionContext: object, field: FormJSField) {
	const templating = formManager.get('templating');
	let label = field.label;

	if (label !== undefined && templating.isTemplate(label)) {
...
function getHighestParentLabel(formManager: FormManager, expressionContext: object, field: FormJSField) {
	const formFieldRegistry = formManager.get('formFieldRegistry');
	let parentLabel: string | undefined;
	let f = field;

	while (f._path.length >= 2) {
		const parent = formFieldRegistry.get(f._parent) as FormJSField;
...
function getFieldLabels(formManager: FormManager, fieldIds: string[]) {
	...
	return fieldIds
		.map((id) => {
			...
			const parentLabel = getHighestParentLabel(formManager, expressionContext, field);
