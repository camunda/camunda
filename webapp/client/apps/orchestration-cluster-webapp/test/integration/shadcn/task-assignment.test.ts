...
test.describe('Task assignment', () => {
	test('should assign an unassigned task to the current user', async ({network, shadcnTaskDetailPage}) => {
		network.use(
			mockGetUserTaskEndpoint({
		...
		await expect(shadcnTaskDetailPage.assignButton).toBeVisible();
		...
		await shadcnTaskDetailPage.assignButton.click();

		await expect(shadcnTaskDetailPage.unassignButton).toBeVisible();
	});

	test('should unassign a task from the current user', async ({network, shadcnTaskDetailPage}) => {
		network.use(
			mockGetUserTaskEndpoint({
				successResponse: HttpResponse.json(assignedTask),
			}),
		...
		await shadcnTaskDetailPage.unassignButton.click();

		await expect(shadcnTaskDetailPage.assignButton).toBeVisible();
	});

	test('should show error notification when assignment fails with 403', async ({network, shadcnTaskDetailPage}) => {
		network.use(
			mockGetUserTaskEndpoint({
				successResponse: HttpResponse.json(unassignedTask),
			}),
		...
		await expect(
			shadcnTaskDetailPage.header.notifications.getByNotificationTitle('Task could not be assigned'),
		).toBeVisible();
		await expect(shadcnTaskDetailPage.assignButton).toBeVisible();
	});

	test('should handle tasks with task listeners', async ({network, shadcnTaskDetailPage}) => {
		network.use(
			mockQueryUserTasksEndpoint({
				successResponse: HttpResponse.json(createQueryUserTasksResponse()),
			}),
		...
		await expect(
			shadcnTaskDetailPage.header.notifications.getByNotificationTitle('Task assignment delayed'),
		).toBeVisible();
	...
	test('should not show assign button for completed task', async ({network, shadcnTaskDetailPage}) => {
		...
		await expect(shadcnTaskDetailPage.assignButton).not.toBeVisible();
		await expect(shadcnTaskDetailPage.unassignButton).not.toBeVisible();
	});
});
