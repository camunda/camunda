...
test('should match snapshot with the filter select closed', async ({shadcnTasklistIndexPage, page}) => {
	await shadcnTasklistIndexPage.goto();

	await expect(shadcnTasklistIndexPage.taskItem('Review purchase contract')).toBeVisible();
	await expect(shadcnTasklistIndexPage.filterSelect).toBeVisible();
