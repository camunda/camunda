...
public class ElasticsearchBackupRepository implements BackupRepository {
  ...
  private class CreateSnapshotListener implements ActionListener<CreateSnapshotResponse> {
    ...
    public void onFailure(Exception ex) {
      if (ex instanceof SocketTimeoutException) {
        ...
        // Keep waiting
        while (true) {
          List<SnapshotInfo> snapshotInfos = findSnapshots(snapshotRequest.repositoryName(), backupId);
          SnapshotInfo currentSnapshot = snapshotInfos.stream().filter(x -> Objects.equals(x.snapshotId().getName(), snapshotRequest.snapshotName())).findFirst().orElse(null);
        ...
        logger.error(String.format("Exception while creating snapshot [%s] for backup id [%d].", snapshotRequest.snapshotName(), backupId), ex);
        // No need to continue
        onFailure.run();
      }
    }
  ...
  public void validateRepositoryExists(String repositoryName) {
    ...
    try {
      GetRepositoriesResponse repository = esClient.snapshot().getRepository(getRepositoriesRequest, RequestOptions.DEFAULT);
  ...
  public void executeSnapshotting(BackupService.SnapshotRequest snapshotRequest, Runnable onSuccess, Runnable onFailure) {
    var request = new CreateSnapshotRequest()
      ...
      .userMetadata(objectMapper.convertValue(snapshotRequest.metadata(), new TypeReference<>() {}))
  ...
  private List<SnapshotInfo> findSnapshots(String repositoryName, Long backupId) {
    ...
    try {
      ...
      if (isRepositoryMissingException(e)) {
        final String reason = String.format("No repository with name [%s] could be found.", repositoryName);
        throw new OperateRuntimeException(reason);
      }
  ...
  public List<GetBackupStateResponseDto> getBackups(String repositoryName) {
    final GetSnapshotsRequest snapshotsStatusRequest = new GetSnapshotsRequest().repository(repositoryName)
