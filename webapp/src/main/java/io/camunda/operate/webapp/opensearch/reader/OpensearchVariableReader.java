...
public class OpensearchVariableReader implements VariableReader {
  ...
  private void adjustResponse(final List<VariableDto> response, final String processInstanceId, final VariableRequestDto request) {
  ...
  private List<VariableDto> queryVariables(final String processInstanceId, VariableRequestDto variableRequest){
  ...
  private void applySorting(final SearchRequest.Builder searchRequest, final VariableRequestDto request) {
    ...
    if (directSorting) { //this sorting is also the default one for 1st page
      searchRequest
          .sort(sortOptions(NAME, SortOrder.Asc));
      ...
      //reverse sorting
      searchRequest
          .sort(sortOptions(NAME, SortOrder.Desc));
      if (request.getSearchBefore() != null) {
        searchRequest.searchAfter(toSafeListOfStrings(request.getSearchBefore(objectMapper)));
      } else if (request.getSearchBeforeOrEqual() != null) {
        searchRequest.searchAfter(toSafeListOfStrings(request.getSearchBeforeOrEqual(objectMapper)));
      }
  ...
  private VariableDto toVariableDto(VariableEntity variableEntity){
    return VariableDto.createFrom(variableEntity, null, true, operateProperties.getImporter().getVariableSizeThreshold(), objectMapper);
  }
}
