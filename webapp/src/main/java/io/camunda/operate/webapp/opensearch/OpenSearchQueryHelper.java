...
public class OpenSearchQueryHelper {
  ...
  private Query runningFinishedQuery(ListViewQueryDto query, RequestDSL.QueryType queryType) {
    ...
    if (finished && (completed || canceled)) {
      ...
      Query completedQuery = query.isCompleted() ? term(STATE, ProcessInstanceState.COMPLETED.toString()) : null;
      Query canceledQuery = query.isCanceled() ? term(STATE, ProcessInstanceState.CANCELED.toString()) : null;
  ...
  private Query activityIdQuery(String activityId, FlowNodeState state) {
    final Query query = and(
      ...
      state == FlowNodeState.COMPLETED ? term(ACTIVITY_TYPE, FlowNodeType.END_EVENT.name()) : null
    );
  ...
  private Query activityIdQuery(ListViewQueryDto query, RequestDSL.QueryType queryType) {
    ...
    return or(
      ...
      query.isCompleted() ? activityIdQuery(query.getActivityId(), FlowNodeState.COMPLETED) : null,
      query.isCanceled() ? activityIdQuery(query.getActivityId(), FlowNodeState.TERMINATED) : null
    );
  }
  ...
  private Query processDefinitionKeysQuery(ListViewQueryDto query) {
    return CollectionUtil.isNotEmpty(query.getProcessIds()) ? stringTerms(ListViewTemplate.PROCESS_KEY, query.getProcessIds()) : null;
  }
  ...
  private Query bpmnProcessIdQuery(ListViewQueryDto query) {
    if (!StringUtils.isEmpty(query.getBpmnProcessId())) {
      return and(
        ...
        query.getProcessVersion() != null ? term(ListViewTemplate.PROCESS_VERSION, query.getProcessVersion()) : null
      );
    }
    return null;
  }
  ...
  private Query excludeIdsQuery(ListViewQueryDto query) {
    return CollectionUtil.isNotEmpty(query.getExcludeIds()) ?not(stringTerms(ListViewTemplate.ID, query.getExcludeIds())) : null;
  }
  ...
  private Query batchOperationIdQuery(ListViewQueryDto query) {
    return query.getBatchOperationId() != null ? term(ListViewTemplate.BATCH_OPERATION_IDS, query.getBatchOperationId()) : null;
  }

  private Query parentInstanceIdQuery(ListViewQueryDto query) {
    return query.getParentInstanceId() != null ? term(ListViewTemplate.PARENT_PROCESS_INSTANCE_KEY, query.getParentInstanceId()) : null;
  }

  private Query tenantIdQuery(ListViewQueryDto query) {
    return query.getTenantId() != null ? term(ListViewTemplate.TENANT_ID, query.getTenantId()) : null;
  }
