...
public class ProcessInstanceReader {
  ...
  public ListViewProcessInstanceDto getProcessInstanceWithOperationsByKey(Long processInstanceKey) {
      final ProcessInstanceForListViewEntity processInstance = processStore.getProcessInstanceListViewByKey(processInstanceKey);
  ...
  private List<ProcessInstanceReferenceDto> createCallHierarchy(final String treePath,
    ...
    return processStore.createCallHierarchyFor(processInstanceIds, currentProcessInstanceId)
        .stream().map(r ->
  ...
  public ProcessInstanceCoreStatisticsDto getCoreStatistics() {
    ...
    if( permissionsService != null) {
      PermissionsService.ResourcesAllowed allowed = permissionsService.getProcessesWithPermission(IdentityPermission.READ);
      statistics = processStore.getCoreStatistics((allowed == null || allowed.isAll()) ? null : allowed.getIds());
    } else {
