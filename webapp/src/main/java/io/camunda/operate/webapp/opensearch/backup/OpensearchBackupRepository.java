...
public class OpensearchBackupRepository implements BackupRepository {
  ...
  public void deleteSnapshot(String repositoryName, String snapshotName) {
    ...
    richOpenSearchClient.async().snapshot().delete(requestBuilder)
      .thenAccept(response -> logger.debug("Delete snapshot was acknowledged by Opensearch node: " + response.acknowledged()))
      .exceptionally(t -> {
        if (isSnapshotMissingException(t)) {
          ...
          logger.error("Exception occurred while deleting the snapshot: " + t.getMessage(), t);
        }
        return null;
      });
  }
  ...
  private boolean isSnapshotMissingException(Throwable t) {
    return t instanceof OpenSearchException && t.getMessage().contains(SNAPSHOT_MISSING_EXCEPTION_TYPE);
  }

  private boolean isRepositoryMissingException(Exception e) {
    return e instanceof OpenSearchException && e.getMessage().contains(REPOSITORY_MISSING_EXCEPTION_TYPE);
  }
  ...
  public void validateRepositoryExists(String repositoryName) {
    try {
      ...
      if (isRepositoryMissingException(e)) {
        final String reason = format("No repository with name [%s] could be found.", repositoryName);
        throw new OperateRuntimeException(reason);
      }
      final String reason = format("Exception occurred when validating existence of repository with name [%s].", repositoryName);
  ...
  public void validateNoDuplicateBackupId(String repositoryName, Long backupId) {
    ...
    try {
      response = richOpenSearchClient.snapshot().get(getSnapshotRequestBuilder(repositoryName, snapshot));
    } catch (IOException e) {
      final String reason = format(
      ...
      final String reason = format("Exception occurred when validating whether backup with ID [%s] already exists.", backupId);
      throw new OperateRuntimeException(reason, e);
    }
  ...
  public void executeSnapshotting(BackupService.SnapshotRequest snapshotRequest, Runnable onSuccess, Runnable onFailure) {
    ...
    var requestBuilder = createSnapshotRequestBuilder(snapshotRequest.repositoryName(), snapshotRequest.snapshotName(), snapshotRequest.indices())
      .ignoreUnavailable(false) //ignoreUnavailable = false - indices defined by their exact name MUST be present
      .metadata(objectMapper.convertValue(snapshotRequest.metadata(), new TypeReference<>() {}))
      .featureStates("none")
      .waitForCompletion(true);

    richOpenSearchClient.async().snapshot().create(requestBuilder)
      ...
      .exceptionally( e -> {
        if (e instanceof SocketTimeoutException) {
          ...
          logger.warn(format("Timeout while creating snapshot [%s] for backup id [%d]. Need to keep waiting with polling...",
          ...
          while (true) {
            ...
            Optional<SnapshotInfo> maybeCurrentSnapshot = snapshotInfos.stream().filter(x -> Objects.equals(x.snapshot(), snapshotRequest.snapshotName())).findFirst();

            if (maybeCurrentSnapshot.isEmpty()) {
              logger.error(format("Expected (but not found) snapshot [%s] for backupId [%d].", snapshotRequest.snapshotName(), backupId));
              // No need to continue
              onFailure.run();
              break;
          ...
          logger.error(format("Exception while creating snapshot [%s] for backup id [%d].", snapshotRequest.snapshotName(), backupId), e);
          // No need to continue
          onFailure.run();
        }
  ...
  private void handleSnapshotReceived(SnapshotInfo snapshotInfo, Runnable onSuccess, Runnable onFailure) {
    if (SUCCESS.equals(snapshotInfo.state())) {
      ...
      logger.error(format("Snapshot taking failed for %s, reason %s", snapshotInfo.uuid()), snapshotInfo.reason());
      // No need to continue
      onFailure.run();
    } else {
      logger.warn(format("Snapshot state is %s for snapshot %s", snapshotInfo.state(), snapshotInfo.uuid()));
      onSuccess.run();
    }
  }

  private List<SnapshotInfo> findSnapshots(String repositoryName, Long backupId) {
    var requestBuilder = getSnapshotRequestBuilder(repositoryName, Metadata.buildSnapshotNamePrefix(backupId) + "*");
    ...
    try {
      ...
      final String reason = format("Exception occurred when searching for backup with ID [%s].", backupId);
      throw new OperateRuntimeException(reason, e);
    }
  }
  ...
  private GetBackupStateResponseDto toGetBackupStateResponseDto(Long backupId, List<SnapshotInfo> snapshots) {
    ...
    if (snapshots.size() == expectedSnapshotsCount && snapshots.stream().map(SnapshotInfo::state).allMatch(SUCCESS::equals)) {
    ...
    for (SnapshotInfo snapshot : snapshots) {
      ...
      detail.setStartTime(OffsetDateTime.ofInstant(Instant.ofEpochMilli(Long.parseLong(snapshot.startTimeInMillis())), ZoneId.systemDefault()));
    ...
    if (response.getState() == BackupStateDto.FAILED) {
      ...
      String failedSnapshots = snapshots.stream()
        .filter(s -> FAILED.equals(s.state()))
        .map(SnapshotInfo::snapshot)
        .collect(Collectors.joining(", "));
      if (!failedSnapshots.isEmpty()) {
        failureReason = format("There were failures with the following snapshots: %s", failedSnapshots);
      } else {
        String partialSnapshot = snapshots.stream()
  ...
  public List<GetBackupStateResponseDto> getBackups(String repositoryName) {
    var requestBuilder = getSnapshotRequestBuilder(repositoryName, Metadata.SNAPSHOT_NAME_PREFIX + "*");
      // TODO check unsupported: it looks like sorting as well as size/offset are not working, need to sort additionally before return
...
//      .order(SortOrder.DESC);
    ...
    try {
      ...
      LinkedHashMap<Long, List<SnapshotInfo>> groupedSnapshotInfos = snapshots.stream()
        .collect(
          groupingBy(
            si -> {
              Metadata metadata = objectMapper.convertValue(si.metadata(), Metadata.class);
      ...
      List<GetBackupStateResponseDto> responses = groupedSnapshotInfos.entrySet()
        .stream()
      ...
      final String reason = format("Exception occurred when searching for backups: %s", e.getMessage());
      throw new OperateRuntimeException(reason, e);
    }
  }
}
