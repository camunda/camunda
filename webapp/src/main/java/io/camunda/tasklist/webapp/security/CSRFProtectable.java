...
import static io.camunda.tasklist.webapp.security.TasklistURIs.X_CSRF_PARAM;
...
public interface CSRFProtectable {
  ...
  CookieCsrfTokenRepository COOKIE_CSRF_TOKEN_REPOSITORY = new CookieCsrfTokenRepository();

  default void configureCSRF(final HttpSecurity http) throws Exception {
    COOKIE_CSRF_TOKEN_REPOSITORY.setCookieName(X_CSRF_TOKEN);
    COOKIE_CSRF_TOKEN_REPOSITORY.setHeaderName(X_CSRF_TOKEN);
    COOKIE_CSRF_TOKEN_REPOSITORY.setParameterName(X_CSRF_PARAM);
    COOKIE_CSRF_TOKEN_REPOSITORY.setCookieHttpOnly(false);
    http.csrf()
        .csrfTokenRepository(COOKIE_CSRF_TOKEN_REPOSITORY)
  ...
  default HttpServletResponse addCSRFTokenWhenAvailable(
