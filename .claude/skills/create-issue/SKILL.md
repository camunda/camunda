...
This branch's checkout doesn't have a CVE or tech-debt template (`5. CVE.yml` / `6. tech debt.yml`
only exist on `main`) — Step 1's `ls` reads the local filesystem, so those issue types can't be
created from a `stable/8.8` checkout. If the user needs one, check out `main` (or a branch based on
it) first, or create the issue via the GitHub UI instead.
