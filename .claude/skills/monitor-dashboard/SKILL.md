...
- Every dashboard JSON file must have a unique `uid`. Use `teamname-dashboardname` format (e.g., `zeebe-processing-heatmap`). Never reuse UIDs across dashboards.
...
- Every panel query must use the `datasource` template variable, not a hardcoded datasource. Create a `datasource` variable on every new dashboard.
...
- [ ] UID is unique and follows `teamname-dashboardname` format
- [ ] Dashboard title ≠ folder name
- [ ] `as code` tag present on dashboard
- [ ] `datasource` variable created and used in all queries
...
- [ ] Related panels grouped into rows; expensive rows collapsed
