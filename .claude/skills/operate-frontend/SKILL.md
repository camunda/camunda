...
Operate is the process monitoring frontend at `operate/client/`. It is **legacy code being phased out** in favor of a replacement webapp (not yet on this branch). Work in Operate should be limited to bug fixes, small adjustments, and maintenance.
...
Each endpoint gets a thin typed function. Endpoints come from `@camunda/camunda-api-zod-schemas/8.8`:
...
import type {MyResponseType} from '@camunda/camunda-api-zod-schemas/8.8';
