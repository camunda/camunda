...
- Templates:
  ...
  - `zeebe/engine/src/test/java/io/camunda/zeebe/engine/state/appliers/ClockEventAppliersTest.java`
...
- Templates:
  ...
  - **Per-test (exception):** `zeebe/engine/src/test/java/io/camunda/zeebe/engine/processing/processinstance/CreateProcessInstanceRejectionTest.java` — uses an instance `@Rule EngineRule`. Useful reference for record assertions and test-client usage; only mirror the per-test isolation when siblings can't share state.
...
- `zeebe/docs/failing-tests.md` — investigating flaky/failing engine tests.
