...
- **Missing direct consumer dependency** — Gradle `implementation()` is absent from a consumer's
  compile classpath, while Maven compile dependencies are normally transitively available there.
  If consumer source imports a class that Maven supplied transitively, add that external or reactor
  dependency directly to the consumer and choose `api()` versus `implementation()` from the
  consumer's own public API. Do not make the provider's dependency `api()` just to preserve an
  accidental transitive compile path.
- **Wrong `api()` vs `implementation()`** — Maven has no compile/api split. Use Gradle `api()`
  only when the dependency's classes cross the module boundary through public/protected method
  parameters or return types, fields, constructors, thrown types, generic bounds, implemented
  interfaces, superclasses, or consumer-resolved annotations. If the dependency is used only by
  implementation code, including code in other packages within the same module, use
  `implementation()`. A direct Maven compile dependency or a currently-working transitive path is
  not enough evidence for `api`; inspect the exposed signatures and downstream consumers. `api()`
  when the dependency should stay internal leaks it to consumers (see the optional-dependency
  section in SKILL.md).
