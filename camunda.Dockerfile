...
### jattach and tini download stage ###
...
# Copy tools - tini maintains backwards compatibility within 8.8.x minor releases.
# Previous 8.8.x images included tini, and removing it could break customer
# deployments that rely on it (custom entrypoints, monitoring, signal handling).
COPY --from=tools --chown=1001:0 /tini /usr/bin/tini
COPY --from=tools --chown=1001:0 /jattach /usr/local/bin/jattach
