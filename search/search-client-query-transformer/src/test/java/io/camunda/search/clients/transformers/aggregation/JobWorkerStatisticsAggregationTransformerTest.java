...
class JobWorkerStatisticsAggregationTransformerTest {

  private final JobWorkerStatisticsAggregationTransformer transformer =
      new JobWorkerStatisticsAggregationTransformer();
  ...
  void shouldBuildExpectedAggregationStructure() {
    ...
    final var aggregation = new JobWorkerStatisticsAggregation(page);
    ...
    // Verify composite aggregation by worker
    assertThat(aggregations.get(0))
        .isInstanceOfSatisfying(
            SearchCompositeAggregator.class,
            compositeAgg -> {
              assertThat(compositeAgg.name()).isEqualTo(AGGREGATION_BY_WORKER);
              ...
              // 3 filter buckets (created, completed, failed) — no workers cardinality agg
  ...
  void shouldBuildCompositeAggregationWithTermsSource() {
    ...
    // Verify composite has one terms source for worker
    assertThat(compositeAgg.sources()).hasSize(1);
    assertThat(compositeAgg.sources().get(0))
        .isInstanceOfSatisfying(
            SearchTermsAggregator.class,
            termsSource -> {
              assertThat(termsSource.name()).isEqualTo(AGGREGATION_SOURCE_NAME_WORKER);
              assertThat(termsSource.field()).isEqualTo(FIELD_WORKER);
            });
  }
  ...
  void shouldUseDefaultSizeWhenPageIsNull() {
    // given
    final var aggregation = new JobWorkerStatisticsAggregation(null);
