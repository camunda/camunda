...
public class GroupMemberQueryTransformerTest extends AbstractTransformerTest {

  @Test
  public void shouldQueryMembersByGroupId() {
    // given
    final var filter =
        FilterBuilders.groupMember((f) -> f.groupId("test-parent-id").memberType(USER));
  ...
  public void shouldApplyAuthorizationCheck() {
    ...
    final var authorization =
        RequiredAuthorization.of(a -> a.group().read().resourceIds(List.of("1", "2")));
    ...
    final var searchQuery =
        transformQuery(FilterBuilders.groupMember(b -> b), resourceAccessChecks);
  ...
  public void shouldApplyFilterAndChecks() {
    ...
    // when
    final var searchQuery =
        transformQuery(FilterBuilders.groupMember(b -> b.groupId("a")), resourceAccessChecks);
  ...
  public void shouldReturnNonMatchWhenNoResourceIdsProvided() {
    // given
    final var authorization = RequiredAuthorization.of(a -> a.group().read());
    final var authorizationCheck = AuthorizationCheck.enabled(authorization);
    final var resourceAccessChecks =
        ResourceAccessChecks.of(authorizationCheck, TenantCheck.disabled());
