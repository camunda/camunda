...
public class FlowNodeInstanceDocumentReader extends DocumentBasedReader
  ...
  public SearchQueryResult<FlowNodeInstanceEntity> search(
    ...
    if (filter.elementInstanceScopeKeys() != null && !filter.elementInstanceScopeKeys().isEmpty()) {
  ...
  /**
   * Executes a two-step fallback search when the filter includes elementInstanceScopeKeys.
   *
  ...
  private SearchQueryResult<FlowNodeInstanceEntity> searchWithScopeKeyFallback(
    ...
    final var firstResult =
        search(
            FlowNodeInstanceQuery.of(
                b -> b.filter(f -> f.flowNodeInstanceKeys(filter.elementInstanceScopeKeys()))),
            resourceAccessChecks);
          ...
          FlowNodeInstanceQuery.of(
              b ->
                  b.filter(
                          f ->
                              f.copyFrom(filter)
                                  .processInstanceKeys(filter.elementInstanceScopeKeys())
                                  .levels(1))
                      .sort(query.sort())
                      .page(query.page()));
