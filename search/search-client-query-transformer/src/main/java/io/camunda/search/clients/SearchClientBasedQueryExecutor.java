...
public final class SearchClientBasedQueryExecutor {
  ...
  public <F extends FilterBase, S extends SortOption, T, R> SearchQueryResult<R> search(
    ...
    final var pageSize = query.page().size();
    return executeSearch(
        query,
        q -> {
          final var unlimited = SearchQueryResultType.UNLIMITED.equals(type);
          if (pageSize > DocumentBasedSearchClient.QUERY_MAX_SIZE || unlimited) {
            return responseTransformer.apply(
                searchClient.scroll(q, documentClass, true), unlimited);
          } else {
            return responseTransformer.apply(
