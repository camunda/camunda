...
import io.camunda.data.clients.core.search.ElasticsearchSearchHit;
import java.util.Collections;
import java.util.List;

public class ElasticsearchSearchResponse<T> implements DataStoreSearchResponse<T> {

  private final long totalHits;
  private final String scrollId;
  private final List<DataStoreSearchHit<T>> hits;

  public ElasticsearchSearchResponse(final Builder<T> builder) {
  ...
  public static <T> DataStoreSearchResponse<T> from(final ResponseBody<T> response) {
    final var builder = new ElasticsearchSearchResponse.Builder<T>();

    final var totalHits = response.hits().total().value();
    final var scrollId = response.scrollId();
    final var hits = response.hits().hits().stream().map(ElasticsearchSearchHit::from).toList();
  ...
  public static final class Builder<T> implements DataStoreSearchResponse.Builder<T> {
    ...
    public DataStoreSearchResponse<T> build() {
      return new ElasticsearchSearchResponse<T>(this);
    }
  }
}
