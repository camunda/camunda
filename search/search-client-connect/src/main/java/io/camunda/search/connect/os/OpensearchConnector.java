...
import io.camunda.search.connect.os.json.WrappedJacksonJsonpMapper;
...
public final class OpensearchConnector {
  ...
  private OpenSearchTransport createAWSBasedTransport(final ConnectConfiguration configuration) {
    ...
    return new AwsSdk2Transport(
        ...
        AwsSdk2TransportOptions.builder()
            .setMapper(new WrappedJacksonJsonpMapper(objectMapper))
            .build());
  }

  private OpenSearchTransport createDefaultTransport(final ConnectConfiguration configuration) {
    final HttpHost host = getHttpHost(configuration);
    ...
    final var jsonpMapper = new WrappedJacksonJsonpMapper(objectMapper);
    builder.setMapper(jsonpMapper);

    return builder.build();
  }

  private boolean shouldCreateAWSBasedClient() {
  ...
  protected HttpAsyncClientBuilder configureHttpClient(
      HttpAsyncClientBuilder httpAsyncClientBuilder, ConnectConfiguration osConfig) {
