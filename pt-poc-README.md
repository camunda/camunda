...
**`PhysicalTenantLoginPageController` removed.** Unprefixed `/login` is served by CSL's `OidcWebapp` chain's `DefaultLoginPageGeneratingFilter` (lists every registration in the standard repo — no `providers.assigned` filtering on the unprefixed picker, accepted trade-off). On the prefixed PT chains we install our own `DefaultLoginPageGeneratingFilter` in `PerTenantSecurityChainFactory#buildWebappChain` (`addFilterAfter(picker, CsrfFilter.class)`) — the picker renders at `/physical-tenant/<id>/login` with the PT's assigned providers and PT-prefixed authorization URLs. Root cause of why we had to install it ourselves: Spring Security 7's `DefaultLoginPageConfigurer.configure()` only auto-installs the picker when no custom `AuthenticationEntryPoint` is set, and our PT chain sets one to route unauthenticated requests to the per-PT picker URL.
