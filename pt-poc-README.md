...
**`PhysicalTenantLoginPageController` removed.** Unprefixed `/login` is now served by CSL's `OidcWebapp` chain's `DefaultLoginPageGeneratingFilter` — which lists every registration in the standard repo (no `providers.assigned` filtering on the unprefixed picker, accepted trade-off). The prefixed PT entry point always redirects directly to the first assigned registration's authorization URL — no picker UI on the prefixed access path. Multi-IdP selection for a prefixed PT is still reachable via `/physical-tenant/<id>/oauth2/authorization/<regId>` if needed.
