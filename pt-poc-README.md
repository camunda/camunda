...
**Picker rendering** (follow-up after Task 19 spotted multi-picker behaviour): CSL doesn't ship a working picker — its `OidcWebapp` chain configures a custom `AuthenticationEntryPoint`, which trips Spring Security 7's gate in `DefaultLoginPageConfigurer.configure()` (auto-picker only activates when `authenticationEntryPoint == null`). The PoC renders pickers itself: unprefixed `/login` is served by `PhysicalTenantLoginPageController` (lists every registration in the standard repo — no `providers.assigned` filtering on the unprefixed picker, accepted trade-off), and the PT-prefixed `prefix + /login` is rendered by an explicit `DefaultLoginPageGeneratingFilter` instance installed on the PT chain in `PerTenantSecurityChainFactory#buildWebappChain` (`addFilterAfter(picker, CsrfFilter.class)`), populated from the PT's `ClientRegistrationRepository`. Both pickers carry the same provider set for the default PT (root config = default's assigned set), so `/app` and `/physical-tenant/default/app` show identical options.
