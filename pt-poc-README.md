...
| 20 | CSL: install DefaultLoginPageGeneratingFilter on OidcWebapp chain ([CSL#269](https://github.com/camunda/camunda-security-library/issues/269)) | ⏳ follow-up    |
...
**Picker rendering** (follow-up after Task 19 spotted multi-picker behaviour): CSL doesn't ship a working picker — its `OidcWebapp` chain configures a custom `AuthenticationEntryPoint`, which trips Spring Security 7's gate in `DefaultLoginPageConfigurer.configure()` (auto-picker only activates when `authenticationEntryPoint == null`). On the **prefixed PT chains** the PoC installs Spring's standard `DefaultLoginPageGeneratingFilter` explicitly in `PerTenantSecurityChainFactory#buildWebappChain` (`addFilterAfter(picker, CsrfFilter.class)`), populated from the PT's `ClientRegistrationRepository` — so `/physical-tenant/<id>/login` works for any PT. The **unprefixed `/login`** currently has no picker; multi-IdP deployments need either CSL to install the picker filter itself on the OidcWebapp chain (Task 20) or each host to keep providing its own picker controller. The PoC deliberately does NOT ship a host-side controller for `/login` — the controller had become a long-running source of confusion and the right place to fix the gap is CSL.
